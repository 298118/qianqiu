...
- During the PixiJS ink-map phase, Gemini CLI is the frontend collaborator. Gemini may modify or create scoped frontend files, maintain frontend context notes when assigned, and provide verification notes, but must not run `git add`, `git commit`, `git push`, or create PRs. Codex owns backend work, asset generation, final diff review, documentation sync, staging, and commits.
