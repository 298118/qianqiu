...
- 2026-05-14 起，按用户要求停止与 Gemini CLI 共同开发；S72 后续工作与未来开发全部由 Codex 负责，S72.8 已完成归档。
...
- 不让 AI 或前端代码绕过服务器 resolver；AI 仍只能读取安全 `mapContextView` / capped summary，并通过 proposal-only 工具表达地理意图。
...
| 角色 | 主要职责 | 禁止事项 |
| --- | --- | --- |
| Codex | 后端地图 runtime view、API/schema、服务器裁决边界、AI 工具权限、Mock/no-key fallback、前端 PixiJS 地图渲染、图层系统、动效、交互、响应式布局、测试、使用 `gpt-image-2` 生成素材、第三方素材筛选、素材视觉审核、素材台账、最终提交 | 不把素材来源或视觉审核结论只留在聊天里；不使用其他 AI 生图工具生成 S72 地图素材；不让前端代码直写 canonical 状态 |
| Codex 子代理 | 只读复审或明确 scoped patch；用于提交前风险检查 | 不提交、不推送、不创建 PR |

历史上的 S72.4-S72.6 Gemini patch 作为已完成事实保留；S72 现已完成归档，后续新功能不再分配给 Gemini CLI。
...
| S72.2 | DONE | Codex | 后端地图 runtime view 与布局契约 | 新增 `mapRuntimeView`、显示 layout seed、route payload 接线和测试；确保 raw/hidden/坐标污染不外泄 |
| S72.3 | DONE | Codex | 水墨底图、图标、纹理与素材 manifest | 已生成并视觉审核 `ink-map-v1` 首批 AI 资产：水墨底图、宣纸纹理、路线笔触、红/青涟漪和 9 个图标；已新增 manifest、素材台账和尺寸/透明度/路径安全测试 |
| S72.4 | DONE | Gemini CLI，Codex 审核提交 | PixiJS 地图 shell 与图层系统 | 已接入本地 `pixi.js@7.4.3` vendor、CDN fallback、`mapRenderer.js`、`mapPanel.js`、地图 DOM/CSS、manifest-driven 素材加载、tooltip/label、行动草稿回填和资源失败 fallback；Codex 已浏览器复核 |
| S72.5 | DONE | Gemini CLI，Codex 审核提交 | 地图与游戏系统联动 | 已完成地点、路线和事件点击选中态，tooltip 只读安全 `mapRuntimeView` ref/sourceRefs，可跳转已公开渲染的局势簿卡片；行动草稿仍只回填输入框，不直写状态 |
| S72.6 | DONE | Gemini CLI，Codex 审核提交 | 水墨动效与视觉 polish | 已完成路线墨线 alpha 呼吸、事件涟漪 scale/alpha 扩散、深朱砂双圈选中态、tooltip/fallback 案头舆图视觉、reduced-motion 降级和可见性暂停守卫；Codex 已修正脚本残片与路线动效 `NaN` 风险 |
| S72.7 | DONE | Codex | 验收、性能与安全回归 | 已扩展 browser smoke 覆盖 PixiJS canvas 非空、资源失败静态降级、reduced-motion、桌面/窄屏布局、hidden/raw 防线和完整 Mock 主线；聚焦 node tests、docs governance、dual-mode storage-only 与浏览器 smoke 已通过 |
...
| S72.8 | DONE | Codex | 专项归档 | 已将完成范围、风险和后续地图玩法迁入归档，压缩活动台账 |
...
Gemini 的首个只读前端约束报告曾由用户转交给 Codex，并已沉淀到 [PIXIJS_INK_MAP_RUNTIME_CONTRACT.md](PIXIJS_INK_MAP_RUNTIME_CONTRACT.md)。2026-05-14 起后续前端工作由 Codex 直接接手，必须先阅读本路线图、运行时契约、`public/index.html`、`public/app.js` 和 `public/styles.css`，并遵守以下边界：
...
S72 已完成归档。后续若继续扩展地图玩法，应从新的小步骤重新开题，并继续保持前端只读安全 view，地图点击不得直接写 canonical 状态。
