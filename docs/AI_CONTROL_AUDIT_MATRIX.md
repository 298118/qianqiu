...
| 检索式 prompt context assembler | AI 可读取 `retrievalContext` 中按任务/玩家行动排序的可见国家、城市、路线、NPC、关系、官署、官职、任所、考成、迁转、实体和事件摘要；不可把排序结果当作任免、事件结局或隐藏信息公开裁决。 | S53.1 的 `src/ai/promptContextAssembler.js` 只从服务器可见 view 和 prompt summary helper 组装上下文，保留原 capped summary 字段并新增只读 ranked index。它不读取 raw `worldState.worldGeography`、raw `worldPeople`、raw `officialPostings`、raw `worldThreads`、raw `longTermEvents.queue`、raw audit sidecar 或 SQLite 审计表；provider schema 和稳定前缀不变。 | 这是 provider-only prompt 对象，不直接进入浏览器 UI。S53.2 浏览器信息面板仍必须读取 route view，而不是 `retrievalContext`。 | `src/ai/promptContextAssembler.js`、`src/ai/prompts.js`、`test/promptContextAssembler.test.js`、`test/prompts.test.js`。 | 后续若加入审计检索、SQLite business table 检索或向浏览器暴露事件档案，必须先新增 sanitized projection、分页权限、hidden-token corpus 和 raw-ledger/audit 泄漏探针。 |
...
| 人物、家族、资产与田产 schema / bridge | AI 可在叙事中解释可见人物、人情、家产或田产压力；当前只能沿用 `relationshipChanges` 建议既有可见关系，不可写人物/家产/田产/关系表。 | S51.1 的 `worldPeopleSchemas` 归一化独立 schema bundle、过滤 hidden 行和 nested hidden refs、构造 capped prompt summary。S51.2 的 `worldPeople` 从旧 `characters`、`relationshipLedger` 和可见 active request 近期札记派生 `worldState.worldPeople`、`worldPeopleView` 与 prompt summary；它只保存当前可见 projection，不存新增 hidden 私档。普通 `statePatch.worldPeople` 被 schema、remote normalization、`applyStatePatch()`、provider long-run 和 route red-team 拒绝或忽略；审计脱敏会 redacts `worldPeople` / `npcs` / `assets` 等 proposal 内容。 | 游戏、SSE 和考试 route 返回 `worldPeopleView`；现有 `relationshipView` / `activeNpcRequestView` 不变。当前无新增浏览器人物面板；未来人物谱牒/家产面板必须读取服务器 view，不能读 raw ledger。 | `docs/NPC_HOUSEHOLD_ASSET_RELATIONSHIP_CONTRACT.md`、`src/game/worldPeopleSchemas.js`、`src/game/worldPeople.js`、`test/worldPeopleSchemas.test.js`、`test/worldPeopleBridge.test.js`、`test/gameTurnRelationships.test.js`、`test/prompts.test.js`、`test/aiControlRedTeam.test.js`。 | 后续若保存真正 hidden NPC/家产私档，必须先新增 route redaction 或非 raw player state API，并补浏览器 hidden-token corpus、raw-ledger fallback 探针和 SQLite 表级权限边界。 |
...
| 官职任所数据库契约 / bridge | AI 可读取 capped 可见官署、官职、任所、考成和迁转摘要并解释公开职责或传闻；不可写 `officialPostings`、任命记录、考成记录或迁转记录。 | S52.1 的 `officialPostingSchemas` 定义 schema normalization、hidden-filtered view 和 prompt cap。S52.2 的 `officialPostings` 从 `officialCatalog`、`officialCareer`、地方官 role state 和可见 `worldGeographyView` 派生 `worldState.officialPostings`、`officialPostingsView` 与 prompt summary；存储的 raw ledger 也只是可见 projection。普通 `statePatch.officialPostings` 会被 schema、remote normalization、`applyStatePatch()`、audit redaction 和 provider long-run 拒绝、忽略或脱敏。实际任免仍由 `officialCareer` 裁决。 | 游戏、SSE 和考试 route 返回 `officialPostingsView`；当前无浏览器官职簿/任所地理面板，后续 UI 必须读取服务器 view，不能读 raw ledger。hidden 任所、密札考成、未公开调任和 hidden 地理 refs 不进入 view/prompt。 | `docs/OFFICIAL_POSTING_DATABASE_CONTRACT.md`、`src/game/officialPostingSchemas.js`、`src/game/officialPostings.js`、`test/officialPostingSchemas.test.js`、`test/officialPostings.test.js`、`test/gameTurnOfficialPostings.test.js`、`test/prompts.test.js`、`test/auditRoute.test.js`、`test/providerLongRunScript.test.js`。 | 后续若保存真正 hidden 官员私档、密札考成或未公开调任，必须先新增 route redaction 或非 raw player state API，并补浏览器 hidden-token corpus、raw-ledger fallback 探针和 SQLite 表级权限边界。 |
...
| 天下地理账本 | AI 可读取 capped `worldGeography` prompt 摘要，并在叙事中解释公开地名、路线、边面、贡道和官署辖区；不可写 `worldGeography`，不可裁决战争、外交、城市财政或隐藏情报公开。 | `src/game/worldGeographySeeds.js` 提供静态 seed；`src/game/worldGeography.js` 创建/归一化 server-owned `worldState.worldGeography`，从现有顶层指标刷新轻量压力快照，并构造 `worldGeographyView` 与 prompt summary。普通 `statePatch.worldGeography` 被 schema、remote normalization、`applyStatePatch()` 和 provider long-run 检查拒绝或忽略。 | 路由返回 `worldGeographyView`；隐藏路线/边面、`hiddenNotes`、hidden nested refs 和书生不可见的 `role_visible` 行不进入 view/prompt。现有 route 仍返回完整本地 `worldState` 以保持开发兼容，浏览器后续地理面板必须读取 view 而不是 raw ledger。 | `docs/WORLD_GEOGRAPHY_SEED_CONTRACT.md`、`src/game/worldGeographySeeds.js`、`src/game/worldGeography.js`、`test/worldGeographySeeds.test.js`、`test/worldGeography.test.js`、`test/gameTurnWorldGeography.test.js`、`test/prompts.test.js`。 | S53 做浏览器地理面板时，补 smoke hidden-token 扫描和 raw-ledger fallback 探针；若未来拆 SQLite 国家/城市业务表，继续保证 AI 只能读 projection、不能写 SQL/table rows。 |
...
| 事件日志与 AI proposal 审计 | AI 不可写审计表、不可决定审计 status；它的输出只作为被审查 proposal 的输入摘要。 | S49.4 由 `src/game/audit.js` 在服务器裁决后生成摘要；`src/storage/sessionAudit.js` 统一脱敏 API key、本地路径、隐藏字段和 raw `worldState`；JSON adapter 尽力写 `data/audit/*.jsonl` 诊断 sidecar，SQLite adapter 在同一事务写 `event_log` / `ai_change_proposals`，route 通过 `mutateSession()` context 排队，`skipWrite` 会丢弃队列。 | 当前不暴露玩家 API；未来事件档案 UI 必须读取服务器 projection，不能直接展示 raw audit payload。 | `src/storage/sessionAudit.js`、`src/game/audit.js`、`src/storage/jsonSessionAdapter.js`、`src/storage/sqliteSessionAdapter.js`、`test/auditRoute.test.js`、`test/sessionStoreAdapterContract.test.js`。 | 未来若开放浏览器事件档案，需补 projection 可见性、隐藏 token corpus、分页/检索权限和审计 payload 泄漏测试。 |
