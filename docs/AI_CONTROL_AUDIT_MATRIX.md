...
| 天下地理静态种子 | S50.1 阶段 AI 不读取也不可写该 catalog；未来只可读取服务器过滤后的国家/城市/边境 projection，并在叙事中解释公开地理事实。 | `src/game/worldGeographySeeds.js` 只提供静态 seed、归一化、引用校验和隐藏过滤 view；它不写 `worldState`、不改 route payload、不进入 prompt。后续若新增 `worldState.worldGeography`，必须按 server-owned ledger 更新 schema/stateRules/prompt tests。 | 当前不暴露玩家 API；`buildWorldGeographySeedView()` 会过滤 hidden 行和 `hiddenNotes`，可作为后续 projection 的起点。 | `docs/WORLD_GEOGRAPHY_SEED_CONTRACT.md`、`src/game/worldGeographySeeds.js`、`test/worldGeographySeeds.test.js`。 | S50.2 接入 per-session projection 时，需补 provider 伪造 `worldGeography`、hidden route/frontier 泄漏、prompt 全量过载和浏览器 raw ledger 渲染探针。 |
