...
| 世界 tick / 全局时间 | AI 不可写 `turnCount`、日历推进、`tenDayPeriod` 或自然漂移；可在叙事中解释当月民情。 | `src/game/time.js` 归一化年月旬和格式化日期；`src/game/worldTick.js` 当前仍按服务器状态推进年月、府库、粮储、民心、腐败、军心、边患和已知派系，后续 S48.3 才改为旬推进/月末结算；route 用 server-owned patch 应用。 | 玩家看到月度摘要、事件和状态栏年月；S48 后续会统一为年月旬。 | `docs/WORLD_TICK_CONTRACT.md`、`src/game/time.js`、`src/game/worldTick.js`、`test/time.test.js`、`test/worldTick.test.js`、`test/gameTurnTick.test.js`。 | 增加 provider 试图 patch `turnCount/year/month/tenDayPeriod` 同时触发考试窗口的跨模块探针。 |
