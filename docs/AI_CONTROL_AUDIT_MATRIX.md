...
| 官职任所数据库契约 | AI 可读取未来 capped 可见官署、官职、任所、考成和迁转摘要并解释公开职责或传闻；不可写 `officialPostings`、任命记录、考成记录或迁转记录。 | S52.1 的 `officialPostingSchemas` 只定义 schema normalization、hidden-filtered view 和 prompt cap；未来 `worldState.officialPostings` 被视为 server-owned key，普通 `statePatch.officialPostings` 会被 schema、remote normalization、`applyStatePatch()` 和 provider long-run 拒绝或忽略。实际任免仍由 `officialCareer`/后续服务器 ledger 裁决。 | S52.1 暂不新增 route view 或浏览器官职簿；后续 UI 必须读取服务器 view，不能读 raw ledger。hidden 任所、密札考成、未公开调任和 hidden nested refs 不进入 view/prompt。 | `docs/OFFICIAL_POSTING_DATABASE_CONTRACT.md`、`src/game/officialPostingSchemas.js`、`test/officialPostingSchemas.test.js`、`test/stateRules.test.js`、`test/aiSchemas.test.js`、`test/providerLongRunScript.test.js`。 | S52.2 写 per-session ledger 时，补 route/prompt/browser hidden-token 探针，并确认 `player.officeTitle`、`officialCareer.currentPosting`、`officeId/currentPostingId` 不产生错配。 |
