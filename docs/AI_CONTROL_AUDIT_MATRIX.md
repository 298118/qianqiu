...
| 人物、家族、资产与田产 schema | AI 可在叙事中解释可见人物、人情、家产或田产压力；当前只能沿用 `relationshipChanges` 建议既有可见关系，不可写人物/家产/田产/关系表。 | S51.1 的 `worldPeopleSchemas` 只归一化独立 schema bundle、过滤 hidden 行和 nested hidden refs、构造 capped prompt summary；当前不写 `worldState.worldPeople`、不进 route payload、不建 SQLite 业务表。普通 `statePatch.worldPeople` 被 AI schema 拒绝并被 `applyStatePatch()` 忽略。 | 当前无新增玩家面板；未来人物谱牒/家产面板必须读取服务器 view，不能读 raw bundle。 | `docs/NPC_HOUSEHOLD_ASSET_RELATIONSHIP_CONTRACT.md`、`src/game/worldPeopleSchemas.js`、`test/worldPeopleSchemas.test.js`。 | S51.2 桥接旧 `characters` / `relationshipLedger` / active request 时，补 provider 伪造 NPC、公开 hidden 家产、hidden patron nested ref 泄漏、prompt cap 和浏览器 hidden token 扫描。 |
