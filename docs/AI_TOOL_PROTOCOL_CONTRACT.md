...
- S70.7：已落地 `judicial.propose_case_resolution`、`city.propose_policy`、`military.propose_order`、`diplomacy.propose_move`、`exam.request_ranking_adjudication`、`office.request_appointment_adjudication`、`career.propose_reward_or_promotion` 和 `career.request_discipline_adjudication`。这些工具使用 strict input schema，要求领域动作、`publicSummary`、`evidenceRefs`、`targetRefs`、`jurisdictionRef`、`visibility`、`confidence`、`riskLevel`、`cooldownKey`、`expectedBenefits`、`counterCosts`、`riskDisclosure`、空 `privateResultRefs` 和 `riskTags`；默认 registry 会按 actor 权限过滤，resolver 只返回 `pending` / `rejected`，不写状态、SQLite、事件档案、官职、榜单、判决、军令、战和、赏罚或成案审计。
- S71.5：`city.propose_policy.policyKind` 已扩展为财政与城市政策全集（赈济、平粜、征粮、修堤、清丈、减免、追赃、盐漕、徭役、市场、治安）。默认模型可见 runner 仍只记录 pending/rejected proposal；服务器内部可把该 proposal 交给 `cityPolicyResolver` 裁决。`server.adjudicate_policy` 仍不是模型可见工具名、provider function name、MCP `tools/list` 条目或玩家可伪造 call。
- S71.6：`judicial.propose_case_resolution.caseAction` 已扩展为刑名动作全集（受理、传唤、查证、调解、罚银、羁押、驳回、判决、申详、移交、缓审）。默认模型可见 runner 仍只记录 pending/rejected proposal；服务器内部可把该 proposal 交给 `judicialCaseResolver` 裁决。`server.resolve_case` 仍不是模型可见工具名、provider function name、MCP `tools/list` 条目或玩家可伪造 call；公开 docket/event 只显示可见证据摘要，不公开 evidence ref、raw proposal 或 hidden 证据。
- S71.7：`military.propose_order.orderKind` 与 `diplomacy.propose_move.moveKind` 已改为从 `militaryDiplomacyResolverConfig` 动作全集生成，覆盖侦察、固守、练兵、调粮、动员、出击、会战、大会战、撤军、遣使、互市、和议、朝贡、边境警告、宣战 request、会盟和扣使。默认模型可见 runner 仍只记录 pending/rejected proposal；服务器内部可把该 proposal 交给 `militaryDiplomacyResolver` 裁决。`server.resolve_battle` / `server.apply_diplomacy` 仍不是模型可见工具名、provider function name、MCP `tools/list` 条目或玩家可伪造 call；公开 event 只显示可见证据摘要，不公开 evidence ref、raw proposal 或 hidden 情报。
- S70.8/S70.14：补完整 provider prompt/tool eval、真实 MiMo required smoke、browser/JSON/SQLite parity 和归档。
- S92.4：已新增旁路 `src/ai/tools/gameToolLoop.js`、`toolCallNormalizer.js`、`toolGuardrails.js` 与 `toolResultProjector.js`。该 loop 先用于 Mock/test-only，复用 S70.3 runner 的 schema、权限、冷却、server-owned resolver 和 audit 边界；provider-visible tool list 只含 function name、description、input schema；模型可见 tool result 只含 `status`、`publicResult`、`rejectionReasons`、`auditRef`、`modelFollowUpHint`。`privateResultRefs`、`appliedEventIds`、`counterCosts`、`followUpHooks`、developer audit、resolver label、permission/cooldown/mockFallback、raw provider payload、prompt、key、base URL、本地路径、`worldState`、`statePatch` 和内部 `server.*` 不得进入 provider-visible 回填。默认普通 turn、真实 provider tool calling 和服务器裁决能力仍未切换。
