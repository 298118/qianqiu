...
Qianqiu was intentionally buildless through the first playable browser slice. S74.0 has installed the React/Vite toolchain for the next default frontend, but the current runtime entry remains the existing `public/` app until S74.1 creates `client/` and Express history fallback.

- Backend: Node.js + Express in `server.js`.
- Frontend: current entry is plain HTML/CSS/JS in `public/`; S74.1+ will add a React + TypeScript + Vite client under the contract in [docs/FRONTEND_REACT_MIGRATION_CONTRACT.md](FRONTEND_REACT_MIGRATION_CONTRACT.md).
