...
Qianqiu was intentionally buildless through the first playable browser slice. S74 now uses a React + TypeScript + Vite frontend under `client/`; the production build in `dist/client/` is the default `/` entry served by Express with React Router Data Mode history fallback.

- Backend: Node.js + Express in `server.js`.
- Frontend: current entry is React + TypeScript + Vite in `client/`; `public/assets/`, `public/vendor/`, and `public/mapRenderer.js` remain runtime asset/map resources. The old `public/index.html`, `public/app.js`, `public/styles.css`, and `public/mapPanel.js` are migration references, not the delivered browser shell. S74.7 binds top navigation to the active safe `sessionId` after Mock start or route refresh.
...
- Browser smoke: `playwright-core` driving an installed Chrome/Edge browser through `scripts/clientSmoke.js` for the React default entry. `scripts/browserSmoke.js` remains as the legacy pre-React reference script.
...
Reads the session through the storage facade and returns `sessionId`, client-safe `worldState`, the player-facing `examCalendarView`, `examRivalView`, `examProcedureView`, `examinerPanelView`, `examHonorView`, `appointmentTrackView`, `studyProfileView`, `relationshipView`, `activeNpcRequestView`, `roleWorldCouplingView`, `worldGeographyView`, `mapContextView`, `mapRuntimeView`, `worldEntityView`, `worldPeopleView`, `worldThreadView`, `longTermEventView`, `officialCareerView`, `officialPostingsView`, `localAffairsDocketView`, `militaryDiplomacyView`, `economicFiscalView`, `historicalEventArchiveView`, `intelligenceRumorView`, `actorMemoryView`, `sessionSummaryView`, `eventArchiveView`, and `informationPanelPageView`. S70.12 strips raw `actorMemoryLedger` and `sessionSummary` from this response-level `worldState`; S72.2 map runtime remains a safe display projection, not a raw coordinate/source table.
...
S74 uses `scripts/clientSmoke.js` as the default browser acceptance entrypoint:
...
The script uses `playwright-core` with an installed Chrome or Edge executable. It resolves `BROWSER_EXECUTABLE_PATH`, `--browser <path>`, or common platform install paths. Without `--url`, it temporarily forces `AI_PROVIDER=mock`, starts the Express app on a free local port, verifies React Data Router boot at `/`, starts a scholar game through the real home form, confirms navigation is bound to the current UUID `sessionId`, opens and reloads `/game/:sessionId/map|people|archive|exam|ranking|court|settings`, checks the PixiJS map canvas and manifest-backed portrait pagination, and rejects unsafe `/api/game/state/*` or `/api/dev/*` requests. JSON/SQLite parity and no-browser storage checks now belong to `npm run smoke:dual-mode` / `npm run smoke:dual-mode -- --storage-only`.

The older `scripts/browserSmoke.js` journey remains in the repository as a pre-React migration reference:
