...
- `src/ai/promptContextAssembler.js` owns the S53.1 prompt context assembler. It centralizes dynamic prompt summaries and builds ranked `retrievalContext` from server-visible projections only; S57.1 makes recent-event retrieval and `src/ai/prompts.js` compact `recentEvents` read sanitized `eventArchiveView` items instead of raw `eventHistory` text. It does not read raw hidden ledgers, raw audit sidecars, SQLite audit tables, provider configuration, or local paths.
