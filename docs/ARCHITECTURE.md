...
- Storage: local JSON files under `data/sessions/`. The dynamic database direction is documented in [docs/DYNAMIC_WORLD_DATABASE_PLAN.md](DYNAMIC_WORLD_DATABASE_PLAN.md); JSON remains the default until a storage adapter and SQLite prototype are implemented.
