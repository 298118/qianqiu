...
- `src/game/studyProfile.js` and `src/game/studyProfileConfig.js` own S68.2 读书账本与学业计划. `worldState.studyProfile` is server-owned and normalized on old saves; route payloads expose only `studyProfileView`, and prompt context receives only the capped `studyProfile` summary. Ordinary study-like player actions update the ledger after provider patch boundaries, while exam submission refreshes weaknesses, strengths, teacher advice, book recommendations, and the next plan from saved score/audit history. Providers may read the summary and suggest narrative feedback, but ordinary provider patches cannot write `studyProfile`, exam ranks, appointments, or official posts.
