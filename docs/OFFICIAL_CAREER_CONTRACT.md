...
office-forgery filtering. Browser panel expansion remains S42.3.
...
目标：入仕后不再只是六项仪表变化和偶发升降，而是进入有官职、衙门、差遣、考成、人脉、政敌、弹劾、外放、处分和履历档案的长期生涯。S42.1 定义契约；S42.2 已实现字段、规则和 Mock 行动扩展；S42.3 才扩展浏览器 UI 与验收。
...
S42.2 route payloads already expose `bureau`, `assignmentSummary`, `assignments`, `assessment`, `networkSummary`, and `procedureSummary` from server-built view data without `hiddenNotes`. The current browser panel still renders the compact S34/S42.1 official summary; S42.3 owns the expanded 官署/差事/关系/风险 panels, stable selectors, screenshot checks, and horizontal-overflow acceptance.
