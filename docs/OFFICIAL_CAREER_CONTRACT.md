...
archive. A further “朝议跟进 / 部院覆奏 / 御前摘报 / 考成观察” submission records
server-owned `courtEntryFollowUps` and exposes `latestFollowUp`,
`followUpHistory`, `followUpScenePreview`, `followUpNextActions`, monthly
briefing, event archive `official_court_follow_up`, and world-thread signals.
These records may make bounded progress/assessment adjustments, but they do not
