...
engine. S34 remains the current runtime implementation; S42.1 defines the next
server-owned model for offices, bureaus, assignments, patronage networks,
assessment dossiers, impeachment procedure, transfers, punishment, and browser
archives. Runtime expansion in S42.2/S42.3 must follow the Chinese sections
below.
...
目标：入仕后不再只是六项仪表变化和偶发升降，而是进入有官职、衙门、差遣、考成、人脉、政敌、弹劾、外放、处分和履历档案的长期生涯。S42.1 只定义契约，不改变运行时 schema；S42.2 才实现字段、规则和 Mock 行动扩展，S42.3 才扩展浏览器 UI 与验收。
...
- S42.2 若新增 `worldState.officialCareer` 字段，应通过 feature-level `schemaVersion` 归一化旧存档；不需要提升 JSON storage envelope 版本。
...
S42.2 推荐新增轻量官职目录，而不是把更多中文官名散落在规则里。目录项建议包含：
...
S42.2 可以先用静态 JS 目录，不引入新依赖。AI prompt 读取的是玩家可见摘要，不能读取隐藏弹劾线索、未公开政敌意图或未来考成结果。

### 差遣与考成

差遣是 S42 深度官场的主要长期目标。建议将未来字段落在 `worldState.officialCareer.assignments`，由服务器创建、归一化、推进和归档。差遣条目建议包含：
...
考成不应是单次升迁按钮。S42.2 应建立可解释的考成卷宗，至少综合：
...
S34 已有 `impeachment` 与 `punishment` 结果。S42 后续应把弹劾扩为流程，而不是一次性改职：
...
S42.3 前端只能消费服务器构造的 view，不应扫描 raw `worldState.officialCareer.hiddenNotes` 或 provider 文本。推荐扩展 `officialCareerView`：
...
```json
{
  ...
  "assessment": { "careerScore": 62, "riskScore": 34, "nextReviewInMonths": 5 },
...
玩家可扫描的面板目标是“官署/差事/履历/关系/风险”，每个栏位都应有稳定 DOM selector 或 `data-*` 属性供 browser smoke 验收。隐藏信息只显示为“另有未明”“风闻未定”等摘要。
