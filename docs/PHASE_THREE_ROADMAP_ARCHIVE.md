...
> 归档说明：本文件冻结第三阶段 S30-S39.1 的规划与执行记录。当前活动路线图已切换到 `docs/DEVELOPMENT_STEPS.md` 的第四阶段规划；不要在本归档继续追加新步骤。

本文件曾是 Codex 与 Claude Code 共同维护的第三阶段活动路线图与进度台账；归档后仅作为历史记录保留。
...
Step: DeepSeek provider task-model configuration

Commit: current DeepSeek provider routing commit

Completed:
- Configured the local ignored `.env` for `AI_PROVIDER=deepseek` using the user-supplied DeepSeek key without adding secrets to tracked files.
- Added task-specific DeepSeek model routing in `src/ai/providers/deepseek.js`: V4 Flash remains the fallback/default for ordinary turn/streaming and exam question generation; `DEEPSEEK_OPENING_MODEL=deepseek-v4-pro` and `DEEPSEEK_GRADE_MODEL=deepseek-v4-pro` are used for low-frequency, quality-critical opening and essay grading.
- Added remote-provider normalization that drops malformed display-only `attributeChanges` rows before turn schema validation, after keyed DeepSeek smoke showed V4 Flash sometimes returns loose change-note objects while the authoritative `statePatch` remains valid.
- Tightened the opening prompt so real-provider event strings must include concrete historical anchors for the provider long-run tone gate.
- Updated `.env.example`, README, the development brief, architecture notes, and shared context with the DeepSeek task-model strategy.

Verification:
- `node --check src\ai\providers\deepseek.js`
- `node --check src\ai\providers\remoteHelpers.js`
- `node --check test\deepseekProvider.test.js`
- `node --check test\remoteHelpers.test.js`
- `node --check src\ai\prompts.js`
- `node --check test\prompts.test.js`
- `node --test test\deepseekProvider.test.js test\remoteHelpers.test.js test\aiSchemas.test.js test\providerSmokeScript.test.js`
- `npm run smoke:provider -- --provider deepseek`
- `npm run smoke:provider -- --provider deepseek --stream`
- `npm run smoke:provider:long -- --provider deepseek --turns 3`

Risk/leftover:
- Real DeepSeek keyed smoke now passes for start, turn, turn streaming, question, and grading. A short long-run probe initially showed Flash opening events can be too generic for the historical-anchor gate, so opening was moved to V4 Pro; route-level browser SSE with DeepSeek was not run in this slice.
- The local `.env` contains a real secret and remains ignored by Git.

Next:
- Run full `npm test`, `git diff --check`, and a read-only pre-commit subagent review before committing.
