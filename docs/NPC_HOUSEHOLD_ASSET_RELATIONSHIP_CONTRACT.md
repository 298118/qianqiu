...
S51 是本地动态世界数据库专项的人物域切片。S51.1 先固定 NPC、家族、资产、田产、关系与可见性 schema；S51.2 在此基础上桥接当前 `characters`、`relationshipLedger` 和可见 `activeNpcRequest`。S55.1 在本文固定本地 SQLite 人物域业务表契约，供后续 S55.2/S55.3 持久化与生命周期事件使用。

S51 切片本身不新增 SQLite 业务表，不替换旧 `relationshipView` / `activeNpcRequestView`，也不让 AI 直接创建人物或改写家产。S51.2 新增的 `worldState.worldPeople` 是当前可见旧系统数据的安全 projection，不是隐藏人物私档总库。S53.5 后浏览器“人物谱牒”已经读取 `worldPeopleView`，但仍只读 route player-facing view，不读取 raw ledger。S55.1 仍只定义契约：不改运行时代码、不新增 route 字段、不创建 SQLite 表；默认 JSON 路径和 Mock 可玩性不变。
...
## S55.1 SQLite 人物域业务表契约

S55.1 固定人物域本地 SQLite 表形状；实现留给 S55.2/S55.3。SQLite 表是本机索引、修复和长期检索层，不替代当前 route-facing `worldState` snapshot。因为当前 `GET /api/game/state/:sessionId` 仍返回完整本地 `worldState`，真正 hidden NPC 私档、资产真数、隐藏意图和密札备注不得回写到 `worldState.worldPeople`；后续如要让玩家 API 不再返回 raw state，必须另立 redaction/API 切片。

### 总体原则

- 所有人物域业务表都以 `session_id` 分区；同一 NPC、家族、资产或关系在不同存档中是不同动态行。
- `world_sessions.world_state_json` 仍保存兼容 snapshot。S55.2 前，`worldState.worldPeople` 继续只是可见桥接 projection；SQLite 私档不得借这个字段泄漏到 route。
...
| `source` | `TEXT NOT NULL` | `legacy_bridge` / `server_derived` / `server_event` / `import` / `repair` / `migration`。AI 不可成为 source。 |
...
| `last_event_id` | `TEXT` | 最近关联的安全事件 id；S55.3/S57 可回填。 |
...
- S55.2 读取 SQLite 行时，应从 `world_sessions.world_state_json` / `worldState.worldPeople` 修复缺失或陈旧的可见桥接行；不得从 route raw state 反向制造 hidden 私档。若 raw table 私档行引用损坏，优先降级为 `visibility = hidden` 或裁剪 dangling refs，并记录脱敏 repair 事件。
