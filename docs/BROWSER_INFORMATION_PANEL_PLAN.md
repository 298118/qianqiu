...
S53.3 已完成前端接线基础：`public/app.js` 缓存 S50-S52 相关 view，并在 `#information-panel` 中建立五类 tab 壳和稳定 selector。该壳只显示分类与计数，事件档案 tab 仍禁用；下面的 S53.4-S53.6 继续负责细内容与安全 projection。
...
浏览器已渲染的主要信息面包括：状态栏、身份/书生面板、人脉簿、主动请托、官场档案、世界议程、科期、同场对手、考试弹窗、考试档案、存档簿和 AI 连接面板。尚未独立渲染的是 `worldGeographyView`、`worldEntityView`、`worldPeopleView`、`officialPostingsView` 和 `longTermEventView`。S53 后续面板应补这些 view 的玩家可读入口，同时避免把 `#scholar-panel` 挤成难以扫描的长列表。
...
- 现有 `officialCareerView` 的官场档案仍是任免、差事、考成和履历的第一玩家入口；`officialPostingsView` 后续用于官职簿/任所地理，不替代官场结算。
...
- S53.4：实现“天下格局”和“任所地理”。只读 `worldGeographyView`、`officialPostingsView` 和必要的 `officialCareerView`。补地理 hidden token、role-visible、桌面/移动溢出和 official start 验收。
