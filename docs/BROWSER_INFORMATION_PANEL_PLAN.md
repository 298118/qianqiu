...
`eventArchiveView` 带 `schemaVersion`、`generatedAtTurn`、`dateLabel`、`items[]`、`counts` 和 `hiddenNotice`，并给每条 item 标注 `id`、`sourceType`、`sourceLabel`、`kind`、`title`、`summary`、`year/month/tenDayPeriod/turn`、`dateLabel`、`visibility`、`status`、`statusLabel`。服务器负责 cap、排序、hidden token 过滤和本地路径/密钥脱敏。若未来要把本地 audit 用于事件档案，也必须先转成 sanitized projection，并补分页或检索权限，而不是让浏览器读 JSONL 或 SQLite audit table。
...
- S53.6：已实现事件档案安全 projection 与浏览器面板。`eventArchiveView` 只读 sanitized projection；`#event-archive-panel` 渲染 `.event-archive-item[data-event-id][data-source-type][data-status]`，browser smoke 默认要求事件 tab 启用、条目具备来源/status/日期数据属性，并继续检查 raw audit/proposal/path/key 泄漏、cap、年月旬排序和考试/官场/长期事件合并验收。
