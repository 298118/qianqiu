...
执行 S73.2：新增 `public/assets/ui/ink-ui-manifest.json` 草案和 `docs/FRONTEND_ASSET_LEDGER.md`，把 S73.1 的 usage、尺寸、缩略图、fallback、safeArea、focalPoint、移动裁切、审核状态、许可/来源和敏感字段防线转成可测试资产台账，并为 S73.10 的 300-400 张全量立绘池预留 `portraitRef`、状态变体、缩略图、懒加载和审核字段。S74.0 前不得直接安装 React Router 或其他新依赖；继续保持 JSON/Mock 默认可玩、SQLite local-only、AI proposal-only、server resolver 裁决、地图前端只读安全 view 和完整书生路径不回退。
