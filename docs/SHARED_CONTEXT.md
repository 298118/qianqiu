...
- Storage: `src/storage/sessionStore.js` is the route-facing facade. Default adapter is `src/storage/jsonSessionAdapter.js`, storing JSON session records under `data/sessions/*.json` with schema envelope, redacted metadata, nested `worldState`, atomic temp writes, revision checks, and per-session lock. Optional `src/storage/sqliteSessionAdapter.js` is selected by `STORAGE_ADAPTER=sqlite`, using local `world_sessions` rows and JSON `world_state_json`; S54.2 also syncs normalized geography rows into local `geo_*` tables through `src/storage/sqliteGeographyTables.js`. S55.1 defines future `people_*` SQLite table contracts only; no runtime people tables are created yet. Local audit records use JSON sidecars or SQLite `event_log` / `ai_change_proposals`. JSON remains default. No remote save, account, multiplayer, cloud sync, or hosted DB scope.
- Active roadmap: local dynamic database specialty in `docs/DEVELOPMENT_STEPS.md`. S49-S53 foundation is complete and archived in `docs/LOCAL_DATABASE_FOUNDATION_ARCHIVE.md`; S54 geography split is complete, S55.1 people table contract is complete, and remaining work continues with people persistence, official posting, safe event data, SQLite prompt retrieval, and full dual-mode hardening while preserving JSON/default route contracts.
...
- S55.2: next. Persist visible `worldPeople` bridge rows in SQLite mode and prove JSON/SQLite `worldPeopleView` parity while preserving hidden filtering and the relationship/active-request lifecycle.
...
Start S55.2: persist the visible `worldPeople` bridge rows in SQLite mode and prove JSON/SQLite `worldPeopleView` parity. Keep JSON/default Mock playability, SQLite local-only scope, AI-no-SQL, current relationship/active-request server ownership, and route-view-only browser/prompt surfaces as the baseline.
