...
- Active roadmap: S49-S67 本地数据库与大世界内容已统一归档，S68-S69 科举、读书、评卷、榜单、同年座师、授官和 Provider/Mock 验收已归档。S70.1 AI prompt pack 与工具协议契约已落地，S70.2 actor 权限模型已实现，下一步进入 S70.3 `game_ai_tools` registry/runner。S71 已作为 S70 后的数据库玩法化、维护、安全检索和 redacted API 专项挂入台账，不抢占 S70。
...
启动 S70.3：内部 `game_ai_tools` registry 与 runner。重点新增工具 registry、按 S70.2 actor profile 的 `listToolsForActor()`、read/proposal/request-adjudication runner、Mock fallback、resolver bridge、冷却和 hidden-safe 审计摘要；继续保持 `server.*` 内部化、proposal-only、无 SQL/raw table/raw state patch 和 Mock/no-key 降级边界。
