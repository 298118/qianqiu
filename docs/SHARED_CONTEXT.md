...
- S46: dependency/plugin governance. `docs/DEPENDENCY_PLUGIN_GOVERNANCE.md` is the S46.1 template for dependency/plugin/open-source-reference purpose, license, maintenance, alternatives, testing, docs landing points, and rollback.
- S47.1: provider/browser acceptance expansion around route-level connection tests and explicit keyed health checks. Browser smoke has an opt-in `--check-ai-connection` button check; keyed route health has `npm run smoke:provider:route`. S47.1 deliberately does not add model cost/speed ledgers, per user scope.
- S47.2: DeepSeek context-cache-friendly prompt verification. Prompt packs should maximize stable reusable prefixes and keep dynamic world/player/exam content after the prefix, while never reducing necessary game context or narrative quality. Per latest user scope, S47.2 must not add provider usage cache hit/miss token recording.
...
After S47.2, choose the next `TODO`/`IN_PROGRESS` roadmap step from `docs/DEVELOPMENT_STEPS.md`. Do not reintroduce provider usage cache hit/miss token recording unless a new explicit roadmap step restores that scope.
