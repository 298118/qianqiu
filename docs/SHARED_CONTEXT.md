...
- Latest implementation: S91.6 科举落墨校阅与候榜状态 polish 已完成。科举页新增 `s91-6-exam-writing-reader` 四读校阅，从当前安全考试 view、本地文章字数、route 支持状态、交卷 readiness 和本案卷近次交卷评定派生“试别 / 草稿 / 交卷 / 候榜”读法；本地文章只显示字数，不回显正文，评定只显示清洗后的考试名与分数摘要，榜次、同年、座师与授官仍往皇榜公开 view 细看。仍不新增 route/API/schema/AI 权限/依赖/素材、存档字段、prompt 能力或服务器裁决。
