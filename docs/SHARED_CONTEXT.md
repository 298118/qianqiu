...
- S56.2: persist `officialPostings` into SQLite `office_*` rows while preserving server-owned appointments and scholar -> official continuity.
...
Start S56.2: implement SQLite persistence for `officialPostings` into `office_*` rows. Preserve server-owned appointments, JSON/default Mock playability, route payload shape, one-way `world_state_json -> office_*` repair, hidden geo/people refs filtering, and the complete scholar -> official path.
