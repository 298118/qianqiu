...
- Frontend: 当前发布入口仍是 plain HTML/CSS/JS，无 build step；S74.0 已规划可在依赖治理通过后引入 React + TypeScript + Vite + React Router Data Mode，并按用户最新要求由新前端直接接管默认 `/`，旧 `public/index.html`、`public/app.js` 和 `public/styles.css` 可被替换或删除，不再要求 `/ink-client/` 并行入口、`/legacy.html` 或旧 `/` 保持可用。Backend: Node.js + Express，plain JavaScript。
...
- Roadmap status: S49-S67 本地数据库与大世界内容、S68-S69 科举深化、S70 AI 编排、S71 数据库玩法化和 S72 PixiJS 水墨地图均已完成并归档。当前活动路线进入 S73-S77 前端水墨重构，任务书见 `docs/FRONTEND_INK_REDESIGN_ROADMAP.md`；S73.0a 已把规划细化为素材矩阵、React/Vite 迁移路线、身份/场景专题和小步骤验收，S73.0b 已按用户要求改为 React Router 多页 SPA 并把地图重构为独立“舆图”页，S73.0c 已补齐 S73.10 并改为 S73 内完成 300-400 张全量玩家/NPC 立绘池，S73.0d 已按用户要求移除旧前端保持可用约束，S74 起新 React/Vite 前端直接接管默认 `/`，不再规划 `/ink-client/` 双入口或 `/legacy.html`；S73.1 已新增 `docs/FRONTEND_VISUAL_ASSET_GUIDE.md` 固定前端视觉资产标准，S73.2 已新增 `public/assets/ui/ink-ui-manifest.json` 和 `docs/FRONTEND_ASSET_LEDGER.md` 固定前端素材 manifest/台账 schema，S73.3 已生成并审核 16 个 UI 材质素材，S73.4 已生成并审核 6 个首页资产，S73.5 已生成并审核 10 个场景插画，S73.6 已生成并审核 6 个身份背景，S73.7 已生成并审核 24 张玩家/NPC 立绘风格基准，S73.8 已生成并审核 8 个动效/fallback 素材，S73.9 已新增可复用素材 QA 脚本、预览页和报告，当前覆盖 450 个 active 素材与 19 个透明素材；S73.10.1 已新增全量立绘矩阵，锁定 336 张 planned 立绘和 12 条创意 prompt 母版；S73.10.2 已生成并审核 192 张玩家池立绘（72 张身份阶段 + 60 张女性风格补充 + 60 张男性风格补充），玩家可选池男女各 96 张；S73.10.3 已生成并审核 188 张通用 NPC 立绘，包括一开始的 120 张矩阵主体、20 张旧版 bonus 和 48 张宫装/唐装女性风格扩展。下一步是 S73.10.4 重要 NPC 专属池。S72 归档见 `docs/PIXIJS_INK_MAP_ARCHIVE.md`。
...
执行 S73.10.4：生成重要 NPC 专属立绘池。重要 NPC 不混入通用头像池，未进入公开剧情视野的人物只登记安全 `portraitRef`，不得泄漏 hidden 私档、隐藏动机、未公开任免或未公开关系；继续使用 medium 子代理做创意草案，Codex 负责 prompt 定稿、生成、视觉审核、manifest 入库、QA 和最终提交。
