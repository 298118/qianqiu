...
- Active roadmap: S49-S67 本地数据库与大世界内容已统一归档，S68-S69 科举、读书、评卷、榜单、同年座师、授官和 Provider/Mock 验收已归档。S70.1 AI prompt pack 与工具协议契约已落地，S70.2 actor 权限模型和 S70.3 内部 `game_ai_tools` registry/runner 已实现，下一步进入 S70.4 NPC mind 与记忆基础。S71 已作为 S70 后的数据库玩法化、维护、安全检索和 redacted API 专项挂入台账，不抢占 S70。
...
启动 S70.4：NPC mind 与记忆基础。重点基于 S70.2 actor profile 和 S70.3 runner 增加高显著度 NPC 排序、背景 NPC heuristic、NPC prompt context / proposal 形状、Mock/no-key deterministic fallback 和 hidden-safe 记忆候选；暂不保存深层 private memory，真正记忆 ledger 留给 S70.12 深化。
