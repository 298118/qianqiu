...
执行 S73.5：场景插画包。继续沿用“medium 子代理创意发散 prompt 草案，Codex 负责 prompt 定稿、`gpt-image-2` 生图、视觉审核、压缩、缩略图、manifest/台账入库和提交”的分工，优先生成书斋、贡院号舍、放榜、殿试、县衙、公堂、军帐、御案、城市街巷等 8-12 张场景图。S74.0 前不得直接安装 React Router 或其他新依赖；继续保持 JSON/Mock 默认可玩、SQLite local-only、AI proposal-only、server resolver 裁决、地图前端只读安全 view 和完整书生路径不回退。
