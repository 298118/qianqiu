...
执行 S73.10：在 S73 内完成 300-400 张全量玩家/NPC 立绘池的生成、缩略图、压缩、视觉/安全审核、manifest 入库和台账登记。继续使用 S73.9 的 `npm run qa:frontend-assets:write` 刷新素材 QA 报告，后续 S74-S77 只能通过已审核 `portraitRef`、缩略图和 fallback 使用，不得硬编码路径、显示未审核素材或一次性加载全量池。
