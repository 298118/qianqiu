...
执行 S73.3：在 S73.2 manifest/台账 schema 之上生成首批 UI 材质包，优先覆盖宣纸背景、奏折纸面、边框、墨迹分隔线、印章/朱砂点缀、按钮 hover/active 所需纹理，并逐项完成 Codex 视觉审核、压缩、缩略图或 fallback 记录、manifest 入库和台账登记。S74.0 前不得直接安装 React Router 或其他新依赖；继续保持 JSON/Mock 默认可玩、SQLite local-only、AI proposal-only、server resolver 裁决、地图前端只读安全 view 和完整书生路径不回退。
