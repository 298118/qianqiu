...
- Active roadmap: S49-S67 本地数据库与大世界内容已统一归档，S68-S69 科举、读书、评卷、榜单、同年座师、授官和 Provider/Mock 验收已归档。S70.1 AI prompt pack 与工具协议契约已落地，下一步进入 S70.2 actor 权限模型，再接 S70.3 `game_ai_tools` registry/runner。S71 已作为 S70 后的数据库玩法化、维护、安全检索和 redacted API 专项挂入台账，不抢占 S70。
...
启动 S70.2：AI actor 与权限模型。重点新增 authority tier、actor type、tool group、visibility preset、budget preset 和 actor tool allowlist，先让书生、老师、考官、县令、大臣、将领、皇帝、系统引擎只能列出各自可见工具；继续保持 S70.1 的 prompt/tool 契约、`server.*` 内部化、proposal-only 和 Mock/no-key 降级边界。
