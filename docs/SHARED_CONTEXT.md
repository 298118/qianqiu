...
- Latest implementation: S91.3 主卷行止校阅与快捷建议状态 polish 已完成。主卷案桌新增 `s91-3-main-turn-reader` 四读校阅，从既有 route/session 状态、案主身份、场景、公开卷宗计数、quick action 状态/条数、上一回批和本地草稿长度派生“身份 / 草稿 / 快捷 / 回批”读法；草稿只显示来源与字数，不回显全文。仍不新增 route/API/schema/AI 权限/依赖/素材、存档字段、prompt 能力或服务器裁决。
...
可继续 CSS 预算后续瘦身，或按具体页面/route 新开下一轮 polish 小步骤；若继续主卷/六身份方向，应只围绕现有 role cycle view、quick action 状态、底部奏折、本地草稿和玩家可见安全读法，不新增 provider 能力、存档字段或服务器裁决面。
