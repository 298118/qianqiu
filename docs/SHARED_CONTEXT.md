...
- S65.2 新增的情报传闻是服务器可见 projection：AI 可以读取 `intelligenceRumorView.publicRumors`、顶层 `intelligenceRumors` 摘要与 capped `intel.rumors`，解释角色可见的坊间传闻、衙门案牍、官署奏报、同僚私信、军中侦报、粮道风声、部院奏报、御史风闻或御前摘报；普通 prompt 下顶层摘要和检索摘要都只保留 1 条短线索，高相关 profile 最多保留 4 条。它只提供可信度、来源归因和线索，不得裁决传闻真伪、公开密情、任免、战和、刑赏、财政结算、事件成案、审计或持久化。隐藏情报真值、密档链、raw table、raw audit、provider proposal、完整 prompt、本地路径和 key 不能进入该 view、prompt retrieval 或 SQLite prompt 索引。
- S66.1 新增的大规模 prompt retrieval 策略是只读上下文编排，不是新权限层：AI 可以看到 `retrievalContext.strategy` 中的 profile、排序信号、行/字符预算、候选/选中/丢弃域统计、角色视野和分页边界；显式内部检索源会过滤 hidden canary、raw table/index、审计、本地路径和 key 形状文本。普通/high profile 仍控制在 48/72 行与约 20,000/30,000 字符；AI 仍不得写数据库、裁决事件、任免、战和、财政结算或公开 hidden。
...
- 2026-05-10：S66.1 大规模 prompt retrieval 策略已完成于本次提交。`src/ai/promptContextAssembler.js` 新增 `retrievalContext.strategy`，记录 ordinary/high profile、`maxRows`、`maxChars`、实际选中行数、候选/丢弃域统计、全局裁剪、字符预算裁剪、排序信号、角色视野和分页边界；显式 `promptRetrievalSource` / `retrievalSource` 内部 hook 会过滤 hidden canary、raw table/index 名、审计表名、本地路径、key 形状和密封测试 token，补上 S65.2 复审提出的 source 污染防线。`src/game/worldContentFixtures.js` 的检索行计数纳入 `intel` 域，避免情报传闻漏算预算。普通自由回合仍按 48 行 / 约 20,000 字符，高相关检索仍按 72 行 / 约 30,000 字符；S66.1 不新增 SQLite STRICT 表列、不改变 provider schema、不让 AI 获得数据库写入、事件裁决、任免、战和、财政结算或 hidden 公开权。已通过 `node --check src/ai/promptContextAssembler.js`、`node --check src/game/worldContentFixtures.js`、`node --test test/promptContextAssembler.test.js`、`node --test test/worldContentFixtures.test.js`、`node --test test/promptContextAssembler.test.js test/worldContentFixtures.test.js test/prompts.test.js test/sqlitePromptRetrieval.test.js`、`node --test test/sessionStoreAdapterContract.test.js`。下一步建议启动 S66.2 大数据量浏览器信息面板。
- 2026-05-10：S65.2 情报、传闻与可见性系统已完成于本次提交。新增 `src/game/intelligenceRumorsConfig.js` 与 `src/game/intelligenceRumors.js`，从服务器可见 `worldGeographyView`、`localAffairsDocketView`、`militaryDiplomacyView`、`economicFiscalView`、`worldPeopleView.relationships` 和公开历史事件链派生 `intelligenceRumorView`。同一事实按身份投影为坊间传闻、地方风声、衙门案牍、官署奏报、同僚私信、军中侦报、粮道风声、部院奏报、御史风闻或御前摘报，并带可信度、来源归因、相关 refs 和服务器裁决边界。游戏 start/state/turn/SSE 与考试 question/progress/submit payload 返回 `intelligenceRumorView`；`eventArchiveView` 新增 `intelligence_rumor`；`promptContextAssembler` 新增顶层 `intelligenceRumors` 与 `retrievalContext.intel.rumors`，两者都会随 `promptBudgetProfile` 裁剪，普通 profile 只留 1 条短传闻，高相关 profile 最多 4 条；SQLite `prompt_retrieval_index` 新增 `intel.rumors` compact rows，仍从 `world_sessions.world_state_json -> server views` 单向修复。S65.2 不新增 SQLite STRICT 表列、不新增 provider schema、不让 AI 裁决传闻真伪、公开密情、写审计、改状态或落库。small fixture 的 route-safe prompt 检索总行数增至约 601 行，测试天花板同步调到 625；普通 prompt 仍控制在 48 行 / 约 20,000 字符以内，高相关检索在 72 行 / 约 30,000 字符以内。已通过 focused 情报/事件档案/prompt/SQLite/路由/fixture 测试、`node --test test/sessionStoreAdapterContract.test.js`、docs-governance、documentationGovernance、`git diff --check` 和 `npm test`（505 tests）；提交前只读复审未发现阻断问题，P3 预算口径已补为代码回归。最终复审只余非阻断 P3：显式 `promptRetrievalSource` / `retrievalSource` 是内部可信 hook，后续若暴露给外部输入需加 source 认证或同等敏感词过滤。下一步建议启动 S66.1 大规模 prompt retrieval 策略。
...
Start S66.2 large-scale browser information panels: use S62/S63/S64/S65 projections plus the S60.2 fixture/page helper to build real browser search, filtering, pagination, and hidden-token smoke for city, NPC, office, and event views. S67.1 should then fold S66.1 strategy metadata and S66.2 UI paging into large fixture performance/dual-mode acceptance.
