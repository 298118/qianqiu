...
- Storage: JSON session records under `data/sessions/*.json`, using a `storageSchemaVersion: 1` envelope with redacted metadata, nested `worldState`, atomic temp writes, revision checks, and a per-session local lock file; temp cleanup must respect fresh locks.
- Active roadmap: S48 time-specialty project in `docs/DEVELOPMENT_STEPS.md`; fourth phase is frozen in `docs/PHASE_FOUR_ROADMAP_ARCHIVE.md`.
...
- `docs/QIANQIU_DEVELOPMENT_HISTORY_ARCHIVE.md`、`docs/FOURTH_PHASE_PROGRESS_ARCHIVE.md` 和 `docs/PHASE_FOUR_ROADMAP_ARCHIVE.md` 是按需查阅归档；日常启动跳过，只有追溯旧实现笔记、第四阶段路线图或旧验证细节时再读。
...
After S48.4, begin S48.5 from `docs/DEVELOPMENT_STEPS.md`: audit remaining long-term, official-career, World Threads, World Entities, browser/provider long-run, and wording assumptions that still imply one turn equals one month. Keep development rules unchanged.
