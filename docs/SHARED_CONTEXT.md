...
- Latest implementation: S91.4 人物往来校阅与本地草稿状态 polish 已完成。人物详情工作台新增 `s91-4-people-workbench-reader` 四读校阅，从当前安全人物 view、当前页签、四类本地输入字数、近次公开回批和当前人物明确绑定的公开记录数量派生“照面 / 本地稿 / 回批 / 留痕”读法；草稿只显示字数，不回显对话、报价、命令或交游呈词。仍不新增 route/API/schema/AI 权限/依赖/素材、存档字段、prompt 能力或服务器裁决。
...
可继续 CSS 预算后续瘦身，或按具体页面/route 新开下一轮 polish 小步骤；若继续人物/囊箧方向，应只围绕现有安全人物 view、公开记录、本地草稿字数和玩家可见安全读法，不新增 provider 能力、存档字段或服务器裁决面。
