...
- Active roadmap: S49-S53、S54-S59、S60-S67 已完成并归档。S68.1 科举制度契约、S68.2 读书账本基础、S68.3 老师/书院/同窗互动与 S68.4 科场制度流程已完成；后续应从 S68.5 科场事件与多考官阅卷开始，再推进 S69 榜单、同年/座师和授官深化，最后进入 S70 AI prompt pack、工具协议、actor 权限和多 AI 编排。
...
- 2026-05-11：S68.4 科场制度流程已完成，代码提交 `9a53791`。新增 `src/game/examProcedureConfig.js` 和 `src/game/examProcedure.js`，建立 server-owned `activeExam.procedure` 与 route `examProcedureView`；外层四级考试 API 保持兼容，内部公开摘要展示童试县试/府试/院试三关，乡试/会试三场多卷，殿试御前策问，以及保结、报名、搜检、号舍、发题、草稿、誊清、交卷、弥封、誊录、对读、磨勘、放榜和归档。`/api/game/start`、`/api/game/state/:sessionId`、普通/流式 `/api/game/turn`、`/api/exam/question`、`/api/exam/progress` 和 `/api/exam/submit` 返回 `examProcedureView`；场内推进只同步 procedure phase，不推进全局年月旬；交卷后 `player.examHistory[].examProcedure` 保存安全快照。`src/ai/prompts.js` 和 `promptContextAssembler` 只给 provider capped `examProcedure` 摘要，`stateRules` 明确 `examProcedure` 为 server-owned，公开文本会遮蔽 sealed/internal/key/path/raw-shaped 字符串。浏览器新增“科场程式”面板，考试弹窗与考试档案展示流程/卷件/复核公开摘要，且只读 route view 或历史安全快照；兼容 fallback 已收紧为无 route `examProcedureView` 就不渲染流程。已通过 focused 语法检查、治理检查、diff 检查和相关测试 81/81；提交前只读子代理复审未发现阻断问题，残留风险是 S68.4 文本脱敏仍是模式匹配式，安全性依赖本步不保存 hidden/raw 科场真值，S68.5 引入科场事件和多考官 proposal 时必须继续补 key-shaped 字符串、考官 hidden intent、弥封映射和 raw proposal 红队。本步未保存弥封身份映射、保结密注、考官私意、模型原始建议或内部审计；S68.5 再实现科场事件池与多考官阅卷。

## Next Recommended Step

启动 S68.5：科场事件与多考官阅卷。优先把夹带、病倒、誊录误差、考官偏好、房官/同考官/主考官 proposal 接入服务器 ranking/处罚前的安全流程，并保持 `examProcedureView`、prompt 和浏览器只读脱敏摘要。
