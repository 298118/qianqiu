...
Start S56.1: define the SQLite contract for offices, bureaus, postings, assessments, and transfers. Preserve server-owned appointments, JSON/default Mock playability, SQLite local-only scope, AI-no-SQL/table-write boundaries, and the complete scholar -> official path.
