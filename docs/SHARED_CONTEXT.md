...
- Storage: `src/storage/sessionStore.js` is the route-facing facade. Default adapter is `src/storage/jsonSessionAdapter.js`, storing JSON session records under `data/sessions/*.json` with schema envelope, redacted metadata, nested `worldState`, atomic temp writes, revision checks, and per-session lock. Optional `src/storage/sqliteSessionAdapter.js` is selected by `STORAGE_ADAPTER=sqlite`, using local `world_sessions` rows and JSON `world_state_json`; S54.2 also syncs normalized geography rows into local `geo_*` tables through `src/storage/sqliteGeographyTables.js`, S55.2 syncs normalized visible `worldPeople` bridge rows into local `people_*` tables through `src/storage/sqlitePeopleTables.js`, S55.3 records server people events with local `people_*.last_event_id` links, S56.2/S56.3 syncs normalized safe `officialPostings` projection rows into local `office_*` tables through `src/storage/sqliteOfficialPostingTables.js`, S57.1 syncs sanitized `eventArchiveView` rows into local `event_archive_index` through `src/storage/sqliteEventArchiveTables.js`, and S58.1 syncs compact server-visible prompt retrieval rows into local `prompt_retrieval_index` through `src/storage/sqlitePromptRetrievalTables.js`. SQLite read repair still flows one way from `world_sessions.world_state_json` to derived business rows; S56.3/S57.1/S58.1 add content-hash drift detection for same `row_id` / same revision pollution and old missing-hash rows in `office_*` / `event_archive_index` / `prompt_retrieval_index`. Raw `people_*` / `office_*` / `event_archive_index` / raw business rows are not route state, browser, appointment, audit, or prompt truth sources; prompt retrieval only reads the safe derived index when SQLite read state provides a non-enumerable source. Local audit records use JSON sidecars or SQLite `event_log` / `ai_change_proposals`; S57.2 adds `src/game/auditPublicProjection.js` and `scripts/auditEventArchiveTool.js` for local public-audit status/export without exposing raw audit or proposal content. JSON remains default. No remote save, account, multiplayer, cloud sync, or hosted DB scope.
- Active roadmap: local dynamic database specialty in `docs/DEVELOPMENT_STEPS.md`. S49-S53 foundation is complete and archived in `docs/LOCAL_DATABASE_FOUNDATION_ARCHIVE.md`; S54 geography split, S55 people persistence/event association, S56 official-posting persistence/reference hardening, S57 safe event archive index/pagination plus audit-to-public projection tooling, S58.1 SQLite safe prompt retrieval index, S58.2 browser information-panel JSON/SQLite parity smoke, and S59.1 dual-mode integration acceptance are complete. Remaining work is S59.2 archive/context compression before S60 multi-AI planning.
...
- Audit public projection / 本地审计公开 projection: `src/game/auditPublicProjection.js`, `scripts/auditEventArchiveTool.js`
...
- Session storage facade/adapters: `src/storage/sessionStore.js`, `src/storage/jsonSessionAdapter.js`, `src/storage/sqliteSessionAdapter.js`, `src/storage/sqliteGeographyTables.js`, `src/storage/sqlitePeopleTables.js`, `src/storage/sqliteOfficialPostingTables.js`, `src/storage/sqliteEventArchiveTables.js`, `src/storage/sqlitePromptRetrievalTables.js`, `src/storage/sessionRecord.js`, `src/storage/sessionAudit.js`
...
- S57.2: DONE. `storage:audit-events` can status/export a local public-audit projection from JSON sidecars or SQLite audit tables; it only projects allowlisted public summaries, counts AI proposals without returning them, and does not feed route/prompt/browser/event_archive_index.
- S58.1: DONE. SQLite mode now syncs `prompt_retrieval_index` from `worldGeographyView`、`worldPeopleView`、`officialPostingsView` and `eventArchiveView` compact visible rows; `promptContextAssembler` uses that non-enumerable safe source in SQLite read states while JSON fallback and provider schema stay unchanged.
- S58.2: DONE. `npm run smoke:browser -- --information-parity` now starts JSON/SQLite Mock servers, runs the same official-assignment 局势簿 flow, compares normalized DOM snapshots, route view counts, and paged event archive metadata, and checks hidden raw table/index/audit/prompt tokens.
- S59.1: DONE. `npm run smoke:dual-mode` now chains JSON/SQLite full Mock browser smoke, information-panel parity, and storage/tooling acceptance; `--storage-only` covers JSON -> SQLite dry-run/import, derived table counts, geography repair/export, audit projection, visible payload parity, and hidden-token redaction without a browser.
- S59.2: TODO. Compress S54-S59 completed details into a future archive and trim the active ledger.

## Current Work Note

- 2026-05-08：S59.1 JSON/SQLite 双模式集成硬化已完成于本次提交。新增 `scripts/dualModeAcceptance.js` 与 `npm run smoke:dual-mode`：默认串联 JSON 完整 Mock browser smoke、SQLite 完整 Mock browser smoke、`--information-parity` 局势簿 parity 和 storage acceptance；`--storage-only` 可在无浏览器环境只跑 JSON -> SQLite dry-run/正式导入、派生表计数、地理 drift dry-run/repair/export、JSON/SQLite audit public projection、route/prompt 可见 payload parity 和 hidden-token 防线。`scripts/importJsonSessionsToSqlite.js` 新增 `syncedDerivedTables` 并更新帮助文案，明确导入会通过 SQLite adapter 同步 `geo_*`、`people_*`、`office_*`、`event_archive_index` 和 `prompt_retrieval_index`，保留旧 `syncedGeographyTables` 字段兼容既有脚本。本轮新增 `test/dualModeAcceptanceScript.test.js`，已通过 `node --check scripts\dualModeAcceptance.js test\dualModeAcceptanceScript.test.js scripts\importJsonSessionsToSqlite.js`、`node --test test\dualModeAcceptanceScript.test.js test\browserSmokeScript.test.js test\sqliteGeographyTool.test.js test\auditEventArchiveTool.test.js test\sqlitePromptRetrieval.test.js`（54 tests）、`npm run smoke:dual-mode -- --storage-only`、`$env:AI_PROVIDER='mock'; npm run smoke:dual-mode`（JSON/SQLite 完整 Mock browser journey + 局势簿 parity + storage acceptance）、`npm run check:docs-governance`、`git diff --check` 和 `npm test`（443 tests）。Descartes 与 Hilbert 完成 S59.1 前置只读梳理，均未编辑文件或运行 Git 写入命令；其建议已纳入：新增总验收入口、补 import 派生表口径、保留 information parity 与 full browser journey 的分工、storage-only 不扩大跨域 repair CLI。Hubble 完成提交前只读复审，未发现 P0/P1/P2/P3，确认清理/脱敏/兼容口径/验收覆盖和文档边界无阻塞，且未编辑文件或运行 Git 命令。下一步 S59.2 做 S54-S59 细节再归档与活动台账压缩。
- 2026-05-08：S58.2 浏览器局势簿 JSON/SQLite 双模式 parity smoke 已完成于本次提交。`public/app.js` 为 `#event-archive-panel` 增加稳定分页 `data-*`（page、pageSize、totalItems、totalPages、hasNextPage、pageItemCount），浏览器仍只读 route `eventArchiveView`。`scripts/browserSmoke.js` 新增 `--information-parity`，自动启动 JSON 与 SQLite 两个 Mock 临时服务器，执行同一套 official-assignment 局势簿流程，比较桌面、分页态、移动端 normalized DOM snapshot、route view counts，以及 `eventArchivePageSize=2` 的分页 route metadata；该模式不与 `--url` 或单一 `--storage-adapter` 混用。局势簿 helper 同步扩展 raw `world_state_json`、`event_archive_index`、`prompt_retrieval_index`、`geo_` / `people_` / `office_`、raw audit、provider proposal、prompt、本地路径和 key 的隐藏词防线，并补 event archive grid overflow、parity helper、显式 SQLite DB 归属边界单测。已通过 `node --check public\app.js scripts\browserSmoke.js test\browserSmokeScript.test.js`、`node --test test\browserSmokeScript.test.js`（39 tests）、`$env:AI_PROVIDER='mock'; npm run smoke:browser -- --information-parity`（JSON/SQLite 双服务器，6 screenshots checked）、`npm run check:docs-governance`、`git diff --check`、`npm test`（438 tests）。Bernoulli 完成 S58.2 前置只读梳理，未编辑文件或运行 Git 写入命令；其分页 `data-*`、显式 parity 入口、hidden-token 和 overflow 建议已纳入实现。Carson 完成提交前只读复审，未发现 P0/P1/P2；其两个 P3（architecture 摘要未提 S58.2、显式 SQLite DB 清理边界缺 focused regression）已修正，且该子代理未编辑文件或运行 Git 命令。下一步 S59.1 做 JSON/SQLite 双模式整体验收，覆盖 Mock 主线、导入导出、修复脚本和文档。
- 2026-05-08：S58.1 SQLite 索引驱动的 prompt context 已完成于本次提交。新增 `src/storage/sqlitePromptRetrievalTables.js`，SQLite 模式把 `worldGeographyView`、`worldPeopleView`、`officialPostingsView` 和 `eventArchiveView` 的 compact 可见 projection 同步到 `prompt_retrieval_index`，每行带 `metadata_json.contentHash`，读档按 `world_sessions.world_state_json -> server views -> prompt index` 单向修复缺失、陈旧、错 id 和同 id/同 revision 内容污染。`sqliteSessionAdapter` 在写入、导入、删除和读档修复路径维护该索引，并只给 SQLite 读档后的 `worldState` 挂载非枚举安全来源；route payload、save-list、JSON.stringify、浏览器和存档 JSON 不暴露该来源、数据库路径、raw row 或事件 id。`promptContextAssembler` 在有安全来源时从 prompt index 读取地理、人物、官职任所和近事检索行；JSON/default fallback 继续调用现有 view helper，`retrievalContext` schema、provider schema 和稳定 prompt 前缀不变。新增 `test/sqlitePromptRetrieval.test.js`，覆盖 JSON fallback parity、SQLite 检索一致、hidden token 防泄漏、prompt index 同 id 内容污染修复、缺失行修复、raw `geo_*` / `people_*` / `event_log` / `ai_change_proposals` 不被 prompt 读取。已通过 `node --check src\ai\promptContextSource.js src\ai\promptContextAssembler.js src\storage\sqlitePromptRetrievalTables.js src\storage\sqliteSessionAdapter.js test\sqlitePromptRetrieval.test.js`、`node --test test\sqlitePromptRetrieval.test.js`（3 tests）、`node --test test\promptContextAssembler.test.js test\geographyStorageParity.test.js`（5 tests）、`node --test test\sessionStoreAdapterContract.test.js`（51 tests）、`node --test test\prompts.test.js test\eventArchive.test.js test\auditEventArchiveTool.test.js`（23 tests）。Huygens 完成 S58.1 前置只读梳理，建议使用专用安全派生索引而非直接扫 raw business/audit tables；Locke 完成 S58.2 前置只读梳理，建议后续复用 browser smoke SQLite 参数做双模式 parity。二者均未编辑文件或运行 Git 写入命令。S58.2 已在上方完成记录中接续实现。
- 2026-05-08：S57.2 审计到公开事件 projection 本地工具与脱敏测试已完成于本次提交。新增 `src/game/auditPublicProjection.js`，只把 `visibility: "public"` 的审计摘要经过 `eventArchive` sanitizer 与 related/applied allowlist 投影为本地 public items；developer/private 审计、敏感或空摘要、hidden notes、prompt、path、key、raw state-like 文本均丢弃或遮蔽。新增 `scripts/auditEventArchiveTool.js` 与 `npm run storage:audit-events`，支持 `status` / `export`、`--adapter json|sqlite`、`--session`、`--limit`、`--out` 和 `--db`；JSON sidecar 与 SQLite audit 共用 adapter API，缺失 SQLite DB 不会被 status/export 创建。`ai_change_proposals` 在工具中只计数，不输出 provider proposal 原文；工具输出也不写回 `eventArchiveView`、`event_archive_index`、route state、prompt context 或浏览器信息面板。新增 `test/auditPublicProjection.test.js` 与 `test/auditEventArchiveTool.test.js`，覆盖 JSON/SQLite 双来源、public/developer visibility、缺失 SQLite DB、metadata / dateLabel / createdAt 污染、hidden/path/key/prompt/raw audit token 和 AI proposal 原文防泄漏。已通过 `node --check src\game\auditPublicProjection.js scripts\auditEventArchiveTool.js test\auditPublicProjection.test.js test\auditEventArchiveTool.test.js`、`node --test test\auditPublicProjection.test.js test\auditEventArchiveTool.test.js test\auditRoute.test.js test\eventArchive.test.js test\sessionStoreAdapterContract.test.js`（63 tests）、`npm run check:docs-governance`、`git diff --check`、`npm test`（431 tests）；Mendel 完成前置只读梳理，未编辑文件或运行 Git 写入命令。Popper 首轮提交前只读复审发现 2 个 P2（export metadata 未脱敏、audit `createdAt` 原样输出），均已修复并补 metadata / createdAt 泄漏回归；第二轮基于修复后最终 diff 与验证证据复审未发现 P0/P1/P2。下一步 S58.1 做 SQLite 索引驱动的检索式 prompt context，保持 JSON fallback 与 hidden-row 防泄漏。
- 2026-05-08：S57.1 安全事件索引与事件档案分页已完成于本次提交。`src/game/eventArchive.js` 新增 `buildEventArchiveIndexItems()` 与分页 metadata，默认 `eventArchiveView.items[]` 仍可直接渲染，同时提供 `pagination` / `pageCounts`；`GET /api/game/state/:sessionId` 支持 `eventArchivePage` / `eventArchivePageSize` 查询。新增 `src/storage/sqliteEventArchiveTables.js`，SQLite 模式创建并同步 `event_archive_index`，只保存 `eventArchiveView` 已脱敏的 public projection，并用 `metadata_json.contentHash` 检测缺 hash、错 id、陈旧 revision 或同 id/同 revision 内容污染；读档仍按 `world_sessions.world_state_json -> eventArchiveView -> event_archive_index` 单向修复，不读取 raw `event_log` / `ai_change_proposals`，也不让 raw index row 反向成为 route/prompt/browser 来源。`src/ai/promptContextAssembler.js` 的近事检索和 `src/ai/prompts.js` 顶层 `recentEvents` 都改读安全事件档案条目，`public/app.js` 的事件档案与读档叙事回放也只读 route `eventArchiveView`。已通过 `node --check src\game\eventArchive.js src\storage\sqliteEventArchiveTables.js src\storage\sqliteSessionAdapter.js src\ai\promptContextAssembler.js src\ai\prompts.js src\routes\game.js public\app.js`、`node --test test\eventArchive.test.js test\promptContextAssembler.test.js test\gameTurnEventArchive.test.js test\sessionStoreAdapterContract.test.js test\prompts.test.js test\publicAppSource.test.js`（77 tests）、`npm run check:docs-governance`、`git diff --check`、`npm test`（425 tests）；Meitner 完成前置只读梳理，未编辑文件或运行 Git 写入命令。Planck 首轮提交前只读复审发现 3 个问题（prompt 顶层 raw 近事、读档 raw 叙事回放、README 过期限制），均已修复并补回归；Archimedes 最终只读复审确认代码侧无 P0/P1/P2，并发现 1 个 P2 文档口径问题，已修正 brief 中 S57.1/S57.2 剩余方向。S57.2 已在上方完成记录中接续实现。
...
Start S59.2: archive/compress S54-S59 implementation detail while preserving JSON default, SQLite local-only, AI/server authority, and view-first safety boundaries.
