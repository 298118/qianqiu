...
执行 S73.4：首页资产包。继续沿用“medium 子代理创意发散 prompt 草案，Codex 负责 prompt 定稿、`gpt-image-2` 生图、视觉审核、压缩、缩略图、manifest/台账入库和提交”的分工，优先生成首页山水画卷、云雾层、题名册/户籍册表单底、朱印开始按钮和首页案卷存档素材。S74.0 前不得直接安装 React Router 或其他新依赖；继续保持 JSON/Mock 默认可玩、SQLite local-only、AI proposal-only、server resolver 裁决、地图前端只读安全 view 和完整书生路径不回退。
