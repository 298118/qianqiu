...
Start S57.1: add a safe event index / event archive pagination projection without exposing raw audit, prompt text, provider proposals, local paths, hidden notes, or database internals. Preserve JSON/default Mock playability, route payload safety, and the complete scholar -> official path.
