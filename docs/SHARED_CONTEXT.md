...
- S56.3: continue official-posting city/person cross-reference integrity and safe view degradation.
...
Start S56.3: tighten official-posting cross-domain reference integrity and safe view degradation. Preserve server-owned appointments, JSON/default Mock playability, route payload shape, one-way `world_state_json -> office_*` repair, hidden geo/people refs filtering, and the complete scholar -> official path.
