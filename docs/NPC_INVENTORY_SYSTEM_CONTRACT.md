...
S88.7 后续簿切片新增 `npcActiveRequestView.followUpTasks`。该视图只从已清洗的 resolved active request item 与 `outcome.followUpView` 派生，公开 `taskRoute`、状态、`draftText`、NPC 摘要、证据 refs、风险标签和 proposal-only/server-owned/browser-draft-only 边界；`deferred`、`refused`、`expired` 和仍 active 的来函不得进入后续簿。浏览器“拟后续”只写本地行动草稿，不创建真实任务、不直接结算资源、婚姻、弹劾、背叛、人情债或 hidden 私档。

S88.7 后续登记切片新增 `npc_active_request_follow_up_resolver`。普通回合提交包含“续办 / 后续 / 复核 / 核验 / 登记”等意图且命中安全 `followUpTasks` 的文本时，服务器必须先从 `buildNpcActiveRequestView()` 重建 task evidence，再回找 canonical `npcActiveRequestLedger.activeRequests` 记录并校验 `publicResolutionRef` 与 `serverOwnsFollowUp` 边界。通过后只向该请求的 `outcome.followUpResolutions` 追加公开登记，并同步 `outcome.followUpView.latestResolution`、`auditRefs`、`[NPC 后续]` 事件、turn feedback 的 `npcActiveRequests.outcome.followUpResolved` / `followUpResolutions` 和 `followUpTasks[].latestResolution`。该登记不是资源结算器、婚姻流程、弹劾/处分/定罪流程、背叛事实裁定、人情债月结或真实任务队列；污染输入、ref 不匹配或同旬重复登记不得写入 resolution。

S88.7 后续公开回响切片允许 `eventArchiveView`、`actorMemoryView` 与 `worldThreadView` 只读消费最新安全 follow-up resolution。事件档案必须沿用原主动来函 public ref，展示近次 public summary / next step / status，不以 resolutionId 生成新的真实事件来源；NPC 记忆只接受当前回合 `followUpResolutions`，且必须经 `buildNpcActiveRequestView({ includeResolved: true })` 命中 canonical resolutionId/public ref、原 active-request resolver trace 与 `serverOwnsFollowUp` 边界后，才写 `npc_active_request_follow_up` 可见记忆；world thread 只把最新安全摘要写入 `active_npc_request` 议题。三者均不得回显 raw ledger、inputEcho、provider payload、privateSignalTags、hiddenDossier、safe_search_index、state_patch 等污染别名，也不得暗示资源、婚姻、弹劾、处分、定罪、背叛真相、人情债或真实任务已经结算。

S88.7 后续领域 evidence 切片新增 `npcActiveRequestView.followUpEvidence`。该视图不是新持久账本，只从安全 `followUpTasks`、`latestResolution` 和已清洗边界派生 `items`、`people`、`events`、`economy`、counts 与 safeguards；每条 evidence 必须标明 `sourceType: "npc_active_request_follow_up_evidence"`、公开 `evidenceId` / `sourceId`、taskRoute、evidenceKind、domain/searchDomain、topicSurfaceIds、NPC 公开摘要、publicSummary、nextStep、riskTags/sourceRefs 和 proposal-only/server-owned/read-only boundaries。它可以把引荐拜会/同年师友、人情债月账、请托案牍、献策公开证据、廉政/风宪/背叛风险 watchlist 提供给 resolver input、`memorial-review` / `court-debate` / `trial` / `npc-profile` topic surface、安全搜索和 SQLite `safe_search_index`。这些 evidence 仍只是公开材料和草稿引用，不创建真实任务、不结算银钱/物品/人情债、不写婚姻/关系终局、不成弹劾/处分/定罪、不裁定背叛 hidden truth，也不得绕过服务器后续 resolver。

普通回合、SSE、考试 payload 和 player-state 返回 `npcActiveRequestView`；NPC 详情返回 `relationshipActionEligibilityView`。兼容 `worldState`、redacted player API、SQLite 派生表和 prompt context 必须剥离 raw `npcActiveRequestLedger` 与 NPC 私档，只暴露安全标题、请求类型、NPC 公开名、期限、风险标签、允许回应、服务器边界、公开 outcome、安全 resolver trace 和公开 follow-up resolution 摘要。
