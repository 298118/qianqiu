...
本文承接 S49-S67 本地数据库与超大动态世界内容归档，以及 S70 AI prompt / tool / actor 编排规划。目标是把现有“安全索引 / projection / 局势簿”继续推进为真正驱动玩法的服务器 resolver 输入，同时补齐本地数据库维护、全文检索、redacted API、AI actor 角色卡、压力事件、场景推演、NPC 记忆和 AI 调动可观测性。

## 1. 台账位置建议

本规划不插队 S69。S69 正在做科举、榜单、同年座师和授官深化，当前应继续从 S69.3 往后推进。
...
| ID | 目标 | 主要产物 | 验收重点 |
| --- | --- | --- | --- |
| S71.0 | 专项契约与接入点 | 本文件、brief/context/AI 矩阵补充、resolver 输入清单 | S69 不被改动；明确 S70 依赖和 S71 边界 |
| S71.1 | 数据库作为玩法 resolver 输入 | `resolverInputContext` 契约；城市/国家/NPC/官职/事件/情报 projection 到 resolver 的只读输入 | resolver 不读 raw table；AI 不能写输入；JSON/SQLite 输入 parity |
| S71.2 | 本地 schema migration 与维护层 | `schema_migrations`、维护命令、备份、VACUUM、索引健康、数据库体积提示 | dry-run 不改库；输出不泄漏路径/key/hidden；Windows/Node SQLite 可用 |
...
S71.1 需要新增一个明确的 resolver 输入层，避免各玩法模块随意读取 raw ledger 或 raw SQLite table。
...
S71.2 的维护层建议先做本地命令，不改变默认启动方式：

- `schema_migrations`：记录本地 SQLite schema 版本、迁移 id、应用时间、校验摘要和失败原因。早期 `CREATE TABLE IF NOT EXISTS` 仍保留，但新增/变更表要有 migration 入口。
- `storage:sqlite:status`：输出表数量、索引数量、session 数、派生表漂移摘要、FTS 状态和数据库体积；输出必须脱敏。
- `storage:sqlite:backup`：生成本地 `.bak` 或 `.sqlite.backup`，默认不覆盖；路径只在命令行显示，不进入游戏 API/prompt/browser。
- `storage:sqlite:vacuum`：在显式命令下执行 `VACUUM` / `PRAGMA optimize`，支持 `--dry-run`。
- `storage:sqlite:health`：检查缺表、缺索引、orphan session rows、contentHash 漂移、FTS 行数与 projection 行数是否一致。
- `storage:sqlite:export-safe`：只导出 player-facing projection 和统计，不导出 hidden/raw/provider proposal。

验收至少覆盖 dry-run 不创建数据库、缺 `node:sqlite` 时受控 skip、Windows 路径脱敏、损坏/缺失派生行可从 `world_state_json -> server view` 修复。
...
- 把本文件升级为正式执行契约，确认 S71 不抢 S69、不绕 S70 工具协议。
- 建立 `resolverInputContext` 字段清单、禁止源清单和测试矩阵。
...
- 可选新增 `docs/DATABASE_RESOLVER_INPUT_CONTRACT.md`，如果后续实现前需要更细 schema。
...
- 输出对象按领域分组：`geography`、`people`、`offices`、`economy`、`military`、`events`、`intel`、`player`、`safety`。
...
- 开发诊断 route 必须默认关闭或仅在 `NODE_ENV !== "production"` 且 `ENABLE_DEV_DIAGNOSTICS=true` 时启用；同时限制无 `Origin` 或本机 localhost Origin，复用本地 CORS allowlist，并在测试中覆盖默认不可访问、production 不可访问和开启后仍只返回脱敏统计。
- 短期保留 `GET /api/game/state/:sessionId` 作为开发兼容 route，但文档标注它不能承载 future hidden 私档。
- 浏览器逐步改为优先读取 player-state；debug 面板才读取 diagnostics。
...
- hidden-canary 不进 player-state；diagnostics 只显示脱敏统计；诊断 route 默认关闭、production 关闭、localhost/Origin 门禁生效。
...
- 支持动作：受理、传唤、查证、调解、羁押、驳回、判决、申详、移交。
- 工具建议：
  - `judicial.propose_case_resolution`
  - `judicial.request_case_adjudication`
- 输出后果：公开案牍、关系变化、人情债/怨怼、治安/声望/考成变化、后续事件 hook。
