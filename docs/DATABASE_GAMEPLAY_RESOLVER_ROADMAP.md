...
本文承接 S49-S67 本地数据库与超大动态世界内容归档，以及 S70 AI prompt / tool / actor 编排归档。目标是把现有“安全索引 / projection / 局势簿”继续推进为真正驱动玩法的服务器 resolver 输入，同时补齐本地数据库维护、全文检索、redacted API、AI actor 角色卡、压力事件、场景推演、NPC 记忆和 AI 调动可观测性。S71.0-S71.12 已完成并归档到 [DATABASE_GAMEPLAY_RESOLVER_ARCHIVE.md](DATABASE_GAMEPLAY_RESOLVER_ARCHIVE.md)：resolver 输入边界、只读 `resolverInputContext`、本地 SQLite migration/maintenance、安全全文检索、redacted player API 与本机开发诊断 API、财政/城市政策、刑名、军务外交、压力事件、多 actor 场景运行时、NPC 记忆账本、AI 调动审计面板和验收证据均已落地。后续新步骤应复用该输入层、本地维护工具、玩家/诊断分层、scene-local actor context、actor memory 安全 view、AI 调动安全 view 和 proposal-only 领域工具边界。

## 1. 台账位置建议

本规划最初用于避免 S71 插队 S69/S70；当前 S68-S69 科举深化、S70 AI 编排和 S71 数据库玩法化均已完成并归档。本文件保留为 S71 设计与实现追溯，后续新步骤应复用这里沉淀的输入层、维护层、安全 API、场景/记忆/审计边界，而不是继续把新任务塞进 S71 活动队列。
...
3. **S71.0-S71.12 已完成数据库玩法化专项并归档**：数据库 projection 输入已正式接入财政、地方政策、战役、外交、案件、事件链、全文检索、维护工具和安全 API。

后续若另起新阶段并行开发，只能复用 S71 的稳定边界：文档、维护和检索基础可以作为输入层；任何会调用 AI tool、保存 hidden 私档或改变玩法裁决的实现，仍必须经过 S70 工具协议、actor 权限层、S71 redacted API 和服务器 resolver 裁决边界。
...
| S71.8 | 压力驱动事件生成器 | 已落地：从粮价、水利、腐败、边防、NPC 怨怼、财政缺口生成事件候选 | 服务器成案；冷却/概率配置集中；hidden 不泄漏 |
| S71.9 | 多 actor 场景运行时 | 已落地：朝议、堂审、会盟、战役 scene-local time；多 actor 发言和提案 | 场景不乱推全局旬；每个 actor 只见自身视野；只返回 resolve-only 摘要 |
| S71.10 | NPC 记忆账本 | 已落地：高显著 NPC 目标、人情债、恩怨、家族风险、畏惧、野心；背景 NPC heuristic | 只写安全 actor memory view；不暴露 private/hidden/raw 记忆 |
...
- S71.10 当前实现复用 S70.12 `actorMemoryLedger`，没有新增 raw 私档表：NPC mind 的安全 `memoryCandidates` 会通过服务器 proposal 边界入账；背景 NPC heuristic 只读 `worldPeopleView` 与可见关系行，生成目标、人情债、恩怨、家族风险、畏惧、野心等可见记忆；private/hidden/污染文本、不可见 actor、raw `actorMemoryLedger`、SQLite raw row、prompt retrieval row、本地路径和 key 均不得进入普通 player-state、prompt、安全搜索或场景 context。
...
当前 S71.11 实现采用 `src/game/aiControlAudit.js` 与 `aiControlAuditView`，先复用 bounded `aiInvocationSummaryView`、公开事件档案、可见记忆、官职月报和经历摘要生成玩家/开发双层安全投影；暂不新增 raw audit 浏览器或 SQLite 审计业务表。开发诊断只在 `diagnostics.ai.controlAudit` 返回同一 hidden-safe 摘要。
...
5. S71.12 已完成验收、归档和台账收束；后续开发另起新步骤。
...
状态：已实现。当前代码入口为 `src/game/worldPressureEventConfig.js` 与 `src/game/worldPressureEventGenerator.js`。
...
当前实现写入：受控 `treasury` / `grainReserve` / `publicOrder` / `corruption` / `armyMorale` / `borderThreat` 等 world meters、公开 `eventHistory` 摘要和内部 `worldPressureEventLedger`。生成器只从 S70.6 可见 pressure refs 与 S71.1 `resolverInputContext` 收集脱敏信号，按配置进行组合、优先级、确定性概率、冷却和每旬上限裁决；公开事件只显示可见线索摘要，不公开 sourcePressureRefs、raw proposal、hidden 私档、密情真值或 `server.*`，`worldPressureEventLedger` 不进入 redacted player state。
...
当前实现：`sceneRuntimeConfig` 已定义 `court_debate`、`judicial_hearing`、`diplomatic_summit` 和 `battle_council` 的参与者、允许意见类型、默认动作、证据域和预算；`sceneRuntime` 已实现 scene-local 创建、逐 actor context、Mock/no-key deterministic 场景意见、AI runtime proposal sanitizer、场景意见收集与 resolver outcome 收束。运行时传给 `aiRuntime.generateSceneProposal()` 的只是安全 scene / participant / options 摘要，不传完整 caller options、raw resolver input 或审计上下文；返回 proposal 会被锚定参与者身份、过滤不可见 evidence refs、拒绝 hidden/raw/path/key/server.* 污染、清空模型工具调用/private refs/直写字段，并只桥接到 S71.5-S71.7 resolve-only 接口。`collectSceneProposals()` 只整理 `runSceneRound()` 产物，外部 proposal 仍必须由 `resolveSceneOutcome()` 用完整 scene 重审；运行时不推进全局时间、不写 `worldState`、session、SQLite、审计表或内部 ledger。
...
当前实现：已扩展 S70.12 `actorMemoryLedger`，把 NPC mind 安全候选和背景 NPC heuristic 统一转为服务器 memory proposal。背景 heuristic 只读取 `worldPeopleView` 与可见关系行，跳过 hidden/不可见/死亡 NPC；普通回合通过 `npcMemory.includeBackground` 生成 capped 目标、人情债、恩怨、家族风险、畏惧和野心记忆，并继续走既有 actor 可见性、文本清洗、去重、衰减、prompt cap 和 route response raw ledger redaction。
...
- 当前 S71.10 不新增独立 SQLite 记忆表、浏览器私心面板或 actor-private 记忆；这些若后续实现，必须先补角色视野和 redacted API 分层。
...
- 已新增 `src/game/aiControlAudit.js`：
  - `buildAiControlAuditView(worldState, options)`
  - `buildAiControlAuditDiagnostics(worldState, options)`
- 游戏、考试、SSE、player-state 和 AI 设置 route 已返回 `aiControlAuditView`；开发诊断已并入 `diagnostics.ai.controlAudit`。
- 前端在 `public/app.js` / `public/styles.css` 增加 `#ai-control-audit-panel`，默认显示玩家可见公开结果、工具调用/拒绝摘要和本机安全边界。

后续若需要读取本地审计 sidecar 或 SQLite 审计表，只能在该 view 外另做 allowlist 投影，不得把 raw prompt、raw provider proposal、raw audit row、SQLite rowid、本地路径、key 或 hidden ledger 交给浏览器。

测试：

- `test/aiControlAudit.test.js`
- `test/aiSettingsRoute.test.js`
- `test/redactedState.test.js`
- `test/streamingTurnRoute.test.js`
...
状态：已实现。完成归档见 [DATABASE_GAMEPLAY_RESOLVER_ARCHIVE.md](DATABASE_GAMEPLAY_RESOLVER_ARCHIVE.md)。本步新增 S71 统一归档，回填 JSON/SQLite parity、Mock/no-key、provider smoke、browser smoke、hidden-token、完整书生路径和全量 `npm test` 回归证据，并把 `scripts/dualModeAcceptance.js` 中 S67 large fixture 生成、局势簿 projection 与 SQLite read-repair 性能阈值更新为当前 S71 派生索引规模下的守门值。
...
- `node --test test/worldPressureEventGenerator.test.js test/sceneRuntime.test.js test/actorMemoryLedger.test.js test/aiControlAudit.test.js`
