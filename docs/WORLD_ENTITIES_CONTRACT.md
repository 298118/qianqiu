...
- `recentImpacts` 只保存服务器已应用影响的公开追踪证据，按上限裁剪；它不是新的裁决账本。
...
- `recentImpacts[]`：从已应用实体影响整理出的公开压力留痕，包含 title、publicSummary、实体名、状态/风险标签、受影响指标标签、安全 sourceRef、topicSurfaceIds、relatedRefs/scopeRefs 和发生回合；隐藏实体、hidden/private/raw/provider/prompt/path/key/SQLite/search-index 污染行和密钥片段不会进入该数组。
...
- 只包含可见实体。
- 最多保留 6 个高关注实体。
- 可包含最多 6 条公开 `recentImpacts` 摘要，帮助 AI 追溯实体压力来源。
- 不包含 `hiddenNotes`、隐藏实体、隐藏联系人、provider 配置、raw ledger、SQLite 行、本地路径、key 或原始内部 source refs。

游戏和考试路由返回 `worldEntityView`。Prompt `compactWorldState()` 读取 `worldEntities` 摘要，让 ordinary turn、官场、地方、军事等 prompt pack 能看到制度实体背景。S88.7 起，`worldEntityView.recentImpacts` 还会进入 resolver input 的 `events` evidence、奏折/拟旨/朝议/人物档案 topic surface、安全搜索和 SQLite `safe_search_index` 单向派生；这些出口只读公开 evidence，不新增写 API 或结算权。
...
- `applyWorldEntityInfluences(worldState, influences)`：夹取 metric delta、刷新 `status/publicSummary/lastUpdatedTurn`、写入 `recentNotes` / `recentImpacts` 或 `hiddenNotes`，并返回本回合 `worldEntityImpacts`。
...
- S88.7 关系网压力：canonical 主动来函 follow-up resolution 和已裁决 NPC 关系行动可进入实体压力；已应用影响会同步形成 `recentImpacts`，供后续 resolver/topic/search/SQLite 只读追踪。该证据链仍不裁决资源、人情债、关系终局、婚姻、弹劾、定罪、背叛、NPC 行动或 hidden 私档。
...
- `test/resolverInputContext.test.js`、`test/topicSurfaceView.test.js`、`test/safeWorldSearch.test.js`、`test/sqliteSafeSearch.test.js` 覆盖 `worldEntityView.recentImpacts` 只读 evidence、topic allowlist、安全搜索行和 SQLite 派生行。
