...
S60 内容充实阶段的官署、官职、任所、任命、考成和迁转总量目标见 [S49-S67 本地数据库与大世界内容归档](LOCAL_DATABASE_AND_WORLD_CONTENT_ARCHIVE.md) 中的“S60 内容规模与可见性契约”章节。这些目标表示数据库/fixture 总量，不自动放宽当前 `officialPostings` bundle cap；后续若要承载大规模任命池，应通过安全分页 view、prompt capped summary 或另行调整 schema cap 与测试，不能让 raw `office_*` 行反向决定任免或泄漏 hidden 任所/密札考成。
