...
4. **每个工具必须有权限声明。** 工具定义包含 `readScope`、`proposalScope`、`authorityPredicate`、`resolver`、`auditEventType`、`cooldown`、`riskTags` 和 failure contract。
5. **AI actor 按身份加载工具。** 不把皇帝工具给书生，不把刑名工具给商贾，不把军事调度工具给县令。大工具库后期可用 tool search / MCP allowlist 延迟加载。
6. **大多数 NPC 不每旬全量调用大模型。** 高显著度 NPC、场景参与者、权力核心、玩家近关系和事件相关者使用 LLM mind；背景 NPC 用服务器启发式、记忆摘要和批处理 proposal。
7. **每次高影响工具调用都可追溯。** 记录 actor、可见输入摘要、tool name、arguments 摘要、服务器接受/拒绝原因、applied event id、后果和成本。
8. **Mock/no-key 仍完整可玩。** Mock actor mind、规则式 NPC 和 deterministic tool runner 必须覆盖 S70 主流程，真实 provider 只增强质量。
