...
本文是 S70 的提前规划稿，承接 S60-S67 “超大动态世界数据库内容充实”之后的 AI 深化方向。目标不是让模型绕过服务器直接改库，而是把 AI 从“叙事与评分器”提升为“世界中有身份、有记忆、有权限、有后果的行动者网络”。
...
1. **AI 调工具，服务器执法。** 模型只返回 tool call 或 structured proposal；服务器根据 actor 身份、权限、辖区、证据、冷却、资源和状态边界决定是否执行。
2. **工具不等于数据库写入。** `game_ai_tools` 只暴露 read view、proposal 和 request-adjudication；任何 `worldState`、SQLite 派生表、审计表和 revision 写入仍由服务器模块完成。
3. **每个工具必须有权限声明。** 工具定义包含 `readScope`、`proposalScope`、`authorityPredicate`、`resolver`、`auditEventType`、`cooldown`、`riskTags` 和 failure contract。
4. **AI actor 按身份加载工具。** 不把皇帝工具给书生，不把刑名工具给商贾，不把军事调度工具给县令。大工具库后期可用 tool search / MCP allowlist 延迟加载。
5. **大多数 NPC 不每旬全量调用大模型。** 高显著度 NPC、场景参与者、权力核心、玩家近关系和事件相关者使用 LLM mind；背景 NPC 用服务器启发式、记忆摘要和批处理 proposal。
6. **每次高影响工具调用都可追溯。** 记录 actor、可见输入摘要、tool name、arguments 摘要、服务器接受/拒绝原因、applied event id、后果和成本。
7. **Mock/no-key 仍完整可玩。** Mock actor mind、规则式 NPC 和 deterministic tool runner 必须覆盖 S70 主流程，真实 provider 只增强质量。
...
| ID | 目标 | 主要产物 | 验收重点 |
| --- | --- | --- | --- |
| S70.1 | AI 工具协议契约 | `docs/AI_TOOL_PROTOCOL_CONTRACT.md`、tool envelope schema、proposal/result schema | 模型只能请求工具；服务器执行；schema/strict/fallback 明确 |
| S70.2 | AI actor 与权限模型 | actor profile schema、authority tier、allowed tool groups | 书生/县令/大臣/将领/皇帝工具隔离，hidden 视野隔离 |
| S70.3 | 内部工具运行时 | `game_ai_tools` registry、read/proposal tool runner、audit hook | 无 AI 直写 SQL/table；所有工具有权限、冷却、审计 |
...
- AI proposal 夹带 `worldState` raw patch、SQL、hidden notes、provider key、本地路径或 prompt，必须脱敏并拒绝。
