...
| S89.4 save shelf and archive digest | Verifies the home old-save shelf exposes mutually exclusive loading/empty/error/ready states, safe retry copy, readable counts, and no start-form error pollution even after a failed start; invalid save `sessionId` rows must be disabled without `/game/...` links. The archive route must expose the “案卷索引 / 近次线索” digest, preserve mobile overflow safety, and continue blocking raw/provider/key/path/hidden pollution. |
...
Relevant implementation: S89.4 React 首页旧案状态与史册信息密度 polish。

Commands run during S89.4:
...
npx vitest --config vitest.config.mjs run client/src/__tests__/App.test.tsx --pool=vmThreads --fileParallelism=false --maxWorkers=1 -t "unsupported archive route|S89.4|archive route entries"
...
- Focused React coverage passed for home old-save loading/empty mutual exclusion, save error isolation and safe retry, save retry failure after a start error, invalid save id no-link handling, unsupported archive route recovery, archive digest rendering, and archive route entries.
- `npm run test:client` passed with 6 files / 129 tests. Earlier S89.4 runs on this machine hit the known Vitest fork worker startup timeout, so the suite was also rechecked with `npx vitest --config vitest.config.mjs run --pool=vmThreads --fileParallelism=false --maxWorkers=1`, which passed the same 6 files / 129 tests.
- Governance and full repository gates passed: `npm run check:docs-governance`, `node --test test/documentationGovernance.test.js`, and `npm test` completed with 1160 passing tests.
