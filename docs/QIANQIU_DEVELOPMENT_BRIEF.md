...
- 后端：Node.js + Express，当前以 CommonJS JavaScript 为主；S86 起已建立渐进 TypeScript 检查地基，优先覆盖契约、API/view 类型、安全 projection、AI schema/provider facade、storage adapter 和核心 resolver，不做全仓重写；S87 规划继续覆盖 route/API response shape，先用 contract 与局部 JSDoc typedef，避免大型 route 一次性 whole-file `@ts-check`。
...
S87 规划：后端 route/API 响应类型覆盖已作为下一轮专项，规划见 [TYPESCRIPT_ROUTE_RESPONSE_COVERAGE_ROADMAP.md](TYPESCRIPT_ROUTE_RESPONSE_COVERAGE_ROADMAP.md)。本轮优先覆盖 `src/contracts/serverContracts.ts` 中的 public response contract、`src/routes/game.js` 的 `buildCommonTurnViews` 局部 JSDoc typedef、start/state/player-state/turn、inventory/NPC/trade/delegation、exam route 和 AI route response shape。不得为了启用类型检查一次性 whole-file `@ts-check` 大型 route 文件；所有类型收束都必须继续保护安全 projection、raw ledger 剥离、Ajv/runtime 校验和服务器裁决。
