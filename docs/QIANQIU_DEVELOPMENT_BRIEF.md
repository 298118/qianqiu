...
S52.1 已新增 `docs/OFFICIAL_POSTING_DATABASE_CONTRACT.md` 和 `src/game/officialPostingSchemas.js`。该契约固定未来 `bureaus`、`offices`、`cityJurisdictions`、`postings`、`assessmentRecords`、`transferRecords` 的字段、数值范围、`office_visible` 等可见性规则、hidden notes 边界、nested 引用裁剪和 prompt cap。该步骤不写 `worldState.officialPostings`，不改变 `officialCareerView`，不新增浏览器官职簿/任所地理面板，不创建 SQLite 业务表；普通 provider 夹带 `statePatch.officialPostings` 会被 schema、remote normalization、stateRules、provider long-run 与 red-team 边界拒绝或忽略。S52.2 才进入地方官/入仕官员任所与城市动态指标联动。
