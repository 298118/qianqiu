...
- `roleCycleView`：S88.5 新增六身份循环矩阵与跨域入口。该 view 不新增 persistent ledger，而是从当前 `player.role` 和已有安全 view 即时派生当前身份的本旬事务、风险、待办、AI read scope、工具权限、proposal 边界、服务器裁决说明和六身份矩阵占位；只展开当前身份，非当前身份显示“待任后展开”，避免低权限身份读取其他身份 hidden 军务、任所、私档、raw ledger 或 provider/prompt 细节。S88.5.2 后，该 view 还会为当前身份生成 capped `entryPoints`、`items[].evidenceRefs` 和 `currentRole.evidenceRefs`：地方官可只读引用 `marketPriceView` 与 `npcEconomyView`，将领可只读引用 `mapRuntimeView` 与 `eventArchiveView`。prompt context 只读取 compact ref/label/target summary，不读取地图 layout、layoutPath、mapBounds、assetSetId、viewportHint、坐标或 runtime 原始结构；resolver input 仍只采集 capped 当前身份安全 evidence，避免挤占旧 evidence cap。React `RoleCycleSection` 在六身份主面板只读显示证据与入口；入口只做 route allowlist 跳转或打开本地 surface，草稿按钮写本地行动草稿，不调用 turn API，不裁决身份事务结果。
