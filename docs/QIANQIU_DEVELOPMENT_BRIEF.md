...
S88.4 更新：入仕官员首轮官场体验已完成第一个 coherent slice。新增 `src/game/officialFirstMonthConfig.js` / `src/game/officialFirstMonth.js`，由服务器从 `officialCareer.assignments` 中的首月差事派生 `officialCareerView.firstMonthExperience`，公开官署首月进度、风险档、限期、上官同僚反馈、回署回执、考成信号、下一步草稿建议和权限边界；普通回合推进首月差事时会追加 `[官署回执]` 安全反馈，`summarizeOfficialCareerForPrompt()` 只读取 capped 摘要。`playerMonthlyBriefingView` 的“本职差事 / 上官同僚 / 下月可行”会优先摘录该首月体验，仍不改官职、财政、案牍、军务、NPC、SQLite 或审计表。React `OfficialMinisterPanel` 新增“官署首月”区块，只读 `officialCareerView.firstMonthExperience` 与 `playerMonthlyBriefingView`，按钮只写底部奏折草稿；前端和 AI 都不能直接写差遣成败、任免、考成、弹劾或 hidden 私档。`officialCareer` 公开文本清洗同步收紧，拒绝 provider/proposal/prompt/raw/statePatch/worldState/key/path/SQLite 形态污染后才进入浏览器、prompt 和月报。
...
S76.4 更新：入仕官员/大臣面板已完成。React 主卷在 `player.role === "official"` 或 `player.role === "minister"` 时渲染 `OfficialMinisterPanel`，通过 manifest registry 读取已审核 `role_background` 部院/大臣素材，并只组合安全 `officialCareerView`、`appointmentTrackView`、`officialPostingsView`、`playerMonthlyBriefingView`、`actorMemoryView`、`aiControlAuditView` 和玩家公开摘要。面板显示官职履历、部院公文、同年座师、人脉记忆、派系/朝局风险、考成、弹劾和奏疏入口；S88.4 起，若服务器提供 `officialCareerView.firstMonthExperience`，面板还显示“官署首月”的首月差事、进度风险、回署回执、上官同僚反馈、考成信号和月报摘录提示。按钮只把“整理履历”“查办公文”“拟回堂官”“拜会座师”“探问朝局”“自陈考成”“回应弹劾”“拟具奏疏”等意图写入底部奏折草稿，不提交回合、不调用 resolver、不推进时间、不直接任免、奖惩、处分、弹劾成案或改写考成。官缺、升降、铨选、考成、处分、派系影响、时间推进、首月差事成败和持久化仍由服务器 resolver 与普通回合裁决完成。
