...
S88.4 更新：入仕官员首轮官场体验已完成五个 coherent slice。新增 `src/game/officialFirstMonthConfig.js` / `src/game/officialFirstMonth.js`，由服务器从 `officialCareer.assignments` 中的首月差事派生 `officialCareerView.firstMonthExperience`，公开官署首月进度、风险档、限期、上官同僚反馈、回署回执、考成信号、下一步草稿建议和权限边界；普通回合推进首月差事时会追加 `[官署回执]` 安全反馈，`summarizeOfficialCareerForPrompt()` 只读取 capped 摘要。第二切片新增 `src/game/officialCourtEntryConfig.js` / `src/game/officialCourtEntry.js`，把首月回署材料整理为 `officialCareerView.courtEntry` / `courtEntries`：包含奏折队列、朝议筹议、长期考成 trace、上官同僚后续回响、可引用 sourceRefs 和 draft-only 下一步。第三切片让普通回合识别“入奏折队列 / 付朝议筹议 / 续记考成”等首月回署提交，由服务器写入 `officialCareer.courtEntryResolutions`，并把近次裁决投影到 `officialCareerView.courtEntry.latestResolution`、`courtEntryResolutions`、`eventArchiveView` 的 `official_court_entry` 条目和后续官职月报；裁决结果只包括准入复核、转部核议、留中补查、驳回补据、续入考成等公开状态、受限进度/考成微调和下一步。第四切片新增朝议/部院/御前跟进：普通回合提交“朝议跟进 / 部院覆奏 / 御前摘报 / 考成观察”后，服务器写入 `officialCareer.courtEntryFollowUps`，并把 `latestFollowUp`、`followUpHistory`、`followUpScenePreview` 和 `followUpNextActions` 投影到 `officialCareerView.courtEntry`、prompt 摘要、`eventArchiveView` 的 `official_court_follow_up` 条目、`worldThreadView` 和月报。第五切片新增 `src/game/officialCourtResponseConfig.js` / `src/game/officialCourtResponse.js`、server-owned `worldState.officialCourtResponses` 和 `courtResponseView`：皇帝、大臣、官员或有司可围绕公开奏议写入朱批留览、票拟覆奏、补据、朝议回应或考成观察，普通回合只记录 `[奏议回应记录]` 与 `official_court_response` 中间态；兼容 public `worldState` 不暴露该 raw ledger，事件档案、`worldThreadView`、topic surface 和 React 皇帝/官员面板只读消费。跟进和回应只记录朝议成议、部院待覆、御前留览、补据再议或考成观察等中间反馈、参与 actor 摘要、回应 docket 和受限进度/风险/考成观察，不直接任免、奖惩、处分、拨钱粮、采纳奏折或成弹劾。`topicSurfaceView` 的 `memorial-review`、`edict-draft`、`court-debate` 可从 `officialCareerView.courtEntries` 与 `courtResponseView` 读取这类安全 evidence，`topic_draft` 可引用它拟稿，但仍不执行 resolver、不推进时间、不写 canonical state。React `OfficialMinisterPanel` 新增“官署首月”和“奏折朝议入口”区块，只读 `officialCareerView.firstMonthExperience`、`courtEntry`、近次裁决、朝议/部院/御前跟进、跨身份回应与 `playerMonthlyBriefingView`，`EmperorPanel` 新增“奏议回应”；按钮只写底部奏折草稿，前端和 AI 都不能直接写差遣成败、任免、考成、弹劾或 hidden 私档。`officialCareer` / `officialCourtResponse` 公开文本清洗同步收紧，拒绝 provider/proposal/prompt/raw/statePatch/worldState/key/path/SQLite 形态污染后才进入浏览器、prompt、topic evidence、月报和事件档案。
...
S78 更新：官署专题玩法化已完成。新增统一 `topicSurfaceView` 安全投影，六类专题 surface（奏折队列、拟圣旨、朝议、堂审、军议、人物公开档案）现在都通过 `GET /api/game/topic-surface/:sessionId/:surfaceId` 读取玩家可见材料、证据 ref、人物公开摘要、可选草稿模板和上一轮公开结果。S88.4 起，奏折队列、拟圣旨和朝议可读取 `officialCareerView.courtEntries` 中的首月回署 evidence；该 evidence 会附带近次裁决和朝议/部院/御前跟进摘要，但仍只是安全材料来源，并新增 `official_first_month_memorial` / `official_first_month_debate` 草稿模板。本轮起这些 surface 还可读取跨身份 `courtResponseView.responseItems` / `recentResponses`，把朱批留览、票拟覆奏、补据、朝议回应或考成观察作为 draft-only evidence，不读取 raw `officialCourtResponses` 账本。新增只读 `POST /api/ai/topic-draft/:sessionId`、`topic_draft` AI task、prompt pack、schema、模型路由和 Mock/no-key fallback；AI 只生成标题、草稿正文、引用 evidenceRefs、风险提示和建议下一步，不提交普通回合、不推进时间、不调用 resolver、不写 canonical state。服务端会校验证据白名单、结构化 JSON、污染词、伪造“已裁决/已结案/已任命/已获胜”等成案话术，并在 provider 异常、坏 JSON、越权证据或污染文本时降级本地草稿。React `SurfaceHost` 将六类专题升级为材料/筹议/草稿三栏工作台，玩家可勾选证据、选择批红/拟旨/廷议/首月回署/问案/军议/拜访等草稿模板，请 AI 拟稿、手动改稿并写入底部奏折；真正后果仍只由 `/api/game/turn` 以及 `officialCourtResponse`、`cityPolicyResolver`、`judicialCaseResolver`、`militaryDiplomacyResolver`、`sceneRuntime` 等服务器裁决链处理。
...
- `officialCareer` / `officialCareerView` / `courtResponseView`：服务器拥有官职履历、当前任所、差遣、考成簿、弹劾程序、官场结算和跨身份奏议回应。普通 provider 不能 patch 这些账本、不能改官职/考成/弹劾/差遣成败；`runOfficialCareerStep()` 只按公开行动、世界 tick 和服务器规则推进差遣、履历与结算，`runOfficialCourtResponseStep()` 只把朱批、票拟覆奏、补据、朝议回应或考成观察写成 `officialCourtResponses` 中间态。S88.4 起 `officialCareerView.firstMonthExperience` 会从首月差事派生官署首月进度、风险、限期、回署回执、上官同僚反馈、考成信号和下一步草稿建议；普通回合推进首月差事时生成 `[官署回执]` 反馈，月报读取该安全 view。`officialCareerView.courtEntry` / `courtEntries` 会把首月回署材料、奏折/朝议目标 surface、长期考成 trace、上官同僚后续回响和 draft-only 下一步整理为安全 evidence，供 `topicSurfaceView` 和 `topic_draft` 引用；普通回合提交后，服务器会把准入复核、转部核议、留中补查、驳回补据或续入考成写入 `courtEntryResolutions`，并投影为近次裁决、事件档案和月报材料。朝议/部院/御前跟进由服务器写入 `courtEntryFollowUps`，投影为 `latestFollowUp`、`followUpHistory`、`followUpScenePreview`、`followUpNextActions`、事件档案 `official_court_follow_up`、月报条目和世界议程线索；跨身份回应由 `courtResponseView` 投影为回应 docket、近次回应、事件档案 `official_court_response` 和世界议程线索，兼容 public `worldState` 不暴露 raw `officialCourtResponses`。这些跟进/回应只表示朝议成议、部院待覆、御前留览、补据再议或考成观察，不直接任免、奖惩、处分、拨钱粮、采纳奏折或成弹劾。相关 view 和 prompt summary 会清洗 provider/proposal/prompt/raw/statePatch/worldState/key/path/SQLite 形态污染，不暴露 `hiddenNotes`、raw response ledger 或 hidden 私档。
