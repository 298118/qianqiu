...
- `worldEntities` / `worldEntityView`：朝廷衙门、地方士绅、书院、军镇、盐漕、赈务等制度实体压力。S88.7 起，NPC 主动来函 follow-up 与已裁决 NPC 关系行动也可通过安全 resolver trace 派生公开关系网压力；该派生只接受服务器拥有边界，不读取 raw ledger、provider payload、prompt、SQLite 行、visual-only refs、本地路径或 key。
