...
- S73.10.2：玩家身份阶段立绘池已入库，`public/assets/ui/portraits/s73-10/` 下有 72 张玩家主图，`public/assets/ui/thumbs/` 和 `public/assets/ui/portraits/placeholders/` 分别提供缩略图与低清占位，QA sidecar 为 `public/assets/ui/portraits/portrait-player-pool-qa-v1.json`，校验入口为 `npm run qa:player-portraits`。
