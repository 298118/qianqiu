...
- `roleCycleView`：S88.5.1 新增六身份循环矩阵首片。该 view 不新增 persistent ledger，而是从当前 `player.role` 和已有安全 view 即时派生当前身份的本旬事务、风险、待办、AI read scope、工具权限、proposal 边界、服务器裁决说明和六身份矩阵占位；只展开当前身份，非当前身份显示“待任后展开”，避免低权限身份读取其他身份 hidden 军务、任所、私档、raw ledger 或 provider/prompt 细节。prompt context 只读取 capped 当前身份摘要；resolver input 只采集 `currentRole.items` 作为低量安全 evidence，避免挤占旧 evidence cap；React `RoleCycleSection` 在六身份主面板只读显示并把按钮写成本地行动草稿，不调用 turn API，不裁决身份事务结果。
