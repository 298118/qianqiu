...
当前活动路线图：`docs/DEVELOPMENT_STEPS.md`，已切换为第四阶段规划与进度台账。第四阶段优先处理 AI 连接可见化、优秀提示词、AI 调动/控制适配性审查、官场深度、世界议程、多实体世界模型和依赖/插件治理。
...
S43.1 后，游戏与考试路由会返回服务器归一化的 `worldThreadView`。该视图来自 `worldState.worldThreads`，把主动 NPC 请托、长期事件、官场差遣/结果、角色世界联动、边事、派系压力和地方案链整理为玩家可见的跨月议题摘要；AI 可读 prompt 摘要，但普通 provider 不得通过 `statePatch.worldThreads` 直接写入。
...
- 可观测：DeepSeek adapter 后续应读取 `usage.prompt_cache_hit_tokens` 和 `usage.prompt_cache_miss_tokens`，在 diagnostics/smoke 中报告命中率趋势。
...
第三阶段以“长期模拟骨架、关系可视化、官场结果、科举日历、身份联动和验收硬化”为中心，已完成并归档到 `docs/PHASE_THREE_ROADMAP_ARCHIVE.md`。第四阶段活动台账见 `docs/DEVELOPMENT_STEPS.md`。

第四阶段优先级顺序：

1. AI 连接校验：开局页提供 `POST /api/ai/connection-test`，不落盘检查 provider、模型、streaming 能力和脱敏错误。
2. 提示词与 AI 编排：把世界引擎、科举、官场、地方、军事、皇帝/大臣等提示词拆成 prompt pack，并补离线 eval fixtures。
3. 官场深度：扩展官职谱系、衙门职责、差事、考成、座主/同年/上官/政敌、弹劾、廷推、外放和处分。
4. 世界议程：把主动 NPC、长期事件、官场结果和角色联动整理为跨月议题，让玩家可追踪历史压力和介入点。
5. AI 调动/控制审查：逐系统标明 AI 可生成/可建议/不可裁决的边界，以及服务器裁决点、玩家可见性和 red-team 测试。
...
S43.1 已新增 `docs/WORLD_THREADS_CONTRACT.md` 与 `src/game/worldThreads.js`：`worldState.worldThreads.schemaVersion = 1` 是服务器拥有的议题索引，当前从主动请托、长期事件、官场差遣、官场结果、角色世界联动、边镇压力、派系压力和地方案链派生 `worldThreadView`。它只做归一化、可见性过滤和 prompt/API 摘要，不替代原有结算器；S43.2 再实现前端世界议程检查视图、玩家介入点和后续结算。
...
S11-S38.3 的逐步实现笔记已经迁入 `docs/QIANQIU_DEVELOPMENT_HISTORY_ARCHIVE.md`，避免每次开发启动都读取数百行历史记录。当前必读 brief 只保留产品定位、核心契约、运行规范、AI/服务器边界、第四阶段摘要和验收标准；需要追溯旧阶段细节时再打开归档文件。
