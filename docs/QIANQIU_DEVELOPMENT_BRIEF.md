...
当前活动路线图：`docs/DEVELOPMENT_STEPS.md`，已切换为 S48 时间专项规划与进度台账。时间专项优先处理全局旬制回合、月末大结算、考试等密集场景的局部时间、多系统期限语义和玩家可见“年月旬”展示。
...
6. 服务器拥有时间：`year`、`month`、未来的 `tenDayPeriod` 和场景时间推进都属于服务器裁决；provider 不得通过普通 `statePatch` 写入。
