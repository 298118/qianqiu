...
S53.2 后，浏览器信息面板的规划入口是 `docs/BROWSER_INFORMATION_PANEL_PLAN.md`。该规划不新增 runtime UI，但固定后续天下格局、任所地理、人物谱牒、官职簿和事件档案必须读取 route player-facing view：`worldGeographyView`、`worldPeopleView`、`officialPostingsView`、`worldEntityView`、`worldThreadView`、`longTermEventView`、`officialCareerView` 等，而不是 raw `worldState` ledger 或 S53.1 provider-only `retrievalContext`。事件档案属于高风险面板，必须先由服务器生成 sanitized `eventArchiveView` 或等价 projection，不能直接读取 JSON/SQLite audit payload、provider proposal、prompt、本地路径或 key。

S53.3 后，浏览器会在 `#scholar-panel` 中渲染一个紧凑的 `#information-panel`“局势簿”tab 壳，包含 `#world-geography-panel`、`#posting-geography-panel`、`#world-people-panel`、`#official-postings-panel` 和禁用的 `#event-archive-panel`。`renderWorldState()` 现在缓存 `worldGeographyView`、`worldEntityView`、`worldPeopleView`、`officialPostingsView` 等 route view；该壳只显示分类和计数，不渲染国家/城市/人物/官职细卡，不读取 raw ledger，不把 `retrievalContext` 暴露给 UI，也不提供事件档案内容。

S53.4 后，`#world-geography-panel` 会渲染可见国家、城市、路线、边面和官署辖区的紧凑卡片；`#posting-geography-panel` 会渲染当前任所、公开辖区、地方指标和相关通路。S53.5 后，`#world-people-panel` 会渲染可见人物、家族、资产、田产和关系的 `.world-people-card[data-kind][data-entity-id]`，`#official-postings-panel` 会渲染官署、官职、任命、考成和迁转的 `.official-posting-card[data-kind][data-entity-id]`。这些面板继续只读 route view，不读取 raw `worldState.worldGeography` / `worldState.worldPeople` / `worldState.officialPostings`，不展示隐藏 notes、hidden intent、未公开任所、raw id 文本、provider proposal、prompt、本地路径或 key；事件档案仍不在本步骤展开。
