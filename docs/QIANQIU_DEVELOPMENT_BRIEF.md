...
当前实现中，新生成题目前会先由服务器应用赶考准备：按考试等级扣除盘费，金钱不足时转化为小幅疲劳、心性或声望风险，并写入 `activeExam.entryPreparation`。该步骤不推进 `turnCount`、`year`、`month` 或 `tenDayPeriod`，也不改变晋级规则；已存在的未交卷考试会被复用，不会 retroactively 再扣盘费。
