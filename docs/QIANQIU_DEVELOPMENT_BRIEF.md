...
- S71：数据库玩法化、维护、安全检索和 redacted API，S71.0 契约已固定在 [DATABASE_RESOLVER_INPUT_CONTRACT.md](DATABASE_RESOLVER_INPUT_CONTRACT.md)，S71.1 已落地只读 `resolverInputContext` 输入层，S71.2 已落地本地 SQLite migration/maintenance 初版，S71.3 已落地安全全文检索 / 本地搜索，让安全 projection、本地维护工具和 snippet 搜索成为后续财政、地方政策、案件、军务、外交和事件链 resolver 的基础；下一步补 redacted API、AI actor 场景、NPC 记忆和 AI 调动审计面板。
