...
S81-S85 规划更新：NPC、资产与储物系统专项已加入活动路线图，规划源头见 [NPC_INVENTORY_SYSTEM_ROADMAP.md](NPC_INVENTORY_SYSTEM_ROADMAP.md)。该专项覆盖开局背景兑现、玩家/NPC 资产与资源账本、背包和仓库、重要凭证、阶段 NPC 名册、AI 对话、交易、赠礼、委派任务、长期经济关系、论道/切磋/求爱/婚姻扩展位和总验收。开发顺序必须前后端分离：S81-S83 以后端数据契约、JSON/SQLite 存储、AI schema、Mock fallback、安全 API 和服务器裁决为主；S84 集中接入 React 前端 NPC、背包、仓库、交易和委派体验；S85 再做经济长期演化、预留玩法和总验收。AI 可解析、扮演、提案和回禀，但资产授予、资源扣减、交易成败、物品归属、委派结果、关系落账、科举/官职和持久化仍由服务器拥有。
