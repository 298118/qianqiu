...
- S89.18：科举与皇榜仪式读法 polish 已完成。科举页新增“科举仪程”，按取题启封、场内推进、交卷候批和候榜回音整理当前案卷状态；皇榜页新增“放榜仪程”，按张榜取材、我名、同年座师和授官过渡整理公开榜文读法，并把“防弊检测”改为“弥封复核”。本步零新增 CSS，复用既有科举/皇榜面板、边界列表和状态文案样式。浏览器仍只调用既有科举 API 或读取安全榜文 view，不裁决评分、放榜、晋级、授官、同年座师关系、官职任免或隐藏事实；不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.19：设置与断卷状态读法 polish 已完成。设置目录新增四类工具状态簿，说明推演只改分工、显示只改本地章法、旧案只看公开摘要、案卷摘要不补私记；右上角印匣的推演设置矩阵新增状态簿，并清洗预设、分工 label/purpose/model、状态枚举和错误信息，避免污染 payload/API error 回显 provider 原文、本地路径、key、schema/manifest 或底层诊断；断卷、空卷与畸形主卷恢复页只给安全归路。本步零新增 CSS，复用既有设置卡、状态行、边界列表和案卷空态壳。浏览器仍只打开本地印匣、既有全局 AI settings API 或本地显示偏好；畸形案卷不读取主卷接口、不打开专题层、不写行动草稿；不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.20：前端 CSS 预算瘦身与材质变量清理已完成。全局样式把 `rgba(...)` 改为等价 `rgb(... / ...)`，复用已定义材质变量承载宣纸、折纸、破纸、朱痕、朱印、印匣纹理和墨线背景，合并移动端重复的单列/双列 grid 与竖排 flex 规则，并删除确认无 React 源码引用的 `.actionPanel`；source canary 守住 CSS 体积、禁止旧写法和废弃选择器回归。最终预算回落到 `CSS 99.0 KiB`。本步不改 React 组件行为，不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.21：舆图局势读法与行动线索 polish 已完成。舆图页右侧簿新增“山河局势轴 / 本卷读法”，把当前公开图层、最显著近事、人物视觉锚点、已裁决公开后果和可拟行动数整理为玩家可读读法；“据局势拟稿”只写本地 `map-runtime` 草稿，并继续以当前安全 event/npc refs 作为待服务器重建复核的 hint。坐标、layout、viewport、renderer 画面层级、visual-only effect 和 NPC visual-only anchor 不进入主卷裁决。样式复用既有舆图摘要底座，最终预算保持在 `CSS 99.8 KiB`。本步不新增后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.22：主卷本旬行止笺与草稿状态读法 polish 已完成。主卷右侧案头索引新增“本旬行止笺”，把已载公开卷宗数量、草稿有无、草稿来处和卷宗读法整理成玩家可见状态；草稿来处只显示“手写稿 / 案头摘录 / 舆图摘录 / 史册摘录 / 科举草稿”等中文标签，不展示内部 source enum、草稿全文、`draftContext` 或工程字段。本步零新增 CSS，复用既有主卷侧栏、卷宗格、边界列表和状态行。浏览器仍只消费安全 view、route 状态和本地草稿状态，不裁决资源、关系、交易、NPC 行动、经济、官职、考试、地图行动、婚姻、弹劾、定罪、背叛或隐藏事实；不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.23：囊箧流转候批笺与移置候批读法 polish 已完成。囊箧页新增“流转候批笺”，把资源、资产、物件、凭证数量、可流转物件数、当前移置选择和候批边界整理成玩家可见状态；未获案卷回批前，不写成已入账、已移置、已成交或已影响关系。本步零新增 CSS，复用既有囊箧面板、状态行和边界列表。浏览器仍只消费安全囊箧、资源、资产、经济 trace 和 route-local 选择状态，不裁决资源扣减、物品转移、交易、赠予、借用、NPC 行动、经济、关系、考试、官职或隐藏事实；不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.24：前端 CSS 重复规则折叠与预算缓冲 polish 已完成。全局样式合并舆图右侧簿、四列统计网格和书生 compact 卡片中静态确认重复的基础声明，不删除仍有 React/runtime 源码命中的 class，不触碰画像查看器顺序覆盖依赖；source canary 守住合并 selector 和 `global.css` 源码体积。最终预算回落到 `CSS 99.4 KiB`。本步不改 React 组件行为，不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.25：覆盖层液态玻璃与共享纵深 polish 已完成。印匣抽屉、普通弹窗、本地专题层和高清立绘查看器统一带 `s89-25-liquid-glass` 纵深标记，复用宣纸、印匣和折纸材质加毛玻璃、半透明层、阴影纵深与内沿高光；同步折叠三列统计网格重复 CSS，使最终预算保持在 `CSS 99.5 KiB`。本步不改 React 数据来源，不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.26：人物交游候复笺与案头索引 polish 已完成。人物页顶部新增“交游候复笺 / 人物案头索引”，把入谱人数、相识人物、当前人物页签、来函、后续簿、公开证据、关系网、交游议题、账解数量和本地候复稿有无整理成首屏读法；索引只显示草稿有无，不展示草稿正文、来处枚举、`draftContext` 或内部复核线索。本步零新增 CSS，复用既有人物页材质、标题行、边界列表和状态行；不改 React 数据来源，不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.27 / S89.30：前端 CSS 构建预算校准已完成并由共享材质层继承。S89.27 先把 CSS 总预算/单 CSS asset 上限调整为 `180000` / `128000` bytes，替代 S89 后期只剩数百 bytes 余量的旧单文件硬门；S89.30 在交付产品级共享案卷材质、错落动效、选中朱印、草稿写入反馈和空态纸底后，将 `scripts/clientBuildBudget.js` 的 CSS 总预算/单 CSS asset 上限调整为 `220000` / `200000` bytes。该调整不表示取消预算治理；后续 CSS 继续增长时，应优先做 route/component CSS 拆分、删除无用选择器、合并重复规则或抽出共享样式，再评估是否继续提高硬门。
- S89.28：前端 JS 分包预算稳定已完成。Vite 生产构建用 `manualChunks()` 仅拆分 `node_modules`：React/React Router 为 `vendor-react`，Zustand 为 `vendor-state`，Lucide 为 `vendor-icons`，其余 npm 依赖为 `vendor`，应用源码仍留在主 `index` chunk。该步骤不启用 `React.lazy` 或 route lazy loading，不新增依赖、素材、CSS 或玩家可见 UI，不改浏览器安全 view、草稿、专题层、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决；目的只是守住 `maxSingleJsBytes: 650_000` 的单 chunk 预算并改善 vendor cache 粒度。当前构建预算结果为 `JS 635.0 KiB / CSS 99.5 KiB`，主应用 chunk 约 `358.1 KiB`，`vendor-react` 约 `272.2 KiB`。
- S89.29：史册证据读法与旁注空态 polish 已完成。史册页在“案卷索引 / 近次线索”和公开追踪主列之间新增“史册追索笺 / 史册证据读法”，把近次入册、公开后果、实体余波、来函线索和拟稿候复边界整理成主列、旁注、追索、拟稿四行读法；旁注计数只来自当前案卷安全 `eventArchiveView` counts、`domainConsequenceView`、`npcActiveRequestView.followUpEvidence` 公开计数或分组长度。本步零新增 CSS，复用既有史册/书生卡片、边界列表和状态行；不显示 raw evidence refs、草稿正文、内部枚举或工程词，不改 React 数据来源，不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.30：共享案卷材质与错落动效 polish 已完成。React 壳层带 `data-polish-atmosphere="s89-30-shared-material-motion"`，跨首页、主卷、舆图、人物、囊箧、史册、科举、皇榜、朝议、设置目录和专题层复用宣纸层次、卡片轻浮起、错落进入、朱印选中、草稿已写入状态洗和空态纸底；低动效偏好与系统 `prefers-reduced-motion` 均关闭强动画但保留静态状态反馈。浏览器仍只消费安全 view、route/surface 状态和本地草稿，不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.31：舆图态势罗盘与移动单点札记 polish 已完成。舆图页右侧簿新增“舆图态势罗盘”，按近事、人物、后果、可拟四类整理当前公开图层、NPC visual-only 锚点、公开后果和行动草稿读法；罗盘拟稿只写本地 `map-runtime` 草稿，并继续由服务器从当前 `mapRuntimeView` 重建复核。PixiJS runtime tooltip 新增地点/近事/人物/驿路札记、可见度条、候复边界和移动端底部札记读法；舆图页与 runtime tooltip 的文本/ref sanitizer 同步拦截 `draftContext`、`schema`、`manifest`、`server adjudication`、`AI read scope`、`proposal boundary`、`safe view`、`resolver` 及其 ref 形态。浏览器仍不把坐标、layout、点击位置、route path、visual-only effect、NPC anchor 或 tooltip 变成资源、关系、地图行动、官职、考试或隐藏事实；本步不新增 API、依赖、素材，不改后端 schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.32：首页与全局壳材质动效及入口收束 polish 已完成。首页新开案卷区新增“开卷路径”，把题名、立身、候复三步收进表单读法，并把样卷入口降权为“试阅样卷 / 样卷舆图”；顶栏、主导航、右上角工具区、印匣抽屉/总览/分栏/面板和设置目录新增 S89.32 可检测 marker 与玻璃纸层、卷轴展开、墨线回光、朱印页签反馈。低动效偏好和系统 `prefers-reduced-motion` 均关闭强动画但保留静态层次。设置仍不进入主导航，右上角印匣仍是唯一显眼工具入口，`/game/:sessionId/settings` 继续只是目录/刷新路由；浏览器仍只消费安全 view、route/surface 状态、本地显示偏好和旧案列表，不请求诊断接口、不读取 raw 存档、不裁决资源、身份、交易、NPC 行动、经济、考试、官职、地图行动、关系或 hidden 信息。本步不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.33：科举与皇榜二轮仪式材质动效 polish 已完成。科举页新增“科场仪幕”，按肃场、启封、落墨、候榜整理当前试别、题纸、草稿字数和候榜状态，并给科举 hero、仪幕、试卷纸卷和交卷朱印反馈补 S89.33 marker 与低动效降级；皇榜页新增“金榜仪轨”，按张榜、我名、同年、授官整理公开榜文读法，并给皇榜 hero、榜纸、仪轨和榜行选中态补 S89.33 marker、金光/纸层动效与 `aria-pressed` / `data-selected` 状态。浏览器仍只消费安全 view、既有科举 API、route/local 状态和本地草稿，不请求诊断接口、不读取 raw 存档、不裁决取题、评分、舞弊、放榜、晋级、授官、同年座师关系、官职任免、资源、交易、NPC 行动、经济、关系或 hidden 信息；本步不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.34：主卷与朝议案头中枢二轮材质动效 polish 已完成。主卷新增“案头中枢 / 本卷案桌”，按场景、卷宗、草稿、去处整理当前案卷读法，并把行旅、人物、账解、科举复核公开材料只显示计数和“未载不补造”边界；既有“本旬行止笺”只新增草稿有无状态 marker，不显示草稿正文。朝议页新增“官署议程 / 御案传签”，按章奏、谕旨、朝议、堂审军议说明既有六个专题入口的公开取材和候复边界，畸形案卷仍禁用入口。低动效偏好和系统 `prefers-reduced-motion` 均关闭强动画但保留材质、边框和状态；浏览器仍只消费安全 view、route/local 状态和本地草稿状态，不请求诊断接口、不读取 raw 存档、不裁决资源、身份、交易、NPC 行动、经济、考试、官职、地图行动、关系或 hidden 信息。本步不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.35：人物页与高清立绘查看器二轮高级展示 polish 已完成。人物页新增“人物画屏 / 入谱照面”，按当前案卷公开人物、本页/入谱计数、当前照面、公开小传、题签、高清数、来函证据数、账解数和本地草稿有无整理案读；人物名册、人物谱牒和人物卡新增选中/高清/画像状态 marker。高清立绘查看器新增“画屏案读”栏，复用已清洗的查看器文案显示人物、身份、题签和观画状态；低动效偏好和系统 `prefers-reduced-motion` 均关闭强动画但保留材质、边框、选中态和可读状态。浏览器仍只消费 `worldPeopleView`、`npcRosterView`、`npcDetailView.publicProfile`、既有 follow-up/economy/relationship 安全 view、route/local 状态和已审核 runtime 画像字段，不展示草稿正文、内部来源枚举、完整素材池、全量立绘池、raw 存档、本地路径、source manifest 或完整 manifest，不裁决资源、身份、交易、委派、NPC 行动、经济、考试、官职、地图行动、关系或 hidden 信息。本步不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.36：朝议 / 人物 / 史册跨页追踪读法 polish 已完成。新增共享“跨页追索笺”，在朝议页提示查人物、查史册和回主卷候复，在人物页提示留本页、入朝议、查史册和回主卷候复，在史册页提示留本页、查人物、入朝议和回主卷候复；卡片只整理当前 route session 已公开安全 view、既有页面清洗计数与本地草稿有无，不补造人物、后果、议题或账解。rail 使用折纸材质、错落进入、hover/focus 朱边抬升、移动端单列和低动效关闭守门；unsupported route 链接统一退回首页。浏览器仍不请求诊断接口、不读取 raw 存档、本地路径、source manifest 或完整 manifest，不裁决资源、身份、交易、委派、NPC 行动、经济、考试、官职、地图行动、关系、婚姻、弹劾、定罪、背叛或 hidden 信息。本步不新增依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.37：CSS token 与可访问性渐进重构已完成。全局样式补齐颜色、纸面、边框、间距和动效 token，正文链接恢复默认可识别下划线，按钮/导航型链接由 `a[class]`、`.scholarPanelActions a` 和既有组件类保持 reset；新增 `.paperSurface`、`.rolePanel`、`.statusSurface`、`.ledgerCard` 基础 surface utility，并先接入跨页追索笺。高对比模式同步提高纸面、边框、focus 与按钮/卡片对比；本地低动效和系统 `prefers-reduced-motion` 共用动效变量。CSS 物理拆文件留作后续第二阶段，当前不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.38：CSS 架构重构第二阶段已完成。`client/src/styles/global.css` 保持为 Vite 唯一入口，但物理拆分为 tokens/base/utilities/components/routes/responsive/motion 模块；移动端 760px 覆盖继续拆为 layout、home、game-map、people-inventory、exam-ranking 五个响应式模块。新增状态色、disabled、共享卡片阴影和 disabled surface token，并让高对比模式同步覆盖；共享材质动效样式 hook 改为稳定 `data-material-motion="shared-paper"`，路线图编号 `data-polish-atmosphere` 仅作验证 marker；短编号 keyframes 改为语义名。source canary 和 browser smoke 现在检查拆分入口顺序、合并样式源和稳定 hook。本步不新增依赖或素材，不删除仍有源码命中的 selector，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.39：CSS 语义纸面动效 utility 接入已完成。重复纸面卡、列表项、账册卡和草稿 written 行开始使用 `.paperMotionCard`、`.paperMotionInteractive`、`.paperMotionDraft` 稳定语义 hook，覆盖旧案、舆图、史册、人物、囊箧、来函、账解、皇榜、设置和专题层的低风险重复项；共享材质、高对比和低动效 selector 开始从 route-specific class 收敛到这些 hook。source canary 改为递归解析 CSS import 图，browser smoke 抽样检查语义 hook 与 written draft hook。本步不新增依赖或素材，不删除 selected/empty/跨身份大面板等高风险旧 selector，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.40：selected/empty 语义纸面动效 utility 接入已完成。selected 控件新增 `.paperMotionSelected`，empty/error 状态新增 `.paperMotionEmpty`，覆盖人物名册、囊箧容器、议题草稿槽、印匣页签、人物工作台页签、舆图图层开关、首页旧案空态、跨身份面板空态、快捷建议空态、史册空态、皇榜空态、舆图 runtime/全隐空态和 AI 设置矩阵 error 状态；业务 class、ARIA/checked/data 状态和专用 CSS 均保留。共享 selected/empty 材质、高对比 selected 边框和低动效 selected 动画关闭改读语义 hook，并继续以真实 selected/checked/error 状态门控；source canary 和 browser smoke 守住 semantic hook、数量记录和未选中控件不带 selected 材质。本步不新增依赖或素材，不删除仍有源码命中的 selector，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.41：跨身份大面板语义纸面动效 utility 接入已完成。`.scholarPanelCard` 生成点新增 `.paperMotionPanel`，真正身份面板与身份循环额外新增 `.rolePanel`；领域后果、来函证据、跨页追索和史册证据读法只使用 `.paperMotionPanel`，避免把非身份面板误标为 role panel。共享材质、高对比和低动效 selector 从 `.scholarPanelCard` 与已被语义卡片覆盖的 `.npcRelationshipAgendaCard` 收敛到 `.paperMotionPanel`，route 专用 `.scholarPanelCard` / `.scholarPanelCardHeader` 布局规则继续保留。source canary 和 browser smoke 守住语义 hook、header 不误改、非身份面板不带 rolePanel、运行时书生面板仍有 shared-paper 材质。本步不新增依赖或素材，不删除仍有源码命中的 selector，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.42：静态纸面 surface utility 与结构 selector 收敛已完成。新增 `.paperMotionSurface` 作为只承接共享纸面阴影、不承担入场动画或交互抬升的静态 surface hook；主卷、囊箧、设置、朝议、史册、科举、皇榜、人物、专题层和 AI 设置中的 `.surfaceSafetyList` 行统一接入 `.surfaceSafetyRow paperMotionSurface`，AI 推演分工卡接入 `.aiTaskRoute paperMotionSurface`。共享材质 selector 不再依赖 `.aiTaskRoute`、`.surfaceSafetyList div` 或 `.rankingList button`；皇榜榜行保留 `.paperMotionInteractive`、`aria-pressed` / `data-selected` 和专属金榜选中 CSS，不误接 `.paperMotionSelected`。source canary 和 browser smoke 守住静态 surface 数量、旧结构行缺口、AI 分工卡 hook、皇榜 interactive 与 selected-hook gating。本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.43：皇榜与专题层静态 surface container 语义 hook 接入已完成。皇榜全屏页的 `.rankingNoticeBoard`、`.rankingListPanel`、`.rankingDetailPanel`、`.rankingBoundary` 和专题层/舆图筛选的 `.topicSurfaceColumn`（含 `.topicDraftColumn`）新增 `.paperMotionSurface`，共享材质 selector 不再依赖这些 route-specific class；既有业务 class、ARIA/focus、`data-polish-*`、皇榜榜行 `.paperMotionInteractive` / `data-selected`、专题层材料/证据/草稿和舆图筛选逻辑均保留。source canary 和 browser smoke 守住皇榜四个静态容器、专题层栏目、桌面/移动舆图筛选 hook 数量。本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.44：舆图与史册静态读法 surface utility 接入已完成。舆图页 `.mapSituationLedger`、`.mapVisibleLayerDigest`、`.mapSituationIndex` 与史册页 `.archiveDigestBand`、`.archiveDigestIntro` 新增 `.paperMotionSurface`，共享材质 selector 不再依赖这些 route-specific class，高对比 `.archiveDigestBand` 也改由语义 hook 覆盖；既有业务 class、`data-polish-*`、按钮草稿状态、近次线索、统计、空态、证据读法、跨页追索和移动端布局均保留。source canary 和 browser smoke 守住桌面/移动舆图与史册索引 hook 数量。本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.45：囊箧与账解静态 surface utility 接入已完成。囊箧页 `.inventoryContainerList`、`.inventoryItemList`、三处 `.inventoryLedgerBlock`、两处 `.inventoryTransferPanel` 与账解读法 `.economyTraceSection` 新增 `.paperMotionSurface`，共享材质 selector 不再依赖这些 route-specific class；既有业务 class、容器/物件选择、移置表单、候批笺标记、资源/资产/凭证摘要、经济 trace 证据/边界标记、伪元素、backdrop-filter 和移动端布局均保留。source canary 和 browser smoke 守住桌面/移动囊箧 8 个静态 surface hook，且不引入回合提交、危险 HTML 或 card/panel 误标。本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.46：首页静态 surface utility 接入已完成。首页 `.homeDesk`、`.continueShelf`、`.saveShelf` 新增 `.paperMotionSurface`，共享材质 selector 不再依赖这些 route-specific class；既有业务 class、`data-polish-home-*`、开局表单、旧案读取/刷新、继续本局链接、状态文案、移动端布局和低动效降级均保留。source canary 和 browser smoke 守住默认首页、移动首页和返回首页继续本局路径的静态 surface hook，且不引入回合提交、危险 HTML 或 card/panel 误标。本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.47：主卷静态 surface utility 接入已完成。主卷 `.narrativeScroll`、`.gameSideLedger`、`.openingClaimPanel` 新增 `.paperMotionSurface`，共享材质 selector 不再依赖这些 route-specific class；既有业务 class、`data-polish-game`、`data-draft-state`、案头索引、草稿状态、开局裁决读法、主卷叙事、移动端布局和低动效降级均保留。source canary 和 browser smoke 守住默认 Mock 开局主卷三处静态 surface hook，并在书生面板写入本地草稿后确认 `.gameSideLedger.paperMotionSurface[data-draft-state='written']` 仍存在；本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改回合提交、草稿 `draftContext`、React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.48：人物页静态 surface utility 接入已完成。人物页 `.npcGroupList`、`.npcDetailWorkbench`、`.portraitLedger` 新增 `.paperMotionSurface`，共享材质 selector 不再依赖这些 route-specific class；既有业务 class、`data-polish-people-*`、人物选择、`paperMotionSelected`、`data-gallery-selected`、画像懒加载、人物画屏、交游候复笺、跨页追索、对话/交易/委派/交游 handlers、本地草稿写入、safe view 消费和移动端布局均保留。source canary 和 browser smoke 守住三处人物静态 surface hook 且不误接 card/panel；科举页候选因考试纸卷专属动画、reduced-motion 守门和取题/推进考场/交卷链路留待后续独立审查。本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改 NPC mutation、React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.49：科举页静态 surface utility 接入已完成。科举页 `.examQuestionPanel`、当前/预览 `.examDesk`、两处 `.examRecordPanel`、`.examPeerPanel`、三处 `.examPreviewPanel` 和 `.examRecentSubmitPanel` 新增 `.paperMotionSurface`，共享材质 selector 收敛为 `.paperMotionCard`、`.paperMotionPanel`、`.paperMotionSurface` 三个语义 hook，不再依赖科举 route-specific class；`.examSafetyBoundary` 因承担虚线安全边界读法且不是原 shared-paper 静态材质 selector 成员，不纳入本轮。既有业务 class、`data-polish-exam-ledger`、`data-polish-exam-paper`、取题/推进考场/交卷 handlers、考试 API 调用、反馈拟行动本地草稿、考试纸卷专属 `::before`、展开动画、reduced-motion selector、高对比覆盖和移动端布局均保留。source canary 和 browser smoke 守住科举静态 surface hook、shared selector 纯语义化、三类 exam handler/API 调用和当前试卷材质标记；本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.50：共享纸面动效 keyframe 语义化与 token 化已完成。运行时 CSS 中 S89.30 编号 keyframes 改为 `paperSurfaceRise`、`paperWrittenStateWash`、`paperSelectedSealBloom` 稳定语义名，`draftWrittenPulse` 的朱色 outline 和共享状态洗阴影颜色接入集中 token；`polish-surfaces.css`、browser smoke 与 source canary 同步改读语义 keyframe 名。本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改 React 组件、前端数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.51：共享纸面状态 surface token 化已完成。`tokens.css` 新增 shared-paper selected/written/empty 的朱边、宣纸 inset、状态阴影和状态背景 surface token，`polish-surfaces.css` 的 hover、selected、written、empty 状态改读这些 token，utility 文件不再直接写 `rgb(...)` 色值。source canary 守住 token 定义、状态 utility token 使用、高对比 selected selector 和低动效 shared-paper selector；本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改 React 组件、前端数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.52：共享纸面高对比状态 token 覆盖已完成。高对比模式现在覆盖 shared-paper selected/written/empty 状态所需的朱色 glow、朱边、暖阴影、宣纸 inset 与状态 surface tokens，`--qq-shadow-paper-*` 继续由这些输入 token 组合，不直接重写 composite shadow。source canary 守住高对比 token 覆盖、关键对比数值、shared-paper shadow token 使用、高对比 selected selector 和低动效 shared-paper selector；本步不新增依赖或素材，不删除仍有源码命中的业务布局 selector，不改 React 组件、前端数据流、后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
- S89.17：朝议专题目录与官署案头索引 polish 已完成。朝议页六个专题入口现在逐项显示卷宗取材、可拟草稿、案卷未载不补造和候复边界，入口仍保持唯一按钮并打开既有 `SurfaceHost` 专题层；本步零新增 CSS，复用既有卡片、标签、边界列表和纸按钮样式。浏览器仍只打开本地专题层和写本地草稿，不递交回合、不裁决资源、交易、任免、赏罚、定罪、战和、关系或隐藏事实；不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
