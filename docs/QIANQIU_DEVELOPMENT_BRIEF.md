...
S74.3 更新：已新增 React UI 状态层，`client/src/state/uiState.ts` 管理当前 route page、`sessionId`、安全玩家摘要、drawer/modal/surface、tab、action draft 和 display preferences；`gameSessionState` 在开局、读档、普通回合和交卷成功后同步安全摘要，普通回合成功清空行动草稿。S88.9 起，route-local 专题入口可用 `openSurfaceForSession(surface, sessionId)` 先绑定当前路由案卷并丢弃不匹配的安全摘要，避免朝议、史册、舆图、人物档案和身份循环入口在旧 UI 指针下读取另一案卷 topic surface；该入口只接受可运行 UUID 或明确预览案卷 id，不支持的畸形案卷不会写入 UI session 指针或打开本地 surface。同一阶段 `ActionDraft` 也带 `sessionId`，route/payload/surface 切案卷会清空不匹配草稿和 `draftContext`，主卷提交只消费当前路由案卷草稿。该 store 不保存完整 `worldState`、raw provider payload、hidden ledger、完整 prompt、本地路径或 key；S75.7 起显示偏好通过独立白名单存储层本地持久化，不进入服务器 canonical state。
