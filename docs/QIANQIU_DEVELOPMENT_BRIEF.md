...
- 存储：默认本地 JSON session 文件；S49.3 后可选本地 SQLite session row adapter，S49.4 后有本地事件日志与 AI proposal 审计，便于调试和后续迁移。
...
当前活动路线图：`docs/DEVELOPMENT_STEPS.md`，已切换为本地动态世界数据库专项。数据库方向见 `docs/DYNAMIC_WORLD_DATABASE_PLAN.md`。短期继续默认使用 JSON session 存档；S49.3 已把路由入口 `src/storage/sessionStore.js`、默认 JSON adapter、共享 `sessionRecord` helpers 与可选 SQLite adapter 分开，并让 JSON/SQLite 同跑 storage adapter contract tests。S49.4 已在 adapter contract 上加入本地 `event_log` 与 `ai_change_proposals` 审计：JSON 写 `data/audit/*.jsonl`，SQLite 写本地表，记录模型建议、服务器接受/拒绝和应用事件 id。S50.1/S50.2 已固定天下地理 seed 并新增每局 `worldState.worldGeography`，用过滤后的 `worldGeographyView` 与 capped prompt 摘要提供可见地理上下文。S51.1/S51.2 已新增 `docs/NPC_HOUSEHOLD_ASSET_RELATIONSHIP_CONTRACT.md`、`src/game/worldPeopleSchemas.js` 和 `src/game/worldPeople.js`：先固定 NPC、家族、资产、田产、关系和可见性 schema helper，再把旧 `characters`、`relationshipLedger` 和可见 active request 近期札记桥接为安全 `worldState.worldPeople` / `worldPeopleView` / prompt projection。S52.1/S52.2 已新增 `docs/OFFICIAL_POSTING_DATABASE_CONTRACT.md`、`src/game/officialPostingSchemas.js` 和 `src/game/officialPostings.js`：先固定官署、官职、任所、城市辖区、考成和迁转记录的 future ledger 契约、可见性过滤、prompt cap 与 provider 越权边界，再把官职目录、地方官/入仕官员当前任所和 S50 可见城市/辖区数据桥接为安全 `worldState.officialPostings` / `officialPostingsView` / prompt projection。S53.1 新增 `src/ai/promptContextAssembler.js`，把 prompt 动态上下文集中为服务器可见 projection 摘要与 `retrievalContext` 排序索引；它从 `worldGeographyView`、`worldPeopleView`、`officialPostingsView`、World Entities/Threads、长期事件 view 和可见事件史取材，不读取 raw ledger、raw audit sidecar 或 SQLite 审计表。S53.2 新增 `docs/BROWSER_INFORMATION_PANEL_PLAN.md`，规划天下格局、任所地理、人物谱牒、官职簿和事件档案五类浏览器信息面板，固定它们只能读取 route player-facing view；S53.3 已在 `public/app.js` 缓存 S50-S52 相关 view，并新增浏览器 `#information-panel` tab 壳、五类面板 selector 和 browser smoke 骨架验收；S53.4 已把天下格局与任所地理细面板接入同一壳，只读 `worldGeographyView`、`officialPostingsView` 和可见地理辅助 view；S53.5 已把人物谱牒与官职簿接入同一壳，只读 `worldPeopleView`、`officialPostingsView` 和可见辅助关系/地理 view；S53.6 已新增 `src/game/eventArchive.js` 与 route `eventArchiveView`，并把事件档案接入同一壳，只读公开近事、世界议程、长期事件、官场履历和考试档案整理后的安全 projection。S51.2 桥接仍保留旧字段和旧 route contract；S52.2/S53.4/S53.5/S53.6 不新增 SQLite 业务表或 hidden 人物/官场私档；事件档案不读取 raw audit、provider proposal、prompt、本地路径或 key。后续若要承载大量国家/邻国、城市、NPC、家族、官职任命、地方任所和事件记录，应在该审计底座、地理账本、人物 schema 契约、官职任所契约、S53.1 prompt context assembler 和 S53 浏览器信息面板边界之上逐步拆业务表。当前不规划远程存档、账号体系、多人同步、云端冲突解决或托管数据库。AI 不能直接写数据库或执行 SQL，只能提交结构化建议；服务器继续负责 schema、白名单、clamp、隐藏过滤、科举晋级、官职任免、长期事件、世界实体、世界议程和持久化事务。
...
S51.2 后，人物、家族、资产、田产和关系的未来数据库化入口是 `src/game/worldPeopleSchemas.js`、`src/game/worldPeople.js` 与 `docs/NPC_HOUSEHOLD_ASSET_RELATIONSHIP_CONTRACT.md`。`worldPeopleSchemas` 继续处理 schema bundle 的归一化、可见性过滤、隐藏引用裁剪和 capped prompt 摘要；`worldPeople` 只把当前可见 `characters`、`relationshipLedger` 和 active request 近期札记桥接成安全 `worldState.worldPeople` / `worldPeopleView`。既有 `relationshipView` / `activeNpcRequestView` route contract 不变，active request 生命周期仍由服务器调度、回应和过期；S53.5 浏览器人物谱牒只读 `worldPeopleView` 与可见关系辅助 view，不读 raw `worldState.worldPeople` 或旧 `characters`/`relationshipLedger`。普通 provider 仍不得通过 `statePatch` 写人物库、家族、资产、田产或关系表。

S52.2 后，官职、官署、任所和迁转的当前运行时 projection 是 `src/game/officialPostingSchemas.js`、`src/game/officialPostings.js` 与 `docs/OFFICIAL_POSTING_DATABASE_CONTRACT.md`。`officialPostings` 从 `officialCatalog`、`officialCareer`、地方官 role state 和可见 `worldGeographyView` 派生安全 `worldState.officialPostings` / `officialPostingsView`，并把 capped `officialPostings` 摘要放进 prompt。该 projection 只记录可见官署/官职、城市任所、当前玩家 posting、考成摘要和由服务器 `officialCareer.careerHistory` 派生的迁转；它不改变 `officialCareerView`，S53.4/S53.5 只把它用于任所地理和官职簿查阅，不创建 SQLite 官职业务表，也不允许普通 provider 通过 `statePatch.officialPostings` 写任命、调任、考成或处分。

S53.2 后，浏览器信息面板的规划入口是 `docs/BROWSER_INFORMATION_PANEL_PLAN.md`。该规划固定天下格局、任所地理、人物谱牒、官职簿和事件档案必须读取 route player-facing view：`worldGeographyView`、`worldPeopleView`、`officialPostingsView`、`worldEntityView`、`worldThreadView`、`longTermEventView`、`officialCareerView`、`eventArchiveView` 等，而不是 raw `worldState` ledger 或 S53.1 provider-only `retrievalContext`。事件档案属于高风险面板，S53.6 已由服务器生成 sanitized `eventArchiveView`，不能直接读取 JSON/SQLite audit payload、provider proposal、prompt、本地路径或 key。

S53.3 后，浏览器会在 `#scholar-panel` 中渲染一个紧凑的 `#information-panel`“局势簿”tab 壳，包含 `#world-geography-panel`、`#posting-geography-panel`、`#world-people-panel`、`#official-postings-panel` 和 `#event-archive-panel`。`renderWorldState()` 现在缓存 `worldGeographyView`、`worldEntityView`、`worldPeopleView`、`officialPostingsView`、`eventArchiveView` 等 route view；该壳不读取 raw ledger，也不把 `retrievalContext` 暴露给 UI。

S53.4 后，`#world-geography-panel` 会渲染可见国家、城市、路线、边面和官署辖区的紧凑卡片；`#posting-geography-panel` 会渲染当前任所、公开辖区、地方指标和相关通路。S53.5 后，`#world-people-panel` 会渲染可见人物、家族、资产、田产和关系的 `.world-people-card[data-kind][data-entity-id]`，`#official-postings-panel` 会渲染官署、官职、任命、考成和迁转的 `.official-posting-card[data-kind][data-entity-id]`。S53.6 后，`#event-archive-panel` 会渲染 `.event-archive-item[data-event-id][data-source-type][data-status]`，来源只限服务器整理后的 `eventArchiveView`。这些面板继续只读 route view，不读取 raw `worldState.worldGeography` / `worldState.worldPeople` / `worldState.officialPostings` / raw audit，不展示隐藏 notes、hidden intent、未公开任所、raw id 文本、provider proposal、prompt、本地路径或 key。
...
S49.4 后，开局、普通回合、成功流式回合、考试取题、考试局部推进和交卷评分会写入本地审计记录。普通 provider 的 `statePatch`、关系建议和 `examTrigger` 仍先由服务器裁决；越权 proposal 不写入业务状态，只在 `ai_change_proposals` 中留下脱敏摘要、接受/拒绝原因和应用事件 id。流式叙事如果已经向浏览器发送但最终 provider 失败，仍不保存状态，也不写审计记录。JSON 模式审计 sidecar 是已提交 session 之后的诊断性追加，追加失败不会把已保存回合变成路由失败或触发 SSE 重试；SQLite 模式才提供 session row 与审计表同事务写入。
...
S53.1 把 prompt 动态上下文收束到 `src/ai/promptContextAssembler.js`。`assemblePromptContext()` 继续提供原有 capped 摘要字段，并额外生成 `retrievalContext`：按当前任务、玩家行动和身份视野，从服务器可见 projection 中排序国家/城市/路线/边面、NPC/关系、官署/官职/任所/考成/迁转、World Threads、长期事件、World Entities 和最近可见事件。该 assembler 只读 view / prompt summary helper，不读取 raw `worldState.worldGeography`、raw `worldPeople`、raw `officialPostings`、raw `worldThreads`、raw 审计 sidecar 或 SQLite 审计表；它不改变 provider schema、稳定前缀或服务器裁决权。
...
S51.1 已新增 `docs/NPC_HOUSEHOLD_ASSET_RELATIONSHIP_CONTRACT.md` 和 `src/game/worldPeopleSchemas.js`。该契约固定未来 `npcs`、`households`、`assets`、`estates`、`relationships` 的字段、数值范围、可见性枚举、hidden notes/intent 边界、nested 引用裁剪和 prompt cap。S51.2 在此基础上新增 `src/game/worldPeople.js`：新局写入 `worldState.worldPeople` 的可见桥接层，旧档 route ensure 自动从 `characters` / `relationshipLedger` / active request 回填，游戏/考试/SSE payload 返回 `worldPeopleView`，`compactWorldState()` 加入 capped `worldPeople` prompt 摘要。该桥接不替换旧关系系统，不新增浏览器人物或家产面板，不创建 SQLite 业务表，也不把新增 hidden 人物私档放进 raw route state。

S52.1 已新增 `docs/OFFICIAL_POSTING_DATABASE_CONTRACT.md` 和 `src/game/officialPostingSchemas.js`。该契约固定未来 `bureaus`、`offices`、`cityJurisdictions`、`postings`、`assessmentRecords`、`transferRecords` 的字段、数值范围、`office_visible` 等可见性规则、hidden notes 边界、nested 引用裁剪和 prompt cap。S52.2 已新增 `src/game/officialPostings.js`，把官职目录、当前入仕官员/地方官任所、S50 可见城市/辖区指标和服务器 `officialCareer.careerHistory` 桥接为 `worldState.officialPostings`、`officialPostingsView` 与 capped prompt 摘要。该步骤不改变 `officialCareerView`；S53.4/S53.5 仅把 `officialPostingsView` 用于浏览器任所地理与官职簿查阅，不创建 SQLite 业务表；普通 provider 夹带 `statePatch.officialPostings` 会被 schema、remote normalization、stateRules、provider long-run、audit redaction 与 red-team 边界拒绝、忽略或脱敏。
...
本地数据库专项也必须满足同一边界：默认 JSON/Mock 路径不得被破坏；`sessionStore` facade 的 route payload、revision、legacy 迁移、脱敏存档簿和 `mutateSession` 语义是 SQLite 适配器也必须遵守的 contract。S49.3 的 SQLite 原型只是一行一 session、本地 `world_sessions` 表和 JSON `world_state`，通过 `STORAGE_ADAPTER=sqlite` 可选开启；S49.4 的 `event_log` / `ai_change_proposals` 只记录本地脱敏审计摘要，不进入玩家 API，也不让 AI 直接写表；S50.2 的天下地理账本仍存在 JSON `world_state` 内，浏览器和模型必须读取服务器过滤后的 `worldGeographyView` / prompt summary，而不是 raw ledger；S51.2 的人物桥接账本只保存当前可见 projection，浏览器和模型必须读取 `worldPeopleView` / prompt summary；S52.2 的官职任所桥接账本同样只保存当前可见 projection，浏览器和模型必须读取 `officialPostingsView` / prompt summary；S53.4/S53.5 的浏览器天下格局、任所地理、人物谱牒和官职簿也遵守同一 view-first 边界。数据库只增强本机索引、审计、长期存储、检索式 prompt context 和安全查阅 projection，不把核心裁决交给 AI、SQL 或黑箱库。当前数据库专项不规划远程存档、账号体系、多人同步、云端冲突解决或托管数据库；`session_id` 只表示本机不同存档。
