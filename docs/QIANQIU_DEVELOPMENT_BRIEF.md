...
S70 规划已经扩展为后续 Codex 开发任务书：`docs/AI_ORCHESTRATION_ROADMAP.md` 第 13-16 节逐步列出执行规则、依赖与资料准备、S70.1-S70.14 的前置条件、建议模块/函数、工具/route 接口、测试文件和验收清单。S70 默认优先不新增 npm 依赖，先使用现有 Node/Express/AJV/provider adapters/node:test/browser smoke；若后续需要 MCP SDK、队列、schema 类型库、搜索或可观测依赖，必须先走依赖治理并证明收益。S70.1 已落地契约、schema 与 provider tool smoke；其余任务书列出的模块、脚本和测试仍是后续实施目标，不代表当前运行时都已存在。
...
- S70：AI 提示词、工具协议、actor 权限和多 AI 编排；S70.1 已落 prompt/tool 契约、工具协议 schema 和 MiMo 工具 smoke，后续从 S70.2 actor 权限模型继续。
