...
2. 执行 `git status --short`，确认当前工作树。
3. 判断是否有别人未提交或未说明的改动，不要覆盖。
4. 将本次任务涉及的设计变更写回文档或 README。
5. 保持 Mock 模式可运行。
6. 完成后至少运行与本次改动相关的验证命令。
7. 再次执行 `git status --short`。
8. 用 Git 提交本次 coherent change。
9. 在最终回复中说明改了什么、验证了什么、提交哈希是什么。
