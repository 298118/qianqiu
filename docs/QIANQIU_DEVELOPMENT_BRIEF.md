...
S88.12 provider no-key runtime fallback 补充：真实 provider 仍是可选增强，不得成为本地启动或普通游玩门槛。普通 JSON turn 与 SSE turn 在 `AI_PROVIDER=deepseek` 但缺少 `DEEPSEEK_API_KEY` 时，应经 provider facade 安全回退到 Mock，继续返回安全 route view 和可玩叙事；经济 `draftContext`、topic evidence、canonical echo、角色循环结果和账本结算仍由服务器重新校验与裁决，不能因为 fallback 而跳过 allowlist、污染过滤、重复提交拦截或资源/交易/委派/人情债/关系边界。`/api/ai/connection-test` 的缺 key 503 是诊断语义，故意区别于普通游玩回退，用于提示真实 provider 配置不可用。`scripts/providerToolSmoke.js` 在无 `MIMO_API_KEY` 时先 skip，不要求运行时存在 `fetch`；有 key 但无 fetch 仍失败，以便真实 provider smoke 暴露环境问题。
