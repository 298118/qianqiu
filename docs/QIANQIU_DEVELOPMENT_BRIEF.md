...
- S71：数据库玩法化、维护、安全检索和 redacted API，S71.0 契约已固定在 [DATABASE_RESOLVER_INPUT_CONTRACT.md](DATABASE_RESOLVER_INPUT_CONTRACT.md)；下一步从 S71.1 开始让安全 projection 成为财政、地方政策、案件、军务、外交和事件链 resolver 的输入，后续再补 `schema_migrations`、本地维护、FTS/安全搜索、AI actor 场景、NPC 记忆和 AI 调动审计面板。
...
- [DATABASE_RESOLVER_INPUT_CONTRACT.md](DATABASE_RESOLVER_INPUT_CONTRACT.md)：S71.0 数据库 resolver 输入契约，固定 `resolverInputContext` 字段、允许来源、禁止源、evidence ref、AI 权限和 JSON/SQLite parity 测试矩阵。
