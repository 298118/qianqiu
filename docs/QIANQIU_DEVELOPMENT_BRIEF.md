...
- `GET /api/game/state/:sessionId` 读取开发兼容状态快照和服务器整理后的 route views；可用 `informationTab`、`informationQuery`、`informationFilter`、`informationSort`、`informationPage`、`informationPageSize` 读取局势簿服务器分页。S70.12 起响应 `worldState` 会剥离 raw `actorMemoryLedger` / `sessionSummary`，玩家侧只读 `actorMemoryView` / `sessionSummaryView`；后续如保存 hidden 私档，必须先拆玩家 API 或继续增加 raw state redaction。
...
- S70：AI 提示词、工具协议、actor 权限和多 AI 编排；S70.1 已落 prompt/tool 契约、工具协议 schema 和 MiMo 工具 smoke，S70.2 已落 actor profile 与权限 allowlist，S70.3 已落内部工具 registry/runner，S70.4 已落 NPC mind 基础、启发式 proposal 和可见记忆候选，S70.5 已落朝议/科场制度场景 helper，S70.6 已落压力事件工具协议与 actor proposal，S70.7 已落刑名、财政、军事、外交与科举领域工具，S70.8 已落多模型路由与本地 eval runner，S70.9 已落 AI 设置与可观测性，S70.10 已落玩家官职月报与浏览器面板，S70.11 已落自然语言跳时，S70.12 已落大模型记忆系统，后续从 S70.13 地图系统 AI 接口预留继续。
