...
S88.5-S88.6 更新：S88.5 已建立 `roleCycleView` 六身份循环矩阵，并把地方官市价/人物月账、将领舆图/战事档案入口接入当前身份只读 evidence；S88.5.3 的 `roleCycleDomainAdjudication` 只在普通回合里识别低风险市价处置、侦察和调粮，再交由既有 city/military resolver 裁决。S88.6 首片新增 `domainConsequenceView`，把已由服务器裁决的 `cityPolicyLedger`、`militaryDiplomacyLedger`、`judicialCaseLedger` 和 NPC 经济月账整理为公开领域后果追踪：只暴露来源类型、公开摘要、发生旬、受影响指标标签、风险级别、public refs 和下一步建议，不暴露 resolver outcomeId、证据 refs、`stateDelta` / `playerDelta`、资源消耗、关系信号或审计记录。`domainConsequenceView` 已进入 game/exam route contract，`eventArchiveView` 新增 `domain_consequence` 条目，`worldThreadView` 新增领域后果议题，`playerMonthlyBriefing` 会把近次领域后果纳入 sourceRefs、职责摘要、行动建议和风险提示；兼容 public `worldState`、redacted player state 和 route response helper 现在都会剥离 `judicialCaseLedger`。
...
- `domainConsequenceView`：S88.6 新增的公开领域后果追踪 view，不新增持久账本，只从服务器已裁决的 `cityPolicyLedger`、`militaryDiplomacyLedger`、`judicialCaseLedger` 和安全 `npcEconomyView` 读取公开余波，派生 `city_policy`、`military_diplomacy`、`judicial_case`、`npc_economy` 四类后果。该 view 只暴露公开摘要、来源类型、发生旬、状态、受影响指标标签、风险级别、public consequence refs、AI read scope、工具权限和下一步建议；public sourceId 使用稳定哈希后缀，不复用可能含证据 refs 的 resolver outcomeId。它不公开 `stateDelta` / `playerDelta`、资源消耗、关系信号、evidence refs、auditRecord、raw ledger 名称、本地路径、key、数据库内部行或模型原始提案。game/exam route 返回 `domainConsequenceView`；事件档案以 `domain_consequence` 记录公开后果，`worldThreadView` 生成领域后果议题，官职月报把近次后果作为 sourceRefs、职责摘要、行动建议和风险提示。`judicialCaseLedger` 已进入 public `worldState` raw ledger 剥离清单，后续若接 topic surface、搜索索引或前端专题，只能消费本 view 或对应安全条目。
