...
- S73.10 单张覆盖：`public/assets/ui/portraits/portrait-single-override-qa-v1.json` 当前记录 12 张玩家女性科举阶段单张高清重制覆盖，校验入口为 `npm run qa:single-portrait-overrides`。后续女性立绘重制优先参考压缩 WebP 构图，不上传源图或大图；已重制图减少重复视觉审核，疑似小面积文字风险不阻塞入库。
