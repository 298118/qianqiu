...
- 前端：S74.1 已创建 `client/` React + TypeScript + Vite 默认前端，生产构建输出 `dist/client/` 接管默认 `/`；React Router Data Mode 管理多页 SPA 路由。旧原生 `public/index.html` / `app.js` / `styles.css` 暂留作迁移参考，不再是交付入口。
...
- S73-S77 前端水墨重构：水墨/手绘/宣纸与奏折质感的整体前端重构规划已启动，规划源头见 [FRONTEND_INK_REDESIGN_ROADMAP.md](FRONTEND_INK_REDESIGN_ROADMAP.md)，S73.1 视觉资产指南见 [FRONTEND_VISUAL_ASSET_GUIDE.md](FRONTEND_VISUAL_ASSET_GUIDE.md)，S73.2 前端素材台账见 [FRONTEND_ASSET_LEDGER.md](FRONTEND_ASSET_LEDGER.md)。

当前活动路线图见 [DEVELOPMENT_STEPS.md](DEVELOPMENT_STEPS.md)，数据库方向见 [DYNAMIC_WORLD_DATABASE_PLAN.md](DYNAMIC_WORLD_DATABASE_PLAN.md)。S49-S67 已把本地数据库和“超大动态世界数据库”的内容密度、可见性、prompt 检索、浏览器分页和规模验收收束为统一归档；S68-S69 已完成并归档；S70 AI prompt/tool/actor/多模型路由、AI 设置、月报、跳时、记忆、地图接口、provider AI-first smoke 和 JSON/SQLite parity 已完成并归档到 [AI_ORCHESTRATION_ARCHIVE.md](AI_ORCHESTRATION_ARCHIVE.md)。S71 数据库玩法化、维护、安全检索、redacted player API、财政/刑名/军务外交服务器 resolver、压力驱动事件、多 actor 场景、NPC 记忆账本、AI 调动审计面板和验收结果已归档到 [DATABASE_GAMEPLAY_RESOLVER_ARCHIVE.md](DATABASE_GAMEPLAY_RESOLVER_ARCHIVE.md)，规划源头仍见 [DATABASE_GAMEPLAY_RESOLVER_ROADMAP.md](DATABASE_GAMEPLAY_RESOLVER_ROADMAP.md)，resolver 输入契约仍见 [DATABASE_RESOLVER_INPUT_CONTRACT.md](DATABASE_RESOLVER_INPUT_CONTRACT.md)。S72 PixiJS 水墨地图专项已完成归档，归档见 [PIXIJS_INK_MAP_ARCHIVE.md](PIXIJS_INK_MAP_ARCHIVE.md)；规划源头仍见 [PIXIJS_INK_MAP_ROADMAP.md](PIXIJS_INK_MAP_ROADMAP.md)，运行时契约见 [PIXIJS_INK_MAP_RUNTIME_CONTRACT.md](PIXIJS_INK_MAP_RUNTIME_CONTRACT.md)，素材指南见 [MAP_ASSET_GUIDE.md](MAP_ASSET_GUIDE.md)，素材台账见 [MAP_ASSET_LEDGER.md](MAP_ASSET_LEDGER.md)。S73-S77 已按用户新方向启动并细化前端水墨重构规划，目标是把首页画卷、右上角印匣设置/存档/读档/返回首页、底部奏折输入区、身份专属面板、科举考试/放榜全屏界面、地图舆图页、NPC/玩家立绘 manifest 与视觉审核流程做成完整前端体系。用户已确认旧前端单页壳会让内容过于拥挤，并进一步确认既然是重构，旧前端可以全部抛弃；因此 S74 起采用 React Router 多页 SPA 直接接管默认 `/`，旧 `public/index.html`、`public/app.js` 和 `public/styles.css` 可在实施中替换或删除，不再要求 `/ink-client/` 并行入口、`/legacy.html` 或旧 `/` 保持可用：主叙事页保持清爽，舆图、人物、史册/局势簿、科举考试、放榜、皇帝/朝议或官署公文、设置/存档进入独立页面或全局抽屉。S73.1 已新增视觉资产指南，固定阅读优先、目录命名、页面 usage、颜色材质、文件规格、移动裁切、立绘、动效、fallback、审核状态和 manifest 前置安全字段；S73.2 已新增 `ink-ui-v1` manifest 草案、前端素材台账和 manifest 校验测试，固定审核状态、fallback token、路径策略、usage/role catalog、S73.3-S73.10 planned asset groups，以及 S73.10 全量立绘池的 `portraitRef`、缩略图、低清占位、状态变体、懒加载和审核字段；S73.3 已生成并审核 16 个首批 UI 材质，覆盖宣纸、旧绢、奏折纸面、竹简输入条、卷轴面板、残纸卡片、水渍叠加层、淡墨分隔线、角饰笔触、朱印按钮默认/按下态、印匣纹理、试卷界栏、皇榜纸底、朱砂墨迹和纸页边缘阴影，并写入 `public/assets/ui/materials/`、`public/assets/ui/thumbs/`、manifest 和台账；S73.4 已生成并审核 6 个首页资产，覆盖首页水墨山水画卷、云雾透明层、题名册/户籍册表单底、无字朱印开始按钮、首页案卷存档素材和 reduced-motion 静态底，并写入 `public/assets/ui/home/`、`public/assets/ui/thumbs/`、manifest 和台账；S73.5 已生成并审核 10 个场景插画，覆盖书斋、贡院号舍、放榜、殿试、县衙、公堂、军帐、御案、城市街巷和部院公文，并写入 `public/assets/ui/scenes/`、`public/assets/ui/thumbs/`、manifest 和台账；S73.6 已生成并审核 6 个身份背景，覆盖书生、地方官、入仕官员、大臣、将领和皇帝，并写入 `public/assets/ui/roles/`、`public/assets/ui/thumbs/`、manifest、台账、色彩权重和面板材料建议；S73.7 已生成并审核 24 张玩家/NPC 立绘风格基准，覆盖书生、举人/进士、初任官员、县令、吏员、大臣、将领、皇帝/摄政、老师、考官、女官、宫廷人物、商贾、士绅和市井人物，并写入 `public/assets/ui/portraits/`、`public/assets/ui/thumbs/`、manifest、低清占位、QA sidecar 和台账；S73.8 已生成并审核 8 个动效/fallback 素材，覆盖云雾、墨迹扩散、朱印落章、纸页展开、榜文揭示、交卷朱封、淡墨擦拭和卷边揭页；S73.9 已新增 `scripts/frontendAssetQa.js`、`public/assets/ui/asset-qa-preview.html`、`public/assets/ui/asset-qa-report-v1.json` 和 `qa:frontend-assets`，当前把 618 个 active 素材、19 个透明素材、宣纸/深色合成、chroma-key 色边、路径、尺寸、缩略图、fallback、审核状态和敏感字段检查收束为可重复 QA；S73.10.1 已锁定 336 张全量立绘矩阵，S73.10.2 已生成并审核 192 张玩家池立绘（72 张身份阶段 + 60 张女性风格补充 + 60 张男性风格补充），玩家可选池男女各 96 张；S73.10.3 已生成并审核 188 张通用 NPC 立绘，其中一开始的 120 张矩阵通用 NPC 完整保留，20 张旧版源页作为 bonus 继续使用，48 张宫装/唐装女性风格扩展作为已审核补充池入库；S73.10.4 已生成并审核 72 张重要 NPC 专属立绘，使用 `signature_npc_pool` 与 `portrait_pool_signature_npc_s73_10` 单独隔离，不混入通用头像池；S73.10.5 已生成并审核 72 张状态/姿态与场景锚点立绘，使用 `state_variant_pool`、`scene_anchor_pool`、`portrait_pool_state_variant_s73_10` 和 `portrait_pool_scene_anchor_s73_10` 单独分组；S73.10.7 已生成并审核 24 张年轻成年女性补充立绘，使用 `young_female_style_pool` 与 `portrait_pool_young_female_s73_10_7` 单独分组，Codex 视觉审核确认无中年女性、无发福老态、无中性化；S73.10 全量矩阵计划已完成入库并带有补充池。S74.0 已按治理引入 React + TypeScript + Vite + React Router Data Mode 工具链并新增迁移契约；S74.1 已新增 `client/`、Vite/TS/Vitest 配置、最小多页壳和 Express history fallback，`dist/client/` 接管默认 `/`；S75 实现首页和全局 shell；S76 实现身份/场景专题界面、考试/放榜、独立舆图页和立绘接线，所有人物显示必须通过 S73.10 已审核 `portraitRef`、缩略图和 fallback 使用；S77 做默认入口确认、浏览器、安全、性能、可访问性和归档验收。2026-05-14 起，按用户要求停止与 Gemini CLI 共同开发，后续开发全部由 Codex 负责；Codex 负责后端、前端、素材、审核、验证、文档同步和 Git 提交。AI 生图统一由 Codex 使用 `gpt-image-2` 完成；AI 生成素材和第三方优秀素材入库前，都必须由 Codex 使用视觉能力审核游戏基调、历史/水墨适配、缩放可读性和同批一致性。前端重构中的玩家与 NPC 立绘要求成年、端庄、身份明确、高颜值但不露骨，允许女官和女性 NPC 多样化存在；女性角色可用服饰剪裁、腰封、衣料层次和姿态体现优雅成熟女性身形比例，但不得性化未成年人、不得使用幼态卖点、不得挑逗或过度暴露；状态/姿态和场景锚点女性角色不得中性化，需通过发髻、簪钗、层叠衣料、腰封细腰、肩颈线和端庄姿态表现成熟女性特征；重要 NPC 专属池未进入公开剧情视野的人物只暴露安全 `portraitRef`，不得泄漏 hidden 私档、隐藏动机、未公开任免或未公开关系。真正 hidden 私档、资产真数、隐藏动机、未公开任免、密档事件链和隐藏情报真值仍不回填当前 raw route state；S70.12 已先把原始记忆和经历摘要账本从玩家 route `worldState` 中剥离，S71.4 则提供 future hidden 私档前置的玩家 API 边界。当前不规划远程存档、账号体系、多人同步、云端冲突解决或托管数据库。
...
- `npm run smoke:browser -- --storage-adapter sqlite --sqlite-db <path>` 可让 browser smoke helper 和临时 Mock 服务器共用 SQLite adapter，验证“天下格局/任所地理”等浏览器面板仍只读 route view。S58.2 起，`npm run smoke:browser -- --information-parity` 会顺序启动 JSON/SQLite 临时服务器，比较官方差遣后的局势簿 DOM、route view 摘要、事件档案分页 metadata、hidden-token 与 overflow。S67.1 起，`npm run smoke:dual-mode` 会串联 JSON/SQLite 完整 Mock browser smoke、局势簿 parity、存储维护和 large fixture 规模回归；无浏览器环境可用 `--storage-only` 只跑 JSON -> SQLite dry-run/正式导入、地理修复/导出、审计公开 projection、派生表计数、prompt 策略、局势簿分页、事件档案分页、SQLite 读档修复、内存/耗时门槛和 hidden-token 检查。S67 性能报告中 `rawFixtureGenerationMs` 只表示原始 fixture 数据生成，`fixtureBuildMs` 表示外层构建总耗时且用于报告；完整测试并发下的资源波动不得替代数量、分页、prompt budget、读修复 parity 和防泄漏硬门。
...
- S74.1：Vite/TypeScript 默认前端已完成，新增 `client/`、Vite/TS/Vitest 配置、最小 React Router Data Mode 多页壳、Express history fallback、S74.1 focused browser smoke 和 `prestart` 构建入口；`dist/client/` 接管默认 `/`。真实 start/load/turn/exam/AI settings API client 仍留给 S74.2。
...
- S73.10.7：年轻成年女性补充池已入库，`public/assets/ui/portraits/s73-10/` 下有 24 张 `young_female_style_pool` 主图，覆盖宫署/女官、书院/才女、商事/市井和边关/军务；对应缩略图与低清占位已生成，QA sidecar 为 `public/assets/ui/portraits/portrait-young-female-pool-qa-v1.json`，校验入口为 `npm run qa:young-female-portraits`。Codex 视觉审核确认无中年女性、无发福老态、无中性化，女性特征以发髻簪钗、面容、肩颈线、层叠衣料、腰封细腰和端庄姿态体现。
- S73.10.6：缩略图与压缩总括 QA 已入库并随 S73.10.7 刷新，QA sidecar 为 `public/assets/ui/portraits/portrait-compression-qa-v1.json`，校验入口为 `npm run qa:portrait-compression`。该报告覆盖 572 张 active 立绘，其中 548 张为 S73.10 全量池/补充池立绘，固定 1024x1536 主图、384x576 缩略图、64x96 低清占位、safeArea、focalPoint、移动裁切、文件预算和 `allowEagerLoad=false`。
