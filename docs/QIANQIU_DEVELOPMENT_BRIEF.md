...
- S73.10.6：缩略图与压缩总括 QA 已入库，QA sidecar 为 `public/assets/ui/portraits/portrait-compression-qa-v1.json`，校验入口为 `npm run qa:portrait-compression`。该报告覆盖 548 张 active 立绘，其中 524 张为 S73.10 全量池立绘，固定 1024x1536 主图、384x576 缩略图、64x96 低清占位、safeArea、focalPoint、移动裁切、文件预算和 `allowEagerLoad=false`。
