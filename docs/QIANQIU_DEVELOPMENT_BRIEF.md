...
- S74.3：前端 UI 状态层已完成，新增安全 UI store、route page bridge、显示偏好抽屉、安全摘要 modal 和全局 action draft；只保存安全玩家摘要和 UI 状态，不保存完整 `worldState` 或模型/审计原文。S88.9 起，route bridge 切换到不同案卷时会清空不匹配的 UI safe payload，`gameSessionState` 对 player-state、快捷建议、topic surface/draft、囊箧、NPC 名册和 NPC 详情等安全读取加前端请求序号与 `sessionId` / surface / NPC guard，旧请求晚到或错配回包不得覆盖当前路由，也不得让旧调用方继续执行本地副作用。主卷 `GamePage` 只消费当前路由匹配的 `activeSession`、`activeLastTurn` 和 `activePlayerPayload`，`SurfaceHost` 只消费当前案卷匹配的 `topicSurface` / `topicDraft` 并在案卷切换时失效本地专题草稿、证据选择、草稿类型和 draft context，避免旧案主、旧叙事、旧角色面板、旧专题材料或旧 draft context 串入新案卷。
