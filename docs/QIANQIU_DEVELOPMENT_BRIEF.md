...
S85.1-S85.2 更新：长期经济 tick 与基础市价首轮已接入。新增 `marketPriceLedger` 与 `npcEconomyLedger` 两个 server-owned 原账，分别由 `marketPriceView` 和 `npcEconomyView` 安全投影到普通回合、SSE、player-state 与县令主卷；兼容 `worldState`、redacted player API 和浏览器 UI 均剥离 raw ledger，S85 反馈中的资产/资源/库存变化也只返回公开 `economy.*` 分类，不暴露内部 ledger path。普通回合在世界旬/月推进后运行 `npcEconomy`：每旬刷新书籍、粮食、药材、马匹、兵器、文书、礼物、宅产维护和官署经费的基础价格与身份倍率；月末再结算资产维护/收益、库存损耗、委派到期结果、逾期交易承诺、人情债与 NPC 关系记忆。委派经费继续由服务器裁决，创建时拒绝超过地方库银的预算，月结旧任务也按服务器确认的有效预算计算成功率和扣款，避免客户端数字绕过资源约束。考试入场/场内场景只推进科场局部时间，不触发全局经济结算。AI 与前端只读取安全市价和月账摘要；价格、交易、资产、任务、关系、债务和持久化仍由服务器裁决。S85 剩余工作为 NPC 私人目标与主动性、论道/切磋/求爱/婚姻正式扩展位、总验收和归档。
...
- `assetLedger` / `resourceLedger` / `inventoryLedger` / `npcRoster` / `npcInteractionLedger` / `tradeLedger` / `delegatedTaskLedger` / `openingBackgroundClaims` / `marketPriceLedger` / `npcEconomyLedger`：S81-S85 新增的服务器拥有账本，分别维护长期资产、可计量资源、容器物品、可交互 NPC、NPC 交互、交易记录、委派任务、开局背景裁决、基础市场价格和 NPC 经济月结。provider 不能 patch 这些账本；浏览器、prompt 和 SQLite 只读安全 view 或安全派生表；兼容 route `worldState` 会剥离 raw ledger，player-state 返回 `assetLedgerView`、`resourceLedgerView`、`inventoryView`、`npcRosterView`、`npcInteractionView`、`tradeLedgerView`、`delegatedTaskView`、`openingBackgroundClaimsView`、`marketPriceView` 和 `npcEconomyView`。S85.1-S85.2 的 `npcEconomy` 只在普通回合/跳时共享的世界 tick 后运行：旬更市价，月末结算资产、库存、委派、逾期交易、人情债和 NPC 关系记忆；考试场景不触发全局经济。
...
14. 完成后至少运行与本次改动相关的验证命令。
15. 再次执行 `git status --short`。
16. 对包含代码、测试、运行时行为、API/schema、提示词或验证工具变化的 coherent change，在暂存和提交前至少委派一个只读子代理审查最终 diff 与验证结果。纯文档低风险改动可以跳过，但要在共享上下文或最终回复说明。
17. 用 Git 提交本次 coherent change。
18. 在最终回复中说明改了什么、验证了什么、提交哈希是什么。
