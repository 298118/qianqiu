...
- S89.27：前端 CSS 构建预算校准已完成。`scripts/clientBuildBudget.js` 的 CSS 总预算为 `180000` bytes，单个 CSS asset 上限为 `128000` bytes，用于替代 S89 后期只剩数百 bytes 余量的旧 `102400` bytes 单文件硬门。该调整不表示取消预算治理；若后续单个 CSS asset 超过约 `150 KiB`，应优先做 route/component CSS 拆分、删除无用选择器、合并重复规则或抽出共享样式，再评估是否继续提高硬门。本步不改 React 运行时行为，不新增 CSS、依赖或素材，不改后端 API/schema、AI 权限、prompt、provider、存档、runtime manifest 或服务器裁决。
