...
S88 更新：全面系统打磨专项已启动，规划见 [QIANQIU_POLISHING_ROADMAP.md](QIANQIU_POLISHING_ROADMAP.md)。S88 覆盖完整书生主线、多身份循环、官场与世界后果、AI 世界引擎、NPC/关系、资产经济、React 前端、PixiJS 地图、视觉立绘、后端类型安全和 Mock/真实 provider 验收。S88.1 首先加固 AI remote helper/provider public-safe envelope：`src/ai/providers/remoteHelpers.js` 已纳入 `npm run typecheck:server`，`src/contracts/serverContracts.ts` 新增 AI remote task/requester 与 public forbidden field contract，`/api/ai/connection-test` 的 public response 和 provider fallback 日志不得携带 raw provider payload、完整 prompt、请求/响应体、base URL、key、本地路径、`statePatch` 或 `worldState`。S88.2 已固定 SQLite derived row builder 类型边界：`SqliteWorldSessionRow`、prompt retrieval、safe search、repair status、derived drift 和 safe diagnostics 已进入 contract/JSDoc/typecheck，`world_sessions.world_state_json` 继续是 SQLite 模式唯一事实源，派生表只从服务器安全 view 单向修复，不作为 AI、浏览器、prompt 或 resolver 的 canonical truth。S88.3 正在补强书生主线：殿试授官后，`appointmentTracks` 会按服务器授官轨迹 seed 一条入仕首月 `officialCareer.assignments`，`officialCareerView` 和 Mock 科举 smoke 都能看到首月差事；考试取题后，`examTravel` 会按盘费缺口、路途、保结、准考缺口、学业维度、体力心态和读书计划派生 `entryPreparation.preparationPressure` / `entryFeedback`，直接 `entryPreparation`、兼容 `worldState.activeExam.entryPreparation`、`examProcedureView` 与 `studyProfileView.examPreparation` 都只暴露清洗后的等级、原因、建议和入场反馈，React 书生面板与科举页只读这些安全 view 并最多写入行动草稿；放榜后，`examAftermathView` 会从服务器定榜、`examHonor`、`examNetwork` 与 `appointmentTrack` 整理公开同年座师、座师/考官摘要、下一步草稿建议和权限边界，React 皇榜页只展示这些安全摘要并把“拟行动”写入草稿；读书计划深化切片让 `studyProfileView.nextPlan` 和 prompt summary 增加服务器生成的三旬计划窗口、补弱强度、晨午暮日课、复盘节点、风险提示、首课行动与权限边界，React 书生面板只读展示并把“执行首课”写入草稿；兼容 `worldState` 由 `buildClientWorldState` 递归剥离嵌套 hidden/raw/provider/prompt/statePatch 字段并清洗 active exam / exam history 中的入场准备与放榜过渡快照，`studyProfileView` 自身也会拒绝 provider/statePatch/worldState/本地路径污染文本并回落到服务器默认日课；AI、吏部、皇帝和前端仍不能直接写官职、差事、考成、读书属性、备考压力、入场状态、同年座师关系或官场结果。真实 provider 失败仍走 Mock 或安全失败 envelope，不成为本地启动门槛。
