...
12. 对包含代码、测试、运行时行为、API/schema、提示词或验证工具变化的 coherent change，在暂存和提交前至少委派一个只读子代理审查最终 diff 与验证结果。主代理必须在审查提示词中提供最终 diff 和验证摘要；审查子代理不得编辑文件或运行任何 Git 命令。纯文档低风险改动可以跳过，但要在共享上下文或最终回复说明。
13. 用 Git 提交本次 coherent change。
14. 在最终回复中说明改了什么、验证了什么、提交哈希是什么。
