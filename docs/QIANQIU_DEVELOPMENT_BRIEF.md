...
S53.4 后，`#world-geography-panel` 会渲染可见国家、城市、路线、边面和官署辖区的紧凑卡片；`#posting-geography-panel` 会渲染当前任所、公开辖区、地方指标和相关通路。两者继续只读 route view，不读取 raw `worldState.worldGeography` / `worldState.officialPostings`，不展示隐藏 notes、未公开任所、raw id 文本、provider proposal、prompt、本地路径或 key；人物谱牒、官职簿和事件档案仍不在本步骤展开。
...
S52.1 已新增 `docs/OFFICIAL_POSTING_DATABASE_CONTRACT.md` 和 `src/game/officialPostingSchemas.js`。该契约固定未来 `bureaus`、`offices`、`cityJurisdictions`、`postings`、`assessmentRecords`、`transferRecords` 的字段、数值范围、`office_visible` 等可见性规则、hidden notes 边界、nested 引用裁剪和 prompt cap。S52.2 已新增 `src/game/officialPostings.js`，把官职目录、当前入仕官员/地方官任所、S50 可见城市/辖区指标和服务器 `officialCareer.careerHistory` 桥接为 `worldState.officialPostings`、`officialPostingsView` 与 capped prompt 摘要。该步骤不改变 `officialCareerView`，不新增浏览器官职簿/任所地理面板，不创建 SQLite 业务表；普通 provider 夹带 `statePatch.officialPostings` 会被 schema、remote normalization、stateRules、provider long-run、audit redaction 与 red-team 边界拒绝、忽略或脱敏。
