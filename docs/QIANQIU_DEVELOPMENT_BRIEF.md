...
- 前端：第一阶段运行入口仍可由原生 HTML + CSS + JavaScript 支撑；S74.0 已为后续默认前端重构安装 React + TypeScript + Vite + React Router Data Mode 工具链，S74.1 起创建 `client/` 并逐步接管默认 `/`。
