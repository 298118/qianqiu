...
4. `worldGeographyView`、`worldPeopleView`、`officialPostingsView`、`localAffairsDocketView`、`militaryDiplomacyView`、`economicFiscalView`、`historicalEventArchiveView`、`intelligenceRumorView`、`eventArchiveView` 和 capped `retrievalContext` 是当前 UI/prompt 合法入口。
...
- S66：S66.1 大规模 prompt retrieval 策略已完成；S66.2 待做浏览器城市、NPC、官职和事件搜索、筛选、分页与 hidden-token smoke。
- S67：规模/性能/回归验收与再次归档。
