...
- S55.1 已在人物契约中固定 future `people_npcs`、`people_households`、`people_assets`、`people_estates`、`people_relationships` 表形状；当前仍不创建这些表，不改变 `worldPeopleView`，也不把 hidden NPC 私档回填进 route raw `worldState`。
...
- `worldPeople` / `worldPeopleView`：从当前可见 `characters`、`relationshipLedger` 和 active request 近期札记桥接人物、家族、资产、田产和关系摘要；不保存 hidden 私档。S55.1 的 `people_*` SQLite 表契约只为后续本地持久化和检索准备，当前 prompt/UI 仍只读服务器 view。
...
5. S54 已固定并实现地理 SQLite 业务表与维护工具；S55.1 已固定人物域 SQLite 表契约但尚未创建运行时表；官职任所和安全事件索引尚未拆成 SQLite 业务表，仍按 S55-S59 小步推进。

S54-S59 剩余方向：

- S54：地理 SQLite 表契约、adapter 持久化、导入/修复/导出和更广 view parity 已完成。
- S55：人物、家族、资产、田产、关系 SQLite 表契约已完成；后续实现持久化、桥接 parity 与 NPC 生命周期事件。
