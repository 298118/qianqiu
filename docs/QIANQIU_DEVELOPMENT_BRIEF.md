...
- `officialCareer` / `officialCareerView` / `courtResponseView` / `courtConsequenceView`：服务器拥有官职履历、当前任所、差遣、考成簿、弹劾程序、官场结算、跨身份奏议回应和长期后果信号。普通 provider 不能 patch 这些账本、不能改官职/考成/弹劾/差遣成败；`runOfficialCareerStep()` 只按公开行动、世界 tick 和服务器规则推进差遣、履历与结算，`runOfficialCourtResponseStep()` 只把朱批、票拟覆奏、补据、朝议回应或考成观察写成 `officialCourtResponses` 中间态，`runOfficialCourtConsequenceStep()` 只把公开奏议链路写成 `officialCourtConsequences` 非终局信号。S88.4 起 `officialCareerView.firstMonthExperience` 会从首月差事派生官署首月进度、风险、限期、回署回执、上官同僚反馈、考成信号和下一步草稿建议；普通回合推进首月差事时生成 `[官署回执]` 反馈，月报读取该安全 view。`officialCareerView.courtEntry` / `courtEntries` 会把首月回署材料、奏折/朝议目标 surface、长期考成 trace、上官同僚后续回响和 draft-only 下一步整理为安全 evidence，供 `topicSurfaceView` 和 `topic_draft` 引用；普通回合提交后，服务器会把准入复核、转部核议、留中补查、驳回补据或续入考成写入 `courtEntryResolutions`，并投影为近次裁决、事件档案和月报材料。朝议/部院/御前跟进由服务器写入 `courtEntryFollowUps`，投影为 `latestFollowUp`、`followUpHistory`、`followUpScenePreview`、`followUpNextActions`、事件档案 `official_court_follow_up`、月报条目和世界议程线索；跨身份回应由 `courtResponseView` 投影为回应 docket、近次回应、事件档案 `official_court_response` 和世界议程线索，兼容 public `worldState` 不暴露 raw `officialCourtResponses`；长期后果由 `courtConsequenceView` 投影为待转信号来源、近次信号、后续草稿、事件档案 `official_court_consequence`、月报条目和世界议程线索，兼容 public `worldState` 不暴露 raw `officialCourtConsequences`。这些跟进、回应和长期后果信号只表示朝议成议、部院待覆、御前留览、补据再议、考成观察、证据缺口、风宪关注或功绩留痕，不直接任免、奖惩、处分、财赋动用、奏议终局或风宪定案。相关 view 和 prompt summary 会清洗 provider/proposal/prompt/raw/statePatch/worldState/key/path/SQLite 形态污染，不暴露 `hiddenNotes`、raw response/consequence ledger 或 hidden 私档。
