...
S88.4 更新：入仕官员首轮官场体验已完成四个 coherent slice。新增 `src/game/officialFirstMonthConfig.js` / `src/game/officialFirstMonth.js`，由服务器从 `officialCareer.assignments` 中的首月差事派生 `officialCareerView.firstMonthExperience`，公开官署首月进度、风险档、限期、上官同僚反馈、回署回执、考成信号、下一步草稿建议和权限边界；普通回合推进首月差事时会追加 `[官署回执]` 安全反馈，`summarizeOfficialCareerForPrompt()` 只读取 capped 摘要。第二切片新增 `src/game/officialCourtEntryConfig.js` / `src/game/officialCourtEntry.js`，把首月回署材料整理为 `officialCareerView.courtEntry` / `courtEntries`：包含奏折队列、朝议筹议、长期考成 trace、上官同僚后续回响、可引用 sourceRefs 和 draft-only 下一步。第三切片让普通回合识别“入奏折队列 / 付朝议筹议 / 续记考成”等首月回署提交，由服务器写入 `officialCareer.courtEntryResolutions`，并把近次裁决投影到 `officialCareerView.courtEntry.latestResolution`、`courtEntryResolutions`、`eventArchiveView` 的 `official_court_entry` 条目和后续官职月报；裁决结果只包括准入复核、转部核议、留中补查、驳回补据、续入考成等公开状态、受限进度/考成微调和下一步。第四切片新增朝议/部院/御前跟进：普通回合提交“朝议跟进 / 部院覆奏 / 御前摘报 / 考成观察”后，服务器写入 `officialCareer.courtEntryFollowUps`，并把 `latestFollowUp`、`followUpHistory`、`followUpScenePreview` 和 `followUpNextActions` 投影到 `officialCareerView.courtEntry`、prompt 摘要、`eventArchiveView` 的 `official_court_follow_up` 条目、`worldThreadView` 和月报。跟进只记录朝议成议、部院待覆、御前留览、补据再议或考成观察等中间反馈、参与 actor 摘要和受限进度/风险/考成微调，不直接任免、奖惩、处分、采纳奏折或成弹劾。`topicSurfaceView` 的 `memorial-review`、`edict-draft`、`court-debate` 可从 `officialCareerView.courtEntries` 读取这类安全 evidence，`topic_draft` 可引用它拟稿，但仍不执行 resolver、不推进时间、不写 canonical state。React `OfficialMinisterPanel` 新增“官署首月”和“奏折朝议入口”区块，只读 `officialCareerView.firstMonthExperience`、`courtEntry`、近次裁决、朝议/部院/御前跟进与 `playerMonthlyBriefingView`，按钮只写底部奏折草稿；前端和 AI 都不能直接写差遣成败、任免、考成、弹劾或 hidden 私档。`officialCareer` 公开文本清洗同步收紧，拒绝 provider/proposal/prompt/raw/statePatch/worldState/key/path/SQLite 形态污染后才进入浏览器、prompt、topic evidence、月报和事件档案。
...
S76.4 更新：入仕官员/大臣面板已完成。React 主卷在 `player.role === "official"` 或 `player.role === "minister"` 时渲染 `OfficialMinisterPanel`，通过 manifest registry 读取已审核 `role_background` 部院/大臣素材，并只组合安全 `officialCareerView`、`appointmentTrackView`、`officialPostingsView`、`playerMonthlyBriefingView`、`actorMemoryView`、`aiControlAuditView` 和玩家公开摘要。面板显示官职履历、部院公文、同年座师、人脉记忆、派系/朝局风险、考成、弹劾和奏折朝议入口；S88.4 起，若服务器提供 `officialCareerView.firstMonthExperience`，面板还显示“官署首月”的首月差事、进度风险、回署回执、上官同僚反馈、考成信号和月报摘录提示；若服务器提供 `officialCareerView.courtEntry`，面板会显示首月回署材料、可进入的奏折/朝议 surface、长期考成 trace、上官同僚后续回响、服务器近次裁决，以及朝议/部院/御前跟进的 stage、status、参与 actor 和后续草稿入口。按钮只把“整理履历”“查办公文”“拟回堂官”“拜会座师”“探问朝局”“自陈考成”“回应弹劾”“入奏折队列”“付朝议筹议”“续记考成”“朝议跟进”“部院覆奏”“御前摘报”“考成观察”等意图写入底部奏折草稿，不提交回合、不调用 resolver、不推进时间、不直接任免、奖惩、处分、弹劾成案或改写考成。官缺、升降、铨选、考成、处分、派系影响、时间推进、首月差事成败、奏折采纳、跟进批复和持久化仍由服务器 resolver 与普通回合裁决完成。
...
- `officialCareer` / `officialCareerView`：服务器拥有官职履历、当前任所、差遣、考成簿、弹劾程序和官场结算。普通 provider 不能 patch 该账本、不能改官职/考成/弹劾/差遣成败；`runOfficialCareerStep()` 只按公开行动、世界 tick 和服务器规则推进差遣、履历与结算。S88.4 起 `officialCareerView.firstMonthExperience` 会从首月差事派生官署首月进度、风险、限期、回署回执、上官同僚反馈、考成信号和下一步草稿建议；普通回合推进首月差事时生成 `[官署回执]` 反馈，月报读取该安全 view。`officialCareerView.courtEntry` / `courtEntries` 会把首月回署材料、奏折/朝议目标 surface、长期考成 trace、上官同僚后续回响和 draft-only 下一步整理为安全 evidence，供 `topicSurfaceView` 和 `topic_draft` 引用；普通回合提交后，服务器会把准入复核、转部核议、留中补查、驳回补据或续入考成写入 `courtEntryResolutions`，并投影为近次裁决、事件档案和月报材料。朝议/部院/御前跟进由服务器写入 `courtEntryFollowUps`，投影为 `latestFollowUp`、`followUpHistory`、`followUpScenePreview`、`followUpNextActions`、事件档案 `official_court_follow_up`、月报条目和世界议程线索；这些跟进只表示朝议成议、部院待覆、御前留览、补据再议或考成观察，不直接任免、奖惩、处分、采纳奏折或成弹劾。该 view 和 prompt summary 会清洗 provider/proposal/prompt/raw/statePatch/worldState/key/path/SQLite 形态污染，不暴露 `hiddenNotes` 或 hidden 私档。
