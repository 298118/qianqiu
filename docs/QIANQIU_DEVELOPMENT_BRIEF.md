...
- S73-S77：当前活动方向为前端水墨重构，任务书见 [FRONTEND_INK_REDESIGN_ROADMAP.md](FRONTEND_INK_REDESIGN_ROADMAP.md)；S73 从视觉资产指南、UI manifest、素材台账、UI 材质、首页/场景/身份素材、玩家/NPC 立绘基准、动效/fallback 和素材 QA 开始。S74.0 可在依赖治理通过后引入 React + TypeScript + Vite + React Router Data Mode 并行多页前端岛，S75-S76 再推进首页/全局 shell、设置/存档/返回首页、底部奏折、身份/场景专题、考试/放榜、独立舆图页和立绘接线，S77 做入口切换、浏览器、安全、性能、可访问性和归档验收。
