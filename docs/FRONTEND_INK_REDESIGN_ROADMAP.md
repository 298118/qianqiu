...
- S73.10 单张覆盖：已新增 `scripts/frontendSinglePortraitOverrides.js`、`qa:single-portrait-overrides` / `qa:single-portrait-overrides:write` 和 `public/assets/ui/portraits/portrait-single-override-qa-v1.json`，当前 12 张玩家女性科举阶段画像使用单张高清重制覆盖；后续女性重制优先参考压缩 WebP 构图，不上传源图/大图，已重制图少审节流。
