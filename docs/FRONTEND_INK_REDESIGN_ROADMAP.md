...
状态：已完成并归档。完成范围、稳定边界、验收入口和后续方向见 [FRONTEND_INK_REDESIGN_ARCHIVE.md](FRONTEND_INK_REDESIGN_ARCHIVE.md)；本文保留为规划源头和历史任务书，不再作为当前活动台账。
...
- 已用统一报告覆盖缩略图、fallback、审核状态、性能预算、立绘低清占位和 transparent alpha 标记；当前报告覆盖 642 个 active 素材。
...
当前状态：S73.10.1 已完成矩阵定稿。新增 `docs/FRONTEND_PORTRAIT_MATRIX.md`、`public/assets/ui/portraits/portrait-pool-matrix-v1.json`、`scripts/frontendPortraitMatrix.js` 和 `npm run qa:portrait-matrix`，把全量池锁定为 336 张 planned 立绘：玩家身份阶段 72、通用 NPC 120、重要 NPC 72、状态姿态 48、场景锚点 24。矩阵只预置安全 `portraitRef`、生成目标路径、prompt 母版、fallback、懒加载分组和审核字段；不把未生成素材写入 `ink-ui-manifest.json` 可用集合。S73.10.2 已完成玩家身份阶段 72 张立绘，并保留 60 张已审核女性玩家风格补充与 60 张已审核男性玩家风格补充，使玩家可选池男女各 96 张。S73.10.3 已完成 188 张通用 NPC 立绘，包括一开始的 120 张矩阵通用 NPC、20 张旧版 bonus 和 48 张宫装/唐装女性风格扩展；S73.10.4 已完成 72 张重要 NPC 专属立绘；S73.10.5 已完成 72 张状态/姿态与场景锚点立绘；S73.10.7 已完成 48 张年轻成年女性补充立绘，覆盖宫署/女官、书院/才女、商事/市井和边关/军务，Codex 视觉审核确认无中老年女性、无发福老态、无中性化；S73.10 单张覆盖已完成 60 张女性/偏女性高清重制，配套 `scripts/frontendSinglePortraitOverrides.js`、`public/assets/ui/portraits/portrait-single-override-qa-v1.json` 和 `qa:single-portrait-overrides`；S73.10.6 已完成缩略图与压缩总括 QA，新增 `scripts/frontendPortraitCompressionQa.js`、`public/assets/ui/portraits/portrait-compression-qa-v1.json` 和 `qa:portrait-compression`，当前统一校验 596 张 active 立绘、其中 572 张 S73.10 立绘的主图、缩略图、低清占位、safeArea、focalPoint、移动裁切、文件预算和禁止 eager load。manifest 与 S73.9 QA 报告当前为 642 个 active 素材。
...
- S73.10.6 缩略图与压缩：已完成并随 S73.10.7 刷新。`qa:portrait-compression` 固定半身主图 1024x1536、缩略图 384x576、低清占位 64x96，校验 596 张 active 立绘的 bytes、SHA-256、文件预算、safeArea、focalPoint、移动裁切和 `allowEagerLoad=false`。
- S73.10.7 视觉与安全审核/年轻女性补丁池：已完成。新增 `scripts/frontendYoungFemalePortraitAssets.js`、`qa:young-female-portraits` / `qa:young-female-portraits:write` 和 `public/assets/ui/portraits/portrait-young-female-pool-qa-v1.json`，单独入库 48 张年轻成年女性立绘；候选源页经 Codex 视觉审核，确认不含中老年女性、发福老态、男性化或中性化候选，且完整衣着、无露骨/挑逗/幼态/现代物/文字/水印。
- S73.10 单张覆盖：已新增 `scripts/frontendSinglePortraitOverrides.js`、`qa:single-portrait-overrides` / `qa:single-portrait-overrides:write` 和 `public/assets/ui/portraits/portrait-single-override-qa-v1.json`，当前 60 张女性/偏女性画像使用单张高清重制覆盖；后续女性重制优先参考压缩 WebP 构图，不上传源图/大图，已重制图少审节流。
...
状态：已完成。S75.1 已在 React 默认 `/` 首页接入 S73.4 已审核首页画卷、透明云雾、题名册表单底、朱印和案卷素材：标题“千秋”保持在主视觉中，中央册页表单继续使用现有安全 `POST /api/game/start` 接线，旧案卷只显示 `GET /api/game/saves` 的脱敏 metadata。桌面和移动端均收束无横向溢出；`prefers-reduced-motion: reduce` 下使用静态底并关闭云雾漂移动效。S75.2/S75.3 仍负责完整开局字段和朱印盖章反馈，本步不提前扩大 start payload。
...
状态：已完成。S75.4 已把右上角工具收束为单一“印匣”按钮，抽屉内用 AI 设置、旧案、显示、安全 tab 统一当前案卷 AI preset、provider 试连、旧案刷新/读档、返回首页、运行时显示偏好和安全摘要；Esc 关闭与焦点回收沿用 `SurfaceHost`，旧存档/显示偏好抽屉入口兼容映射到同一印匣。数据继续来自安全 API client、脱敏 saves metadata 和 UI store 安全摘要，不显示密钥、连接地址、本地路径、完整提示词、内部审计原文或模型原始返回。
...
状态：已完成。S76.1 已将主卷整理为案卷状态条、按身份切换的已审核场景插画带、叙事卷、安全投影索引、图标功能页签和固定底部奏折；不新增后端 API 或服务器裁决权限。
...
状态：已完成。S76.2 已新增 React `ScholarPanel`，在书生身份主卷中接入已审核书斋 `role_background`、安全 `studyProfileView`、`examCalendarView` 和玩家公开摘要，展示读书簿、老师点评、师友、保结前置、科期、文章练习、荐书、近课和赶考/皇榜入口。所有按钮只写底部奏折草稿，赶考、入场、评卷、放榜、晋级和授官继续由服务器按既有规则裁决。
...
状态：已完成。S76.7 已将 React `ExamPage` 升级为贡院/殿试全屏考试界面，接入已审核 `exam_page` 场景素材、试卷界栏、朱印按钮和安全考试数据；取题、推进考场和交卷仍只调用既有考试 API，不提交普通回合，不直接评分、判舞弊、放榜、晋级或授官。
...
状态：已完成。S76.8 已将 React `RankingPage` 升级为贡院外皇榜全屏界面，接入已审核 `ranking_page` 放榜场景、`imperial_notice` 皇榜纸底和 `red_ink_smudge` 朱砂墨迹素材，组合服务器 ranking、考试荣誉、阅卷和授官安全投影，展示三鼎甲、正榜、玩家高亮、评语、评分维度、防作弊检测、授官提示和服务器定榜边界。前端不改榜次、不补评分、不判防弊、不推断授官。
