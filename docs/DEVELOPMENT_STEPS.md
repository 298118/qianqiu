...
- S75.9 仍需把真实浏览器 smoke 扩展到右上角印匣点击、旧案 tab 遍历和读档交互。
...
- 已完成：Kepler 只读复审最终 diff 与验证证据，未发现 P0/P1/P2；P3 建议是当前 browser smoke 仍主要验证 `/settings` 路由刷新，未显式点击右上角印匣并遍历 tab，已留给 S75.9 或 S75.5 后扩展真实浏览器交互。
...
- 真实浏览器 smoke 本轮覆盖设置路由刷新，印匣抽屉 tab 的逐项交互由 Vitest/scaffold 覆盖；S75.9 应补右上角印匣点击与 tab 遍历。
