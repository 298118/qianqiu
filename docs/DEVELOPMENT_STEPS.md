...
本轮 S88.9 不支持案卷 id 的本地专题层与链接防线当前验证口径：

- `npm run typecheck:client`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "session routes|unsupported"`（5 项）
