...
- S91-S92 前端读法收束与 AI 编排 v2 已完成项压缩索引：[ACTIVITY_LEDGER_COMPLETED_ARCHIVE.md](ACTIVITY_LEDGER_COMPLETED_ARCHIVE.md)。
...
- S88 已作为阶段性打磨归档到 [QIANQIU_POLISHING_ARCHIVE.md](QIANQIU_POLISHING_ARCHIVE.md)；S89-S92 已完成项已作为完成流水压缩到 [ACTIVITY_LEDGER_COMPLETED_ARCHIVE.md](ACTIVITY_LEDGER_COMPLETED_ARCHIVE.md)。继续打磨或 AI v2 接入时应新开可审查小步骤，不要把旧长流水复制回本文件。
...
| ID | 状态 | 目标 | 范围 / 下一步 |
| --- | --- | --- | --- |
| S92.NEXT | TODO | AI 编排 v2 后续安全接入 | 优先从 `/api/game/turn` 的 `AI_RUNTIME_V2=shadow` 旁路记录设计、EvidenceRef 接入 topic/tool proposal allowlist，或 ProviderAdapter 扩展到 Anthropic/DeepSeek/MiMo 三者之一开新小步。不得直接把默认普通回合切到 v2，不新增默认 provider 写权、存档字段或服务器裁决面。 |

## 5. 最新状态

- 已完成项归档：S90.1-S90.4、S91.1-S91.18、S92.1-S92.10 已从活动路线图和近期流水迁出，压缩索引见 [ACTIVITY_LEDGER_COMPLETED_ARCHIVE.md](ACTIVITY_LEDGER_COMPLETED_ARCHIVE.md)。S89-S90 React 产品 polish 专题仍见 [FRONTEND_PRODUCT_POLISH_ARCHIVE.md](FRONTEND_PRODUCT_POLISH_ARCHIVE.md)。
- 最新实现锚点：S92.10 已修复 SQLite prompt retrieval 可见性 parity。SQLite prompt retrieval 派生 payload 现在只把服务器安全 view 已裁定可见的 `role_visible` / `office_visible` 情报传闻归一为 EvidenceRef allowlist 内的 `actor_visible`，保留 `public` / `player_visible`，不放行 hidden/private，不改变 `intelligenceRumorView` 业务可见性、route/API/schema、SQLite schema、provider 默认、tool loop 或服务器裁决。
- AI v2 已完成 Ticket 0-8 基础：baseline、AI Task Runtime、OpenAI strict adapter 旁路、Mock-first tool loop、AI Eval v2、Prompt Registry v2、EvidenceRef、安全 trace/provider health、前端 public trace debug 和 S92.10 parity 修复。所有这些能力仍是渐进旁路或安全只读/反馈入口，默认 `/api/game/turn`、旧 provider facade、topic draft route、存档格式、浏览器裁决权和服务器最终裁决未被接管。
- 最新全量验证锚点来自 S92.10：`node --check src/storage/sqlitePromptRetrievalTables.js`、`node --test test/sqlitePromptRetrieval.test.js`、`node --test test/dualModeAcceptanceScript.test.js`、`npm run eval:ai`、`npm run eval:ai:v2`、`npm run typecheck:server`、`npm run check:docs-governance` 和 `npm test`（1294 tests）。
- 本轮 2026-05-29 仅做文档压缩与已完成项归档；运行时代码、前端行为、后端 API/schema、AI 权限、prompt、provider、SQLite schema、存档格式、runtime manifest、素材 manifest 和服务器裁决均未改变。按低风险纯文档规则跳过提交前子代理复审。

## 6. 最近完整验证口径

本活动台账只保留当前可接手的最高层验证口径；逐步骤验证命令已迁入 [ACTIVITY_LEDGER_COMPLETED_ARCHIVE.md](ACTIVITY_LEDGER_COMPLETED_ARCHIVE.md) 的压缩索引或保留在对应 Git history。

- 当前最新代码验证：S92.10 的 `npm test` 为 1294/1294 pass，并通过 AI eval v1/v2、server typecheck、SQLite prompt retrieval 聚焦测试和 docs governance。
- 本轮文档维护验证：运行 `npm run check:docs-governance`、`node --test test/documentationGovernance.test.js` 和 `git diff --check`。
- 若下一步涉及后端 route/API/schema、AI schema/provider facade、storage adapter 或 resolver，必须额外运行 `npm run typecheck:server`；涉及 React UI 时补 `npm run typecheck:client`、聚焦 Vitest、`npm run build:client`、`npm run budget:client` 和 browser smoke。

## 7. 近期进度记录

### 2026-05-29：上下文压缩与 S90-S92 完成台账归档

- 范围：压缩 [SHARED_CONTEXT.md](SHARED_CONTEXT.md) 的 Current Work Note；把 [DEVELOPMENT_STEPS.md](DEVELOPMENT_STEPS.md) 中 S90.1-S92.10 的 DONE 活动表、最新状态长清单、逐步骤验证锚点和近期流水迁出；在 [ACTIVITY_LEDGER_COMPLETED_ARCHIVE.md](ACTIVITY_LEDGER_COMPLETED_ARCHIVE.md) 增加 S90-S92 压缩归档入口。
...
- 验证：运行 `npm run check:docs-governance`、`node --test test/documentationGovernance.test.js` 和 `git diff --check`。
