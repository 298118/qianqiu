...
本轮 S88.8 经济解释 trace 第四片当前验证口径：

- `npm run typecheck:client`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2 client/src/state/uiState.test.ts`（19 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2 client/src/components/MemorialComposer.test.tsx client/src/state/uiState.test.ts`（24 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2 client/src/__tests__/App.test.tsx`（41 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（79 项）
