...
| S88.10 | IN_PROGRESS | PixiJS 水墨地图行动入口 | S88.10.1 已启动前端安全行动入口首片：复用 `mapRuntimeView.refs/routes/eventEffects/actionDrafts` 生成舆图行动牌，并把地点、驿路和近事草稿写成带安全 refs 的本地 `draftContext`；仍不新增后端 API 或浏览器裁决权。后续继续打磨地方事务、军务外交、NPC 活动和更长链路服务器复核。 |
...
本轮 S88.10.1 PixiJS 舆图安全行动入口当前验证口径：

- `npm run typecheck:client`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "S72 map|map renderer|舆图"`（1 项）
