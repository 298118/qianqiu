...
| ID | 状态 | 目标 | 范围 / 下一步 |
| --- | --- | --- | --- |
| S89.50 | DONE | 共享纸面动效 keyframe 语义化与 token 化 | 承接 S89.49 后继续收敛带迭代编号的共享 CSS 动效命名，运行时 CSS 中 S89.30 共享纸面 keyframes `s8930PaperRise`、`s8930StateWash`、`s8930SealBloom` 改为稳定语义名 `paperSurfaceRise`、`paperWrittenStateWash`、`paperSelectedSealBloom`；`draftWrittenPulse` 的硬编码朱色 outline 改读新增 `--qq-color-vermilion-glow-outline`，并新增朱色 glow、暖阴影、宣纸 inset token 供共享纸面状态洗复用。`polish-surfaces.css` 与 `scripts/clientSmoke.js` 改读语义 keyframe 名，`test/reactClientScaffold.test.js` 新增 S89.50 source canary，守住语义 keyframes、token 使用和旧 keyframe 名不再出现在运行时 CSS/smoke 中。范围限 tokens、motion keyframes、共享纸面 utility、客户端 smoke/source canary、brief 与交接文档；不新增依赖或素材，不改 React 组件、前端数据流、后端 API/schema、AI 权限、prompt、provider、SQLite、存档、runtime manifest 或素材 manifest。浏览器仍只消费安全 view、route/surface 状态和本地草稿，不新增裁决权。当前已通过完整验证；实现提交：`aca84940`。 |
| S89.49 | DONE | 科举页静态 surface utility 接入 | 承接 S89.48 单独审计科举页静态容器，`.examQuestionPanel`、当前/预览 `.examDesk`、`.examRecordPanel`、`.examPeerPanel`、`.examPreviewPanel` 和 `.examRecentSubmitPanel` 新增 `.paperMotionSurface`；既有业务 class、`data-polish-exam-ledger="s89-18-exam-ritual"`、`data-polish-exam-paper="s89-33-exam-ceremony-material"`、取题/推进考场/交卷 handlers、`requestExamQuestion(sessionId, level)`、`progressExam(sessionId, examId, sceneAction.trim())`、`submitExam(sessionId, examId, essay.trim())`、反馈拟行动本地草稿、考试纸卷专属 `::before` / 展开动画 / reduced-motion selector / 高对比覆盖和移动端布局均保留。`.examSafetyBoundary` 不纳入本轮，因为它不是原 shared-paper 静态材质 selector 成员且承担虚线安全边界读法。`polish-surfaces.css` 第一条 shared-paper selector 收敛为 `.paperMotionCard`、`.paperMotionPanel`、`.paperMotionSurface` 三个语义 hook，不再依赖科举 route-specific class。`test/reactClientScaffold.test.js` 新增 S89.49 source canary；`scripts/clientSmoke.js` 在取题前后检查已渲染科举静态 surface hook，并确认当前试卷仍保留 `[aria-label='当前试卷'].examDesk.paperMotionSurface[data-polish-exam-paper='s89-33-exam-ceremony-material']`。范围限 React className、CSS utility/共享 selector、客户端 smoke/source canary、brief 与交接文档；不新增依赖或素材，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、SQLite、存档、runtime manifest 或素材 manifest。浏览器仍只消费安全 view、route/surface 状态和本地草稿，不新增裁决权。实现提交：`b648f771`。 |
| S89.48 | DONE | 人物页静态 surface utility 接入 | 承接 S89.47 后继续收敛共享材质 selector 中的 route-specific 耦合，人物页 `.npcGroupList`、`.npcDetailWorkbench` 和 `.portraitLedger` 新增 `.paperMotionSurface`；既有业务 class、`data-polish-people-*`、人物分组选择、`paperMotionSelected`、`data-gallery-selected`、人物画屏、交游候复笺、跨页追索、画像懒加载、对话/交易/委派/交游 handlers、本地草稿写入、人物 safe view 消费和移动端布局均保留。`polish-surfaces.css` 第一条共享纸面 selector 不再依赖这三个业务 class；科举页 `.examQuestionPanel`、`.examDesk`、`.examRecordPanel`、`.examPeerPanel`、`.examPreviewPanel`、`.examRecentSubmitPanel` 因考试纸卷专属展开动画、reduced-motion 守门和取题/推进考场/交卷链路，留待后续 S89.49 单独审计。`test/reactClientScaffold.test.js` 新增 S89.48 source canary；`scripts/clientSmoke.js` 在人物页实际 DOM 抽样三处静态 `.paperMotionSurface` 且检查无缺口。范围限 React className、CSS utility/共享 selector、客户端 smoke/source canary、brief 与交接文档；不新增依赖或素材，不改 React 数据流、后端 API/schema、AI 权限、prompt、provider、SQLite、存档、runtime manifest 或素材 manifest。浏览器仍只消费安全 view、route/surface 状态和本地草稿，不新增裁决权。实现提交：`7f5e322f`。 |
...
- 2026-05-26：S89.48 完成人物页静态 surface utility 接入。当前范围限 React className、CSS utility/共享 selector、客户端 smoke/source canary、brief 和开发文档；人物分组、人物详情工作台和人物谱牒新增 `.paperMotionSurface`，共享材质 selector 不再依赖这些 route-specific class，人物选择、`paperMotionSelected`、`data-gallery-selected`、画像懒加载、人物画屏、交游候复笺、跨页追索、对话/交易/委派/交游 handlers 和本地草稿写入保持不变。科举页候选因专属考试纸卷动画、reduced-motion 守门和提交链路留到后续 S89.49。未删除仍有源码命中的业务布局 selector，不新增浏览器裁决权，不改 NPC mutation、后端 API/schema、AI 权限、prompt、provider、SQLite、存档、runtime manifest 或素材 manifest。实现提交 `7f5e322f`；本次 hash 回填仅修改本文与 [SHARED_CONTEXT.md](SHARED_CONTEXT.md)，属于低风险纯文档维护，跳过子代理复审。
- 2026-05-26：S89.49 完成科举页静态 surface utility 接入。当前范围限 React className、CSS utility/共享 selector、客户端 smoke/source canary、brief 和开发文档；取题面板、当前/预览试卷、场中记录、虚拟考生、三处预览/仪程面板和最近交卷提示新增 `.paperMotionSurface`，共享材质 selector 收敛为 `.paperMotionCard`、`.paperMotionPanel`、`.paperMotionSurface` 三个语义 hook；考试纸卷专属动画、伪元素、reduced-motion selector、三类 exam API handler、本地反馈草稿和移动端布局保持不变。`.examSafetyBoundary` 不纳入本轮。未删除仍有源码命中的业务布局 selector，不新增浏览器裁决权，不改科举取题、推进、交卷、评分、舞弊、放榜、晋级、授官、后端 API/schema、AI 权限、prompt、provider、SQLite、存档、runtime manifest 或素材 manifest。实现提交 `b648f771`；本次 hash 回填仅修改本文与 [SHARED_CONTEXT.md](SHARED_CONTEXT.md)，属于低风险纯文档维护，跳过子代理复审。
- 2026-05-26：S89.50 完成共享纸面动效 keyframe 语义化与 token 化。当前范围限 CSS tokens、motion keyframes、共享纸面 utility、客户端 smoke/source canary、brief 和开发文档；把 S89.30 运行时编号 keyframes 改为稳定语义名，并把草稿脉冲 outline 与状态洗阴影颜色接入集中 token。未删除仍有源码命中的业务布局 selector，不新增浏览器裁决权，不改 React 组件、后端 API/schema、AI 权限、prompt、provider、SQLite、存档、runtime manifest 或素材 manifest。实现提交 `aca84940`；本次 hash 回填仅修改本文件与 [SHARED_CONTEXT.md](SHARED_CONTEXT.md)，属于低风险纯文档维护，跳过子代理复审。
- 前一轮 S88 归档是低风险纯文档维护；S89.3-S89.50 涉及前端代码、样式、验证脚本和文档，提交前按子代理复审规则执行。
...
### 2026-05-26：S89.50 共享纸面动效 keyframe 语义化与 token 化

- 范围：`client/src/styles/motion/keyframes.css` 中共享纸面动效 `s8930PaperRise`、`s8930StateWash`、`s8930SealBloom` 改为 `paperSurfaceRise`、`paperWrittenStateWash`、`paperSelectedSealBloom`；`draftWrittenPulse` 不再硬编码 `#8e2f2738`，改用 `--qq-color-vermilion-glow-outline`。`tokens.css` 新增朱色 glow、暖阴影和宣纸 inset token；`polish-surfaces.css` 和 `scripts/clientSmoke.js` 改读语义 keyframe 名。
- 边界：本轮只做共享 CSS 命名与 token 收敛，不改 React 组件、页面 className、前端数据流、后端 API/schema、AI 权限、prompt、provider、SQLite、存档、runtime manifest、素材 manifest 或服务器裁决；浏览器仍不裁决资源、身份、交易、委派、NPC 行动、经济、考试、官职、地图行动、关系、婚姻、弹劾、定罪、背叛或 hidden 信息。
- Smoke/canary：`test/reactClientScaffold.test.js` 新增 S89.50 source canary，守住语义 keyframes、token 使用、旧 S89.30 keyframe 名不再出现在运行时 CSS/smoke 中；既有 S89.5/S89.7/S89.30/S89.38 canary 同步改读语义 keyframe 名。
- 验证：已通过 `node --check scripts/clientSmoke.js`、`node --test test/reactClientScaffold.test.js`（83 tests）、`npm run typecheck:client`、完整串行客户端套件 `npx vitest --config vitest.config.mjs run --pool=vmThreads --fileParallelism=false --maxWorkers=1`（6 files / 134 tests）、`npm run qa:runtime-manifest`、`npm run build:client`、`npm run budget:client`、直接 browser smoke `node scripts/clientSmoke.js`、截图矩阵 `npm run smoke:browser:visual`、`npm run check:docs-governance`、`node --test test/documentationGovernance.test.js` 和 touched-files `git diff --check`。预算输出为 `JS 662.8 KiB / CSS 141.4 KiB / fonts 26288.4 KiB / client-assets 27092.6 KiB`，截图输出到 `artifacts/browser-visual-matrix`；构建仍输出既有 `/assets/ui/...` runtime asset resolution warning、npm `globalignorefile` warning 和 plugin timing warning。
- 提交前复审：只读复审 Harvey 未发现阻断问题，确认本轮只修改 tokens、motion keyframes、共享纸面 utility、browser smoke、source canary 和文档，未触碰 React 页面组件、前端数据流、后端 API/schema、AI 权限、素材 manifest、runtime manifest 或服务器裁决；旧 keyframe 名只保留在历史文档说明和测试负向断言中。非阻断建议为后续可继续把 `polish-surfaces.css` selected/written 静态 box-shadow 中同族硬编码朱色、暖阴影和 inset 色接入本轮新增 token。
- 提交：实现提交 `aca84940`；本次 hash 回填仅修改本文件与 [SHARED_CONTEXT.md](SHARED_CONTEXT.md)，属于低风险纯文档维护，跳过子代理复审。

### 2026-05-26：S89.49 科举页静态 surface utility 接入

- 范围：`ExamPage` 的 `.examQuestionPanel`、当前/预览 `.examDesk`、两处 `.examRecordPanel`、`.examPeerPanel`、三处 `.examPreviewPanel` 和 `.examRecentSubmitPanel` 新增 `.paperMotionSurface`；`polish-surfaces.css` 第一条 shared-paper selector 收敛为 `.paperMotionCard`、`.paperMotionPanel`、`.paperMotionSurface` 三个语义 hook，不再包含科举 route-specific class。
- 边界：本轮只做 className 与共享 selector 收敛，不改 `handleQuestion`、`handleProgress`、`handleSubmit`、`requestExamQuestion(sessionId, level)`、`progressExam(sessionId, examId, sceneAction.trim())`、`submitExam(sessionId, examId, essay.trim())`、route-local session guard、反馈拟行动本地草稿、考试纸卷 `data-polish-exam-paper`、`::before`、展开动画、reduced-motion selector、高对比覆盖、移动端布局、后端 API/schema、AI 权限、prompt、provider、SQLite、存档、runtime manifest 或素材 manifest；浏览器仍不裁决取题、推进考场、交卷、评分、舞弊、放榜、晋级、授官、资源、身份、交易、委派、NPC 行动、经济、官职、地图行动、关系、婚姻、弹劾、定罪、背叛或 hidden 信息。`.examSafetyBoundary` 不纳入本轮，因为它不是原 shared-paper 静态材质 selector 成员且承担虚线安全边界读法。
- Smoke/canary：`test/reactClientScaffold.test.js` 新增 S89.49 source canary，守住六类科举静态 surface hook、至少三处 `.examPreviewPanel`、至少两处 `.examRecordPanel`、三类 exam handler/API 调用、shared selector 纯语义化、未误接 `.paperMotionCard` / `.paperMotionPanel` 和专属 reduced-motion selector；`scripts/clientSmoke.js` 在取题前后检查所有已渲染科举静态面板均带 `.paperMotionSurface`，并在取题后确认当前试卷仍是 `[aria-label='当前试卷'].examDesk.paperMotionSurface[data-polish-exam-paper='s89-33-exam-ceremony-material']`。
- 复审：开工只读审计子代理 Banach 建议 S89.49 可单步迁移科举六类静态容器，保留 `.examSafetyBoundary`、考试 API handler、route guard、反馈草稿、考试纸卷专属动画/伪元素/reduced-motion 和移动端布局；本轮按建议实施。
- 验证：已通过 `node --check scripts/clientSmoke.js`、`node --test test/reactClientScaffold.test.js`（82 tests）、`npm run typecheck:client`、完整串行客户端套件 `npx vitest --config vitest.config.mjs run --pool=vmThreads --fileParallelism=false --maxWorkers=1`（6 files / 134 tests）、`npm run qa:runtime-manifest`、`npm run build:client`、`npm run budget:client`、直接 browser smoke `node scripts/clientSmoke.js`（首次 360s 外层限时截断，600s 重跑通过）、截图矩阵 `npm run smoke:browser:visual`、`npm run check:docs-governance`、`node --test test/documentationGovernance.test.js` 和 touched-files `git diff --check`。预算输出为 `JS 662.8 KiB / CSS 140.8 KiB / fonts 26288.4 KiB / client-assets 27092.0 KiB`，截图输出到 `artifacts/browser-visual-matrix`；构建仍输出既有 `/assets/ui/...` runtime asset resolution warning、npm `globalignorefile` warning 和 plugin timing warning。
- 提交前复审：只读复审 Hume 未发现阻断问题，确认本轮只给科举页静态容器追加 `.paperMotionSurface`、`.examSafetyBoundary` 未纳入、shared-paper selector 已收敛为三个语义 hook，且考试 API/handler、route guard、反馈草稿、考试纸卷专属动画/伪元素/reduced-motion、后端 API/schema、AI 权限、runtime manifest 和服务器裁决边界未变。非阻断建议为后续可显式给 `.examSafetyBoundary` 不接 `.paperMotionSurface` 增加 source canary。
- 提交：实现提交 `b648f771`；本次 hash 回填仅修改本文件与 [SHARED_CONTEXT.md](SHARED_CONTEXT.md)，属于低风险纯文档维护，跳过子代理复审。

### 2026-05-26：S89.48 人物页静态 surface utility 接入

- 范围：`PeoplePage` 的人物分组 `.npcGroupList`、人物详情工作台 `.npcDetailWorkbench` 和人物谱牒 `.portraitLedger` 新增 `.paperMotionSurface`；`polish-surfaces.css` 第一条 shared-paper selector 移除这三处 route-specific class，继续保留科举页静态面板 selector 作为下一步单独审计对象。
- 边界：本轮只做 className 与共享 selector 收敛，不改 NPC 选择逻辑、`paperMotionSelected`、`data-gallery-selected`、画像懒加载、人物画屏、交游候复笺、跨页追索、对话/交易/委派/交游 handlers、本地草稿写入、人物 safe view 消费、移动端布局、后端 API/schema、AI 权限、prompt、provider、SQLite、存档、runtime manifest 或素材 manifest；浏览器仍不裁决资源、身份、交易、委派、NPC 行动、经济、考试、官职、地图行动、关系、婚姻、弹劾、定罪、背叛或 hidden 信息。
- Smoke/canary：`test/reactClientScaffold.test.js` 新增 S89.48 source canary，守住三处人物静态 surface hook、共享 selector 收敛、未误接 `.paperMotionCard` / `.paperMotionPanel`、人物选中态与四类 mutation handler；`scripts/clientSmoke.js` 在人物页实际 DOM 检查 `.portraitLedger.paperMotionSurface`、`.npcGroupList.paperMotionSurface`、`.npcDetailWorkbench.paperMotionSurface` 且无缺口。
- 复审：开工只读审计子代理 Nietzsche 建议 S89.48 聚焦人物页三处，科举页因 `.examDesk[data-polish-exam-paper="s89-33-exam-ceremony-material"]` 专属展开动画、伪元素、reduced-motion 规则和取题/推进/交卷链路拆到后续 S89.49；本轮按建议实施。
- 验证：已通过 `node --check scripts/clientSmoke.js`、`node --test test/reactClientScaffold.test.js`（81 tests）、`npm run typecheck:client`、完整串行客户端套件 `npx vitest --config vitest.config.mjs run --pool=vmThreads --fileParallelism=false --maxWorkers=1`（6 files / 134 tests）、`npm run qa:runtime-manifest`、`npm run build:client`、`npm run budget:client`、直接 browser smoke `node scripts/clientSmoke.js`、截图矩阵 `npm run smoke:browser:visual`、`npm run check:docs-governance`、`node --test test/documentationGovernance.test.js` 和 touched-files `git diff --check`。预算输出为 `JS 662.6 KiB / CSS 140.9 KiB / fonts 26288.4 KiB / client-assets 27091.9 KiB`，截图输出到 `artifacts/browser-visual-matrix`；构建仍输出既有 `/assets/ui/...` runtime asset resolution warning、npm `globalignorefile` warning 和 plugin timing warning。
- 复审：提交前只读复审 Hooke 未发现阻断问题，确认本轮只迁移人物页三处、未提前处理科举候选；新增 `.paperMotionSurface` 继续承接原 shared selector 阴影，视觉风险很低。非阻断建议为后续若人物页新增同类容器需同步调整 `peopleStaticSurfaceCount === 3`。
- 提交：实现提交 `7f5e322f`；本次 hash 回填仅修改本文件与 [SHARED_CONTEXT.md](SHARED_CONTEXT.md)，属于低风险纯文档维护，跳过子代理复审。
