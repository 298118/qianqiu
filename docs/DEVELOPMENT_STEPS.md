...
| S74.0 | TODO | Codex + 只读依赖审阅子代理 | 依赖治理与 React 迁移契约 | 通过治理后再安装 React、TypeScript、Vite、React Router、Zustand、Lucide、Vitest、Testing Library 和 jsdom；新增前端迁移契约，不切默认入口 |
...
- 将 `react-router` 从暂缓项移动到 S74.0 候选依赖；S74.0 需要固定 Data Mode、`createBrowserRouter` / `RouterProvider`、`basename: "/ink-client"`、URL 安全边界、`/ink-client/*` 刷新 fallback、动效和回滚，不采用 React Router Framework Mode 接管 Express 后端。
