...
| S88.5 | IN_PROGRESS | 六身份循环矩阵 | S88.5.1 已先建立服务器派生的 `roleCycleView` 与 React `RoleCycleSection` 首片，把皇帝、大臣、将领、地方官、书生、入仕官员统一为当前身份循环矩阵：只公开当前身份事务、风险、待办、AI read scope、工具权限、proposal 边界和服务器裁决说明，非当前身份只显示待任占位。S88.5.2 已启动跨域入口与证据 refs：地方官循环只读接入市价与人物月账，将领循环只读接入舆图与战事档案，并以前端 route/surface 入口展示，不新增写 API 或持久账本。S88.5.3 已接普通回合后端接缝：低风险市价处置、军议侦察和调粮复用既有 city/military resolver；人物月账保持只读。S88.5 前端矩阵可见化片已完成，实现提交 `5614d6bc`。当前信息密度 polish 已完成：`RoleCycleSection` 只消费既有安全 `roleMatrix`，为六身份矩阵增加日期/回合、职责层级、服务器清洗摘要、固定中文取材域标签和仅当前身份警势，仍不展开其他身份案源、不新增写 API、不替代服务器裁决；实现提交 `2b9e1483`。本轮继续完成交互密度/权责边界 polish：`RoleCycleSection` 新增本身份速览、本身份公开取材和“可读材料与裁决边界”，只从当前 `roleCycleView.currentRole`、`aiReadScope.allowedSourceViews`、`toolPermissions`、`proposalBoundaries`、`serverAdjudication` 与安全 flags 派生中文标签和短句，过滤 provider/raw/key/hidden/本地路径污染，不显示 sourceId，不展开非当前身份案源，按钮仍只写本地草稿；实现提交 `d95f2ac5`。 |
...
| S88.9 | IN_PROGRESS | React 前端操作与状态打磨 | 第一片已启动案卷异步状态防线：`loadSession`、快捷建议、topic surface/draft、囊箧、NPC 名册和 NPC 详情读取都带请求序号、session/surface/NPC guard，旧请求晚到或错配回包不会覆盖当前路由或继续执行旧调用方副作用；`GamePage` 只消费当前路由匹配的 session、lastTurn 和 UI safe payload，路由切换会清空不匹配的 UI payload；`SurfaceHost` 也按当前案卷守住专题材料、本地草稿、证据选择和 draft context，避免同 surface 旧案卷材料串入新案卷。实现提交 `1e1cb7a0`。第二片推进人物页/囊箧页 route-local 状态防线与印匣旧案刷新防线，实现提交 `960e0336`。第三片推进科举、皇榜和舆图 route-local 状态防线，实现提交 `9b32068a`。第四片推进设置页全局 AI 请求 race/loading/empty 防线，实现提交 `ae5b64fd`。第五片实现提交 `2dccdae0`：`openSurfaceForSession(surface, sessionId)` 会先绑定当前 route 案卷并丢弃旧安全摘要，朝议专题、史册“阅奏折”和舆图“筛舆图”不再沿用 stale UI `currentSessionId` 拉取旧案卷 topic surface。第六片已完成 overlay 焦点与皇榜跳转可访问性打磨，实现提交 `ca472648`：抽屉/弹窗/专题层/立绘查看器统一 Tab 焦点循环，史册/舆图 surface 关闭后回到触发按钮，皇榜“跳至我名”会聚焦榜名详情。第七片实现提交 `71328cfa`：史册、皇榜和舆图移动端/长文本溢出 polish 已补 CSS `min-width: 0`、`overflow-wrap`、移动端单列/auto-fit 布局与地图标签省略；browser smoke 扩展移动史册路径和关键文本节点溢出扫描；React scaffold 增加对应静态守门。第八片实现提交 `65868232`：系统级 reduced motion 会关闭舆图 runtime 动效与皇榜装饰动画，设置页 AI 任务矩阵空/载入/错误态改为安全状态块，移动端设置操作全宽触控。第九片顶部主导航 active 精准化实现提交 `7223c036`。第十片实现提交 `9a7d287d`：可运行 `/game/:sessionId/...` 坏路由显示回主卷，预览/畸形案卷不提供案卷恢复；route error 文案固定脱敏。第十一片实现提交 `12dda964`：只在 `/game/:uuid/子路径` 提供回主卷，非 UUID 畸形案卷与根案卷 error 都不提供自指恢复。第十二片实现提交 `48e11b25`：`isRouteLocalSessionId()` 只允许可运行 UUID 和明确预览 id 进入 UI session 指针/local surface/顶栏当前案卷 href，舆图、史册和朝议页对畸形案卷禁用专题按钮、回退链接到首页且不显示原始畸形 id。第十三片实现提交 `1df8b3f5`：人物、囊箧、科举和皇榜页现在也用 `isRouteLocalSessionId()` 守住畸形案卷，固定中文安全空态替代旧案卷 stale 数据，禁用本地人物档案、移置、考试和跳榜操作，并避免发起案卷 API；设置页仍是全局 AI 设置入口，不按案卷局部 surface 处理。上一片把主卷 `actionDraft` 绑定 `sessionId`，route/payload/surface 切案卷会清空不匹配草稿与本地 surface，`GamePage` 只把当前案卷草稿和 `draftContext` 交给底部奏折/提交/快捷建议预览；身份循环入口也改用 `openSurfaceForSession(surface, sessionId)`，防止旧案草稿或旧 UI 指针带入新案卷。上一片补强 NPC/囊箧 route-local 读取：`loadInventory()`、`loadNpcs()` 和 `loadNpcDetail()` 成功/失败写 store 前再次复核当前案卷，旧读取不得拉回 `currentSessionId`、覆盖当前安全投影/status/loading/error；人物页同案卷旧 NPC mutation 回执必须同时匹配当前 NPC 与提交时草稿快照，避免清空当前 NPC 或同 NPC 已继续编辑的新草稿。当前囊箧同案卷移置表单防串扰片实现提交 `607a7fd8`：`InventoryPage` 提交移置时记录物件 id 与目标容器快照，响应回来时只有 route session、回包 session、当前表单物件和目标仍匹配，才显示移置提示、清空物件/去处选择并切换容器焦点；若玩家已改选另一个物件或去处，旧响应只通过 store 合并服务器安全投影，不覆盖当前表单。本片不新增后端 API 或服务器裁决权。后续继续逐页补 loading、empty、error、移动端和操作效率，或转回 S88.12 真实 keyed provider 长跑证据。 |
| S88.10 | IN_PROGRESS | PixiJS 水墨地图行动入口 | S88.10.1 已启动前端安全行动入口首片：复用 `mapRuntimeView.refs/routes/eventEffects/actionDrafts` 生成舆图行动牌，并把地点、驿路和近事草稿写成带安全 refs 的本地 `draftContext`；仍不新增后端 API 或浏览器裁决权。S88.10.2 已完成后端复核链，实现提交 `b3078249`：普通/SSE turn 会在 topic draft 校验不命中后重建当前 `mapRuntimeView`，用 `normalizeMapRuntimeTurnContext()` 只采纳当前安全舆图 refs，并把通过校验的 `mapRuntimeDraftContext` 作为 role-cycle outcome/ledger 审计 hint；地方案牍、军务报告和市价粮价舆图草稿使用服务器模板触发既有低风险 city/military resolver，不引入地图写 API、新工具权限或浏览器裁决。S88.10.3 已补 true `provider.streamTurn()` 分支 focused 回归，实现提交 `d7bf0e7a`，覆盖将领从 `mapRuntimeView` 军务报告行动草稿提交 SSE 回合时的服务器重建、伪造 refs/layout/viewport/visual-only effect 过滤、`actionDraftRefs` 审计和 raw 军务账本剥离。S88.10.4 已启动 NPC 舆图活动锚点首片，实现提交 `44016b01`：`mapRuntimeView.npcActivityAnchors` 只从安全 `npcActiveRequestView` 与当前可见 `worldPeopleView` 地点交叉派生 read-only 视觉锚点，Pixi/React 舆图只展示人物来函/续办线索。S88.10.5 已启动 NPC 关系行动舆图锚点片，实现提交 `85cb1593`：关系行动锚点只从安全 `npcInteractionView` 的 `npc_relationship_action_resolver` 记录与当前可见 `worldPeopleView` 地点交叉派生，继续通过 `npcActivityAnchors` 只读显示论道、切磋、求爱和议婚公开裁决线索，不新增行动草稿、写 API、AI 工具权限、服务器 resolver 或 canonical NPC/地理/关系事实。后续可继续评估真实 keyed provider 长跑证据或 S88.7 实体压力影响。 |
| S88.11 | IN_PROGRESS | 视觉、立绘与氛围一致性 | 首片已启动 runtime manifest 生成层 QA，实现提交 `099d4ab3`。第二片已推进 assetRegistry 消费层 QA，实现提交 `89059f7a`：前端 registry 会拒绝全 manifest 重复 asset id / portraitRef、非成年或缺显式元数据的立绘、懒加载策略缺口、池分组与 subcategory 不一致，并把通用 NPC 画像查询固定在 `generic_npc_pool`，调用方不能用大 limit 突破首屏立绘预算。第三片 browser smoke 细化实现提交 `91539d39`：真实浏览器会显式断言只请求 runtime manifest、不请求完整 source manifest，复核 runtime manifest 字段/路径/审核/成年立绘/lazy-load 边界，并把人物页 `data-portrait-ref` 与重要 NPC 专属池 metadata 交叉验证，首屏立绘/缩略图/占位请求仍限制在 8 张内；本片不新增图片、不改变可用资产、不放宽前端加载策略。 |
...
- 2026-05-24：推进 S88.5 六身份循环交互密度与权责边界 polish，实现提交 `d95f2ac5`。`client/src/components/RoleCycleSection.tsx` 继续只消费当前案卷安全 `roleCycleView`，新增“本身份速览”统计当前身份事务/风险/入口/草稿数量，新增“本身份取材”只显示当前身份顶层 evidence label，新增“可读材料与裁决边界”把 `aiReadScope.allowedSourceViews` 映射为固定中文来源标签，并展示只读视图、前端草稿、服务器裁决和服务器边界短句；`localRoleCyclePathPattern` 会拒绝 Windows / POSIX / macOS / CJK 标点前缀本地路径污染，provider/raw/key/hidden 等旧污染词继续整条回落。非当前身份仍只显示待任占位和泛化取材域，不展示 sourceId、items、entryPoints、evidenceRefs 或真实压力；入口只做 route allowlist 跳转或打开本地 surface，草稿按钮仍只写本地行动草稿，不调用 `/api/game/turn`。本轮同时修复 S88.7 史册实体压力 browser canary 的首屏 cap 缺口：`ArchivePage` 在 12 条可见史册上限内保留至少一条 `world_entity_impact` 公开条目，避免实体压力计数存在但首屏 DOM 被 event/world thread 条目挤出；`localArchivePathPattern` 同步拒绝 Windows / POSIX / macOS / CJK 本地路径污染，污染实体压力条目会先被安全清洗过滤，再由后一条安全实体压力补位。该规则只调整前端展示优先级，不新增后端 API、存档字段、provider schema、SQLite schema、AI 权限、prompt、服务器 resolver、资源/关系/婚姻/弹劾/定罪/背叛/NPC 行动裁决或 hidden 信息读取。已通过本节验证口径中的 client typecheck、focused/full App 与 React scaffold、完整 client Vitest、client build、Mock browser smoke、docs governance、documentation governance、完整 `npm test`（1159 项）和 `git diff --check`；完整 client Vitest 曾命中一次既有立绘选择瞬时断言，原命令重跑通过。McClintock 只读梳理建议把 `aiReadScope`、tool/proposal/server boundary 和 safety flags 呈现为前端只读边界，本轮采纳。Bernoulli 提交前只读初审发现 RoleCycle 漏拦 `/Users` 与 ArchivePage POSIX/macOS 路径污染两个 P2，已补路径 regex、App 污染 fixture 与静态守门；Bernoulli 最终窄复核确认两个 P2 关闭，未发现新的 P0/P1/P2。
- 2026-05-23：补 S88.7 实体压力史册 browser smoke DOM canary，实现提交 `ba65a8d1`。`scripts/clientSmoke.js` 新增真实 UI 路径 canary：默认 Mock 书生案卷进入人物页“礼法”，对默认可见 NPC 提交“请论道”，等待 `/api/game/npc-interaction/:sessionId` 响应，确认响应安全投影里有 `world_entity_impact` 史册条目和 `npc_relationship_action` recent impact，再进入史册 DOM 验证 `li[data-source-type='world_entity_impact']`、“实体”统计和公开压力文案可见。DOM 文本与安全属性会拒绝 unsafe state/dev API、raw/provider/key/path、raw ledger、SQLite index、`sourceRef`、`relatedRefs`、`scopeRefs` 等污染；响应侧污染扫描只覆盖本次实体史册条目和即时实体影响，避免安全 `hiddenNotice` 提示误报。本轮不新增写 API、后端契约、SQLite schema、存档字段、AI 权限、prompt 或服务器裁决口。当前已通过 `node --check scripts/clientSmoke.js`、focused/full React scaffold、`npm run typecheck:client`、Mock browser smoke、docs governance、documentation governance、完整 `npm test`（1152 项）和 `git diff --check`（退出码 0，仅有未触碰归档/QA 文件既有 CRLF 提示）；Arendt 提交前只读复审未发现 P0/P1/P2，残余非阻断风险是 canary 依赖默认 Mock 案卷存在可用“请论道”关系行动，且响应级检查有意只扫目标条目/即时 impacts。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.7 实体压力追踪证据片，实现提交 `7d63e72a`。`applyWorldEntityInfluences()` 现在会把服务器已应用的公开实体影响同步整理为 capped `worldEntities.recentImpacts`，并通过 `worldEntityView.recentImpacts` 暴露安全证据链；每条 evidence 只保留公开 title/summary、实体名、状态/风险、指标标签、安全 `sourceRef`、`topicSurfaceIds`、`relatedRefs/scopeRefs` 和发生回合，隐藏实体、legacy 污染行、provider/prompt/path/key/SQLite/search-index 和已配置密钥片段会被过滤；本轮进一步把配置密钥片段过滤下沉到 `cleanSourceId()`，避免 public id / `sourceRef` / related ref 泄漏。`resolverInputContext` 现在构建 `worldEntityView` 并把 `recentImpacts` 纳入 `events` evidence；`topicSurfaceView` 在奏折、拟旨、朝议和人物档案 surface 只读展示这些 refs，堂审不接入；`safeWorldSearch` 与 SQLite `safe_search_index` 会在全局 cap 下保护 `worldEntityView.recentImpacts` 行。该链路只追溯公开实体压力，不新增写 API，不让浏览器、AI、topic draft 或搜索结果结算资源、人情债、关系终局、婚姻、弹劾、定罪、背叛、NPC 行动或 hidden 私档。已完成 focused 语法、world entity/resolver/topic/search/SQLite 回归、`typecheck:server`、docs governance、documentation governance、完整 `npm test` 和 `git diff --check`；Bernoulli 提交前只读复审未发现 P0/P1/P2。
- 2026-05-23：推进 S88.7 NPC 关系网实体压力片，实现提交 `184a14b2`。`src/game/worldEntities.js` 新增 `npc_relationship_action` 公开影响来源，并把 canonical `npc_active_request_follow_up_resolver` 后续登记纳入公开实体压力；只接受 `serverOwnsFollowUp === true` 的 follow-up resolution，以及 `status: "server_adjudicated"` 且 `serverOwnsOutcome === true` 的关系行动 trace，blocked 关系行动不产生实体压力，source id 会清洗 raw/private/provider/prompt/path/key/SQLite/search-index 污染；提交前复审后进一步把 `normalizeEntityInfluence()` 的 public `sourceId` 出口也改为 `cleanSourceId()` backstop。`POST /api/game/npc-interaction/:sessionId` 会从刚写入的安全 `recordNpcInteraction()` 记录派生并应用实体影响，响应新增 `worldEntityView` / `worldEntityImpacts`；`src/contracts/serverContracts.ts` 与 `client/src/api/types.ts` 已同步该 response shape。React store 只在案卷匹配时合并 `worldEntityView`，人物页新增只读“关系网影响”，仅消费当前安全 `worldEntityView` 与本次同案卷 `worldEntityImpacts`，不展示 source refs，不调用 turn API，不裁决资源、关系、婚姻、弹劾、定罪、NPC 行动或 hidden 信息。已通过 focused 语法、world entity、NPC route、server/client typecheck、focused client 回归、完整 client Vitest、client build、Mock browser smoke、docs governance、documentation governance、完整 `npm test`（1141 项）和 `git diff --check`；McClintock 提交前只读复审发现 public impact `sourceId` backstop 与本条验证状态两个 P2，代码侧已修复并经窄复审确认关闭，剩余验证计数文档 P2 已修正。
- 2026-05-23：推进 S88.5 六身份矩阵信息密度 / 身份差异化 polish，实现提交 `2b9e1483`。`client/src/components/RoleCycleSection.tsx` 的矩阵卡片继续只读既有安全 `roleCycleView.roleMatrix`，新增日期/回合、职责层级、服务器清洗摘要、固定中文取材域标签和仅当前身份的“警势 N”；`sourceViews` 只映射为中文标签，不作为入口，不展示 sourceId、evidence refs、items 或 entryPoints，非当前身份仍只显示“待任后展开”和泛化边界说明。`client/src/styles/global.css` 新增取材域/警势紧凑样式，`client/src/__tests__/App.test.tsx` 扩展 S88.5 行为回归，确认日期回合、取材域、当前警势、非当前占位可见且不调用 `/api/game/turn`；`test/reactClientScaffold.test.js` 同步静态守门。本轮不改后端 API、provider schema、SQLite schema、存档格式、AI 权限、prompt、resolver、`roleCycleDomainAdjudication` 或服务器裁决边界。已通过 `npm run typecheck:client`、focused React scaffold、focused App Vitest、完整 client Vitest、`npm run build:client`、完整 React scaffold、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1137 项）和 `git diff --check`；Turing 提交前只读复审仅发现共享上下文验证状态待回填 P2，代码/安全边界未发现 P0/P1/P2，本轮已修正。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.5 六身份循环矩阵前端可见化片，实现提交 `5614d6bc`。`client/src/components/RoleCycleSection.tsx` 新增 `cycleRoleMatrix()`，只从既有安全 `roleCycleView.roleMatrix` 读取 6 个身份的 `roleLabel`、`loopLabel`、`statusLabel`、`enabled` 和当前身份 `itemCount`，在六身份主面板中展示“六身份矩阵”；当前身份显示“本身份 · N 项可见事务”，非当前身份只显示服务器已给出的“待任后展开”，不读取其他身份案源、source view 内容、hidden 任所、私档、raw ledger 或 provider/prompt 细节。`client/src/styles/global.css` 新增矩阵紧凑布局和移动端单列约束，`client/src/__tests__/App.test.tsx` 增加预览案卷行为回归，确认矩阵展示书生、地方官、入仕官员、皇帝等身份、5 个非当前身份占位，且不调用 `/api/game/turn`；`test/reactClientScaffold.test.js` 同步静态守门。本轮不改后端 API、provider schema、SQLite schema、存档格式、AI 权限、prompt、resolver、`roleCycleDomainAdjudication` 或服务器裁决边界。已通过 `npm run typecheck:client`、focused React scaffold、focused App Vitest、完整 client Vitest、`npm run build:client`、完整 React scaffold、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1137 项）和 `git diff --check`；Tesla 提交前只读复审未发现 P0/P1/P2，并额外确认前端只读 `roleMatrix`、不展开非当前身份案源、不隐式调用 `/api/game/turn`。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.11 browser smoke 视觉/立绘运行时验收第三片，实现提交 `91539d39`。`scripts/clientSmoke.js` 新增 runtime manifest 结构校验和浏览器请求边界：默认 smoke 会记录页面 manifest 请求，要求前端至少请求 `/assets/ui/ink-ui-runtime-manifest.json`，且不得请求完整 `/assets/ui/ink-ui-manifest.json`；manifest 安全复核改由 Node 侧 `fetch` 读取 runtime manifest，避免 smoke 自己的页面请求污染浏览器 request listener，并递归检查允许 key、`/assets/ui/` 安全路径、runtime 可用审核状态、blocked 状态不落入资产、fallback、成年立绘、缩略图/低清占位、`allowEagerLoad=false`、`thumbnailFirst=true`、`maxInitialPortraits <= 8`、池分组与 subcategory 一致、重复 asset id / portraitRef 和 `raw/provider/prompt/key/localHighResSourcePath/sourcePath/SQLite` 等污染词。人物页 smoke 新增 `assertPeoplePortraitRuntimeSafety()`，只在 `.peopleLedgerList` 范围内接受当前可见人物的 `data-portrait-ref`，要求 `data-visible-portraits` / figure 数量与 `data-visible-people` 对齐、图片数量不超过当前页人数、首屏主图/缩略图/占位请求各不超过 8，并把 DOM ref 与 runtime manifest 中 `signature_npc_pool`、`portrait_pool_signature_npc_s73_10`、`important_npc` 专属立绘 ref 交叉验证，避免普通人物页混入重要 NPC 专属池。本轮不新增图片、不改变 836 个 runtime 可用资产、不改 runtime manifest 文件、不新增后端 API、provider schema、SQLite schema、存档格式、AI 权限、prompt 或服务器 resolver。已通过 `node --check scripts/clientSmoke.js`、focused React scaffold（6 项）、`npm run qa:runtime-manifest`、`npm run typecheck:client`、`AI_PROVIDER=mock npm run smoke:browser`、完整 React scaffold（42 项）、完整 client Vitest（118 项）、docs governance、documentation governance、完整 `npm test`（1137 项）和 `git diff --check`；Parfit 提交前只读初审发现 manifest 安全校验通过页面 fetch 可能让 smoke 自己的请求满足 runtime manifest 请求守门的 P2，已改为 Node 侧 fetch 并补旧签名静态回归，窄复审确认 P2 关闭且未发现新的 P0/P1/P2。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.11 assetRegistry 消费层 QA 第二片，实现提交 `89059f7a`。`client/src/assets/assetRegistry.ts` 现在在创建 registry 时先全 manifest 检查重复 asset id / portraitRef，避免 blocked 或 unsafe 记录与 approved 记录共享身份后被间接寻址；active map 写入仍保留重复防线，防止 `assetById` / `portraitByRef` 静默覆盖。立绘消费层进一步要求显式 `genderPresentation`、`ageBand`、`statusVariant`、`lazyLoad.group`、`thumbnailFirst=true`、`lowResPlaceholder=true`、成年 `ageBand`、安全高清母版标记和立绘池 group/subcategory 一致；`getInitialPortraits()` 与 `getPreloadHints()` 会把调用方大 `limit` clamp 到 manifest lazy budget 与 8 张上限。`PeoplePage` 的通用 NPC 画像查询同时要求 `subcategory: "generic_npc_pool"` 与 `portrait_pool_generic_npc_s73_10`，即使 runtime manifest 被污染也不会从重要 NPC 专属池抽通用人物。`client/src/assets/assetRegistry.test.ts` 新增重复身份、blocked duplicate、缺成年/懒加载元数据、signature/generic 池隔离和 preload limit clamp 回归；React scaffold 新增静态守门。本轮不新增图片、不改变 836 个 runtime 可用资产、不改 runtime manifest 文件、不新增后端 API、provider schema、SQLite schema、存档格式、AI 权限、prompt 或服务器 resolver。已通过 client typecheck、focused assetRegistry Vitest（10 项；未加 worker 限制的单文件 Vitest 曾因 forks worker 启动超时失败，随后受控 worker 重跑通过）、focused/full React scaffold（41 项）、runtime manifest QA、完整 client Vitest（118 项）、client build、Mock browser smoke、docs governance、documentation governance、完整 `npm test`（1136 项）和 `git diff --check`；`git diff --check` 仅打印未改动归档/QA 文件既有 CRLF 警告。Socrates 提交前只读复审未发现 P0/P1/P2；非阻断建议是后续可把 `ageBand` 从 `startsWith("adult")` 收紧为明确枚举白名单。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.11 runtime manifest 生成层 QA 首片，实现提交 `099d4ab3`。`scripts/frontendRuntimeManifest.js` 新增 `validateRuntimeManifestSafety()`，`npm run qa:runtime-manifest` 与 `qa:runtime-manifest:write` 在生成后、写入前校验精简 manifest：顶层、fallback、asset、lazyLoad 和 source 只能保留浏览器运行时允许字段；所有 active 资产必须是 `runtimeUsableReviewStatuses`、路径只能位于 `/assets/ui/` 且不能含本地/远程/artifacts/key/raw/provider/hidden/prompt/SQLite 污染；立绘必须满足 `portraitRef === id`、成年 `ageBand`、缩略图、低清占位、fallback、懒加载分组、`thumbnailFirst=true`、`lowResPlaceholder=true`、`maxInitialPortraits <= 8` 和 `allowEagerLoad=false`。React scaffold 新增污染 runtime manifest 回归，确认未审核状态、作者字段、非成年立绘、eager-load、本地高清源路径以及 `providerPayload` / `SQLite` / `safe_search_index` 等 key/value 词形都会被拒绝。本轮不新增图片、不改 manifest 可用资产、不改前端组件加载策略、不放宽重要 NPC / 通用池隔离、不新增后端 API、provider schema、SQLite schema、存档格式、AI 权限或服务器 resolver。已通过 JS 语法检查、runtime manifest QA、focused/完整 React scaffold、完整素材 manifest QA、docs governance、documentation governance、完整 `npm test`（1136 项）、Mock browser smoke 和 `git diff --check`；首次 120 秒 `git diff --check` 包装超时仅输出既有 CRLF 警告，600 秒重跑通过且仍只打印未改动归档/QA 文件既有 CRLF 警告。Averroes 提交前只读初审发现污染扫描未覆盖 key 与 camelCase/SQLite 词形的 P2；本轮已改为递归扫描 key+value、扩展污染词，并补 `providerPayload` / `SQLite` / `safe_search_index` / fallback css token key 回归。P2 修复后已重跑 focused runtime manifest 回归、完整 React scaffold、docs governance、documentation governance、完整 `npm test`、Mock browser smoke 和 `git diff --check`；Averroes 窄复审确认 P2 已关闭，未发现新的 P0/P1/P2，残余非阻断风险是污染词仍为黑名单策略，后续新增 runtime 字段或内部命名口径时需同步补 token 与回归样例。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.10.5 NPC 关系行动舆图锚点片，实现提交 `85cb1593`。`src/game/mapRuntimeView.js` 现在只从安全 `npcInteractionView.items` 中 `resolverTrace.resolver === "npc_relationship_action_resolver"` 的论道、切磋、求爱和议婚记录派生 `npc_relationship_action` 锚点，并继续用当前可见 `worldPeopleView` 的人物 `currentPostingId/currentCityId/homeCityId` 绑定已有 `mapRuntimeView.refs`；锚点只携带 `npcInteractionView:*` 与 `npcRelationshipActionResolverTrace:*` visual-only source refs，不复用 trace 内 `npcRosterView`、eligibility refs、raw ledger 或 hidden/private 信号。`normalizeMapRuntimeTurnContext()` 已把 `npcInteractionView:*`、`npcRelationshipAction*`、`npcRelationshipActionEligibilityView:*` 和 `npcInteractionLedger:*` 等关系视觉来源加入全路径拒绝表，覆盖 refs/routes/effects/actionDrafts 所有 source ref 收集路径。边界：本片不新增后端 API、provider schema、SQLite schema、存档格式、地图素材 manifest、AI 工具权限、行动草稿、真实任务、NPC 行动裁决、资源/关系/婚姻/伤亡/弹劾/背叛事实落账或浏览器裁决权；地图 layout、坐标、点击位置和人物关系视觉锚点仍只供浏览器显示。已通过 JS 语法检查、focused/完整 map runtime 与 draft context 回归、地图前端 shell/React scaffold 回归、server/client typecheck、client build、完整 client Vitest（114 项）、Mock browser smoke、docs governance、documentation governance、完整 `npm test`（1135 项）和 `git diff --check`；`git diff --check` 退出码为 0，仅打印未改动归档/QA 文件既有 CRLF 警告。Aquinas 提交前只读复审未发现 P0/P1/P2；非阻断建议是后续新增 visual-only ref 命名时继续补拒绝表/污染测试，以及如需更精确产品文案可把关系锚点 actionType 收紧到 debate/duel/courtship/marriage。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.10.4 NPC 舆图活动锚点首片，实现提交 `44016b01`。`src/game/mapRuntimeView.js` 新增 `buildNpcActivityAnchors()`，仅从安全 `npcActiveRequestView` / `followUpEvidence` 与当前可见 `worldPeopleView` 的公开人物 `currentPostingId/currentCityId/homeCityId` 交叉生成 `npcActivityAnchors`，且必须绑定已有 `mapRuntimeView.refs`；`public/mapRenderer.js` 在事件层低调渲染人物来函/续办线索锚点并提供只读 tooltip，`client/src/pages/MapPage.tsx` 只读展示“舆图人物动向”，不提供行动按钮；`client/src/components/InkMapRuntimeBridge.tsx` 的事件图层筛选会同步隐藏 NPC 锚点。`normalizeMapRuntimeTurnContext()` 显式拒绝 `npcActiveRequestView:*` / `npcActiveRequestFollowUp*` / `npcActivityAnchor:*` visual-only source refs 成为服务器 evidence，并在 refs/routes/effects/actionDrafts 所有 source ref 收集路径统一过滤，即使未来这些前缀出现在 runtime 视觉层或被污染进草稿来源也不会进入 role-cycle resolver 审计。边界：本片不新增后端 API、provider schema、SQLite schema、存档格式、地图素材 manifest、AI 工具权限、真实任务、NPC 行动裁决、资源/关系/婚姻/弹劾/背叛事实落账或浏览器裁决权；地图 layout、坐标、点击位置和人物视觉锚点仍只供浏览器显示。已通过 JS 语法检查、focused map runtime / draft context 回归、完整 `test/mapRuntimeView.test.js`、`test/mapRuntimeDraftContext.test.js`、地图前端 shell/React scaffold 回归、`npm run typecheck:server`、`npm run typecheck:client`、`npm run build:client`、docs governance、documentation governance、完整 `npm test`（1134 项）、完整 client Vitest（114 项）和 `AI_PROVIDER=mock npm run smoke:browser`；首次 client Vitest 与完整 Node 测试并行时出现 forks worker 启动超时，单独重跑通过。Mill 提交前只读初审发现 NPC visual-only ref 拒绝未覆盖 refs/routes/actionDrafts source paths 的 P2，已统一过滤并补回归；Mill 窄复审确认 P2 关闭且未发现新的 P0/P1/P2；`git diff --check` 通过。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.10.3 PixiJS 舆图行动 streaming 长链路回归补强，实现提交 `d7bf0e7a`。`test/streamingTurnRoute.test.js` 新增 true `provider.streamTurn()` 分支用例：将领案卷先从当前安全 `mapRuntimeView` 取得 `military_report` ref 与 action draft，再携带混入伪造 `map:forged:secret`、`draft-forged-secret`、`layoutPath`、`mapBounds`、`viewportHint`、`providerPayload` 和 visual-only `domainConsequenceView:*` 的 `map-runtime` draftContext 走 SSE 普通回合；断言路由确实调用 `streamTurn()` 而非 fallback `runTurn()`，公开 outcome 为低风险 `military_diplomacy` / `scout`，`mapRuntimeDraftContext` 只采纳服务器重建后的安全 target/source/evidence/action draft refs，响应不回传 raw `militaryDiplomacyLedger` / `cityPolicyLedger` / resolver delta / auditRecord，保存态 accepted 军务记录附安全审计 hint 并重点断言 target/action draft refs。本片只补验证工具，不改变 API/schema、运行时行为、AI 权限、地图素材、SQLite schema、存档格式、服务器裁决或前端交互；产品 brief 中 S88.10.2 的行为边界仍适用。当前已通过 streaming route focused/full、docs governance、documentation governance、完整 `npm test`（1133 项）和 `git diff --check`；Galileo 提交前只读初审发现文档口径误称只保留 target/action draft refs 的 P2，已修正为 target/source/evidence/action draft refs 审计边界，Galileo 窄复核确认 P2 关闭且未发现新的 P0/P1/P2。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.10.2 PixiJS 舆图行动后端复核链，实现提交 `b3078249`。`src/game/mapRuntimeDraftContext.js` 新增 `normalizeMapRuntimeTurnContext()`，普通 `/api/game/turn` 与 SSE turn 会先尝试 topic draft 复核，未命中时重建当前 `mapRuntimeView` 复核 `surfaceId: "map-runtime"` 的目标、来源、证据和 action draft refs；伪造 refs、`layout` / `layoutPath` / `mapBounds` / `viewportHint` / 坐标、provider/raw/path/key/hidden 污染和 visual-only `domainConsequenceView:*` source refs 都不会进入服务器上下文。`MAP_RUNTIME_ACTION_DRAFT_TEMPLATES` 让案牍、军务报告和市价粮价舆图草稿改用领域化中文模板与 `domainIntentHint`，军务草稿只写低风险“遣哨侦察”并避开发兵、攻城、交战、奇袭、调兵等高风险命令；市价草稿自然触发既有平粜稳价/市价处置接缝。`roleCycleDomainAdjudication` 只把已复核的 map context 作为 `mapRuntimeDraftContext` 审计 hint 写入 accepted/duplicate outcome 与 accepted city/military ledger 最新记录，不把地图 layout、视觉 effect、浏览器点击或 action draft 变成 resolver evidence，也不新增地图写 API、AI 工具权限、NPC 行动、资源结算或 hidden 信息读取。本轮已通过语法检查、map runtime/draft context/role-cycle/game-turn focused tests、server/client typecheck、完整 client Vitest、client build、React scaffold、Mock browser smoke、docs governance、documentation governance、完整 `npm test`（1132 项）和 `git diff --check`；Laplace 初审发现内部 map context 审计入口未强制 schema/status 的 P2，已收紧为只接受 `s88.10-map-runtime-draft-context.v1` + `verified` 并补伪造 schema 回归，Laplace 窄复核确认 P2 关闭且未发现新的 P0/P1/P2。残余非阻断风险：SSE 与将领军务 map-runtime 审计路径主要依赖共享 normalizer 和现有 role-cycle 覆盖，后续可补专门 focused 回归。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.10.1 PixiJS 舆图安全行动入口首片，实现提交 `b4ba4ff5`。`MapPage` 现在从当前案卷的安全 `mapRuntimeView.refs/routes/actionDrafts` 派生“舆图行动”行动牌，地点与驿路按钮只把服务器预渲染 `actionText` 写入主卷本地草稿；`InkMapRuntimeBridge` tooltip 草稿、舆图行动牌和公开近事“据此拟稿”都会附带 `surfaceId: "map-runtime"`、`draftKind`、`sourceView: "mapRuntimeView"`、`evidenceRefs`、`sourceRefs`、`targetRefs`、`requiresServerTurn` 和 `client_hint` 状态，供后续服务器复核使用。草稿上下文只由安全 ref 字符串构成，显式不携带 renderer position、`layout`、`layoutPath`、`mapBounds`、`viewportHint`、坐标、raw/provider/prompt/path/key/hidden 文本；玩家仍需回主卷提交普通行动，服务器继续拥有移动、查案、军务、财政、外交、NPC 行动、任免和持久化裁决。本轮补齐 `MapRuntimeActionDraft` / `MapRuntimeRoute` / `TurnDraftContext` 前端类型、舆图行动牌 CSS、App 行为回归、React scaffold 静态守门和 browser smoke 结构断言；不新增后端 API、provider schema、SQLite schema、地图素材 manifest、AI 工具权限或 resolver。已通过 `npm run typecheck:client`、React scaffold（40 项）、focused 舆图 App 回归（1 项）、完整 client Vitest（114 项；曾有一次无关立绘选择断言瞬时失败，原命令重跑通过）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1126 项）和 `git diff --check`。Hume 提交前只读初审发现 ref sanitizer 未显式拒绝 layout/coords token 的 P2，已补 `layout` / `layoutPath` / `mapBounds` / `viewportHint` / `position` / `coordinate(s)` / `x` / `y` denylist、污染 fixture 和静态守门；Hume 最终只读复审未发现 P0/P1/P2，残余 P3 是后续可把 `getMapEvents()` 内部 `targetId` 改用 `safeMapPageRefId()` 并抽共享 ref sanitizer。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-23：推进 S88.9 不支持案卷 id 的人物/囊箧/科举/皇榜安全空态，实现提交 `1df8b3f5`。`PeoplePage`、`InventoryPage`、`ExamPage` 和 `RankingPage` 现在统一用 `isRouteLocalSessionId(sessionId)` 区分可运行 UUID / 明确预览案卷与畸形案卷；畸形 `/game/not-a-session/people|inventory|exam|ranking` 不写 UI `currentSessionId`，不消费旧案卷 `currentSession`、NPC/囊箧/科举/榜文 payload、错误态或 mutation 结果，不发起案卷 API，并显示固定中文安全空态。人物页“打开人物档案”改用 `openSurfaceForSession("npc-profile", sessionId)` 且不支持案卷时禁用；囊箧“呈请移置”、科举“取题/推进考场/交卷”和皇榜“跳至我名”在不支持案卷时禁用。设置页无 route `sessionId`，仍是全局 AI 设置入口，不纳入案卷局部 unsupported surface 防线。本轮只改前端安全消费、空态和 App/scaffold 回归，不新增后端 API，不改变 history fallback、route recovery、全局 AI 设置、route view、AI 权限、服务器裁决、考试晋级、放榜、资源、NPC、交易、委派、经济或官职规则。已通过 `npm run typecheck:client`、React scaffold（40 项）、focused unsupported App 回归（7 项）、完整 client Vitest（114 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、完整 `npm test`（1126 项）、docs governance、documentation governance 和 `git diff --check`；Hume 提交前只读复审未发现 P0/P1/P2，非阻断缺口是囊箧/科举 unsupported App 回归未显式断言页面链接不含 `not-a-session`，但当前实现路径未见该 id 链接。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。
- 2026-05-22：推进 S88.9 bad route 与 route error 安全恢复片，实现提交 `9a7d287d`。新增 `client/src/routes/routeRecovery.ts`，统一从当前 pathname 提取可运行 UUID 案卷并构造 `/game/:sessionId` 恢复入口；`NotFoundPage` 在 `/game/:sessionId/...` 未命中子路由时显示“回主卷”和“归首页”，对 `s74-preview` 或畸形案卷只显示归首页。`ErrorPage` 改为固定中文安全文案，不渲染 React Router error `statusText` 或 thrown data，避免 provider/raw/key/path/local diagnostics 进入玩家页面；错误页同样只在当前路径带可运行 UUID 时提供回主卷。App 回归覆盖 runnable bad route、preview bad route 和含 `provider payload` / `data/sessions` / `OPENAI_API_KEY` / `raw audit` 的 loader error 脱敏；React scaffold 增加 route recovery 静态守门，确认恢复入口复用 `isRunnableSessionId()` 且不调用 unsafe API。本轮只改 React 错误/404 展示和测试，不新增后端 API，不改变 history fallback、session 绑定、route view、AI 权限、考试、官职、资源、NPC、交易、委派、经济或服务器裁决。已通过 `npm run typecheck:client`、`node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）、focused App route recovery tests（3 项）、完整 client Vitest（106 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1126 项）和 `git diff --check`；Erdos 提交前只读复审未发现 P0/P1/P2，残余非阻断风险是畸形案卷行为测试只实测 `s74-preview`、未来若 `/game/:sessionId` 根 route 自身报错则“回主卷”可能指向同一路径。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。
- 2026-05-22：推进 S88.9 route recovery 边界补强片，实现提交 `12dda964`。`client/src/routes/routeRecovery.ts` 现在只在 pathname 匹配 `/game/:uuid/子路径` 时生成“回主卷”，避免 `/game/:sessionId` 根 route 自身 error 时给出自指恢复入口；非 UUID 畸形案卷如 `/game/not-a-session/unknown` 仍只给“归首页”。App 回归把 route error 脱敏场景调整到子路径并新增根案卷 error 不显示“回主卷”，同时补非 UUID 畸形案卷行为断言；React scaffold 增加 helper 子路径匹配守门。本轮只改 React route recovery helper、测试和文档，不新增后端 API，不改变 history fallback、session 绑定、route view、AI 权限、考试、官职、资源、NPC、交易、委派、经济或服务器裁决。已通过 `npm run typecheck:client`、React scaffold（40 项）、focused App route recovery tests（4 项）、完整 client Vitest（107 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1126 项）和 `git diff --check`；Erdos 提交前只读复审未发现 P0/P1/P2，残余非阻断风险是复审未重跑主代理列出的验证命令。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。
- 2026-05-22：推进 S88.9 顶部主导航 active 精准化，实现提交 `7223c036`。Erdos 只读梳理指出 React Router `NavLink` 默认前缀匹配会让 `/game/:sessionId/map|people|inventory|archive` 等子主路由同时高亮“主卷”和当前页面，本片采纳该 App 级路由 polish。`client/src/components/AppShell.tsx` 现在仅对 `route.id === "game"` 的顶部主导航链接传入 `end`，使“主卷”只在 `/game/:sessionId` exact route active；“舆图 / 人物 / 囊箧 / 史册”等链接仍保持各自路由 active。`client/src/__tests__/App.test.tsx` 新增 `/game/:sessionId/map` 回归，断言“舆图”获得 `aria-current="page"`，而“主卷”保留当前 session href 但没有 `aria-current` / `active`；`test/reactClientScaffold.test.js` 新增静态守门，防止回退。已通过本节顶部对应验证口径；Erdos 提交前只读复审未发现 P0/P1/P2，残余非阻断风险是 App 回归只实测 `/map`，`/people`、`/inventory`、`/archive` 依赖同一 React Router `NavLink` active 规则。本轮只调整浏览器 active 展示和测试，不新增后端 API，不改变 href session 绑定、route view、AI 权限、考试、官职、资源、NPC、交易、委派、经济或服务器裁决。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。
- 2026-05-22：推进 S88.9 React 科举/皇榜/舆图 route-local 状态防线，实现提交 `9b32068a`。`client/src/state/gameSessionState.ts` 现在为 `requestExamQuestion()`、`progressExam()` 和 `submitExam()` 增加考试 mutation 请求序号与当前案卷 guard，旧案卷成功/失败晚到或回包 `sessionId` 错配时，不覆盖当前 `activeExam`、`lastExamResult`、`currentSession`、全局 `status/error` 或 UI safe payload；交卷成功只有在当前案卷应用后才同步 `uiState`。`client/src/pages/ExamPage.tsx` 在切换案卷时重置试别、场内行动和文章草稿；`client/src/pages/RankingPage.tsx` 在切换案卷时清空本地榜名选中；`client/src/pages/MapPage.tsx` 在切换案卷时恢复地点/驿路/近事图层。`client/src/state/uiState.test.ts` 新增旧案卷取题成功、推进失败和交卷成功晚到不污染当前案卷或 UI 摘要的 store 回归；`test/reactClientScaffold.test.js` 新增科举、皇榜、舆图 route-local reset 静态断言。本轮仍只做前端安全消费与本地状态 guard，不新增后端 API，不改变考试晋级、放榜、地图事实、移动、资源、交易、委派、NPC 行动、经济或官职裁决。验证已通过本节顶部对应验证口径；`AI_PROVIDER=mock npm run smoke:browser` 首次 300 秒包装超时发生在 manifest/build/budget 后，确认无残留进程后用 900 秒超时重跑通过。Gauss 提交前只读复审未发现 P0/P1/P2；残余非阻断风险是三页 route-local reset 主要由 store 回归和静态断言覆盖，后续可补 App 级路由跳转行为测试。
- 2026-05-22：修正 S88.9 第二片复审时序审计口径。代码提交 `960e0336` 已存在后，主代理补做 Helmholtz 只读复审；Helmholtz 未发现代码 P0/P1/P2，仅指出复审时序文档口径 P2。本轮只修正 `docs/SHARED_CONTEXT.md` 与 `docs/DEVELOPMENT_STEPS.md` 的复审时序文字，不改代码、API/schema、运行时行为、提示词或验证工具；按低风险纯文档改动规则跳过额外子代理复审。验证：`npm run check:docs-governance`、`node --test test/documentationGovernance.test.js` 和 `git diff --check`。
- 2026-05-22：推进 S88.9 React 人物/囊箧跨案卷本地状态防线，实现提交 `960e0336`。`client/src/pages/PeoplePage.tsx` 现在按当前路由案卷过滤 `lastNpcInteraction`、`lastTrade`、`lastNpcCommand`、NPC 详情、名册级交互/委派 fallback、mutation/loading 状态和错误提示；案卷切换时会重置选中 NPC、页签、画像分页和对话/交易/委派/礼法草稿，旧案卷回包完成后不再清空新案卷本地草稿。`client/src/pages/InventoryPage.tsx` 现在把容器、物件、目标容器和移置提示绑定到当前 `sessionId`，切换案卷会重置本地选择；旧案卷物品移置请求晚到或回包 `sessionId` 不匹配时只更新 store 的安全投影，不再把旧提示、旧容器或“未入容器”空选中带到新案卷。`client/src/state/gameSessionState.ts` 进一步给物品移置、NPC 对话、交易和委派 mutation 加前端请求序号与当前案卷 guard，旧案卷 mutation 的发起、成功和失败都不得改当前路由的 loading/error/最新结果；`loadSession()` / `loadNpcs()` 切换案卷时会重置旧 NPC mutation loading。`client/src/components/SurfaceHost.tsx` 的印匣旧案 tab 会在当前 runnable 案卷缺于已 ready save list 时自动刷新一次，避免已有当前 session 文件但旧列表未更新时漏显当前案卷。`client/src/__tests__/App.test.tsx` 新增人物页旧案 NPC 结果/名册记录/错误过滤回归、囊箧页旧案移置后切换新案卷的本地状态重置回归，以及当前案卷缺于首次旧案列表时自动刷新并显示当前案卷的回归；`client/src/state/uiState.test.ts` 新增旧移置失败、旧 NPC mutation 失败和人物名册切案卷 loading 重置回归。本轮仍只做前端安全消费与本地状态 guard，不新增后端 API，不改变资源、交易、委派、NPC 行动、经济、考试或官职裁决。代码提交后补做 Helmholtz 只读复审未发现代码 P0/P1/P2，仅指出复审时序文档口径 P2，已由本次文档修正收口。
- 2026-05-22：推进 S88.9 React 前端操作与状态打磨第一片，实现提交 `1e1cb7a0`。`client/src/state/gameSessionState.ts` 为 `loadSession()`、`refreshQuickActions()`、`loadTopicSurface()`、`requestTopicDraft()`、`loadInventory()`、`loadNpcs()` 和 `loadNpcDetail()` 增加前端请求序号与回包 guard：旧请求晚到会 reject，不写 store，也不让旧案加载、旧专题草稿这类调用方继续执行本地副作用；最新回包若 `sessionId`、surfaceId 或 NPC id 与当前请求不一致则进入错误态且不同步 UI safe payload，避免旧案卷 player-state、快捷建议、topic 草稿、囊箧和 NPC 详情覆盖当前路由。`client/src/state/uiState.ts` 在 route bridge 切换到不同案卷时会清空不匹配的 `currentPlayerPayload`，首页返回仍保留同案卷继续入口。`client/src/pages/GamePage.tsx` 改为只从当前路由匹配的 `activeSession`、`activeLastTurn` 和 `activePlayerPayload` 渲染案主、身份、叙事、安全视图、角色面板和底部奏折；无匹配 payload 时只显示读取/占位状态，不展示旧案主、旧身份、旧叙事或旧角色面板。`client/src/components/SurfaceHost.tsx` 进一步按当前 `currentSessionId` 过滤 `topicSurface` / `topicDraft`，案卷切换时本地专题草稿、证据选择、草稿类型和 draft context 立即失效，B 案卷同 surface 读取中或失败时不会继续显示/写入 A 案卷材料。Leibniz 只读前端 gap 扫描建议优先处理主卷 stale session / loading / empty 防线，本轮采纳并扩展到安全读取 store；Faraday 提交前只读初审发现专题 surface 同案卷切换 P1，已按上述方式修复并补 App/scaffold 回归，最终只读复审确认 P1 已关闭，未发现新的 P0/P1/P2。已通过 client typecheck、focused/full client tests、React scaffold、client build、Mock browser smoke、docs governance、documentation governance、完整 `npm test`（1126 项）和 `git diff --check`。
- 2026-05-22：推进 S88.12 provider 长跑经济 trace 与显式 key 边界片，实现提交 `28141071`。`scripts/providerLongRun.js` 的内存结算链现在在 `worldTick` 后运行 `runNpcEconomyTickStep()`，再用 `buildEconomyTraceView()` 生成 S88.8 只读经济解释；长跑每回合校验 trace schema、公开字段、`sourceRefs`、`serverOwnsSettlement` / `browserReadonly` 边界和 raw ledger / provider / key / path / `evidenceRefs` 防泄漏，并在报告中记录 `npcEconomyCadence`、`economyTraceItems` 和 trace type 摘要。`test/providerLongRunScript.test.js` 覆盖月末内存结算产生安全 `market_price_signal` 经济 trace，并断言显式 `AI_PROVIDER=deepseek` 或 `--provider deepseek` 缺 key 时 `smoke:provider:long` 必须失败；`test/providerRouteHealthScript.test.js` 同步覆盖 `smoke:provider:route` 的显式缺 key fail，避免真实 provider 验收入口误用普通游玩 Mock fallback。Darwin 只读梳理指出 provider 长跑/route health 已覆盖真实 adapter、流式、语气、server-owned patch、三旬节奏和 route payload 脱敏，但脚本层尚未明确显式真实 provider 缺 key 必须失败；本轮采纳，并额外把 `smoke:provider:long` 接入 S88.8 经济 trace 验收。已通过语法检查、provider long/route focused tests（20 项）、provider 脚本 focused tests（44 项）、`npm run typecheck:server`、docs governance、documentation governance、完整 `npm test`（1126 项）和 `git diff --check`（退出码 0；仅打印未改动归档/QA 文件既有 CRLF 警告）；Lorentz 提交前只读复审未发现 P0/P1/P2，建议有真实 key 时可选补 keyed `smoke:provider:long` / `smoke:provider:route`，不阻断提交。
- 2026-05-22：推进 S88.8 资产/交易/委派/经济解释性第一片，实现提交 `50b55538`。新增 `src/game/economyTraceConfig.js` / `src/game/economyTraceView.js`，从既有安全 `resourceLedgerView`、`assetLedgerView`、`inventoryView`、`tradeLedgerView`、`delegatedTaskView`、`marketPriceView` 和 `npcEconomyView` 派生 `economyTraceView`，用 `sourceRefs`、trace type、分组、金额摘要、下一步和服务器边界解释资源、资产、囊箧、交易、委派、市价和月账为何变化；公开 view 不读取 raw ledger，不输出 `evidenceRefs`、provider/prompt/key/path/SQLite/raw 字段，也不是结算器。`/api/game/start|state|player-state|turn|turn-stream`、`/api/game/inventory/:sessionId` 和 `/api/game/inventory-transfer/:sessionId` 已返回该安全投影，`src/contracts/serverContracts.ts` 与 React API 类型同步。新增 `client/src/components/EconomyTraceSection.tsx`，囊箧页展示“账本为何变化”，过滤污染行和跨案卷 stale session，只把“拟复核”写入本地行动草稿；前端不成交、不扣款、不转物、不改任务/NPC/关系。Harvey 与 Linnaeus 已做只读后端/API、前端接线探查。Lagrange 提交前只读初审发现两个问题：`transferInventoryItem()` stale 响应可能把旧案卷库存套进当前 `sessionId` 的 P1，以及 `economyTraceView.toolPermissions` 暴露 `SQLite` 实现名的 P2；本轮已让 transfer 响应只在 session 匹配时合并，并把公开文案改为“后端持久化账本”，补 store stale transfer 回归和 `SQLite|sqlite|SQL` canary。已通过本节 S88.8 验证口径全部命令；Wegener 最终只读复审未发现 P0/P1/P2。
- 2026-05-22：推进 S88.8 资产/交易/委派/经济解释性第二片，实现提交 `06c25f4c`。`/api/game/trade/:sessionId` 与 `/api/game/npc-command/:sessionId` 现在随交易议价、服务器挡下交易、委派校验失败、委派计划拒绝和委派创建响应返回即时 `economyTraceView`，并复用当前安全 `tradeLedgerView`、`resourceLedgerView`、`inventoryView` 或 `delegatedTaskView` 构建解释，仍不输出 raw ledger、`evidenceRefs`、provider/prompt/key/path/SQLite/raw 字段。`src/contracts/serverContracts.ts` 与 React API 类型已把 `economyTraceView` 纳入 `TradeResponse` / `NpcCommandResponse`。`EconomyTraceSection` 新增 trace type/group 过滤能力，人物页新增“交易委派账本为何变化”，只展示交易、委派、人情债、市价和月账相关解释；React store 会把交易/委派响应中的 trace 合并到当前 session/inventory/NPC 视图，但 `currentSession` 只在自身 `sessionId` 匹配响应时更新，防止 route 指针已切换而旧案卷 payload 被新响应污染。按钮继续只写本地行动草稿，不成交、不扣款、不转物、不结算任务、不改 NPC 关系。Bernoulli 已做只读前端接线探查；Dirac 提交前只读初审发现 route pointer 与 `currentSession.sessionId` 分叉时仍可能污染旧 payload 的 P1，已按自身 session gate 修复并补回归；Dirac 最终只读复审确认 P1 已关闭，未发现新的 P0/P1/P2。
- 2026-05-22：推进 S88.8 资产/交易/委派/经济解释性第三片，实现提交 `62c9a260`。`economyTraceView.traceItems` 新增稳定 `sourceId`、`label`、`summary`、`visibility`、`confidence`、`topicSurfaceIds`、`relatedRefs` 和 `scopeRefs`，以安全 evidence 形状进入 `resolverInputContext` 的 `economy` source collection；`topicSurfaceView` 让奏折、拟旨、朝议、军议和人物档案按 `topicSurfaceIds` 只读消费经济 trace，堂审不读取普通交易/市价 trace，人物档案也不会显示纯资产维护或普通库存损耗。`safeWorldSearch` 新增 `economyTraceView.traceItems` 搜索行并在 1200 行全局 cap 下保护经济 trace，不让地理/人物噪声挤掉交易议价、委派预算/回禀、人情债和市价解释；SQLite `safe_search_index` 继续只从 `world_state_json` 的安全投影单向派生，不新增经济专用 truth 表。Meitner 提交前只读初审发现污染 id 走数字 fallback 时 `cleanId()` 可能抛 TypeError 的 P2；已强制 fallback 字符串化并补污染 `safe_search_index` id 回归，Meitner 最终复核确认 P2 关闭，未发现新的 P0/P1/P2。已通过语法检查、经济 trace / resolver input / topic surface / safe search focused Node tests（31 项）、SQLite safe-search focused tests（8 项）、`npm run typecheck:server`、docs governance、documentation governance、完整 `npm test`（1114 项）和 `git diff --check`。
- 2026-05-22：推进 S88.8 资产/交易/委派/经济解释性第四片，实现提交 `05fba067`。复用 `EconomyTraceSection` 将 `economyTraceView` 接入地方官和入仕官员/大臣主卷：地方官显示“钱粮与市价为何变化”，官员/大臣显示“经济线索与官署材料”，并只展示 `market_price_signal`、交易、委派、人情债、NPC 关系月账和普通月账解释；纯资源快照、资产维护和囊箧保养仍留在囊箧/相关安全视图，不进入官署主卷。`GamePage` 安全视图索引新增“账解”，React API 类型补齐第三片新增的 evidence 字段，`SurfaceHost` 将 `economyTraceView` 数据来源摘要显示为“经济解释 1 条”，evidence meta 显示为“经济解释 · 月账”。所有按钮继续只写 `role-surface` 本地行动草稿，不调用 `/api/game/turn`，不成交、不扣款、不转物、不结算委派、不改变 NPC 关系。Bacon 已做只读前端/topic 接线梳理，建议本片保持前端只读展示与 topic source label polish，不新增后端 resolver 或交易/委派写入口。已通过 `npm run typecheck:client`、`node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）、focused `uiState` / `MemorialComposer` client tests（24 项）、focused `App` client tests（41 项）、完整 client Vitest（79 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1115 项）和 `git diff --check`；Erdos 提交前只读初审发现 topic 数据来源摘要仍显示内部 `economyTraceView` 的 P2，已改为复用 `topicSourceLabel()` 并补 App/scaffold 回归，修复后重跑 `npm run typecheck:client`、`node --test --test-reporter=spec test/reactClientScaffold.test.js`、focused `App` client tests（41 项）和 `git diff --check` 通过。Erdos 最终只读复审确认 P2 关闭，未发现新的 P0/P1/P2；Erdos 复跑 client typecheck、scaffold 和 docs 守门通过，App focused 在其环境因缺少 `@rolldown/binding-linux-x64-gnu` optional native binding 未启动，主代理本地同一 App focused 41 项已通过。最终提交前再次通过 docs governance、documentation governance、完整 `npm test`（1115 项）和 `git diff --check`。
- 2026-05-22：推进 S88.8 资产/交易/委派/经济解释性第五片，实现提交 `1becde0c`。`topic_draft` 现在会把经济解释 evidence 发给 provider 时附带中文 `sourceLabel=经济解释`、`domainLabel=月账` 和明确 adjudication boundary；本地 fallback 草稿在引用经济材料时会写明“经济解释仅作账解材料，不视为交易成交、资源扣减、委派结算、人情债或关系变化”，仍只写底部奏折草稿。新增后端回归覆盖 provider context 不泄露 raw economy ledgers、provider 伪造 refs 被裁剪、provider 失败时 fallback 保留经济 refs 且不宣称已结算，以及普通 turn 的 `normalizeTopicDraftTurnContext()` 会从当前 topic surface 重建 allowlist，丢弃伪造 economy refs、伪造 canonical echo 和堂审 surface 的经济 refs。React App 回归覆盖用户勾选“经济解释”后 `/api/ai/topic-draft/:sessionId` 请求携带该 ref，“写入底部奏折”只写 `role-surface` action draft 并保留 `draftContext.evidenceRefs`，不调用 `/api/game/turn`。本片先经 Anscombe 只读探查确认范围应聚焦 topic draft 草稿链路验收，不新增经济写权。已通过 `node --check src/game/topicDrafts.js`、`node --check test/topicDraftRoute.test.js`、`npm run typecheck:client`、`npm run typecheck:server`、`node --test --test-reporter=spec test/economyTraceView.test.js test/resolverInputContext.test.js test/topicSurfaceView.test.js test/topicDraftRoute.test.js`（38 项）、focused `App` client tests（41 项）、docs governance、documentation governance、完整 `npm test`（1118 项）和 `git diff --check`；首次 App focused 因本机缺少 `@rolldown/binding-linux-x64-gnu` optional native binding 未启动，执行 `npm install` 修复本地 `node_modules` 后重跑通过，lockfile 未变化。Schrodinger 提交前只读复审确认未发现 P0/P1/P2，且认可本片不需要额外 economy-specific route/SSE 阻断才满足最低安全线。
- 2026-05-22：推进 S88.8 经济解释 draftContext turn/SSE 长链路第六片，实现提交 `ecfdc3c0`。`test/gameTurnRoleCycleConsequences.test.js` 新增经济信号种子、SSE 解析工具和保存态快照断言：先让地方官普通 turn 写入一次服务器裁决后的 city policy 基线，再用当前 `memorial-review` surface 的 `economyTraceView` evidence 和伪造 `economyTraceView:forged-secret` / `domainConsequenceEcho:forged` 提交第二次普通 turn 与 SSE turn。两条路径都必须返回 `duplicate_recent`，且 outcome 不携带 `canonicalEchoRefs` / `topicDraftContext`；public payload 不泄露 raw trade/delegated/market/npc economy ledger key、内部 delta、auditRecord、污染文本或伪造 refs；保存态中的 city policy 数量、交易记录、委派任务、NPC 经济 recentEvents 和地方库银在第二次草稿提交后保持不变。Heisenberg 只读探查确认生产代码的普通 turn 与 SSE 都调用 `normalizeTopicDraftTurnContext()`，本片不需要改生产逻辑。已通过本节第六片验证口径：语法检查、focused route/topic/economy tests（47 项）、`npm run typecheck:server`、docs governance、documentation governance、完整 `npm test`（1120 项）和 `git diff --check`；全量 diff check 仅打印未改动归档/QA 文件既有 CRLF 警告。Ramanujan 提交前只读复审未发现 P0/P1/P2，并额外确认普通 turn/SSE 均在 `finalizeTurn()` 前重校验 `draftContext`，经济 evidence 无 canonical echo 时不会写入 `topicDraftContext`。
- 2026-05-22：推进 S88.8 经济解释 draftContext true streaming provider 第七片，实现提交 `f533d1c1`。`test/streamingTurnRoute.test.js` 新增经济信号种子、当前 `memorial-review` surface 经济 evidence 草稿、保存态快照和安全响应断言：同一 stub provider 先通过普通 JSON turn 写入一次 city policy 基线，再让 `provider.streamTurn()` 以可见 narrative chunks 返回同一叙事，并携带经济 `draftContext` 重复提交市价处置。测试确认 true streaming 分支实际调用 `streamTurn()` 一次、不会回落成第二次 `runTurn()`；最终 `final_state` 返回 `duplicate_recent`，不携带 `canonicalEchoRefs` / `topicDraftContext`，不泄露 raw 交易/委派/市价/NPC 经济账本、内部 delta、auditRecord、污染文本或伪造 refs；保存态中的 city policy 数量、军务裁决数量、交易记录、委派任务、资源账本、NPC 经济 recentEvents、玩家银两和地方库银在 streamed 草稿提交后保持不变，公开关系反馈不包含经济草稿、交易、委派、人情债或 `relationshipSignals` 结算信号。同步加宽 `test/gameTurnRoleCycleConsequences.test.js` 的普通 turn/SSE 第六片断言，避免三条路径安全口径分裂。本片不改生产代码，补齐第六片未覆盖的 visible streaming provider 路径。已通过本节第七片验证口径：语法检查、streaming focused tests（6 项）、gameTurn focused tests（9 项）、combined route/topic/economy tests（53 项）、`npm run typecheck:server`、docs governance、documentation governance、完整 `npm test`（1121 项）和 `git diff --check`；全量 diff check 仅打印未改动归档/QA 文件既有 CRLF 警告。Russell 提交前只读初审发现快照未覆盖军务、资源与关系信号面的 P2；主代理已补军务 ledger、资源账本、玩家金银/地方库银不变断言，并用关系结算 canary 避免把普通 NPC/官署关系系统误判为经济草稿结算；Russell 最终只读复审确认 P2 已关闭，未发现新的 P0/P1/P2，残余风险是若未来经济结算回归生成完全无经济词的泛化关系反馈，当前关系 canary 可能无法捕获。
- 2026-05-22：推进 S88.12 Mock/真实 provider 验收首片，实现提交 `53dc1f3d`。`test/streamingTurnRoute.test.js` 新增运行期 no-key fallback route 回归：在 `AI_PROVIDER=deepseek` 且无 `DEEPSEEK_API_KEY` 时，普通 JSON turn 仍由 provider facade 回退到 Mock 并写入一次市价裁决基线；随后第二次 JSON turn 携带当前 `memorial-review` surface 的经济 `draftContext`、伪造 economy ref 和伪造 canonical echo 重复提交，再从当前 topic surface 重建 `draftContext` 走 SSE turn 重复提交。两条 draftContext 路径都必须返回 `duplicate_recent`，不写 `topicDraftContext`、不接受伪造 refs/echo、不泄露 provider/key/path/raw ledger，也不结算交易、委派、人情债、金银或地方库银；普通 Mock fallback 仍允许自身旬 tick 维护资源和公开月账，但新增月账不得携带 draft/forged/raw 污染。`test/aiConnectionRoute.test.js` 继续固定 `/api/ai/connection-test` 缺真实 provider key 返回 503 的诊断语义，避免把诊断误改成普通游玩回退。`scripts/providerToolSmoke.js` 先判断无 `MIMO_API_KEY` 的 skip，再要求 `fetch` 支持，确保无 key 环境不会因运行时缺少 fetch 而失败；`test/providerToolSmokeScript.test.js` 已补 no-key/no-fetch 回归和 keyed/no-fetch fail 回归。Pascal 提交前只读初审发现 no-key JSON + `draftContext` 未覆盖的 P2，主代理已补第二次 JSON draft duplicate；Singer 复审确认实现 P2 已关闭，并指出本条台账表述未同步，本轮已修正文档口径；Boyle 最终窄复核确认 Singer 文档 P2 已关闭，未发现新的 P0/P1/P2。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。
...
本轮 S88.9 NPC/囊箧 route-local 读取补强当前验证口径：

- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/state/uiState.test.ts -t "route-local inventory|NPC read|same-session|stale inventory transfer"`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx -t "older same-session NPC reply|stale S88.9 NPC workbench|follow-up evidence"`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`
- `npm run test:client -- --pool=vmForks --maxWorkers=2`
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`（300 秒包装超时后以 600 秒重跑通过）
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`
- `git diff --check`
- 提交前只读复审：Harvey 未发现 P0/P1/P2；窄复核确认同 NPC 草稿续写风险已关闭且最终 diff 可提交。

上一轮 S88.5 六身份循环交互密度与史册实体压力首屏保留当前验证口径：

- `npm run typecheck:client`
- `node --test --test-reporter=spec --test-name-pattern 'S88.5 role cycle section is wired to all six identity panels as draft-only UI' test/reactClientScaffold.test.js`
- `node --test --test-reporter=spec --test-name-pattern 'S88.5 role cycle section is wired|S88.7 archive consumes world entity impact evidence' test/reactClientScaffold.test.js`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 --reporter=verbose src/__tests__/App.test.tsx -t "renders the S88.5 six-role matrix"`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 --reporter=verbose src/__tests__/App.test.tsx -t "renders the S88.5 six-role matrix|renders archive route entries"`
- `npm run test:client -- --pool=vmForks --maxWorkers=2`
- `npm run build:client`
...
- `npm test`
- `git diff --check`
- 提交前只读子代理复审最终 diff 与验证证据；本片包含前端代码、测试和运行时展示行为，必须在暂存和提交前完成只读复审。

本轮 S88.7 人物页交游议题消费层当前验证口径：

- `npm run typecheck:client`
- `node --test --test-reporter=spec --test-name-pattern 'S88.7 topic surfaces label NPC relationship action evidence|S88.7 people page consumes persistent world entity impact evidence' test/reactClientScaffold.test.js`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 --reporter=verbose src/__tests__/App.test.tsx -t "loads the S76.10 current people ledger"`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 --reporter=verbose src/__tests__/App.test.tsx -t "does not render stale S88.7 follow-up evidence"`
- `npm run test:client -- --pool=vmForks --maxWorkers=2`
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
...
- `node --test --test-reporter=spec test/npcAiSafety.test.js test/npcRelationshipActions.test.js test/npcInventoryRoutes.test.js`
- `node --test --test-reporter=spec test/sqliteSafeSearch.test.js`
- `npm run typecheck:server`
- `npm run typecheck:client`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`
- `npm run test:client -- --pool=vmForks --maxWorkers=2 --reporter=verbose src/__tests__/App.test.tsx -t "loads S78 topic surface materials"`
- `npm run test:client -- --pool=vmForks --maxWorkers=2`
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
...
- `npm test`
- `git diff --check`
- 提交前只读子代理复审最终 diff 与验证证据：Mencius 未发现 P0/P1/P2；本片包含后端安全 view、resolver/topic/search/SQLite 接线、React source label 和测试。

上一轮 S88.7 实体压力史册 browser smoke DOM canary 当前验证口径：

- `node --check scripts/clientSmoke.js`
- `node --test --test-reporter=spec --test-name-pattern 'S88.7 archive consumes world entity impact evidence|S74.7 client smoke verifies default UI start and safe route recovery' test/reactClientScaffold.test.js`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`
- `npm run typecheck:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
...
- `npm test`
- `git diff --check`
- 提交前只读子代理复审最终 diff 与验证证据；本片包含 browser smoke 验证工具、React 静态守门和真实浏览器运行时 canary，必须在暂存和提交前完成只读复审。

上一轮 S88.7 实体压力史册归档片当前验证口径：

- `node --check src/game/eventArchive.js`
- `node --test --test-reporter=spec test/eventArchive.test.js`
- `node --test --test-reporter=spec test/gameTurnEventArchive.test.js`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`
- `node --test --test-reporter=spec --test-name-pattern 'SQLite storage adapter syncs and repairs the safe event archive index' test/sessionStoreAdapterContract.test.js`
- `npm run typecheck:server`
- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=2`
- `npm run build:client`
...
- `npm test`
- `git diff --check`
- 提交前只读子代理复审最终 diff 与验证证据；本片包含后端 view、SQLite 派生、React 史册展示、测试和运行时行为，必须在暂存和提交前完成只读复审。

上一轮 S88.7 人物页持久实体压力展示片当前验证口径：

- `npm run typecheck:client`
- `node --test --test-reporter=spec --test-name-pattern 'S88.7 people page consumes persistent world entity impact evidence|S76.10 people portraits' test/reactClientScaffold.test.js`
- `npm run test:client -- --pool=vmForks --maxWorkers=2 --reporter=verbose src/__tests__/App.test.tsx -t "loads the S76.10 current people ledger"`
- `npm run test:client -- --pool=vmForks --maxWorkers=2`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
...
上一轮 S88.7 实体压力追踪证据片当前验证口径：

- `node --check src/game/worldEntities.js && node --check src/game/resolverInputContext.js && node --check src/game/resolverInputConfig.js && node --check src/game/topicSurfaceView.js && node --check src/game/safeWorldSearch.js`
- `node --test --test-reporter=spec test/worldEntities.test.js`
- `node --test --test-reporter=spec test/resolverInputContext.test.js test/topicSurfaceView.test.js test/safeWorldSearch.test.js`
- `node --test --test-reporter=spec test/sqliteSafeSearch.test.js`
- `node --test --test-reporter=spec --test-name-pattern 'world entity impact|world entity impacts|world entity recent impact|world entity impact rows|world entity impact evidence|world entity impact source refs' test/worldEntities.test.js test/resolverInputContext.test.js test/topicSurfaceView.test.js test/safeWorldSearch.test.js test/sqliteSafeSearch.test.js`
- `npm run typecheck:server`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`
- `git diff --check`
- 提交前只读子代理复审最终 diff 与验证证据；本片包含代码、测试、运行时 view/evidence 接线和 SQLite safe-search 派生行，必须在暂存和提交前完成只读复审。

上一轮 S88.11 browser smoke 视觉/立绘运行时验收当前验证口径：

- `node --check scripts/clientSmoke.js`
- `node --test --test-reporter=spec --test-name-pattern 'S88.11|S74.5 asset registry|S76.10 people portraits|S77.5 client smoke resource budget|S77.4 client smoke' test/reactClientScaffold.test.js`（6 项）
- `npm run qa:runtime-manifest`
- `npm run typecheck:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（42 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（118 项）
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`（1137 项）
- `git diff --check`（退出码 0，仅打印未改动归档/QA 文件既有 CRLF 警告）
- Parfit 提交前只读初审发现 manifest 安全校验通过页面 fetch 会污染 manifest 请求记录的 P2；本轮已改为 Node 侧 `fetch(`${baseUrl}${runtimeAssetManifestPath}`)`，并用 React scaffold 静态断言拒绝旧 `assertManifestRuntimeSafety(page, baseUrl)` 签名。Parfit 窄复审确认 P2 已关闭，未发现新的 P0/P1/P2。

上一轮 S88.11 assetRegistry 消费层 QA 第二片当前验证口径：

- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 src/assets/assetRegistry.test.ts --reporter=verbose`（10 项；未加 worker 限制的单文件 Vitest 曾因 forks worker 启动超时失败，随后受控 worker 重跑通过）
- `node --test --test-reporter=spec --test-name-pattern 'S74.5 asset registry|S88.11 runtime manifest QA|S77.5 runtime manifest' test/reactClientScaffold.test.js`（3 项）
- `npm run qa:runtime-manifest`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（41 项）
- `npm run build:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（118 项）
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
...
- `npm test`（1136 项）
- `git diff --check`（退出码 0，仅打印未改动归档/QA 文件既有 CRLF 警告）
- 提交前只读复审：Socrates 未发现 P0/P1/P2；非阻断建议是后续可把 `ageBand` 从 `startsWith("adult")` 收紧为明确枚举白名单。

上一轮 S88.11 runtime manifest 生成层 QA 首片当前验证口径：

- `node --check scripts/frontendRuntimeManifest.js`
- `npm run qa:runtime-manifest`
- `node --test --test-reporter=spec --test-name-pattern 'S77.5 runtime manifest|S88.11 runtime manifest QA' test/reactClientScaffold.test.js`（2 项）
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（41 项）
- `node --test --test-reporter=spec test/frontendInkAssetsManifest.test.js`（17 项）
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`（1136 项）
- `AI_PROVIDER=mock npm run smoke:browser`
- `git diff --check`（首次 120 秒包装超时仅输出既有 CRLF 警告；600 秒重跑退出码 0，仍只打印未改动归档/QA 文件既有 CRLF 警告）
- 提交前只读复审：Averroes 初审发现污染扫描未覆盖 key 与 camelCase/SQLite 词形的 P2；已修复并重跑 focused runtime manifest 回归、完整 React scaffold、docs governance、documentation governance、完整 `npm test`、Mock browser smoke 和 `git diff --check`；Averroes 窄复审确认 P2 已关闭，未发现新的 P0/P1/P2。

上一轮 S88.10.5 NPC 关系行动舆图锚点片当前验证口径：

- `node --check src/game/mapRuntimeView.js`
- `node --check src/game/mapRuntimeConfig.js`
- `node --check src/game/mapRuntimeDraftContext.js`
- `node --test --test-reporter=spec --test-name-pattern 'S88.10 map runtime view adds read-only NPC activity anchors|S88.10 map-runtime draftContext rejects visual-only NPC refs' test/mapRuntimeView.test.js test/mapRuntimeDraftContext.test.js`（2 项）
- `node --test --test-reporter=spec test/mapRuntimeView.test.js`（9 项）
- `node --test --test-reporter=spec test/mapRuntimeDraftContext.test.js`（3 项）
- `node --test --test-reporter=spec test/mapFrontendShell.test.js test/reactClientScaffold.test.js`（44 项）
- `npm run typecheck:server`
- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（114 项）
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`（1135 项）
- `git diff --check`（退出码 0；仅打印未改动归档/QA 文件既有 CRLF 警告）
- 提交前只读复审：Aquinas 未发现 P0/P1/P2；非阻断建议同上。

上一轮 S88.10.3 PixiJS 舆图行动 streaming 长链路回归当前验证口径：

- `node --test --test-reporter=spec --test-name-pattern 'S88.10 true provider stream revalidates map-runtime military draftContext' test/streamingTurnRoute.test.js`（1 项）
- `node --check test/streamingTurnRoute.test.js`
- `node --test --test-reporter=spec test/streamingTurnRoute.test.js`（8 项）
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`（1133 项）
- `git diff --check`
- 提交前只读复审：Galileo 初审发现文档口径误称只保留 target/action draft refs 的 P2；已修正为 target/source/evidence/action draft refs 审计边界，Galileo 窄复核确认 P2 关闭且未发现新的 P0/P1/P2。

上一轮 S88.10.2 PixiJS 舆图行动后端复核链验证口径：

- `node --check src/game/mapRuntimeView.js`
- `node --check src/game/mapRuntimeDraftContext.js`
- `node --check src/game/roleCycleDomainAdjudication.js`
- `node --check src/routes/game.js`
- `node --test --test-reporter=spec test/mapRuntimeView.test.js test/mapRuntimeDraftContext.test.js`（10 项）
- `node --test --test-reporter=spec test/roleCycleDomainAdjudication.test.js`（14 项）
- `node --test --test-reporter=spec test/gameTurnRoleCycleConsequences.test.js`（10 项）
- `npm run typecheck:server`
- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（114 项）
- `npm run build:client`
...
- `npm test`（1132 项）
- `git diff --check`
- 提交前只读复审：Laplace 初审发现内部 map context 审计入口未强制 schema/status 的 P2；本轮已修复并补回归，Laplace 窄复核确认 P2 关闭且未发现新的 P0/P1/P2。残余非阻断风险是 SSE 与将领军务 map-runtime 审计路径主要依赖共享 normalizer 和现有 role-cycle 覆盖。

本轮 S88.9 bad route 与 route error 安全恢复当前验证口径：

- `npm run typecheck:client`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "safe recovery|malformed bad routes|sanitizes route error diagnostics|self-recovery"`（4 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（107 项）
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`（1126 项）
- `git diff --check`
- 提交前只读复审：Erdos 未发现 P0/P1/P2；残余非阻断风险是复审未重跑主代理列出的验证命令。上一片 Erdos 残余非阻断风险（非 UUID 畸形案卷、根案卷 error 自指恢复）已由本片 focused 回归覆盖。

本轮 S88.9 React 顶部主导航 active 精准化当前验证口径：

- `npm run typecheck:client`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "keeps the top navigation active state scoped"`（1 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（103 项）
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`（1126 项）
- `git diff --check`
- 提交前只读复审：Erdos 未发现 P0/P1/P2；残余非阻断风险是 App 回归只实测 `/map`，其余子主路由依赖同一 React Router `NavLink` active 规则。

本轮 S88.9 React 低动效与设置页空错态 polish 当前验证口径：

- `npm run typecheck:client`
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "keeps map runtime motion reduced"`（1 项；一次与其他 focused tests 并行执行时 120 秒包装超时，单独重跑通过）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "refreshes the S75.4 old-case list once"`（1 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "renders the S76.8 ranking page"`（1 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1`（102 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（102 项；一次与 `npm run build:client` 并行执行时发生进程级 segmentation fault，无断言失败；单独重跑通过）
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `git diff --check`
- `npm run check:docs-governance`
...
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "renders archive route entries"`（1 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "renders the S76.8 ranking page"`（1 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "wraps the S72 map renderer"`（1 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
...
本轮 S88.9 React overlay 焦点与皇榜跳转可访问性打磨当前验证口径：

- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "opens registry-backed local surfaces"`（1 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "opens court, archive, and map surfaces against the current route session"`（1 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "renders the S76.8 ranking page from server-owned exam views"`（1 项）
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose`（49 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（101 项）
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`
- `git diff --check`
- 提交前只读复审：Confucius 未发现 P0/P1/P2；非阻断残余缺口是抽屉、弹窗和高清立绘查看器的 Tab 循环主要由共享实现与静态守门覆盖，暂无逐一交互测试。

本轮 S88.9 React 朝议/史册/舆图 route-bound surface 入口当前验证口径：

- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/state/uiState.test.ts --reporter=verbose`（33 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "opens court, archive, and map surfaces against the current route session"`（1 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose`（49 项）
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（101 项）
...
- `npm test`（1126 项）
- `git diff --check`
- 提交前只读复审：Raman 未发现 P0/P1/P2；非阻断建议是后续 S88.9 App 级路由 polish 可补焦点回收/overlay trigger 行为回归。
...
- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/state/uiState.test.ts --reporter=verbose`（27 项）
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（92 项）
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`（首次 300 秒包装超时发生在 manifest/build/budget 后；确认无残留进程后以 900 秒超时重跑通过）
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`（1126 项）
- `git diff --check`
- 提交前只读复审：Gauss 未发现 P0/P1/P2；残余非阻断风险是三页 route-local reset 主要由 store 回归和静态断言覆盖，后续可补 App 级路由跳转行为测试。

本轮 S88.9 React 人物/囊箧 route-local 状态防线当前验证口径：

- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/state/uiState.test.ts --reporter=verbose`（24 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose`（46 项）
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（89 项）
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`
- `git diff --check`
- 代码提交后补做 Helmholtz 只读复审最终 diff 与验证证据。

本轮 S88.9 React stale session / loading 防线第一片当前验证口径：

- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=2 client/src/state/uiState.test.ts client/src/__tests__/App.test.tsx`（65 项）
- `node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（84 项）
- `npm run build:client`
- `AI_PROVIDER=mock npm run smoke:browser`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`（1126 项）
- `git diff --check`
- Faraday 提交前最终只读复审确认此前 P1 已关闭，未发现新的 P0/P1/P2。

本轮 S88.12 provider 长跑经济 trace 与显式 key 边界片当前验证口径：
...
- `node --test --test-reporter=spec test/providerLongRunScript.test.js test/providerRouteHealthScript.test.js`（20 项）
- `node --test --test-reporter=spec test/providerSmokeScript.test.js test/providerToolSmokeScript.test.js test/providerAiFirstSmokeScript.test.js test/providerLongRunScript.test.js test/providerRouteHealthScript.test.js`（44 项）
- `npm run typecheck:server`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`
...
本轮 S88.12 provider no-key fallback 首片当前验证口径：

- `node --check scripts/providerToolSmoke.js`
- `node --check test/providerToolSmokeScript.test.js`
- `node --check test/streamingTurnRoute.test.js`
- `node --test --test-reporter=spec test/providerToolSmokeScript.test.js`（8 项）
- `node --test --test-reporter=spec test/streamingTurnRoute.test.js`（7 项）
- `node --test --test-reporter=spec test/aiConnectionRoute.test.js`（6 项）
- `npm run typecheck:server`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`
...
本轮 S88.8 经济解释 draftContext true streaming provider 第七片当前验证口径：

- `node --check test/streamingTurnRoute.test.js`
- `node --test --test-reporter=spec test/streamingTurnRoute.test.js`（6 项）
- `node --check test/gameTurnRoleCycleConsequences.test.js`
- `node --test --test-reporter=spec test/gameTurnRoleCycleConsequences.test.js`（9 项）
- `node --test --test-reporter=spec test/streamingTurnRoute.test.js test/gameTurnRoleCycleConsequences.test.js test/topicDraftRoute.test.js test/economyTraceView.test.js test/resolverInputContext.test.js test/topicSurfaceView.test.js`（53 项）
- `npm run typecheck:server`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `npm test`（1121 项）
...
### 2026-05-24：推进 S88.9 囊箧同案卷移置表单防串扰

- 范围：继续 S88.9 React 前端操作与状态打磨，聚焦囊箧页在同一案卷内提交物品移置后，玩家又改选其他物件或目标容器时，旧移置响应不应清空当前表单或切换容器焦点。本轮只改 `InventoryPage` 页面本地副作用、App 行为回归、React scaffold 静态守门和文档；不新增后端 API、route response shape、AI 工具、SQLite schema、prompt 权限、服务器 resolver 或资源/身份/交易/NPC/经济/考试/官职裁决。
- 实现：`client/src/pages/InventoryPage.tsx` 新增 `latestTransferSelectionRef`，在提交移置时记录发起请求的 `itemId` 与 `toContainerId`，并在物件/去处选择变化时更新当前表单快照。移置响应回来后，只有 route session、响应 `sessionId`、当前物件和当前目标容器仍与请求快照一致，才显示“服务器已校验并更新物件位置。”或失败提示、清空物件/去处选择并把仓储焦点切到目标容器；若玩家已改选，旧响应只让 Zustand store 按既有同案卷 guard 合并服务器安全投影，不覆盖当前表单。
- 回归：`client/src/__tests__/App.test.tsx` 新增同案卷旧移置响应晚到回归：先提交“待移旧稿 -> 文库木柜”，再在响应前改选“待选收据 -> 书案小箧”，旧响应落地后仍保留当前 `物件/去处` 选择且不显示旧成功提示。`test/reactClientScaffold.test.js` 增加 `latestTransferSelectionRef` 与 `isLatestTransferSelection(requestItemId, requestTargetContainerId)` 静态守门。
- 验证：已通过 `npm run typecheck:client`、focused `App.test.tsx` 新旧囊箧 S88.9 回归、完整 React scaffold（45 项）、完整 client Vitest（126 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、`npm run check:docs-governance`、`node --test test/documentationGovernance.test.js`、完整 `npm test`（1159 项）和 `git diff --check`。首次 focused Vitest 使用 repo-root `client/src/...` 路径导致 120 秒包装超时且无断言输出，随后改用 client-root `src/__tests__/App.test.tsx` 路径重跑通过；构建和 smoke 仅保留既有 runtime `/assets/ui/...` build-time 解析、chunk size/plugin timing 与 npm `globalignorefile` 警告。
- 子代理：Pauli 提交前只读复审未发现 P0/P1/P2，并只读复跑 focused 新增 App 回归通过。残余非阻断风险是当前 guard 基于当前表单值匹配请求快照，若未来允许同物件/同目标并发请求，或玩家改选后又切回同一组合，较旧响应仍可能通过；当前按钮 loading 已基本挡住重复提交，后续可用本地 request id 加硬。accepted:false 与网络失败的改选后不提示未单独做 App 回归，但实现复用同一 `isLatestTransferSelection()` 守门。
- 提交：实现提交 `607a7fd8`；本哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。

### 2026-05-24：推进 S88.9 NPC/囊箧 route-local 读取补强

- 范围：继续 S88.9 React 前端操作与状态打磨，聚焦囊箧、NPC 名册和 NPC 详情读取在切案卷后成功/失败晚到，以及人物页同案卷切换 NPC 后旧 mutation 回执清空当前 NPC 草稿的前端风险。本轮只改 React store、人物页本地副作用、App/store 回归和静态守门；不新增后端 API、route response shape、AI 工具、SQLite schema、prompt 权限、服务器 resolver 或资源/身份/交易/NPC/经济/考试/官职裁决。
- 实现：`client/src/state/gameSessionState.ts` 让 `loadInventory()`、`loadNpcs()` 和 `loadNpcDetail()` 在成功与失败路径写 store 前再次用当前 route session 复核，旧案卷读取晚到时不得把 `currentSessionId` 拉回旧案卷，也不得覆盖当前囊箧/NPC 安全投影、status/loading 或 error。`client/src/pages/PeoplePage.tsx` 为当前选中 NPC 增加 ref，NPC 对话、交易、委派和礼法提交回执只有在 route session、发起时 NPC id 与提交时草稿快照都仍匹配时才清空本地草稿，避免旧 NPC 回执清掉玩家刚给当前 NPC 输入的内容，也避免同 NPC 回执清掉玩家继续编辑的新草稿。
...
- 范围：继续 S88.9 React 前端操作与状态打磨，聚焦跨案卷切换时底部奏折 `actionDraft`、`draftContext` 与身份循环 surface 入口的 session 归属。本轮只改前端 UI 状态生命周期、主卷消费过滤、React/App 回归和静态守门；不新增后端 API、route response shape、AI 工具、SQLite schema、prompt 权限、服务器 resolver 或资源/身份/交易/NPC/经济/考试/官职裁决。
- 实现：`client/src/state/uiState.ts` 为 `ActionDraft` 增加 `sessionId`，`setActionDraft()` 只在当前 UI session 是可运行 UUID 或明确预览案卷时写入案卷绑定草稿；`setCurrentPage()`、`syncSessionPayload()` 和 `openSurfaceForSession()` 会按目标案卷保留同案卷草稿，清空旧案卷草稿、旧本地 surface 和旧画像查看器。`GamePage` 新增 `activeActionDraft`，底部奏折、`submitTurn()` 的 `draftContext` 和 quick-action 刷新预览都只消费当前路由案卷草稿；身份循环 `onOpenRoleCycleSurface` 改为 `openSurfaceForSession(surface, sessionId)`，不再沿用未绑定案卷的 `openSurface()`。
...
- 范围：继续 S88.7 NPC 与关系深化的前端消费层，聚焦人物页如何展示已由服务器写入 `worldThreadView` 的 `npc_relationship_action` 公开议题。本轮只消费当前案卷 safe view，不新增后端 API、route response shape、AI 工具、SQLite schema、prompt 权限或服务器结算器。
- 实现：`client/src/pages/PeoplePage.tsx` 新增 `collectRelationshipAgendaThreads()`，从 `activeSession.worldThreadView.activeThreads` 只收录 `sourceType === "npc_relationship_action"` 的安全 thread，过滤 source id/label、标题、摘要、目标和后续提示中的 provider/raw/path/key/hidden 污染，按 thread id 去重并 capped 展示。“交游议题”卡片显示来源、状态、风险、公开摘要、follow-up hint、关联标签和首条干预提示；“拟跟进”只写 `role-surface` 本地行动草稿，明确资源、婚姻、关系终局、伤损、弹劾、定罪、背叛和 NPC 行动仍由服务器裁决。`client/src/styles/global.css` 补同源面板与卡片布局。
- 回归：`client/src/__tests__/App.test.tsx` 在人物页当前案卷 fixture 中加入安全交游议题、污染交游议题和非关系行动 thread，断言只显示安全议题、非关系和污染内容不渲染，并点击“拟跟进”确认只写行动草稿；stale 案卷回归补 `worldThreadView.activeThreads`，确认跨案卷交游议题和按钮不出现。`test/reactClientScaffold.test.js` 增加静态守门，要求人物页筛选 `npc_relationship_action`、复用 `peopleTextLooksUnsafe`、渲染“交游议题 / 拟跟进”，且不引入 turn API、raw ledger、provider payload、key、本地路径或 browser storage。
- 验证：已通过 `npm run typecheck:client`、两个 S88.7 React scaffold focused 测试、完整 `test/reactClientScaffold.test.js`（45 项）、两个 App focused 回归、完整 client Vitest（120 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、`npm run check:docs-governance`、`node --test test/documentationGovernance.test.js`、完整 `npm test`（1159 项）和 `git diff --check`。`App.test.tsx` 单测使用 `--maxWorkers=2` 的 focused 首轮曾在 worker 启动阶段 180 秒包装超时；随后以 `--maxWorkers=1` 同目标重跑通过，无断言失败。client build / browser smoke 仅出现既有 runtime asset 解析、chunk size 和 npm `globalignorefile` 警告。
- 子代理：Galileo 提交前只读初审发现 POSIX path-shaped 污染可进入交游议题的 P2；主代理已补本地路径形态检测、`/mnt` / `/home` fixture 和静态守门。Galileo 窄复审继续发现中文标点前缀路径绕过 P2；主代理已把常见 ASCII/CJK 标点加入 path pattern，并补 `，/home`、`、/mnt` fixture。Galileo 最终复审确认 P2 关闭，未发现新的 P0/P1/P2；残余非阻断风险是 path detector 仍是启发式分隔符 allowlist，未来可继续补不常见分隔符。
- 下一步：完成本轮全量验证和提交前只读复审后提交；后续可继续 S88.7 更深关系后果、S88.12 真实 keyed provider 长跑证据、S88.11 视觉素材 QA 消费层巡检或 S88.5 矩阵交互密度，但必须继续保持 safe-view-only、browser-draft-only 和服务器裁决。
- 提交：实现提交 `7105c222`；本哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。
...
- 提交：实现提交 `cca14150`；本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。

### 2026-05-23：推进 S88.7 交游记录 evidence 接线

- 范围：继续 S88.7 NPC 与关系深化，把已经由服务器裁决的 `/npc-interaction` 论道、切磋、求爱、议婚关系行动，从安全 `npcInteractionView` 推进为可被 resolver、topic surface、安全搜索和 SQLite safe-search 复用的只读 evidence。本轮不新增写 API、不改 SQLite schema、不新增 AI 工具或 prompt 权限，不让浏览器、AI、topic draft、搜索或 SQLite 派生行裁决资源、人情债、关系终局、婚姻、伤损、弹劾、定罪、背叛、NPC 行动或 hidden 私档。
- 实现：`src/game/npcInteractions.js` 新增 capped `relationshipActionEvidence`，只收录 `npc_relationship_action_resolver` 且 `serverOwnsOutcome=true` 的安全记录，公开“交游记录”标题、摘要、NPC 名、动作标签、裁决/挡下状态、风险标签、`npc-profile` / `court-debate` / `memorial-review` topic allowlist、公开 refs 和服务器边界；`buildNpcInteractionLedgerView()` 继续清洗 legacy 公开字段、resolver trace、eligibility/resource/relationship/worldPeople impact view 与 ignored client fields。`resolverInputContext` 构建 `npcInteractionView`，`resolverInputConfig` 将 `relationshipActionEvidence` 纳入 `events`；`topicSurfaceView` 让奏折队列、朝议和人物档案消费该 source view，堂审不接入；`safeWorldSearch` 和 SQLite `safe_search_index` 同步索引 `npcInteractionView.relationshipActionEvidence`；React `SurfaceHost` 把来源显示为“交游记录”，evidence meta 显示为“交游记录 · 案牍”。
- 回归：新增 resolver input、topic surface、safe search、SQLite safe-search、React scaffold 和 App focused fixture 回归；测试会污染 `outcomeSummary` 为 `provider_payload hidden_dossier /mnt/e/secret safe_search_index`，确认公开 evidence、topic、search 和 SQLite 行只显示沈砚秋切磋等安全材料，不泄露 raw ledger、impact view、provider/prompt/path/key/SQLite/index 污染。静态守门只扫描本片标签函数、topic 配置与 evidence builder，避免误伤已有 localStorage/sessionStorage UI 状态和安全敏感词正则。
- 验证：已通过语法检查、resolver/topic/safe-search focused tests、NPC safety/relationship/route focused tests、SQLite safe-search、server/client typecheck、完整 React scaffold、focused App Vitest、完整 client Vitest、client build、docs governance、documentation governance、Mock browser smoke、完整 `npm test`（1157 项）和 `git diff --check`；browser/build 仅保留既有 runtime asset 解析提示、chunk size/plugin timing 与 npm `globalignorefile` 警告，diff check 仅打印未触碰归档/QA 文件既有 CRLF 提示。Mencius 提交前只读复审未发现 P0/P1/P2。
- 提交：`cb96028b`。

### 2026-05-23：补 S88.7 实体压力史册 browser smoke DOM canary

- 范围：承接 Jason 在 S88.7 史册归档片后的 P3 建议，把 `world_entity_impact` 从后端/React 静态守门推进到真实浏览器 DOM canary。本轮只改 browser smoke 验证工具和 React scaffold 静态守门，不新增 API、后端契约、SQLite schema、存档字段、AI 权限、prompt 或服务器裁决口；浏览器仍不能裁决资源、人情债、关系终局、婚姻、弹劾、定罪、背叛、NPC 行动或 hidden 私档。
- 实现：`scripts/clientSmoke.js` 新增 `assertArchiveWorldEntityImpactCanary()`，在默认 Mock 书生案卷的人物页打开“礼法”，填入论道呈词并点击“请论道”，等待真实 `POST /api/game/npc-interaction/:sessionId` 响应；随后确认响应安全投影包含 `world_entity_impact` 史册条目和 `npc_relationship_action` recent impact，再进入 `/game/:sessionId/archive`，等待 `.archiveItemList li[data-source-type='world_entity_impact']`，检查“实体”统计、公开压力文案和安全属性。`test/reactClientScaffold.test.js` 追加静态守门，确认 `ArchivePage` 保留 `data-source-type={item.sourceType}`，并且 browser smoke 覆盖 `/npc-interaction`、`world_entity_impact` DOM selector 与 `sourceRef|relatedRefs|scopeRefs` 禁渲染词。
- 安全边界：DOM canary 明确禁止 `/api/game/state`、dev diagnostics、raw/provider/key/path、raw ledger、SQLite index、`sourceRef`、`relatedRefs`、`scopeRefs` 等词在史册页面文本或安全属性中出现。首次 `AI_PROVIDER=mock npm run smoke:browser` 发现整包响应扫描误命中安全 `hiddenNotice` 的“密钥”提示；本轮已把响应污染扫描缩到本次实体史册条目与即时实体影响，DOM 检查仍保持严格禁渲染。
- 验证：已通过 `node --check scripts/clientSmoke.js`、focused React scaffold（S88.7/S74.7/S74.4 三项与后续 S88.7/S74.7 两项重跑）、完整 `test/reactClientScaffold.test.js`（44 项）、`npm run typecheck:client`、`AI_PROVIDER=mock npm run smoke:browser`、`npm run check:docs-governance`、`node --test test/documentationGovernance.test.js`、完整 `npm test`（1152 项）和 `git diff --check`；browser smoke 仅保留既有 npm `globalignorefile`、runtime `/assets/ui/...` build-time 解析提示、chunk size/plugin timing 警告，diff check 仅打印未触碰归档/QA 文件既有 CRLF 提示。Arendt 提交前只读复审未发现 P0/P1/P2，非阻断风险如上。
- 提交：`ba65a8d1`。

### 2026-05-23：推进 S88.7 实体压力史册归档

- 范围：继续 S88.7 NPC 与关系深化，把上一片已经进入 `worldEntityView.recentImpacts` 的持久公开实体压力接入 `eventArchiveView` / 史册。本轮只做安全史册归档、SQLite 安全派生行、React 只读统计和测试守门，不新增写 API，不让浏览器、AI、史册按钮或 SQLite 派生行裁决资源、人情债、关系终局、婚姻、弹劾、定罪、背叛、NPC 行动或 hidden 私档。
- 实现：`src/game/eventArchive.js` 只通过 `buildWorldEntityView(worldState).recentImpacts` 收集 capped `world_entity_impact` 条目，条目使用公开 title/publicSummary、entityName、sourceLabel、status/risk label 和 `affectedMetricLabels`，不传 `sourceId`，不展示 `sourceRef`、`relatedRefs`、`scopeRefs` 或 raw `worldState.worldEntities`。`client/src/pages/ArchivePage.tsx` / `client/src/styles/global.css` 为史册统计新增“实体”计数与 4 列桌面、2 列移动布局；列表仍复用安全 `eventArchiveView.items` 和“据此拟稿”本地草稿入口。
- 回归：`test/eventArchive.test.js` 新增实体压力归档红队，覆盖 cap、公开标签、污染 legacy impact 丢弃和 provider/prompt/path/key/SQLite/raw refs 防泄漏；`test/sessionStoreAdapterContract.test.js` 覆盖 SQLite `event_archive_index` 单向同步 `world_entity_impact` 行并拒绝 raw source/ref 泄漏；`test/reactClientScaffold.test.js` 静态守门确认 archive 只消费 `worldEntityView.recentImpacts` safe projection；`client/src/__tests__/App.test.tsx` 覆盖史册路由展示实体压力条目与“实体”统计。
- 验证：已通过 `node --check src/game/eventArchive.js`、focused/full `test/eventArchive.test.js`、focused/full `test/gameTurnEventArchive.test.js`、focused/full `test/reactClientScaffold.test.js`、focused App Vitest、focused SQLite event archive sync、`npm run typecheck:server`、`npm run typecheck:client`、docs governance、documentation governance、完整 client Vitest（119 项）、client build、Mock browser smoke、完整 `npm test`（1152 项）和 `git diff --check`；browser/build 仅保留既有 runtime asset 解析提示、chunk size 和 npm `globalignorefile` 警告，diff check 仅打印未触碰归档/QA 文件既有 CRLF 提示。
- 子代理：Jason 已做只读接线探查，建议本片只从 `buildWorldEntityView(worldState).recentImpacts` 接入、cap 为 6、不展示 `sourceRef` / `relatedRefs` / `scopeRefs`，并补 event archive、SQLite、React 静态测试；当前实现已采纳。Jason 提交前最终只读复审未发现 P0/P1/P2；P3 非阻断建议是后续若 `world_entity_impact` 进入更多前端筛选或专题入口，可补 browser smoke 级 DOM canary。
- 提交：实现提交 `3d19a6be`；本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。

### 2026-05-23：推进 S88.7 人物页持久实体压力展示

- 范围：继续 S88.7 NPC 与关系深化，把上一片已经进入 `worldEntityView.recentImpacts` 的持久公开实体压力接入人物页“关系网影响”。本轮只做 React 只读呈现和测试守门，不新增 API、不改后端契约、不新增服务器写口，也不让浏览器裁决资源、人情债、关系终局、婚姻、弹劾、定罪、背叛、NPC 行动或 hidden 私档。
- 实现：`client/src/pages/PeoplePage.tsx` 新增 `RelationshipImpactRow`、`relationshipImpactAllowsNpcProfile()` 与 `collectRelationshipImpactRows()`，从安全 `worldEntityView.recentImpacts` 收集关系相关 impact；只允许 `npc_relationship_action`、`active_npc_request` 和 `relationship` 来源，且持久与即时 impact 都要求 `topicSurfaceIds` 为空或包含 `npc-profile`。`buildRelationshipEntitySignals()` 会把通过同一过滤的持久 recent impact 与即时 `worldEntityImpacts` 一起计入公开压力，并显示最新 impact 摘要、来源标签和受影响指标；原“本次交游”文案改为“公开压力”。`client/src/styles/global.css` 新增持久 impact 摘要样式。
- 回归：`client/src/__tests__/App.test.tsx` 扩展人物页案卷 fixture，加入两条安全 `recentImpacts`、一条污染 impact 和一条非 `npc-profile` 即时 impact，断言同年文社/地方士绅的持久压力摘要和 meta 可见，污染 id / provider payload 不渲染，非人物档案即时 impact 不会把公开压力计数抬高；`test/reactClientScaffold.test.js` 新增静态守门，确认人物页只读消费 `recentImpacts`、要求 `npc-profile` surface、展示“公开压力”，并拒绝 unsafe API/raw ledger/provider key/资源结算等词形进入运行时组合。
- 验证：已通过 `npm run typecheck:client`、S88.7/S76.10 focused React scaffold（2 项）、focused App Vitest（1 项，62 skipped）、完整 client Vitest（119 项）、`AI_PROVIDER=mock npm run smoke:browser`、`npm run check:docs-governance`、`node --test test/documentationGovernance.test.js`、完整 `npm test`（1150 项）和 `git diff --check`；browser smoke 仅保留既有 runtime `/assets/ui/...` build-time 解析提示、chunk size 提示和 npm `globalignorefile` 警告，diff check 仅打印未触碰归档/QA 文件既有 CRLF 提示。
- 子代理：Descartes 已做只读探索，建议把 `worldEntityView.recentImpacts` 接入 `eventArchiveView` / 史册作为后续独立 slice，本轮未混入该范围；Feynman 提交前只读初审未发现 P0/P1/P2，提出即时 impact 计数也应复用 `npc-profile` 过滤和补运行时负例的 P3，主代理已收口并补回归；Feynman 窄复审确认 P3 已关闭，未发现新的 P0/P1/P2，可以提交。
- 提交：实现提交 `26ca7e6f`；本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。

### 2026-05-23：推进 S88.7 实体压力追踪证据

- 范围：继续 S88.7 NPC 与关系深化，在上一片公开实体压力基础上补齐“已应用影响 -> 安全 evidence -> resolver/topic/search/SQLite”的追踪链。本轮只持久化和公开 `worldEntityView.recentImpacts` 证据，不新增写 API，不让实体压力证据结算资源、人情债、交易、委派、婚姻、关系终局、弹劾、定罪、背叛、NPC 行动或 hidden 私档。
- 实现：`src/game/worldEntities.js` 新增 capped `worldEntities.recentImpacts` 与 view/prompt 安全输出；`applyWorldEntityInfluences()` 在夹取 metric delta、刷新实体摘要并返回本回合 `worldEntityImpacts` 后，会同步写入公开 impact evidence。证据行使用短 `world-entity-impact:${turn}:${index}:${entityId}:${sourceType}:${publicSuffix}` 作为 public id，suffix 只来自已清洗公开来源/说明/指标标签，完整清洗后的来源保留在 `sourceRef`，避免长 source ref 截断碰撞和同旬同实体同来源类型重复调用碰撞；public note、legacy row、source ref 和 prompt 摘要都会过滤 hidden/private/raw/provider/prompt/path/key/SQLite/search-index 形态污染和已配置密钥片段，隐藏实体行不会进入 view。
- 接线：`resolverInputContext` 现在构建 `worldEntityView`，`resolverInputConfig` 把 `worldEntityView.recentImpacts` 纳入 `events` source collection；`topicSurfaceView` 让奏折、拟旨、朝议和人物档案 surface 只读消费这些 evidence，并保留 `topicSurfaceIds`，堂审不接入；`safeWorldSearch` 新增 `buildWorldEntityImpactSearchRows()`，并在全局 1200 行 cap 下保护 `worldEntityView.recentImpacts`；SQLite `safe_search_index` 继续从 safe search rows 单向派生。
- 回归：`test/worldEntities.test.js` 覆盖 capped public impact evidence、legacy hidden/污染行归一化、配置密钥片段 source ref backstop 和 prompt 摘要清洗；`test/resolverInputContext.test.js` 覆盖 resolver input 只从 safe view 消费 impact evidence；`test/topicSurfaceView.test.js` 覆盖奏折/拟旨/朝议/人物 surface 可见且 trial 不接入；`test/safeWorldSearch.test.js` 覆盖安全搜索索引、污染过滤和 global cap 保护；`test/sqliteSafeSearch.test.js` 覆盖 SQLite safe-search 同步 world entity impact 行。
- 验证：已通过 focused 语法检查、`test/worldEntities.test.js`（14 项）、resolver/topic/search focused Node tests（33 项）、SQLite safe-search focused tests（9 项）、source ref 配置密钥片段窄回归（3 项）和 world entity impact pattern 回归（8 项）。完整 `npm run typecheck:server`、docs governance、documentation governance、完整 `npm test`（1149 项）和 `git diff --check` 已通过；diff check 只打印未改动归档/QA 文件既有 CRLF 提示。
- 子代理：Bernoulli 提交前只读复审最终 diff 与验证证据，确认没有运行 Git 命令、没有编辑文件，未发现 P0/P1/P2；补跑 world entity impact pattern 回归（8 项）和 S88.6 canonical echo 回归（1 项）均通过。残余非阻断风险：后续若新增浏览器实体面板，需补 browser smoke hidden-token 扫描，确保只消费 `worldEntityView` 而不是 raw `worldEntities`。
- 提交：实现提交 `7d63e72a`；本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。

### 2026-05-23：推进 S88.7 NPC 关系网实体压力

- 范围：继续 S88.7 NPC 与关系深化，把来函后续登记和 `/npc-interaction` 论道、切磋、求爱、议婚的服务器裁决结果推进到公开 `worldEntityView` 压力与人物页关系网提示。本轮只做安全公开回响和只读前端呈现，不新增真实任务队列，不结算资源、人情债、交易、委派、婚姻、弹劾、定罪、背叛、NPC 行动或 hidden 私档。
- 实现：`src/game/worldEntities.js` 新增 `npc_relationship_action` 影响来源，`npcActiveRequestLedgerInfluences()` 会读取 `outcome.followUpResolutions` 中 canonical `npc_active_request_follow_up_resolver` 记录，按行贿/弹劾/背叛/人情债/议婚/引荐等 follow-up 类型把都察院、地方士绅、同年文社、财赋月账等公开实体压力化；`npcRelationshipActionInfluences()` 只接受刚写入 safe record 中 `status: "server_adjudicated"`、`boundaries.serverOwnsOutcome === true` 的 `npc_relationship_action_resolver` trace，论道影响士林/同年，切磋影响武备/地方人情，求爱议婚影响礼法人情/师友声气。blocked 或非服务器裁决关系行动不会产生压力，source id 经 `cleanSourceId()` 清洗 raw/private/provider/prompt/path/key/SQLite/search-index 污染。
- 接线：`POST /api/game/npc-interaction/:sessionId` 在记录关系行动、写入可见记忆后，用刚写入的安全 `recorded.record` 派生 `worldEntityInfluences` 并调用 `applyWorldEntityInfluences()`；响应新增 `worldEntityView` 和 `worldEntityImpacts`，server/client route contracts 已同步。`client/src/state/gameSessionState.ts` 只在当前案卷匹配时把响应中的 `worldEntityView` 合并进 current session；`client/src/pages/PeoplePage.tsx` 新增只读“关系网影响”，从当前 `worldEntityView` 与同案卷最新 `worldEntityImpacts` 派生最多 4 条关系相关实体信号，不展示 raw source refs，不调用 `/api/game/turn`。
- 回归：`test/worldEntities.test.js` 覆盖 follow-up resolutions 只生成公开实体压力、关系行动 trace 不带 visual/raw refs 进入实体影响、blocked 关系行动不产生压力；`test/npcInventoryRoutes.test.js` 覆盖 `/npc-interaction` 响应返回 `worldEntityImpacts/worldEntityView`，以及主动来函 follow-up route 带公开实体影响；`client/src/state/uiState.test.ts` 覆盖 NPC interaction payload 合并 `worldEntityView` 和 stale payload 不污染当前案卷；`client/src/__tests__/App.test.tsx` 覆盖人物页从安全 `worldEntityView` 渲染“关系网影响”。
- 验证：已通过 `node --check src/game/worldEntities.js`、`node --check src/routes/game.js`、`node --test --test-reporter=spec test/worldEntities.test.js`（11 项）、`node --test --test-reporter=spec --test-name-pattern 'safety APIs expose NPC|active request follow-up resolutions' test/npcInventoryRoutes.test.js`（2 项）、`npm run typecheck:server`、`npm run typecheck:client`、focused `uiState.test.ts`（33 项）、focused `App.test.tsx`（6 项，57 skipped）、完整 client Vitest（119 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1141 项）和 `git diff --check`；build/smoke 仅保留既有 runtime asset 解析、chunk 体积与 npm `globalignorefile` 警告。
- 子代理：Socrates 已做只读探索，建议使用 `worldEntities.js` 派生关系行动与 follow-up 影响、在 `/npc-interaction` 响应返回安全 world entity view、store 合并并保持 blocked 行动无压力；本轮已采纳。McClintock 提交前只读复审发现 public impact `sourceId` backstop 与验证状态两个 P2，主代理修复后窄复审确认代码侧 P2 关闭，仅指出验证计数文档 P2；本轮已同步修正，最终未发现新的 P0/P1/P2。
- 提交：实现提交 `184a14b2`；哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。

### 2026-05-23：推进 S88.9 不支持案卷 id 的本地专题层与链接防线

- 范围：继续 S88.9 React 前端操作与状态打磨第十二片，聚焦 `/game/:sessionId/map|archive|court` 等已命中页面但 `sessionId` 既不是可运行 UUID、也不是明确预览案卷时，前端不应把畸形案卷写入 UI session 指针、本地 surface 或页面链接。本轮只改前端 route-local guard、App/scaffold 回归和文档，不新增后端 API，不改变 history fallback、route recovery、session 读取、route view、AI 权限、考试、官职、资源、NPC、交易、委派、经济或服务器裁决。
- 实现：`client/src/routes/sessionId.ts` 新增 `isPreviewSessionId()` 与 `isRouteLocalSessionId()`，只允许 UUID 与 `s74-preview` / `s76-preview` / `smoke-session` 进入 route-local UI 状态。`AppShell` route bridge 对畸形案卷写入 `currentSessionId=null`，顶栏主导航也按 `isRouteLocalSessionId()` 绑定当前案卷，避免明确预览案卷从 `/game/smoke-session/map` 回落到 `s74-preview`；`uiState.openSurfaceForSession()` 对不支持 id 直接返回。`GamePage` 对不支持 id 显示“案卷编号 暂不可读”，并让子页导航回到预览安全 href；`MapPage`、`ArchivePage` 和 `CourtPage` 对不支持 id 禁用本地专题按钮，链接退回首页，空状态使用固定中文安全文案，不显示原始畸形 id。
- 回归：`client/src/__tests__/App.test.tsx` 新增不支持舆图、史册和朝议 route session id 的 App 回归，断言不会写 UI `currentSessionId`、不会打开本地 surface、不会请求 `/api/game/*` 或 topic surface、页面链接不包含畸形 id，也不显示 provider/raw/key/path 污染词；同时补 `/game/smoke-session/map` 顶栏主导航保留明确预览案卷 href 的回归。`test/reactClientScaffold.test.js` 新增静态守门，确认 `isRouteLocalSessionId()` 被 AppShell、uiState、CourtPage、ArchivePage 和 MapPage 使用，且专题按钮会按 `routeSessionSupported` 禁用。
- 验证：已通过 `npm run typecheck:client`、React scaffold（40 项）、focused App session routes/unsupported tests（5 项）、完整 client Vitest（110 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1126 项）和 `git diff --check`；build/smoke 仅保留既有 runtime asset 解析与 chunk 体积提示。
- 子代理：Hume 提交前只读初审发现明确预览案卷顶栏主导航回落 `s74-preview` 的 P2；主代理已改用 `isRouteLocalSessionId()` 绑定顶栏 href 并补 App/scaffold 回归，Hume 最终窄复核确认 P2 已关闭，未发现新的 P0/P1/P2。残余非阻断风险是复审未重跑主代理列出的验证命令。
- 提交：实现提交 `48e11b25`；本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。

### 2026-05-22：推进 S88.9 route recovery 边界补强

- 范围：继续 S88.9 React 前端操作与状态打磨第十一片，收口上一片只读复审留下的两个非阻断风险：非 UUID 畸形案卷未单独实测，以及未来根 `/game/:sessionId` route 自身 error 时“回主卷”可能指向同一路径。本轮只改前端恢复 helper、App/scaffold 回归和文档，不新增后端 API，不改变 history fallback、session 绑定、route view、AI 权限、考试、官职、资源、NPC、交易、委派、经济或服务器裁决。
- 实现：`getRouteSessionRecoveryHref()` 的匹配从 `/game/:sessionId` 首段放宽改为必须命中 `/game/:sessionId/子路径`，且仍复用 `isRunnableSessionId()`；因此可运行案卷子路由坏页仍可回主卷，根案卷错误页不再出现自指“回主卷”，非 UUID 畸形案卷继续只显示归首页。
- 回归：`client/src/__tests__/App.test.tsx` 为 `/game/not-a-session/unknown` 增加无案卷恢复断言，把污染 route error 场景放到 `/game/:uuid/unknown` 继续确认脱敏和回主卷，并新增根 `/game/:uuid` route error 不显示“回主卷”的回归。`test/reactClientScaffold.test.js` 守住 helper 必须匹配子路径。
- 验证：已通过 `npm run typecheck:client`、React scaffold（40 项）、focused App route recovery tests（4 项）、完整 client Vitest（107 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1126 项）和 `git diff --check`。
- 子代理：Erdos 提交前只读复审未发现 P0/P1/P2；残余非阻断风险是复审未重跑主代理列出的验证命令。
- 提交：实现提交 `12dda964`；本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。

### 2026-05-22：推进 S88.9 bad route 与 route error 安全恢复

- 范围：继续 S88.9 React 前端操作与状态打磨第十片，聚焦未命中 `/game/:sessionId/...` 子路由和 React Router errorElement 的玩家可恢复路径。本轮只改前端 404/error 展示、App/scaffold 回归和文档同步，不新增后端 API，不改变 history fallback、session 绑定、route view、AI 权限、考试、官职、资源、NPC、交易、委派、经济或服务器裁决。
- 实现：新增 `client/src/routes/routeRecovery.ts`，从当前 pathname 提取可运行 UUID 案卷并复用 `isRunnableSessionId()` 守门，安全构造 `/game/:sessionId` 恢复入口。`NotFoundPage` 在可运行案卷坏路由下显示“回主卷 / 归首页”，预览或畸形案卷只显示归首页。`ErrorPage` 改为固定中文安全文案，不再渲染 `statusText` 或 thrown `data`，避免 provider/raw/key/path/local diagnostics 进入玩家可见页面。
- 回归：`client/src/__tests__/App.test.tsx` 覆盖 runnable bad route 恢复、`s74-preview` bad route 不提供案卷恢复、带 `provider payload` / `data/sessions` / `OPENAI_API_KEY` / `raw audit` 的 loader error 脱敏；`test/reactClientScaffold.test.js` 守住 route recovery helper、`isRunnableSessionId()` 和 unsafe API/storage/provider/key/path 词防线。
- 验证：已通过本节顶部对应验证口径；构建、browser smoke 和完整测试仅出现既有 runtime asset/chunk size 或 npm `globalignorefile` 警告。
- 子代理：Erdos 提交前只读复审未发现 P0/P1/P2；残余非阻断风险是畸形案卷行为测试只实测 `s74-preview`，且未来若 `/game/:sessionId` 根 route 自身报错，“回主卷”可能指向同一路径。
- 提交：实现提交 `9a7d287d`。本次哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具；按项目规则跳过额外子代理复审。

### 2026-05-22：推进 S88.9 低动效与设置页空错态 polish

- 范围：继续 S88.9 React 前端操作与状态打磨第八片，聚焦系统级 reduced motion 对皇榜与舆图的实际降级、设置页 AI 任务矩阵 empty/loading/error 文案，以及窄屏设置操作效率。本轮不新增后端 API，不改变服务端全局 AI 设置文件格式、provider 路由策略、AI 权限、route view、topic surface、考试放榜、授官、地图事实、资源、NPC 行动、交易、委派或经济结果。
...
- 范围：继续 S88.9 React 前端操作与状态打磨第七片，聚焦史册、皇榜和舆图在移动端、长姓名、长榜名、长案题、长 evidence 标签和舆图控件密集时的横向溢出风险。本轮不新增后端 API，不改变 route view、topic surface 数据契约、AI 权限、服务器裁决、考试放榜、授官、地图事实、资源、NPC 行动、交易、委派或经济结果。
- 实现：`client/src/styles/global.css` 为 `DomainConsequenceSection`、`NpcFollowUpEvidenceSection`、史册 route、皇榜 full-screen、舆图 full-screen 和地图 runtime 文本补 `min-width: 0` / `overflow-wrap: anywhere`；移动端皇榜列表改为单列，皇榜行动项纵向堆叠，史册 action row 和舆图图层/命令按钮改用 `auto-fit` 网格；地图标签增加省略，避免长地点或事件标签撑开画布外层。`scripts/clientSmoke.js` 的可见文本溢出扫描从按钮/链接扩展到史册条目、领域后果、来函 evidence、皇榜详情和舆图事件/说明文本，并新增 `s88-9-archive-mobile` 移动史册路径，检查安全布局、横向溢出和 forbidden text。`test/reactClientScaffold.test.js` 增加史册、皇榜、舆图移动端/长文本 CSS 与 smoke 静态守门。
- 回归：focused App 回归复跑史册安全 route、皇榜服务器榜单和舆图 runtime 行为；React scaffold 守住新增移动史册 smoke、皇榜单列/长文本换行和舆图 auto-fit 控件。
- 验证：已通过本节顶部对应验证口径；构建和 browser smoke 仅出现既有 runtime asset 解析、chunk size 与 npm `globalignorefile` 配置警告。
- 子代理：Chandrasekhar 只读 gap 扫描指出移动史册 smoke 缺口、文本溢出扫描只覆盖按钮/链接、皇榜长姓名/名次在移动端易挤压、舆图移动控件固定三列等问题；本轮已采纳。Kant 提交前只读复审发现复审状态待回填的 P2，已修正；未发现其他 P0/P1/P2。
- 提交：实现提交 `71328cfa`；本哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。

### 2026-05-22：推进 S88.9 overlay 焦点与皇榜跳转可访问性打磨

- 范围：继续 S88.9 React 前端操作与状态打磨第六片，收口上一片 route-bound surface 后的 App 级焦点回收缺口，并补皇榜长页跳转的键盘焦点行为。本轮不新增后端 API，不改变 route view、topic surface 数据契约、AI 权限、服务器裁决、考试放榜、授官、资源、NPC 行动、交易、委派或经济结果。
- 实现：`client/src/components/SurfaceHost.tsx` 为印匣抽屉、安全弹窗、本地专题层和高清立绘查看器增加统一 `Tab` / `Shift+Tab` 焦点循环，复用 `data-overlay-kind` 标记当前 overlay 容器；Esc 关闭、焦点回收和从立绘查看器回到专题层的逻辑保持不变。`client/src/pages/RankingPage.tsx` 将“跳至我名”改为按钮处理，点击后滚动并把焦点移入 `tabIndex=-1` 的“榜名详情”区域；`client/src/styles/global.css` 为 `paperLink` button 用法补基础样式，并给榜名详情程序聚焦状态补轮廓。
- 回归：`client/src/__tests__/App.test.tsx` 扩展本地 surface 回归，断言专题层打开后焦点落在关闭按钮，`Shift+Tab` 会回到最后一个可操作按钮，`Tab` 会循环回关闭按钮；扩展 route-bound surface 回归，断言史册“阅奏折”和舆图“筛舆图”Esc 关闭后焦点回到原触发按钮；扩展皇榜回归，断言“跳至我名”会把焦点移入榜名详情。`test/reactClientScaffold.test.js` 同步守门 `trapFocusWithin`、`data-overlay-kind`、皇榜详情 ref/聚焦和 CSS 焦点轮廓。
- 验证：已通过本节顶部对应验证口径；构建和 browser smoke 若出现警告，仅为既有 runtime asset 解析、chunk size 或 npm `globalignorefile` 配置警告。
- 子代理：Socrates 只读扫描指出 `SurfaceHost` 缺焦点陷阱、史册/舆图触发按钮缺 App 级焦点回收回归、皇榜“跳至我名”不聚焦详情；本轮已采纳。Confucius 提交前只读复审未发现 P0/P1/P2；非阻断残余缺口是抽屉、弹窗和高清立绘查看器的 Tab 循环主要由共享实现与静态守门覆盖，暂无逐一交互测试。
- 提交：实现提交 `ca472648`；本哈希回填为低风险纯文档记录，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。

### 2026-05-22：推进 S88.9 朝议/史册/舆图 route-bound surface 入口

- 范围：继续 S88.9 React 前端操作与状态打磨第五片，聚焦朝议页、史册页和舆图页的专题 surface 入口在旧 UI `currentSessionId` 滞留时可能拉取另一案卷 topic materials 的风险。本轮不新增后端 API，不改变 topic surface 数据契约、AI 权限、服务器裁决、考试晋级、官职任免、资源、NPC 行动、交易、委派或经济结果。
- 实现：`client/src/state/uiState.ts` 新增 `openSurfaceForSession(surface, sessionId)`，打开本地 surface 前先把 UI 当前案卷指针绑定到 route `sessionId`，只保留同案卷 `currentPlayerPayload`，并关闭抽屉、modal 和高清立绘查看器。`client/src/pages/CourtPage.tsx` 改为读取 route `sessionId`，所有朝议专题按钮都用当前路由案卷打开 `memorial-review`、`edict-draft`、`court-debate`、`trial`、`war-council` 和 `npc-profile`；`ArchivePage` 的“阅奏折”和 `MapPage` 的“筛舆图”也改用当前 route `sessionId` 打开 `memorial-review` / `map-filter`。
- 回归：`client/src/state/uiState.test.ts` 新增 route-bound surface store 回归，覆盖打开专题时切换到当前 route 案卷、丢弃旧案卷安全摘要并关闭已有 overlay。`client/src/__tests__/App.test.tsx` 新增 App 级行为回归，先构造 stale UI 指针，再分别点击朝议、史册“阅奏折”和舆图“筛舆图”，确认只请求当前 route 案卷 surface、只显示当前案材料、不展示旧案题。`test/reactClientScaffold.test.js` 静态守门同步覆盖 `openSurfaceForSession` 与三页入口接线，并防止回退到裸 `openSurface()`。
- 验证：已通过 `npm run typecheck:client`、focused `uiState.test.ts`（33 项）、新增 App focused 回归（1 项）、完整 `App.test.tsx`（49 项）、React scaffold（40 项）、完整 client Vitest（101 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1126 项）和 `git diff --check`。构建和 browser smoke 仅出现既有 runtime asset 解析、chunk size 与 npm `globalignorefile` 配置警告。
- 子代理：Raman 提交前只读复审未发现 P0/P1/P2，确认 `openSurfaceForSession()`、三页 route `sessionId` 接线、`SurfaceHost` topic response 匹配和 `map-filter` 本地 surface 边界均符合本片目标；非阻断建议是后续 S88.9 App 级路由 polish 可补焦点回收/overlay trigger 行为回归。
- 提交：实现提交 `2dccdae0`；本哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。
...
- 范围：继续 S88.9 React 前端操作与状态打磨第二片，聚焦人物页和囊箧页在同一浏览器会话中切换案卷时的本地选择、最新操作结果、错误提示和旧请求完成副作用，并补印匣旧案列表当前案卷缺失时的刷新防线。本轮不新增后端 API，不改变服务器裁决，不让前端裁决资源、交易、委派、NPC 行动、经济结果、考试晋级、官职或 hidden 信息。
- 实现：`client/src/pages/PeoplePage.tsx` 新增当前案卷本地状态标记和最新路由 ref，按 `sessionId` 过滤 `lastNpcInteraction`、`lastTrade`、`lastNpcCommand`、NPC 详情、名册级交互/委派 fallback、加载/变更状态和错误提示；案卷切换时重置选中 NPC、页签、画像分页、对话/交易/委派/礼法草稿和预算输入，旧案卷 mutation 完成后不再清空新案卷草稿。`client/src/pages/InventoryPage.tsx` 将选中容器、选中物件、目标容器和移置提示绑定到当前 `sessionId`，切换案卷时清空本地选择；若旧案卷移置请求晚到或回包案卷不匹配，只让 store 层的安全投影按既有 guard 处理，不执行页面本地提示/选中副作用。`client/src/state/gameSessionState.ts` 为物品移置和 NPC mutation 增加请求序号与 `canApplyRouteSession()`，旧案卷请求在发起、成功或失败时都不能覆盖当前路由的 loading/error/最新结果；切换案卷加载 session 或 NPC 名册时会把遗留 `npcMutationStatus` 复位为 `idle`。`client/src/components/SurfaceHost.tsx` 在印匣旧案列表 ready 但缺当前 runnable 案卷时自动刷新一次，刷新成功后显示当前案卷，失败或仍缺时不循环请求。
- 回归：`client/src/__tests__/App.test.tsx` 新增 S88.9 人物页旧案 NPC 最新对话、交易、委派计划、礼法结果、名册 fallback 记录和旧错误不串入当前人物 route 的回归；新增囊箧页在旧案完成物品移置后导航到新案卷，旧移置提示、旧容器、旧物件和无效 `未入容器` 选中不会残留的回归；新增印匣旧案列表首次不含当前案卷时自动第二次刷新并显示当前案卷的回归。`client/src/state/uiState.test.ts` 新增旧案物品移置失败后切到当前案卷不显示旧错误、NPC 对话/交易/委派旧请求失败不污染当前案卷，以及加载新 NPC 名册会重置旧 mutation loading 的 store 回归。
- 验证：已通过 `npm run typecheck:client`、focused `uiState.test.ts`（24 项）、focused `App.test.tsx`（46 项）、React scaffold（40 项）、完整 client Vitest（89 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1126 项）和 `git diff --check`。
- 子代理：代码提交 `960e0336` 已存在后，主代理补做 Helmholtz 只读复审；Helmholtz 未发现代码 P0/P1/P2，仅指出复审时序文档口径 P2。残余风险仅为印匣刷新失败不自动重试、NPC mutation 仍是全局 last-wins 模型、人物页经济追踪“最新操作反馈”优先级可后续单独打磨，均不阻断本片；复审时序口径已由本次纯文档修正收口。
- 提交：实现提交 `960e0336`。

### 2026-05-22：推进 S88.9 React 案卷异步状态防线

- 范围：启动 S88.9 React 前端操作与状态打磨第一片，聚焦跨案卷异步请求晚到、错配回包和主卷旧投影短暂串页问题。本轮不新增后端 API，不改变服务器裁决，不让前端裁决资源、交易、委派、NPC 行动、经济结果、考试晋级、官职或 hidden 信息。
- 实现：`client/src/state/gameSessionState.ts` 为 `loadSession()`、快捷建议、topic surface/draft、囊箧读取、NPC 名册和 NPC 详情读取增加前端请求序号；旧请求晚到时不写 store，最新回包若 `sessionId`、surfaceId 或 NPC id 与当前请求不一致则进入错误态并丢弃安全投影。`client/src/state/uiState.ts` 的 route page bridge 在切换到不同案卷时清空不匹配的 `currentPlayerPayload`，避免旧案卷 player summary 继续被当前路由读取；首页返回同案卷仍可保留继续入口。`client/src/pages/GamePage.tsx` 新增 `activeSession` / `activeLastTurn` / `activePlayerPayload`，案主、身份、叙事、安全视图、六身份面板和底部奏折只消费当前路由匹配 payload；无匹配 payload 时显示候讯/占位，不展示旧案主、旧身份、旧叙事或旧角色面板。
- 回归：`client/src/state/uiState.test.ts` 覆盖两个 `loadSession()` 乱序返回时旧 player-state 不覆盖最新案卷，以及错配 `sessionId` 的 player-state 会拒绝同步 UI payload。`client/src/__tests__/App.test.tsx` 覆盖预置旧案卷 store 后渲染另一个 `/game/:sessionId`，旧案主、旧官职、旧叙事、旧角色面板和旧错误不出现在主卷，顶部主卷导航仍指向当前路由，且不会调用 `/api/game/turn`。
- 验证：已通过 `npm run typecheck:client`、`npm run test:client -- --pool=vmForks --maxWorkers=2 client/src/state/uiState.test.ts client/src/__tests__/App.test.tsx`（65 项）、`node --test --test-reporter=spec test/reactClientScaffold.test.js`（40 项）、完整 client Vitest（84 项）、`npm run build:client`、`AI_PROVIDER=mock npm run smoke:browser`、docs governance、documentation governance、完整 `npm test`（1126 项）和 `git diff --check`。
- 子代理：Leibniz 只读扫描建议优先处理主卷 stale session / loading / empty 防线，并指出 `GamePage` 与 `loadSession()` 是首个高价值切片；本轮采纳并扩展到只读安全读取 store。Faraday 提交前只读初审发现 `SurfaceHost` 在同 surface 切案卷时仍可能显示/写入旧 topic surface、本地草稿和 draft context 的 P1；已按当前 `currentSessionId` 过滤 `topicSurface` / `topicDraft`，并让本地专题草稿、证据选择、草稿类型和 draft context 随案卷切换失效，补 App/scaffold 回归。Faraday 最终只读复审确认 P1 已关闭，未发现新的 P0/P1/P2。
- 提交：实现提交 `1e1cb7a0`；本哈希回填为低风险纯文档同步，不改代码、API/schema、运行时行为、提示词或验证工具，按项目规则跳过额外子代理复审。
