...
| S88.5 | IN_PROGRESS | 六身份循环矩阵 | S88.5.1 已先建立服务器派生的 `roleCycleView` 与 React `RoleCycleSection` 首片，把皇帝、大臣、将领、地方官、书生、入仕官员统一为当前身份循环矩阵：只公开当前身份事务、风险、待办、AI read scope、工具权限、proposal 边界和服务器裁决说明，非当前身份只显示待任占位；后续切片再把这些事务接入更专门 resolver、长期后果 refs 或地图/NPC/经济入口。 |
...
- S88.5 当前进展：S88.5.1 已建立 `roleCycleView` 首片，作为服务器即时派生的六身份循环矩阵安全 view；它只展开当前身份的事务、风险、待办、AI 读取范围、工具权限、proposal 边界和服务器裁决说明，非当前身份只显示“待任后展开”占位，避免低权限身份读取其他身份的 hidden 军务、任所、私档或 raw ledger。React 六身份主面板共用 `RoleCycleSection` 展示“本旬身份循环 / 本旬事务 / 风险 / 可拟草稿”，按钮仅写本地行动草稿，不调用 turn API，不替代服务器裁决。
