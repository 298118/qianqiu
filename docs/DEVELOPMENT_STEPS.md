...
本轮 S88.5 六身份矩阵信息密度 / 身份差异化 polish 当前验证口径：

- `npm run typecheck:client`
- `node --test --test-reporter=spec --test-name-pattern 'S88.5 role cycle section is wired' test/reactClientScaffold.test.js`
- `npm run test:client -- --pool=vmForks --maxWorkers=2 src/__tests__/App.test.tsx --reporter=verbose`（63 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（119 项）
...
- Turing 提交前只读复审仅发现共享上下文验证状态待回填 P2，代码/安全边界未发现 P0/P1/P2；本轮已修正并需随提交前最后文档守门确认。
