...
| S88.11 | IN_PROGRESS | 视觉、立绘与氛围一致性 | 首片已启动 runtime manifest 生成层 QA，实现提交 `099d4ab3`。第二片已推进 assetRegistry 消费层 QA，实现提交 `89059f7a`：前端 registry 会拒绝全 manifest 重复 asset id / portraitRef、非成年或缺显式元数据的立绘、懒加载策略缺口、池分组与 subcategory 不一致，并把通用 NPC 画像查询固定在 `generic_npc_pool`，调用方不能用大 limit 突破首屏立绘预算。第三片已启动 browser smoke 细化：真实浏览器会显式断言只请求 runtime manifest、不请求完整 source manifest，复核 runtime manifest 字段/路径/审核/成年立绘/lazy-load 边界，并把人物页 `data-portrait-ref` 与重要 NPC 专属池 metadata 交叉验证，首屏立绘/缩略图/占位请求仍限制在 8 张内；本片不新增图片、不改变可用资产、不放宽前端加载策略。 |
