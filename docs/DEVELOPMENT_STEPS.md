...
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx --reporter=verbose -t "safe recovery|malformed bad routes|sanitizes route error diagnostics"`（3 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2`（106 项）
...
- `npm test`（1126 项）
- `git diff --check`
- 提交前只读复审：Erdos 未发现 P0/P1/P2；残余非阻断风险是畸形案卷行为测试只实测 `s74-preview`，且未来若 `/game/:sessionId` 根 route 自身报错，“回主卷”可能指向同一路径。
