...
- 当前下一步是 S76.4：入仕官员/大臣面板，围绕官职履历、部院公文、同年座师、派系、考成、弹劾和奏疏入口继续保护草稿-only 与服务器 resolver 边界。
...
- `npm run smoke:browser -- --client react --screenshots artifacts/s76-3-magistrate-panel`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `git diff --check`
- `npm test`

当前已知遗留：浏览器级 `prefers-reduced-motion` context 深测留给 S77.6；真实专题 `surface` 来源和完整专题 composer 留给 S76.11；S76.4 继续推进入仕官员/大臣面板。
