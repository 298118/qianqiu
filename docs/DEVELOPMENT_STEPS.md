...
- 2026-05-26：S89.46 完成首页静态 surface utility 接入。当前范围限 React className、CSS utility/共享 selector、客户端 smoke/source canary、brief 和开发文档；首页开卷案桌、当前本局和旧案架新增 `.paperMotionSurface`，共享材质 selector 不再依赖这些 route-specific class。未删除仍有源码命中的业务布局 selector，不新增浏览器裁决权，不改开局表单、旧案读取、继续本局链接、后端 API/schema、AI 权限、prompt、provider、SQLite、存档、runtime manifest 或素材 manifest。实现提交待回填。
- 前一轮 S88 归档是低风险纯文档维护；S89.3-S89.46 涉及前端代码、样式、验证脚本和文档，提交前按子代理复审规则执行。
