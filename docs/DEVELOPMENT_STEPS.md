...
- 当前下一步是 S76.2：书生面板，围绕书斋、读书簿、老师点评、师友、科期、文章练习和赶考入口继续保护完整科举路径。
...
- `node --test test/reactClientScaffold.test.js test/browserSmokeScript.test.js`
- `npm run typecheck:client`
- `npm run test:client`
- `npm run smoke:browser -- --client react --screenshots artifacts/s75-10-browser-acceptance`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `git diff --check`
- `npm test`

当前已知遗留：浏览器级 `prefers-reduced-motion` context 深测留给 S77.6；真实专题 `surface` 来源和完整专题 composer 留给 S76.11；S76.2 继续推进书生面板。
