...
| S88.11 | IN_PROGRESS | 视觉、立绘与氛围一致性 | 首片已启动 runtime manifest 生成层 QA，实现提交 `099d4ab3`：`npm run qa:runtime-manifest` 现在会在写入/校验精简 manifest 前拒绝未审核状态、非 `/assets/ui/` 路径、作者侧字段、本地/远程/artifacts/key/raw/provider/hidden/prompt 污染、非成年立绘元数据、缺缩略图/低清占位和 eager-load 立绘；本片不新增图片、不改变可用资产、不放宽前端加载策略。 |
...
本轮 S88.11 runtime manifest 生成层 QA 首片当前验证口径：
