...
| S73-S77 | ACTIVE | 前端水墨重构：素材体系、React/Vite 渐进迁移、首页/全局 shell、身份/场景专题、考试/放榜、舆图、立绘管线、验收与归档 | [FRONTEND_INK_REDESIGN_ROADMAP.md](FRONTEND_INK_REDESIGN_ROADMAP.md) |
...
| S73.0a | DONE | Codex + 只读子代理 | 详细蓝图、依赖路线与素材分期 | 已按用户补充要求把 S73-S77 扩展为可执行小步骤，记录 React/Vite 候选、素材矩阵、NPC 立绘分期和女性立绘安全边界 |
| S73.1 | TODO | Codex | 前端视觉资产指南 | 新增 `FRONTEND_VISUAL_ASSET_GUIDE.md`，固定水墨/宣纸/奏折/身份/场景/立绘/动效/fallback 的尺寸、命名、颜色、裁切和审核标准 |
| S73.2 | TODO | Codex | UI manifest schema 与素材台账 | 新增 `public/assets/ui/ink-ui-manifest.json` 草案和 `FRONTEND_ASSET_LEDGER.md`，覆盖审核状态、缩略图、fallback、性能和安全字段 |
| S73.3 | TODO | Codex | UI 材质包 | 生成宣纸、旧绢、奏折、竹简、卷轴、残纸、水渍、淡墨线、朱印按钮、印匣纹理、试卷界栏和皇榜纸底等 12-20 个素材 |
| S73.4 | TODO | Codex + 子代理创意草案 | 首页资产包 | 生成首页山水画卷、云雾层、题名册表单底、朱印开始按钮和首页案卷存档素材，登记 manifest 与台账 |
| S73.5 | TODO | Codex + 子代理创意草案 | 场景插画包 | 生成书斋、贡院号舍、放榜、殿试、县衙、公堂、军帐、御案、城市街巷等 8-12 张场景图 |
| S73.6 | TODO | Codex + 子代理创意草案 | 身份背景包 | 为书生、地方官、入仕官员、大臣、将领、皇帝准备专属背景、色彩权重和面板材料建议 |
| S73.7 | TODO | Codex + 子代理创意草案 | 玩家/NPC 立绘风格基准 | 首轮只做 24-40 张成人、端庄、高颜值、身份明确且不露骨的基准立绘，预留长期 300-400 张分期管线 |
| S73.8 | TODO | Codex | 动效与 fallback 包 | 生成云雾、墨迹扩散、朱印落章、纸页展开、榜文揭示、交卷火漆印等动效及 reduced-motion 替代 |
| S73.9 | TODO | Codex | 素材预览与 QA | 新增素材预览或测试脚本，校验路径、尺寸、透明度、安全字段、审核状态、fallback 和未审核素材禁用 |
| S74.0 | TODO | Codex + 只读依赖审阅子代理 | 依赖治理与 React 迁移契约 | 通过治理后再安装 React、TypeScript、Vite、Zustand、Lucide、Vitest、Testing Library 和 jsdom；新增前端迁移契约，不切默认入口 |
| S74.1 | TODO | Codex + 实施子代理 | Vite/TypeScript 并行前端岛 | 新增 `client/`、Vite config、TS config、client test config，构建输出到 `public/ink-client/`，旧 `/` 保持可用 |
...
| S74.4 | TODO | Codex + 实施子代理 | Shell 与 surface registry | 建立首页、游戏页、设置抽屉、全屏 surface host、考试/放榜/地图/NPC/圣旨草稿扩展位 |
| S74.5 | TODO | Codex + 实施子代理 | 资产加载层 | 读取 UI manifest，按 role/scene/usage 懒加载素材、缩略图和 fallback，未审核素材默认不可用 |
| S74.6 | TODO | Codex | 旧前端兼容桥 | 旧 `public/app.js` 保留回滚；新前端不依赖旧全局状态，地图桥只包装 S72 安全 map runtime |
| S74.7 | TODO | Codex | S74 回滚与验收 | 记录回滚方式，验证旧 `/` 与新 `/ink-client/` 都能 Mock 开局，client typecheck/test/build 和旧 smoke 通过 |
...
| S76.9 | TODO | Codex + 实施子代理 | 舆图页升级 | 将 S72 地图桥接到 React shell，支持扩大查看、tooltip、局势簿跳转、行动草稿和资源失败 fallback |
| S76.10 | TODO | Codex + 实施子代理 | NPC 与玩家立绘接线 | `Portrait` 只接 `portraitRef` 和安全公开 view；未审核立绘不显示，不加载全量长期池 |
...
| S77.1 | TODO | Codex | 默认入口切换 | 新前端通过验收后切 `/`，旧版保留 `/legacy.html` 或等价回滚入口 |
...
| S77.5 | TODO | Codex | 性能与资源预算 | 首屏只加载必要素材，场景和立绘按需加载，长期立绘池不进首屏 bundle |
...
- 将 S74 之后的技术路线调整为“React + TypeScript + Vite 渐进式接管”：S74.0 先走依赖治理，S74-S76 先在 `/ink-client/` 并行新前端岛开发，S77 验收后才切默认入口，旧前端保留回滚。
- 记录建议引入 React、React DOM、TypeScript、Vite、Zustand、Lucide、Vitest、Testing Library、jsdom；同时暂缓 React Router、TanStack Query、`@pixi/react`、Tailwind/UI kit 和 SSR 框架。
- 将素材生成规划细化为 UI 材质包、首页资产、场景插画、身份背景、玩家/NPC 立绘、动效/fallback、素材预览/QA 和长期立绘分期；首轮立绘限定为 24-40 张风格基准，长期 300-400 张分四期扩展。
...
- S74.0 真正安装前端框架依赖前，必须单独执行依赖治理、记录官方资料、验证 `npm install` / `npm start` 和回滚方式。
