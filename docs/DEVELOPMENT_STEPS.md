...
- 子代理：Hume 初审发现旧案 mutation 失败可能在新案卷显示错误、`loadNpcs()` 切案卷未复位 `npcMutationStatus` 的 P2；本轮已按请求序号与 route-session guard 修复并补回归。Hume 最终只读复审未发现 P0/P1/P2，残余风险仅为印匣刷新失败不自动重试、NPC mutation 仍是全局 last-wins 模型、人物页经济追踪“最新操作反馈”优先级可后续单独打磨，均不阻断本片提交。
- 提交：实现提交待回填。
