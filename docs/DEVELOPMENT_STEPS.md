...
- S88.7 当前人物页交游议题消费层待提交：人物页现在从同案卷安全 `worldThreadView.activeThreads` 只读筛选 `sourceType === "npc_relationship_action"` 的交游议题，显示为“交游议题”卡片；污染标题、摘要、目标、follow-up 提示、source id/label 或 provider/raw/path/key/hidden/本地路径形态会被丢弃，非关系行动 thread 不进入人物页。“拟跟进”只写本地行动草稿，不调用 `/api/game/turn`，不裁决资源、人情债、关系终局、婚姻、伤损、弹劾、定罪、背叛、NPC 行动或 hidden 私档；已通过本节验证口径中的 client typecheck、React scaffold、App focused、完整 client Vitest、client build、Mock browser smoke、docs governance、documentation governance、完整 `npm test` 和 `git diff --check`。Galileo 提交前只读初审发现 POSIX path-shaped 污染绕过 P2，窄复审又指出中文标点前缀绕过；主代理已补 `localPeoplePathPattern`、POSIX/CJK 标点路径 fixture 与静态守门，Galileo 最终复审确认 P2 关闭且未发现新的 P0/P1/P2。提交哈希待本轮完成后回填。
