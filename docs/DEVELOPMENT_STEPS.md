...
本轮 S88.9 主卷草稿与身份循环专题层案卷绑定当前验证口径：

- `npm run typecheck:client`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/state/uiState.test.ts -t "action draft|route session|local surfaces"`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/__tests__/App.test.tsx -t "stale main-shell|role-cycle surfaces|main action draft|quick actions"`
- `npm run test:client -- --pool=vmForks --maxWorkers=1 client/src/components/MemorialComposer.test.tsx`
...
- `npm test`
- `git diff --check`
- 提交前只读子代理复审最终 diff 与验证证据；本片包含前端状态、运行时行为、测试和静态守门，必须在暂存和提交前完成只读复审。
