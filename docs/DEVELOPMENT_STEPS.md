...
工具：Codex，Newton 只读验收缺口探查子代理，提交前只读复审子代理。
...
- `scripts/clientSmoke.js` 兼容路线图里的 `npm run smoke:browser -- --client react` 写法；非 `react` 目标会明确失败，避免误以为旧前端仍是默认验收入口。
- React browser smoke 从真实 Mock 首页开局后，改为通过右上角“印匣”内的“返回首页”验证返回首页/继续本局，不再只依赖左上角品牌链接。
- 新增印匣总验收：桌面端真实点击“打开印匣”，遍历 `AI 设置`、`显示`、`安全`、`旧案` 四个 tab，断言 `aria-selected`、面板内容、快捷建议工具预算固定为 0、关闭后焦点回到“打开印匣”。
- 新增旧案读档验收：在印匣“旧案”tab 刷新案卷，按当前 session 短码定位案卷卡片，点击“载入”，监听 `GET /api/game/player-state/:sessionId`，并继续由全局 unsafe API 拦截确保没有访问 `/api/game/state/*` 或 `/api/dev/*`。
- 新增移动端印匣验收：390x844 视口下打开印匣并遍历四个 tab，检查抽屉在视口内、无横向溢出、无 hidden/raw/key/path/provider payload 泄漏。
- 快捷建议点击后改为等待草稿实际进入 textarea，再继续 Enter 呈上，消除真实浏览器状态同步时序造成的假阴性。
- `test/reactClientScaffold.test.js` 新增 S75.10 源码回归，固定 `--client react`、印匣 tab、旧案 player-state 读档、移动端印匣和焦点关闭路径。
- 本轮不改后端 API/schema、provider schema、SQLite schema、存档格式、AI 权限、科举晋级、官职裁决或服务器 canonical state；S75.10 是验收脚本与文档收口。

验证：

- 已通过：`node --check scripts/clientSmoke.js`。
- 已通过：`node --test test/reactClientScaffold.test.js test/browserSmokeScript.test.js`，61 项通过。
- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，6 个测试文件、50 项通过。
- 已通过：`npm run smoke:browser -- --client react --screenshots artifacts/s75-10-browser-acceptance`，覆盖 desktop home、Mock 开局、印匣返回首页、继续本局、AI 快捷建议草稿、旧案 `player-state` 载入、显示偏好 reload、舆图 reduced motion、人物懒加载、史册/科举/皇榜/朝议/印匣路由刷新、mobile composer、mobile 印匣和 mobile home。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`，1 项通过。
- 已通过：`git diff --check`。
- 已通过：完整 `npm test`，895 项通过、0 失败。
- 已完成：Newton 只读缺口探查建议补右上角印匣、tab 遍历、旧案读档、印匣返回首页、移动端和低动效覆盖，本轮已落实其中关键可复跑项；浏览器级 `prefers-reduced-motion` 深测留给 S77 可访问性/低动效总验收，S75.10 继续使用应用显示偏好验证地图 reduced motion。
- 已完成：提交前只读复审未发现 P0/P1/P2；遗留的浏览器级 reduced-motion context 与真实专题 `surface` 来源验收已明确留给 S77/S76，不阻塞 S75.10。

风险/遗留：

- S75.10 不新增真实专题 composer，因此 S75.9 记录的 `surface` 来源仍是 helper/扩展路径；S76.11 再接实际专题 surface。
- 当前低动效浏览器验收覆盖应用显示偏好与地图 `data-map-motion="reduced"`，还未单独创建 `prefers-reduced-motion: reduce` 浏览器 context；该项可在 S77.6 可访问性总验收补齐。

下一步：

- 执行 S76.1：主游戏壳，把主卷整理为顶栏、叙事卷、场景插画带、功能页签和底部奏折组成的清爽主界面。

### 2026-05-18

工具：Codex，Kierkegaard 只读差距审查子代理。

步骤：S75.9 快捷功能 AI 接入与完善。

提交：本次提交。

完成：

- 新增 `quick_action` AI task、prompt pack、provider schema、Mock provider 建议和远端 provider 接线；`AI_QUICK_ACTION_PROVIDER`、`AI_QUICK_ACTION_MODEL`、`AI_QUICK_ACTION_MAX_OUTPUT_TOKENS`、`AI_QUICK_ACTION_TIMEOUT_MS` 可单独配置，`quick_action` 的工具预算被服务器硬锁为 0，`mayUseTools` / `mayRequestAdjudication` 始终为 false。
- 新增 `src/game/quickActionSuggestions.js` 与 `POST /api/ai/quick-actions/:sessionId`。浏览器只发送当前页、短草稿预览和建议数量；服务器从本地 session 派生 redacted player summary、route view flags、公开 evidence refs 与 actor 工具能力说明，再请求 provider 生成行动草稿。
- 快捷建议只返回 `quickActionSuggestions`：`title`、`label`、`text`、`roleTags`、`toolIntent`、`evidenceRefs`、`source`。服务端会做 schema 校验、身份/工具 intent allowlist、数量与长度 cap、公开 evidence ref 白名单、source 归一化和污染文本清洗；provider 失败、坏 JSON、越权工具意图、伪造 evidence 或污染内容都会降级为本地规则建议。
- endpoint 保持只读，不调用 server resolver，不提交普通回合，不推进时间，不写 canonical state，也不持久写 session；审计边界为响应级 `source/status/fallbackReason/generatedAtTurn` 与现有 AI 设置/route view，不记录 prompt 或 provider payload。若后续需要持久 quick action 调动审计，应单独设计不破坏 read-only 语义的安全记录。
- React `qianqiuApi` 白名单加入 `/api/ai/quick-actions/:sessionId`；`gameSessionState` 新增 `quickActionStatus`、`quickActions` 和 `refreshQuickActions`，在 runnable session + 安全玩家 payload 就绪后刷新建议。
- `MemorialComposer` 显示 `mock-ai`、`provider-ai`、`local-rule`、`map-runtime`、`surface` 来源，支持 loading、stale、failed、applied 状态和刷新入口；点击建议仍只写入行动草稿，只有 Enter 或“呈上”才调用 `/api/game/turn`。
- 印匣和独立设置页新增快捷建议 task 控制，可调整 provider、model、输出长度和温度；工具预算显示为 0 并禁用，避免把 draft-only 建议误解成可执行工具调用。
- `scripts/clientSmoke.js` 更新真实浏览器验收，Mock 开局后等待 `mock-ai` 快捷建议，点击只写 textarea，提交普通回合后等待草稿清空，并用 `data-source` 验证来源标记。
- S75.9 browser smoke 重跑时发现旧 S72 `public/mapRenderer.js` 在本机 Pixi 7 runtime 下可能没有 `app.resize()`，已补 `renderer.resize(width, height)` 兼容分支并在销毁时清理 debounce，避免 React 舆图页刷新/切换时产生页面错误。
- 提交前只读复核发现的 `mimo-deepseek` 未转发快捷建议、舆图派生建议仍显示为 `local-rule`、provider 文本可声称“已审结/已任命”等问题已修正：hybrid provider 现在把 `suggestQuickActions` 交给 MiMo 侧，前端 map/surface 派生建议保留 `map-runtime` / `surface` 来源，服务器会把结果声称类 provider 文本降级为本地规则建议。

验证：

- 已通过：`node --test test/quickActionRoute.test.js test/aiSchemas.test.js test/modelRoutePolicy.test.js test/aiSettings.test.js test/aiSettingsRoute.test.js test/reactClientScaffold.test.js`，59 项通过。
- 已通过：`node --test test/modelRoutePolicy.test.js test/quickActionRoute.test.js`，23 项通过，覆盖 `mimo-deepseek` 快捷建议转发、provider 结果声称降级和 S75.9 endpoint 安全边界。
- 已通过：`node --test test/prompts.test.js`，21 项通过；已把 `quick_action` 加入 prompt pack registry 回归，并确认快捷建议 prompt 为 draft-only 且不继承普通回合 patch 权限。
- 已通过：`node --check public/mapRenderer.js` 与 `node --test test/mapFrontendShell.test.js`，4 项通过，覆盖舆图 runtime 脚本语法与 S72 前端契约。
- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，6 个测试文件、50 项通过；较早聚焦运行曾因本机 `@rolldown/binding-win32-x64-msvc` optional dependency 缺失启动失败，执行 `npm install` 修复本地 `node_modules` 后重跑通过，package/lock 未改。
- 已通过：`npm run build:client`；Vite 对 `/assets/ui/...` 运行时 public 素材路径继续给出既有外部解析提示，构建成功。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s75-9-quick-actions-ai`，覆盖真实 Mock 开局、AI 快捷建议写入草稿、Enter 呈上、草稿清空、显示偏好 reload、舆图/人物/史册/科举/皇榜/朝议/印匣刷新和 mobile composer/home。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`，1 项通过。
- 已通过：`git diff --check`；仅有既有 LF/CRLF warning，退出码为 0。
- 已通过：完整 `npm test`，894 项通过、0 失败；此前一次完整运行只因 `test/prompts.test.js` 的 prompt pack 固定清单未包含 `quick_action` 失败，已补测试并重跑通过。
- 已完成：Kierkegaard 早期只读差距审查发现的 P1 工具预算可配置、P2 降级状态不显性、P3 文档待同步均已修正或记录；P2 持久审计项因 endpoint 需保持 read-only，当前记录为响应级安全审计边界。Dirac 提交前只读复核未发现 P0/P1，其 P2/P3 均已修正并复测通过；补丁后复核确认未发现新的 P0/P1/P2。

风险/遗留：

- 快捷建议仍只是行动草稿推荐，不是领域工具执行器。真实移动、办案、行军、上疏、科举和任免后果继续由普通回合服务器裁决。
- 若后续要把 quick action 调动计入持久 AI 审计，必须新增不暴露 prompt/provider payload 且不破坏 session read-only 期望的安全记录方案。
- `surface` 来源当前是 helper/扩展路径，主卷 `MemorialComposer` 仍固定 `page: "game"`；若 S75.10 要做玩家可见 `surface` 来源验收，需要接入实际专题页 composer 或明确为预留能力。

下一步：

- 执行 S75.10：S75 浏览器验收总收口，覆盖首页、印匣、存档/读档、返回首页、继续本局、显示偏好、底部奏折和 AI 快捷建议在桌面/移动端的综合路径。

### 2026-05-18

工具：Codex，Bernoulli medium 前端实施子代理（早期 scoped patch 已由 Codex 收束接口），Harvey 只读复审。
...
- 新增 `client/src/components/MemorialComposer.tsx`，主卷移除旧内嵌行动表单，改为固定底部奏折/竹简 composer；`AppShell` shell marker 更新为 `s75-8`。
- 输入框使用组件本地输入态，placeholder 按当前安全身份显示，且不再作为 textarea value；空白行动禁用，Enter 呈上，Shift+Enter 换行，普通回合失败时本地输入保留，成功后继续由 `submitTurn` 清空 UI 草稿。
- 新增 `client/src/components/quickActionSuggestions.ts`，定义 `QuickActionSource = "local-rule" | "mock-ai" | "provider-ai" | "map-runtime" | "surface"`、`QuickActionSuggestion` 和 `buildQuickActionSuggestions`，S75.8 只生成本地规则建议。
- 快捷功能区首个入口为“可行事”，根据当前安全玩家身份和 route view flags 显示 2-3 条建议；点击只写入 `role-surface` 行动草稿，不自动提交、不调用 resolver、不写服务器 canonical state。
- `scripts/clientSmoke.js` 扩展真实浏览器验收，覆盖快捷建议写入、`local-rule` source 标记、Enter 呈上后草稿清空、移动端底部 composer 不遮挡/不溢出和 forbidden text 防线。
- 本轮不新增后端 AI endpoint，不改 API/schema、provider schema、SQLite schema、存档格式、AI 权限、科举晋级、官职裁决或服务器 canonical state；不暴露 raw state、完整 prompt、provider payload、本地路径、key 或 hidden 信息。

验证：

- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，6 个测试文件、48 项通过。
- 已通过：`node --test test/reactClientScaffold.test.js`，16 项通过。
- 已通过：`node --check scripts/clientSmoke.js`。
- 已通过：`node --test test/browserSmokeScript.test.js test/reactClientScaffold.test.js`，60 项通过。
- 已通过：`npm run build:client`；Vite 对 `/assets/ui/...` 运行时 public 素材路径继续给出既有外部解析提示，构建成功。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s75-8-memorial-composer`，覆盖 desktop home、真实 Mock start、返回首页/继续本局/快捷“可行事”写入草稿、Enter 呈上、显示偏好 reload、舆图/人物/史册/科举/皇榜/朝议/印匣刷新和 mobile memorial composer/mobile home。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`。
- 已通过：`git diff --check`；仅有既有 LF/CRLF warning，退出码为 0。
- 已通过：完整 `npm test`，879 项通过、0 失败。
- 已完成：Harvey 只读复审最终 diff 与验证证据，未发现 P0/P1/P2；复审中误跑 `npm run test:client -- --runInBand` 因 Vitest 不支持该参数失败，随后用项目原生命令重跑通过，未影响候选 diff。

风险/遗留：

- S75.8 只做本地规则快捷建议和可扩展 UI 插槽；AI/Mock 推荐、安全 endpoint、schema 校验、降级、过期/失败/loading 状态和污染过滤深化留给 S75.9。
- 当前“可行事”建议为确定性短草稿，真实行动后果仍由普通回合服务器裁决。

下一步：

- 执行 S75.9：快捷功能 AI 接入与完善。AI 只能生成可点击草稿建议，不提交行动、不裁决结果、不写 canonical state；无 key/provider 失败/JSON 不合规时回退本地规则。

### 2026-05-18

工具：Codex，Boyle medium 前端实施子代理（超时关闭，无最终报告；Codex 审阅并收束工作树），待提交前只读复审。

步骤：S75.7 显示偏好本地安全持久化。

提交：本次提交。

完成：

- 新增 `client/src/state/displayPreferenceStorage.ts`，把显示偏好限定为 `motion`、`textSize`、`contrast`、`autoScroll` 和 `mapMotion` 五个白名单字段，并写入 `qianqiu.displayPreferences.v1`。
- 本地存储 payload 带 `display-preferences-v1` schema 版本；读取时只接受合法枚举/布尔值，未知字段、旧 schema、损坏 JSON、raw/provider/prompt/path/key 等污染字符串都会回落到默认值或被丢弃。
- `useUiStateStore` 启动与 reset 时读取清洗后的本地偏好，`setDisplayPreference` 在运行时拒绝非法写入并只保存白名单 payload；不把 `currentSessionId`、安全玩家投影、行动草稿、raw state、provider payload、完整 prompt、本地路径或 key 写入该存储。
- `AppShell` shell marker 更新为 `s75-7`；既有 `data-motion`、`data-text-size`、`data-contrast` 继续驱动低动效、大字和高对比度，舆图页继续以 `mapMotion && motion === "full"` 关闭 S72 地图新增动效。
- `scripts/clientSmoke.js` 新增显示偏好真实浏览器验收：打开印匣显示 tab，设置低动效/大字/高对比度/关闭自动滚动/关闭舆图动效，校验 localStorage 白名单、reload 后偏好恢复、地图 runtime `data-map-motion="reduced"` 和 forbidden text 防线。
- 本轮不改后端 API/schema、provider schema、SQLite schema、存档格式、AI 权限、科举晋级、官职裁决或服务器 canonical state。

验证：

- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，5 个测试文件、43 项通过。
- 已通过：`node --test test/reactClientScaffold.test.js`，15 项通过。
- 已通过：`npm run build:client`；Vite 对 `/assets/ui/...` 运行时 public 素材路径继续给出既有外部解析提示，构建成功。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s75-7-display-preferences`，覆盖 desktop home、真实 Mock start、返回首页/继续本局/递送回合、显示偏好 localStorage/reload、map runtime 低动效、people/archive/exam/ranking/court/settings refresh 和 mobile home；首次运行因 Playwright `getByLabel("动效")` 同时命中“舆图动效”复选框而失败，已改为 role 精确定位后重跑通过。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`。
- 已通过：`git diff --check`；仅有既有 LF/CRLF warning，退出码为 0。
- 已通过：完整 `npm test`，878 项通过、0 失败。
- 已完成：Kuhn 只读复审未发现 P0/P1/P2；P3 提醒 App 集成测试清理未清空 localStorage，已补 `window.localStorage.clear()` 并重跑 `npm run test:client`、`node --test test/reactClientScaffold.test.js` 和 `npm run typecheck:client` 通过。

风险/遗留：

- S75.7 只持久化显示偏好，不持久化当前本局指针；刷新首页后仍以旧案列表恢复。
- `autoScroll` 已安全持久化，后续 S75.8/S76 主卷输入和叙事卷轴深化时再把它接入具体叙事滚动行为。

下一步：

- 执行 S75.8：底部奏折输入雏形，保持失败保留草稿、不允许空提交，并继续不暴露 raw/provider/prompt/path/key。

### 2026-05-18

工具：Codex，Pasteur medium 前端实施子代理（样式/测试 patch 已审阅沿用），Poincare 提交前只读复审。
...
- `AppShell` shell marker 更新为 `s75-6`，顶部“返回千秋首页”继续调用 `returnHome`，路由桥在回到 `/` 时保留当前安全 `currentSessionId`。
- `useUiStateStore.returnHome` 只关闭抽屉、弹窗、专题层并清空临时行动草稿；不调用 `POST /api/game/start`、`POST /api/game/turn`、raw state route 或开发诊断，也不删除或重写服务器 session。
- 首页新增“当前本局 / 继续本局”案头条，只在 `currentPlayerPayload.sessionId` 与可运行 `currentSessionId` 匹配时显示，使用安全玩家名、身份/功名/官职、案卷短码和来源；命中 raw/path/key/prompt/provider 污染词时回落中文 fallback。
- `scripts/clientSmoke.js` 扩展真实浏览器验收：默认 Mock 开局后返回首页、确认继续本局入口、继续回主卷、递送一回合，并继续拦截 unsafe `/api/game/state/*` 与 `/api/dev/*`。
- Pasteur medium 子代理在用户中断前已落地 `continueShelf` 样式和 App 集成测试，Codex 审阅后沿用并补充 store/scaffold/smoke 覆盖；Poincare 只读复审指出的继续本局摘要污染覆盖不足已修正，并新增污染 payload 集成测试。
- 本轮不改后端 API/schema、provider schema、SQLite schema、存档格式、AI 权限、科举晋级、官职裁决或显示偏好持久化。

验证：

- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，5 个测试文件、39 项通过。
- 已通过：`node --test test/reactClientScaffold.test.js`，14 项通过。
- 已通过：`npm run build:client`；Vite 对 `/assets/ui/...` 运行时 public 素材路径继续给出既有外部解析提示，构建成功。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s75-6-continue-session`，覆盖 desktop home、真实 Mock start、返回首页/继续本局/递送回合、map runtime、people/archive/exam/ranking/court/settings refresh 和 mobile home。
- 初次 `npm run test:client` 因本机 `node_modules` 缺少 `@rolldown/binding-win32-x64-msvc` optional binding 无法启动 Vitest；执行 `npm install` 补齐本机依赖后重跑通过，未修改 `package.json` 或 lockfile。

风险/遗留：

- S75.6 只处理当前会话指针与首页续玩；显示偏好本地持久化仍留给 S75.7。
- 首页“继续本局”依赖本次浏览器内 UI store 安全投影，刷新首页后仍以 S75.5 旧案列表读档为恢复入口；后续若要跨刷新保留当前本局，需要先设计本地安全偏好/指针持久化边界。

下一步：

- 执行 S75.7：显示偏好本地安全持久化，继续避免把 raw state、prompt、provider payload、本地路径或 key 写入本地存储。

### 2026-05-18

工具：Codex，Ampere medium 前端实施子代理（重复组件已审阅并删除），提交前只读复审。
...
- 新增 React `SaveCaseList` 案卷卡片组件，首页旧案卷和印匣“旧案”tab 复用同一套展示逻辑，避免两套存档 UI 分叉。
- 存档卡片显示脱敏 metadata：sessionId 短码、玩家名、身份/功名/官职、朝代年月旬、回合数、摘要和最近更新时间；字段缺失时显示中文 fallback，如“身份未题”“年月未详”“回合未记”“更新时间未记”和“此卷暂无公开摘要”。
- 首页案卷使用安全路由进入 `/game/:sessionId`，由 `GamePage` 自动通过 `loadSession` 读取 `GET /api/game/player-state/:sessionId`；印匣旧案按钮仍直接调用 `loadSession(sessionId)` 后关闭抽屉并进入主卷。
- `SaveMetadata` 前端类型补齐本步需要的安全 metadata 字段；不展示 storage revision，不新增 raw state、dev diagnostics、provider payload、完整 prompt、本地路径或 key 的读取。
- `AppShell` shell marker 更新为 `s75-5`，React 测试覆盖完整 metadata 展示、缺字段 fallback、读档只走 `player-state` 和 unsafe 文本防线。
- Ampere medium 子代理产出未接入的 `SaveDossierList` 重复组件，Codex 审阅后删除；最终实现以已接入、已测试的 `SaveCaseList` 为准。
- 本轮不改后端 API/schema、provider schema、SQLite schema、存档格式、AI 权限、科举晋级、官职裁决或显示偏好持久化。

验证：

- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，5 个测试文件、36 项通过。
- 已通过：`node --test test/reactClientScaffold.test.js`，13 项通过。
- 已通过：`npm run build:client`；Vite 对 `/assets/ui/...` 运行时 public 素材路径继续给出既有外部解析提示，构建成功。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s75-5-save-cases`，覆盖 desktop home、真实 Mock start、map runtime、people/archive/exam/ranking/court/settings refresh 和 mobile home。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`。
- 已通过：`git diff --check`；仅有既有 LF/CRLF warning，退出码为 0。
- 已通过：完整 `npm test`，876 项通过、0 失败。
- 已完成：Raman 只读复审发现的 P2 文档验证状态滞后和 save metadata summary 污染防线已修正；补充 polluted metadata 测试后重新验证。

风险/遗留：

- S75.5 只深化安全存档 metadata 展示和读档入口；首页“继续本局”仍留给 S75.6。
- S75.10 仍需把真实浏览器 smoke 扩展到右上角印匣点击、旧案 tab 遍历和读档交互。

下一步：

- 执行 S75.6：返回首页与继续本局，保持不删 session、不写空行动、不改服务器状态。

### 2026-05-18

工具：Codex，Mill medium 前端实施子代理尝试（未产出 patch，已关闭），提交前只读复审。
...
- `AppShell` 顶部右侧工具收束为单一“印匣”按钮，使用 lucide `Archive` 图标与既有水墨/印匣纹理样式，不再分散显示存档、显示偏好、安全摘要和设置四个按钮。
- `SurfaceHost` 的设置抽屉改为印匣 tab：AI 设置、旧案、显示、安全四栏；保留 Esc 关闭、焦点回收、滚动锁定和旧 `openDrawer("saves")` / `openDrawer("display-preferences")` 兼容入口。
- 印匣内可保存当前案卷 AI preset、试连 provider、刷新旧案、通过 `loadSession` 载入旧案并跳转主卷、返回首页、调整运行时显示偏好、查看安全玩家摘要。
- 所有数据继续来自安全 API client、脱敏 saves metadata、`player-state` 和 UI store 的安全摘要；不读取 raw state、开发诊断、内部审计原文、模型原始返回、完整提示词、本地路径或 key。
- 本轮不改后端 API/schema、provider schema、SQLite schema、存档格式、AI 权限、科举晋级、官职裁决或显示偏好持久化；S75.5-S75.7 继续深化案卷 metadata、返回首页继续本局和本地安全偏好持久化。

验证：

- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，5 个测试文件、34 项通过。
- 已通过：`node --test test/reactClientScaffold.test.js`，12 项通过。
- 已通过：`npm run build:client`；Vite 对 `/assets/ui/...` 运行时 public 素材路径继续给出既有外部解析提示，构建成功。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s75-4-inkbox`，覆盖 desktop home、真实 Mock start、map runtime、people/archive/exam/ranking/court/settings refresh 和 mobile home。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`。
- 已通过：`git diff --check`；仅有既有 LF/CRLF warning，退出码为 0。
- 已通过：完整 `npm test`，875 项通过、0 失败。
- 已完成：Kepler 只读复审最终 diff 与验证证据，未发现 P0/P1/P2；P3 建议是当前 browser smoke 仍主要验证 `/settings` 路由刷新，未显式点击右上角印匣并遍历 tab，已留给 S75.10 或 S75.5 后扩展真实浏览器交互。

风险/遗留：

- 旧案 tab 目前只显示轻量 metadata 与“载入”按钮；S75.5 仍需补齐案卷式 metadata 展示、缺字段 fallback 和更完整 browser smoke。
- 返回首页目前不删除 session、不改服务器状态；S75.6 继续做首页“继续本局”。
- 显示偏好仍是运行时 UI 状态；S75.7 再做本地安全持久化。
- 真实浏览器 smoke 本轮覆盖设置路由刷新，印匣抽屉 tab 的逐项交互由 Vitest/scaffold 覆盖；S75.10 应补右上角印匣点击与 tab 遍历。

下一步：

- 执行 S75.5：案卷式存档/读档，存档列表显示完整 metadata，读档只走 `player-state`。

### 2026-05-18

工具：Codex，Epicurus medium 前端实施子代理，提交前只读复审。
...
- React 默认 `/` 首页朱印开始按钮新增短促落章反馈、loading 扫光、disabled/error 视觉和 `aria-live` 状态说明。
- 首页 start 提交增加同步 `submitLockRef` 防线，连续 Enter、双击或同一提交周期重复 submit 只会触发一次 `POST /api/game/start`。
- 低动效边界同时读取应用显示偏好与系统 `prefers-reduced-motion: reduce`；低动效下不加落章动画类，CSS 也禁用朱印 loading/error/stamp 动效。
- 首页仍只通过安全 `startNewGame` / `POST /api/game/start` 开局；不读取 raw state、开发诊断、provider payload、完整 prompt、本地路径或 key。
- 补 client 组件测试和源码回归测试，保护重复提交、低动效静态替代、状态属性和安全端点边界。
- 同步 README、brief、前端路线图、活动台账和共享上下文；本轮不改后端 API/schema、provider schema、SQLite schema、存档格式、AI 权限、科举晋级或官职任免规则。

验证：

- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，5 个测试文件、33 项通过。
- 已通过：`node --test test/reactClientScaffold.test.js`，11 项通过。
- 已通过：`npm run build:client`；Vite 对 `/assets/ui/...` 运行时 public 素材路径继续给出既有外部解析提示，构建成功。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s75-3-start-seal`，覆盖 desktop home、真实 Mock start、map runtime、people/archive/exam/ranking/court/settings refresh 和 mobile home。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`。
- 已通过：`git diff --check`；仅有既有 LF/CRLF warning，退出码为 0。
- 已通过：完整 `npm test`，874 项通过、0 失败。
- 已完成：Parfit 只读复审最终 diff 与验证证据，未发现 P0/P1/P2；残余说明是系统级 `prefers-reduced-motion` 真实浏览器行为主要由源码/CSS、Vitest 低动效路径和本轮 browser smoke 共同覆盖。

风险/遗留：

- S75.3 只处理首页开始朱印；右上角印匣、案卷式存档/读档、返回首页与继续本局、显示偏好持久化和底部奏折输入仍在 S75.4-S75.8。
- 低动效目前是运行时 UI 偏好与系统 media query 生效，持久化仍留给 S75.7。

下一步：

- 执行 S75.4：右上角印匣入口，统一 AI 设置、存档/读档、返回首页、显示偏好和安全摘要。

### 2026-05-18

工具：Codex，Hilbert medium 前端实施子代理，提交前只读复审。
...
- React 默认 `/` 首页册页表单补齐朝代、年份、六类身份、姓名、书生家境和自定背景；非书生身份隐藏家境并显示身份底色说明。
- 首页提交继续只走安全 `startNewGame` / `POST /api/game/start`，不读取 raw state、开发诊断、provider payload、完整 prompt、本地路径或 key。
- `src/game/initialState.js` 新增书生公开家境归一化：`familyBackground` 只接受贫寒/普通/世家，非 `scholar` 身份忽略；自定背景写入公开 `customSetting`，不是 hidden 私档。
- 补前端类型与测试，保护 start payload、年份错误、loading/disabled、非书生家境隐藏和安全端点边界。
- 同步 README、brief、架构、前端路线图和共享上下文；本轮不改 provider schema、SQLite schema、存档格式、AI 工具权限或科举/官职服务器裁决规则。

验证：

- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，5 个测试文件、31 项通过。
- 已通过：`node --test test/gameStartRole.test.js test/reactClientScaffold.test.js`，18 项通过。
- 已通过：`npm run build:client`；Vite 对 `/assets/ui/...` 运行时 public 素材路径继续给出既有外部解析提示，构建成功。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s75-2-start-form`，覆盖 desktop home、真实 Mock start、map runtime、people/archive/exam/ranking/court/settings refresh 和 mobile home。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`。
- 已通过：完整 `npm test`，873 项通过、0 失败。
- 已通过：`git diff --check`；仅有既有 LF/CRLF warning，退出码为 0。
- 已完成：Aristotle 只读复审最终 diff 与验证证据，未发现 P0/P1/P2；残余建议是历史 `background` 字段仍作为兼容公开 start 字段保留，后续若要收紧外部 start schema 需另开兼容评估。

风险/遗留：

- S75.2 不实现朱印盖章动效；S75.3 继续处理盖章反馈、Enter 防重复提交和 reduced-motion。
- 家境和自定背景只影响公开开局叙事/安全 setup 字段，不授予功名、官职或绕过服务器身份校验。
...
- 排查 Git 面板 `+45,442/-27,865` 来源，确认该数字对应最近 15 个已提交的 React 前端迁移、S73/S74/S75 素材 manifest/QA 与立绘重制累计 diff，不是未提交变更。
- 当前 `HEAD`、`main` 和 `origin/main` 均为 `e65aaa6`；本轮开始时工作区无未提交变更。
- 删除已被忽略、无需推送 GitHub 的本地生成物：`artifacts/` browser smoke 截图约 1.9GB、`tmp/` 和 `dist/`。未触碰 `.env`、`node_modules/`、`data/` 本地存档或任何已审核 runtime 素材。
- 将历史跟踪的 `game-start-err.log` 与 `game-start-out.log` 移出仓库，并在 `.gitignore` 中新增 `game-start-*.log`，避免本地启动日志再次进入提交。
- 本轮不改运行时代码、API/schema、provider schema、SQLite schema、存档格式、AI 权限、游戏规则或已审核素材 manifest。

验证：

- 已通过：`git status --short` 确认本轮只有 `.gitignore`、两份日志删除和文档记录变更。
- 已通过：目录体积复查，`artifacts/`、`tmp/` 和 `dist/` 已清理；`node_modules/`、`data/`、`.env` 保留。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`。
- 已通过：`git diff --check`；仅有既有 `.gitignore` LF/CRLF warning，退出码为 0。

风险/遗留：

- `public/assets/ui/**`、manifest 和 QA JSON 仍是运行时/验收所需的已跟踪资产，不能仅因体积或 diff 数字大就从 Git 移除；后续若要改为外部对象存储或 Git LFS，需要单独设计交付策略。
- 本轮是低风险仓库卫生和文档记录，跳过子代理复审。

下一步：

- 若继续开发主线，执行 S75.2 开局表单；若继续减小仓库体积，需要先规划 runtime 素材分发方案，不能直接删除已审核 `/public/assets/ui/`。

### 2026-05-18

工具：Codex，Parfit medium 前端实施子代理，Codex 浏览器实看，提交前只读复审。
...
- React 默认 `/` 首页改为水墨画卷第一屏：标题“千秋”保留在主视觉中，不放入卡片；页面接入 S73.4 已审核首页画卷、透明云雾、题名册表单底、朱印、案卷和低动效静态底。
- 首页中央册页继续承载现有“姓名 / 身份 / 新开一卷”安全开局表单，只调用既有 `POST /api/game/start`；旧案卷区域只显示 `GET /api/game/saves` 返回的脱敏 metadata。
- 新增桌面/移动响应式收束和 `prefers-reduced-motion: reduce` 处理；低动效下使用静态底并关闭云雾漂移。
- Codex 修正实施子代理补入的旧 `#start-form` 标记，保持 S74.7 React smoke 对旧原生首页 DOM 的防线；client 测试改为保护 `.homeDesk .startDesk` 新结构。
- 本轮不改后端 API/schema、provider schema、SQLite schema、存档格式、AI 权限、游戏规则或 S75.2/S75.3 的完整表单/朱印交互。

验证：

- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，5 个测试文件、29 项通过。
- 已通过：`node --test test/reactClientScaffold.test.js`，10 项通过。
- 已通过：`npm run build:client`；Vite 对 `/assets/ui/...` 运行时素材路径继续保持外部解析。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s75-1-home-scroll`，覆盖 desktop home、真实 Mock start、map runtime、people/archive/exam/ranking/court/settings refresh 和 mobile home。
- 已通过：Codex 浏览器实看 `http://127.0.0.1:3217/` 桌面首页，确认画卷布局、中央表单、旧案卷提示、无横向溢出、无 hidden/raw/provider/key 文本。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`。
- 已通过：`git diff --check`；仅有既有 LF/CRLF warning，退出码为 0。
- 已通过：完整 `npm test`，872 项通过、0 失败。

风险/遗留：

- S75.1 只完成首页画卷布局；朝代、年份、完整身份、书生家境、自定义背景和错误/禁用状态细化仍在 S75.2。
- S75.3 的朱印盖章反馈、S75.5 的案卷式存档/读档和 S75.6 的继续本局仍未实现。

下一步：

- 执行 S75.2：开局表单，扩展字段时继续使用现有安全 start API，不把自定义背景当作 hidden 私档，不绕过服务器身份校验、科举规则或 Mock/no-key fallback。

### 2026-05-18

工具：Codex，Curie 只读 S74.7 范围探查子代理，Hooke 提交前只读复审。
...
- `AppShell` 顶部主卷、舆图、人物和史册入口现在会在真实案卷中绑定当前安全 `sessionId`，预览案卷仍保留 `s74-preview`；避免玩家从 Mock 开局后点舆图/人物/史册落回预览卷。
- `scripts/clientSmoke.js` 扩展为 S74.7 默认入口验收：无 `--url` 时临时服务器显式固定 `AI_PROVIDER=mock`，从默认 `/` 首页表单真实开局，验证当前 session 导航、舆图 canvas、人物立绘懒加载分页、史册/科举/皇榜/朝议/印匣刷新恢复、移动端首页和 hidden/raw 防线。
- 新 smoke 监听浏览器请求，若 React 前端误碰 `/api/game/state/*` 或 `/api/dev/*` 会失败；普通读档仍只允许走安全 `player-state`。
- 更新 `client/src/__tests__/App.test.tsx` 与 `test/reactClientScaffold.test.js`，覆盖 S74.7 shell 版本、当前 session 导航绑定、首页真实开局 smoke、路由刷新和 Mock 固定逻辑。
- 同步 README、brief、架构、浏览器验收文档、AI 控制矩阵、React 迁移契约、前端路线图和共享上下文，使 `npm run smoke:browser` 明确为 React 默认入口 smoke；JSON/SQLite parity 和 no-browser 存储验收继续由 `npm run smoke:dual-mode` / `--storage-only` 承担。
- 明确产品内不保留 `/legacy.html` 或 `/ink-client/` 双入口；回退方式是 Git revert 或恢复上一提交，本轮不迁移存档、不改后端 API/schema、provider schema、SQLite schema、AI 权限或游戏规则。
...
- 已通过：`npm run test:client`，5 个测试文件、29 项通过。
- 已通过：`node --test test/reactClientScaffold.test.js`，10 项通过。
- 已通过：`npm run build:client`；Vite 提示 `/assets/ui/...` 运行时素材路径保持外部解析，符合 S73/S74 资产边界。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s74-7-default-entry`，覆盖 desktop home、真实 Mock start、map runtime、map refresh、people assets、people refresh、archive refresh、exam/ranking/court/settings refresh 和 mobile home。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`。
- 已通过：`git diff --check`；仅有既有 LF/CRLF warning，退出码为 0。
- 已通过：完整 `npm test`，872 项通过、0 失败。
- 已完成：Curie 只读探查指出的 `App.test.tsx` S74.6 断言、README 旧 smoke 参数、session 子路由刷新和 S74.8 编号漂移均已修正；Hooke 提交前只读复审未发现 P0/P1/P2，P3 `BROWSER_ACCEPTANCE.md` 最新验收结果仍停在 S59.1 的问题已改为 S74.7 结果。

风险/遗留：

- `smoke:browser:legacy` 和旧 `scripts/browserSmoke.js` 仅作迁移参考，不再代表默认入口；如果后续仍要使用旧原生浏览器旅程，需要单独适配 React 默认入口或以 Git 历史追溯。
- S74.7 只做默认入口验收和 smoke 口径收束；完整首页美术、存档/读档、返回首页、底部奏折和显示偏好持久化仍在 S75。

下一步：

- 执行 S75.1：首页画卷布局，继续保持默认 `/` 可 Mock 开局，素材仍只能通过已审核 manifest/fallback 使用。
...
工具：Codex，game-ui-frontend skill，S74.6 只读探索子代理，提交前只读复审。
...
- 新增 `client/src/components/InkMapRuntimeBridge.tsx`，React 侧动态加载 `/vendor/pixi.min.js` 和 `/mapRenderer.js`，直接复用 S72 `window.MapRenderer` 的 PixiJS 渲染能力。
- `MapPage` 现在从当前安全 session payload 读取 `mapRuntimeView`，且要求 `currentSession.sessionId === route sessionId`，避免刷新或切卷时显示上一案卷旧舆图。
- 地图桥只通过 React DOM 管理 label、tooltip 和行动草稿；行动草稿写入 Zustand `actionDraft`，仍需玩家回到主卷提交普通回合，不自动调用 `/api/game/turn`，不直写 canonical 状态。
- 明确隔离旧 `public/mapPanel.js` 的 `#map-panel`、`#map-canvas`、`#map-ui-layer`、`#action-input`、`#information-panel` 和旧 tab DOM 假设；S74.6 不依赖 `public/app.js` 或旧全局前端状态。
- `client/src/api/types.ts` 新增 `MapRuntimeView`、ref、route、event effect 和 action draft 前端只读类型；`uiState` 仍只保存安全摘要和草稿，不保存整份地图 view。
- `public/mapRenderer.js` 新增 `motionEnabled` / `setMotionEnabled`，让 React 显示偏好里的“舆图动效”可以让 S72 动效降级。
- 本轮已视觉查看既有 `ink-world-base-v1` 底图，确认案头山河感、留白和水墨基调可继续使用；S74.6 不重做地图素材，后续若真实截图发现图标比例或移动端可读性不足，再单独开 `ink-map-v2` 或图标 v2 polish。
- 扩展 React client 测试、scaffold 测试和 browser smoke，使 `/game/:sessionId/map` 覆盖真实 Mock 开局后的 map runtime canvas、label、安全文本和 hidden/raw 防线。

验证：

- 已通过：`npm run typecheck:client`。
- 已通过：`npm run test:client`，5 个测试文件、28 项通过。
- 已通过：`node --test test/reactClientScaffold.test.js test/mapFrontendShell.test.js test/mapAssetsManifest.test.js test/mapRuntimeView.test.js test/mapRuntimeRoute.test.js`，22 项通过。
- 已通过：`npm run build:client`，产物主 JS 约 332.58 kB / gzip 105.83 kB。
- 已通过：`npm run smoke:browser -- --screenshots artifacts/s74-6-map-runtime-bridge`，覆盖 desktop home、desktop preview map route、真实 Mock session map runtime、people assets 和 mobile home；真实 `/game/:sessionId/map` 已确认 canvas 非空、公开 label 可见且 hidden/raw 文本未泄漏。
- 已通过：`npm run check:docs-governance`。
- 已通过：`node --test test/documentationGovernance.test.js`。
- 已通过：`git diff --check`；仅有既有 LF/CRLF warning，退出码为 0。
- 已通过：完整 `npm test`，871 项通过、0 失败。
- 已完成：提交前只读复审未发现 P0/P1/P2；P3 建议后续可补路由 session mismatch 与 `mapMotion` toggle 组件测试，并可在回合地图刷新时同步清空 tooltip。

风险/遗留：

- S74.6 是桥接步，不做地图玩法扩张；真实移动、案件、军务、财政、外交和任免后果仍由普通回合服务器裁决。
- 旧 `public/mapPanel.js` 保留供历史 S72 归档和旧前端参考，但 React 新前端不调用它；S76.9 可在独立舆图页继续扩展图层筛选、局势簿联动和更大画布布局。
- 当前底图不重做；若后续浏览器截图暴露图标过具象、缩放不协调或移动端不可读，再按素材治理生成和审核地图 v2。
- `InkMapRuntimeBridge` 当前在地图 view 更新后会重建 labels，但不主动清空已打开 tooltip；这只可能造成旧 tooltip 短暂停留，不会自动提交或泄漏 raw/hidden 内容，后续可随 S76.9 舆图页 polish 一并处理。

下一步：

- 执行 S74.7：S74 默认入口验收与回退说明，确认新默认前端 Mock 开局、路由刷新、安全恢复和 smoke 口径全部收束。
