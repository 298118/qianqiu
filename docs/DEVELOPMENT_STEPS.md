...
| S42.2 | DONE | 实现官场行动与履历系统：差事、奏疏、考成、弹劾、调任、升降、丁忧/起复、清望/贪墨风险 | 2026-05-06 | Codex | 本次提交 |
...
S42.2 已完成首个运行时切片：`src/game/officialCatalog.js` 提供静态官职/衙门目录，`officialCareer.schemaVersion = 2` 兼容旧 v1 存档并新增 `bureauId`、`assignments`、`assessmentDossier`、`impeachmentProcedure`；官场回合会根据玩家输入推进差遣、考成和弹劾流程，`officialCareerView` 暴露官署、差事、考成、关系和风险摘要；普通 provider 仍不能写 `officialCareer` 或 `officeTitle`，也不能通过 `player.position` 伪造明显官职。S42.3 继续负责展开浏览器官场面板、档案和截图/布局验收。
