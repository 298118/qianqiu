...
| S88.10 | IN_PROGRESS | PixiJS 水墨地图行动入口 | S88.10.1 已启动前端安全行动入口首片：复用 `mapRuntimeView.refs/routes/eventEffects/actionDrafts` 生成舆图行动牌，并把地点、驿路和近事草稿写成带安全 refs 的本地 `draftContext`；仍不新增后端 API 或浏览器裁决权。S88.10.2 正在收口后端复核链：普通/SSE turn 会在 topic draft 校验不命中后重建当前 `mapRuntimeView`，用 `normalizeMapRuntimeTurnContext()` 只采纳当前安全舆图 refs，并把通过校验的 `mapRuntimeDraftContext` 作为 role-cycle outcome/ledger 审计 hint；地方案牍、军务报告和市价粮价舆图草稿使用服务器模板触发既有低风险 city/military resolver，不引入地图写 API、新工具权限或浏览器裁决。后续可在安全地理锚点和关系边界明确后再评估 NPC 活动。 |
