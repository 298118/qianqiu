...
- 当前下一步是 S76.5：将领面板，围绕军帐、粮饷、斥候、边患、军令草案和战报入口继续保护草稿-only 与服务器 resolver/军务裁决边界。
...
- `npm run smoke:browser -- --client react --screenshots artifacts/s76-4-official-minister-panel`
- `npm run check:docs-governance`
- `node --test test/documentationGovernance.test.js`
- `git diff --check`
- `npm test`（898 项通过、0 失败）

当前已知遗留：浏览器级 `prefers-reduced-motion` context 深测留给 S77.6；真实专题 `surface` 来源和完整专题 composer 留给 S76.11；S76.5 继续推进将领面板。
