...
本轮 S88.8 经济解释 trace 第五片当前验证口径：

- `node --check src/game/topicDrafts.js`
- `node --check test/topicDraftRoute.test.js`
- `npm run typecheck:client`
- `npm run typecheck:server`
- `node --test --test-reporter=spec test/economyTraceView.test.js test/resolverInputContext.test.js test/topicSurfaceView.test.js test/topicDraftRoute.test.js`（38 项）
- `npm run test:client -- --pool=vmForks --maxWorkers=2 client/src/__tests__/App.test.tsx`（41 项；若缺少 rolldown optional native binding，先 `npm install` 修复本地 `node_modules`）
