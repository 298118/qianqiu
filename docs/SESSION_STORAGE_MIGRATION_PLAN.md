...
- `sessionId` is generated by `crypto.randomUUID()` and must match the UUID-like safe pattern in `src/storage/jsonSessionAdapter.js` before any JSON path is built.
...
Do not let route code depend on JSON-file details. S49.2 formalizes the first storage adapter boundary:
