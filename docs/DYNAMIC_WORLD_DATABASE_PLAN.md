...
结论：**可行，而且中长期值得做；但不建议一步到位替换当前 JSON 存档，也不能让 AI 直接写数据库。** 推荐先保持现有 `worldState` 与服务器裁决边界稳定，再通过存储适配器把本地 SQLite 作为下一层持久化能力引入。AI 仍只生成叙事、建议和结构化 proposal；最终写库必须由服务器模块校验、夹断、归一化、事务提交。
...
- 需要多 session 比较、导入导出、长期世界演化、远程存档或未来多人/账号。
- 需要让 prompt 按“玩家当前可知范围”取上下文，而不是把全部世界状态塞给模型。

所以推荐节奏是：**先规划和适配器，后 SQLite；先本地数据库，后托管数据库；先事件/索引表，后全量实体拆表。**
...
推荐新增一个 `ai_change_proposals` 或等价 ledger：记录模型建议、目标对象、建议字段、审查结果、拒绝原因、应用后的 server event id。这样 AI “影响世界”有痕迹，但世界仍由服务器落笔。
...
- 固定本文档或等价契约。
- 明确 SQLite 是首选本地数据库方向，托管数据库暂不进入当前目标。
