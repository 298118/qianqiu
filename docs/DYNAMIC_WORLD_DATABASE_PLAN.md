...
- S71.7 已证明军务与外交可以从 `resolverInputContext` / S70 domain tool evidence refs 进入服务器裁决：AI 只提交 `military.propose_order` 或 `diplomacy.propose_move`，服务器校验 T4-T5 权限、证据领域/可信度、钱粮/市场证据、兵粮资源、target scope、制度路径和 hidden/raw 污染后，才写受控军务/外交后果、公开事件和内部 `militaryDiplomacyLedger`。
- S71.8 已证明压力事件可以从 `resolverInputContext` / S70 event pressure refs 进入服务器成案：生成器从粮价、水利、腐败、边防、NPC 怨怼、财政缺口、士论和情报压力组合候选，服务器校验可见来源、阈值、确定性概率、冷却、每旬上限和 hidden/raw 污染后，才写受控 world meters、公开事件和内部 `worldPressureEventLedger`。
- S71.9 已证明多 actor 场景可以复用安全 projection 与既有 resolver：场景运行时为朝议、堂审、会盟和战役军议逐 actor 构造可见证据摘要，收集 Mock/AI 场景意见，锚定参与者、过滤不可见 evidence、拒绝工具直写/成案伪造，并只返回 S71.5-S71.7 resolve-only 的 hidden-safe outcome 摘要；是否应用后果仍由调用方显式执行对应 apply helper。
- S71.10 已证明 NPC 记忆可以复用现有 actor memory ledger，而不是新增普通玩家可读 raw 私档：服务器从 NPC mind 安全 `memoryCandidates`、`worldPeopleView` 和可见关系行生成 capped 记忆 proposal，写入目标、人情债、恩怨、家族风险、畏惧和野心等可见记忆；private/hidden、污染文本、不可见 actor、raw `actorMemoryLedger`、SQLite raw row、prompt retrieval row、本地路径和 key 不进入玩家 API、prompt、安全搜索或场景 context。
- S71.11 已证明 AI 调动审计可以做成 route view，而不是 raw audit 浏览器：`aiControlAuditView` 只汇总 bounded AI invocation summary、公开事件、可见记忆、月报和经历摘要；`diagnostics.ai.controlAudit` 也只返回同一 hidden-safe 投影。浏览器面板不读取 raw audit sidecar、SQLite audit row、provider payload、完整 prompt、hidden ledger、本地路径或 key。
...
- AI actor 角色卡、朝议/堂审/会盟/战役场景、NPC 记忆和 AI 调动审计面板都必须继承 S70 的“模型请求、服务器执行”工具边界。
...
9. **S71 数据库玩法化专项**：再让安全 projection 驱动财政、地方政策、案件、军务、外交和事件链 resolver；S71.11 已补本地 migration/maintenance、安全全文检索、redacted player API、开发诊断 API、财政/城市政策服务器 resolver、地方案件/刑名服务器 resolver、军务/外交服务器 resolver、压力驱动事件生成器、多 actor 场景运行时、NPC 记忆账本和 AI 调动审计面板，后续进入 S71 验收与归档。
