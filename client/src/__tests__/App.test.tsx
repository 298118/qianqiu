...
describe("S74.1 React client shell", () => {
  ...
  it("opens registry-backed local surfaces, writes safe drafts, and restores focus on Esc", async () => {
    ...
    expect(courtTrace.textContent || "").toContain("跨页追索笺");
    expect(courtTrace.textContent || "").toContain("从六署专题读议题，从人物页查来人，从史册页看已留痕后果。");
    expect(courtTrace.textContent || "").toContain("这里只指明读卷路径");
    expect(courtTrace.querySelectorAll("[data-cross-trace-target]").length).toBe(3);
    expect(within(courtTrace).getByRole("link", { name: "查人物" }).getAttribute("href")).toBe("/game/smoke-session/people");
    expect(within(courtTrace).getByRole("link", { name: "查史册" }).getAttribute("href")).toBe("/game/smoke-session/archive");
    expect(within(courtTrace).getByRole("link", { name: "回主卷候复" }).getAttribute("href")).toBe("/game/smoke-session");
  ...
  it("renders the S76.8 ranking page from server-owned exam views", async () => {
    ...
    expect(screen.getAllByText("授官").length).toBeGreaterThan(0);
    expect(screen.getByText("放榜仪程")).toBeTruthy();
    expect(screen.getByText("张榜取材")).toBeTruthy();
    const outcomeReader = document.querySelector<HTMLElement>('[data-polish-ranking-reader="s91-7-ranking-reader"]');
    expect(outcomeReader).toBeTruthy();
    expect(outcomeReader?.textContent || "").toContain("题名校阅");
    expect(outcomeReader?.textContent || "").toContain("榜文");
    expect(outcomeReader?.textContent || "").toContain("2 名");
    expect(outcomeReader?.textContent || "").toContain("只数已经张挂的公开榜行");
    expect(outcomeReader?.textContent || "").toContain("荣誉摘要不补成正榜");
    expect(outcomeReader?.textContent || "").toContain("我名");
    expect(outcomeReader?.textContent || "").toContain("状元");
    expect(outcomeReader?.textContent || "").toContain("只按案卷标记的本人榜行呈现");
    expect(outcomeReader?.textContent || "").toContain("细读");
    expect(outcomeReader?.textContent || "").toContain("同年座师公开 2 人");
    expect(outcomeReader?.textContent || "").toContain("不改名次、关系或官职");
    expect(outcomeReader?.textContent || "").toContain("授官");
    expect(outcomeReader?.textContent || "").toContain("翰林院修撰");
    expect(outcomeReader?.textContent || "").toContain("仍候主卷回音");
    expect(outcomeReader?.textContent || "").not.toMatch(/provider payload|raw audit|hiddenNotes|sk-test-secret|path=|C:\\/i);
    ...
    expect(outcomeReader?.textContent || "").toContain("榜名未公开");
    expect(outcomeReader?.textContent || "").not.toMatch(/provider payload|raw audit|hiddenNotes|sk-test-secret|path=|C:\\/i);
  ...
  it("keeps S76.8 ranking empty when the server has not returned a ranking", async () => {
    ...
    const outcomeReader = document.querySelector<HTMLElement>('[data-polish-ranking-reader="s91-7-ranking-reader"]');
    expect(outcomeReader).toBeTruthy();
    expect(outcomeReader?.textContent || "").toContain("未张榜");
    expect(outcomeReader?.textContent || "").toContain("未见我名");
    expect(outcomeReader?.textContent || "").toContain("未选榜名");
    expect(outcomeReader?.textContent || "").toContain("榜文尚未张挂");
    expect(outcomeReader?.textContent || "").not.toMatch(/会元|第一名|弥封复核通过|未见公开扣罚事项/);
  ...
  it("does not mark same-name ranking rows as the player without server flag", async () => {
    ...
    const outcomeReader = document.querySelector<HTMLElement>('[data-polish-ranking-reader="s91-7-ranking-reader"]');
    expect(outcomeReader?.textContent || "").toContain("未见我名");
    expect(outcomeReader?.textContent || "").toContain("同名榜行、本地细读与评语都不补认本人");
    expect(document.querySelector(".rankingGoldenNotice")).toBeFalsy();
  ...
  it("loads the S76.10 current people ledger without exposing the full portrait pool", async () => {
    ...
    const peopleTrace = document.querySelector("[data-polish-cross-trace='s89-36-cross-page-trace'][data-cross-trace-page='people']") as HTMLElement;
    expect(peopleTrace).toBeTruthy();
    expect(peopleTrace.getAttribute("data-cross-trace-state")).toBe("ready");
    expect(peopleTrace.textContent || "").toContain("跨页追索笺");
    expect(peopleTrace.textContent || "").toContain("人物线索先看公开名册与来函，再去朝议成题，归史册留痕。");
    expect(peopleTrace.textContent || "").toContain("人物页不展示草稿正文");
    expect(peopleTrace.querySelectorAll("[data-cross-trace-target]").length).toBe(4);
    expect(within(peopleTrace).getByRole("link", { name: "留本页" }).getAttribute("href")).toBe(`/game/${sessionId}/people`);
    expect(within(peopleTrace).getByRole("link", { name: "入朝议" }).getAttribute("href")).toBe(`/game/${sessionId}/court`);
    expect(within(peopleTrace).getByRole("link", { name: "查史册" }).getAttribute("href")).toBe(`/game/${sessionId}/archive`);
    expect(within(peopleTrace).getByRole("link", { name: "回主卷候复" }).getAttribute("href")).toBe(`/game/${sessionId}`);
  ...
  it("renders archive route entries and domain consequence tracking from safe views", () => {
    ...
    const archiveTrace = archivePanel.querySelector("[data-polish-cross-trace='s89-36-cross-page-trace'][data-cross-trace-page='archive']") as HTMLElement;
    expect(archiveTrace).toBeTruthy();
    expect(archiveTrace.getAttribute("data-cross-trace-state")).toBe("ready");
    expect(archiveTrace.textContent || "").toContain("跨页追索笺");
    expect(archiveTrace.textContent || "").toContain("史册只收已入卷条目；要查来人去人物页，要成议题入朝议页。");
    expect(archiveTrace.textContent || "").toContain("这里只指明读卷路径");
    expect(archiveTrace.querySelectorAll("[data-cross-trace-target]").length).toBe(4);
    expect(within(archiveTrace).getByRole("link", { name: "留本页" }).getAttribute("href")).toBe("/game/74747474-1111-4111-8111-747474747474/archive");
