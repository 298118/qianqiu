...
- 启动 S72 PixiJS 水墨地图专项并完成 S72.2 后端 runtime view：`src/game/mapRuntimeView.js` 从安全 `mapContextView` 派生 `mapRuntimeView`，包含显示 layout、layer/style token、route path、event effect 和服务器预渲染行动草稿；游戏 start/state/player-state/turn/SSE 与考试 question/progress/submit payload 都会返回该安全显示投影。显示坐标只用于未来 PixiJS 前端布局，不进入 prompt、AI 工具、移动裁决、任免、财政、战争或商路收益。
