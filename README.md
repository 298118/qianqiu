...
- 本地存档簿、审计与静态地理种子：JSON session envelope、legacy 迁移、原子写入、revision、轻量 lock、存档列表 API 和浏览器存档簿已经落地；可选 SQLite adapter 可用 `STORAGE_ADAPTER=sqlite` 开启，仍返回同一套路由 payload；S49.4 新增本地 `event_log` / `ai_change_proposals` 审计记录，用于复盘服务器接受/拒绝模型建议；S50.1 新增 `src/game/worldGeographySeeds.js`，作为后续国家/城市动态实例化的静态 catalog。
...
- [docs/WORLD_GEOGRAPHY_SEED_CONTRACT.md](docs/WORLD_GEOGRAPHY_SEED_CONTRACT.md)：S50.1 天下地理静态种子契约，固定国家、邻国、城市、路线、边境、官署辖区和初始可见性。
...
- 当前默认存储仍是本地 JSON。S49.3 已加入可选本地 SQLite session row adapter，S49.4 已加入本地事件日志和 AI proposal 审计；S50.1 只新增静态地理种子，不把国家/城市写入每局动态状态，也不接 prompt/UI；国家、城市、NPC、官职、关系等业务表拆分仍是后续步骤。远程存档、账号体系、多人同步和云端数据库不属于当前范围。
