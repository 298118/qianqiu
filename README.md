...
- 细化 S73-S77 前端水墨重构规划：[docs/FRONTEND_INK_REDESIGN_ROADMAP.md](docs/FRONTEND_INK_REDESIGN_ROADMAP.md) 已扩展为可执行开发蓝图，把 S73 拆成视觉资产指南、UI manifest/台账、UI 材质、首页资产、场景插画、身份背景、玩家/NPC 立绘基准、动效/fallback、素材 QA 和 S73.10 全量立绘生产；S74 规划为通过依赖治理后引入 React + TypeScript + Vite + React Router Data Mode 多页前端岛，S75-S76 接管首页/全局 shell、设置/存档/返回首页、底部奏折、身份/场景专题、考试/放榜、独立舆图页和立绘接线，S77 做入口切换、安全/性能/可访问性和归档验收。
...
- 前端：当前发布入口仍为原生 HTML、CSS、JavaScript，无构建步骤；S74.0 已规划可在依赖治理通过后引入 React + TypeScript + Vite + React Router Data Mode 并行多页前端岛，S77 验收后才切默认入口。
- 后端：Node.js + Express。
