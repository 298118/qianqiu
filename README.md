...
- S88.12 已启动 Mock/真实 provider 验收首片：普通 JSON turn 与 SSE turn 在 `AI_PROVIDER=deepseek` 但无 `DEEPSEEK_API_KEY` 时仍通过 Mock 安全回退保持可玩，经济 `draftContext` 仍由服务器从当前 topic surface 重建 allowlist 后再采纳；`/api/ai/connection-test` 缺 key 仍返回 503 作为诊断，不等同普通游玩回退。`npm run smoke:provider:tools` 在无 `MIMO_API_KEY` 时会先 skip，不要求运行时存在 `fetch`；有 key 但无 fetch 仍会失败以暴露真实 smoke 环境问题。
