...
当前活动路线图已切到本地动态世界数据库专项，见 [动态世界本地数据库规划](docs/DYNAMIC_WORLD_DATABASE_PLAN.md)：短期继续保持 JSON 存档可玩，S49.3 已提供可选本地 SQLite session row 原型，一行一 session，保留 JSON `world_state`，并继续让 JSON 作为默认 adapter；S49.4 已加入本地事件日志与 AI proposal 审计，之后逐步拆国家/邻国、城市、NPC、官职、关系和事件等动态数据。当前不规划远程存档、账号体系、多人同步或云端数据库。AI 只能提交结构化建议，不能直接写数据库或执行 SQL。
...
- 本地存档簿与审计：JSON session envelope、legacy 迁移、原子写入、revision、轻量 lock、存档列表 API 和浏览器存档簿已经落地；可选 SQLite adapter 可用 `STORAGE_ADAPTER=sqlite` 开启，仍返回同一套路由 payload；S49.4 新增本地 `event_log` / `ai_change_proposals` 审计记录，用于复盘服务器接受/拒绝模型建议。
...
- 当前默认存储仍是本地 JSON。S49.3 已加入可选本地 SQLite session row adapter，S49.4 已加入本地事件日志和 AI proposal 审计；国家、城市、NPC、官职、关系等业务表拆分仍是后续步骤。远程存档、账号体系、多人同步和云端数据库不属于当前范围。
