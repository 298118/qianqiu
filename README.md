...
- 细化并启动 S74 前端迁移：[docs/FRONTEND_INK_REDESIGN_ROADMAP.md](docs/FRONTEND_INK_REDESIGN_ROADMAP.md) 已扩展为可执行开发蓝图，S73 完成视觉资产、manifest、素材 QA 和全量立绘池；S74.0 已按 [docs/FRONTEND_REACT_MIGRATION_CONTRACT.md](docs/FRONTEND_REACT_MIGRATION_CONTRACT.md) 安装 React + TypeScript + Vite + React Router Data Mode 工具链并记录依赖治理，S74.1 已创建 `client/`、Vite/TS/Vitest 配置、最小 React Router 多页壳和 Express history fallback，构建产物从 `dist/client/` 接管默认 `/`；S74.2 已新增安全 API client、会话 store 和首页/主卷/科举/印匣最小接线，普通读档只走 `player-state`；S74.3 已新增 UI 状态 store、route page bridge、显示偏好抽屉、安全摘要 modal 和全局 action draft。旧原生前端仍保留在 `public/` 供后续迁移参考，但不再是交付入口；S74.4/S75-S76 继续接管完整 shell、surface registry、首页/全局 shell、设置/存档/返回首页、底部奏折、身份/场景专题、考试/放榜、独立舆图页和立绘接线，S77 做默认入口确认、安全/性能/可访问性和归档验收。
