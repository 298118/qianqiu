...
- 完成 S73.2-S73.9 UI manifest、素材台账、首批材质包、首页资产包、场景插画包、身份背景包、立绘风格基准、动效/fallback 包与素材 QA：`public/assets/ui/ink-ui-manifest.json` 已从 schema 草案进入 `assets_active`，登记 16 个已审核 UI 材质、6 个已审核/限用首页素材、10 个已审核场景插画、6 个已审核身份背景、24 张已审核玩家/NPC 立绘基准和 8 个已审核动效/fallback 素材；新增 `public/assets/ui/asset-qa-preview.html` 与 `public/assets/ui/asset-qa-report-v1.json`，可预览并校验 70 个 active 素材和 19 个透明素材的路径、尺寸、alpha、缩略图、fallback、审核状态、宣纸底/深色底合成和 chroma-key 色边风险。`test/frontendInkAssetsManifest.test.js` 现在校验真实图片存在性、尺寸、alpha、性能预算、审核状态、场景/身份覆盖、角色色彩权重、`portraitRef`、缩略图、低清占位、懒加载、`motion`/`reducedMotionFallback`、S73.9 QA 报告和安全字段。
- S73.7 已完成 24 张玩家/NPC 立绘风格基准并登记 `portrait-baseline-qa-v1.json`；S73.8 已完成 8 个动效/fallback 素材并登记 `effect-motion-qa-v1.json`；S73.9 已新增 `npm run qa:frontend-assets` 与 `npm run qa:frontend-assets:write`，后者复用现有 Playwright/Chrome 工具链刷新透明素材像素 QA；S73.10 仍将在 S73 内完成 300-400 张全量立绘的生成、缩略图、压缩、视觉/安全审核、manifest 入库和台账登记，S74-S77 的人物显示必须通过已审核 `portraitRef`、缩略图和 fallback 使用，动效接入必须尊重 reduced-motion。
