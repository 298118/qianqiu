...
项目已经完成第一阶段可玩纵切、第二阶段本地验收，并推进完第三阶段 S31-S39.1 的主要硬化与体验扩展。第四阶段已经开启，当前优先方向是 AI 连接可见化、提示词与 AI 权限审查、官场深度、世界议程和多实体模拟。

当前重点成果：

- 完整书生主线：书生可以从日常读书、拜师、游学、辩经、谋生进入童试、乡试、会试、殿试，最终成为入仕官员。
- 多身份玩法：支持书生、皇帝、大臣、将领、地方官、入仕官员开局与代表性行动循环。
- 长期模拟骨架：世界月度 tick、关系账本、主动 NPC 请托、长期事件、官场结果、科举日历、角色世界联动和 World Threads 世界议程索引已经接入。
...
- 世界议程：S43.2 已把服务器拥有的 `worldThreads` 聚合层接入浏览器“世界议程”面板；主动 NPC、长期事件、官场差遣/结果、身份联动、地方压力、边事和派系斗争会显示目标、期限、风险、牵连对象、介入提示和近归档余波。
- AI 权限审查：S44.1 已新增 [AI 调动与控制审查矩阵](docs/AI_CONTROL_AUDIT_MATRIX.md)，逐项固定哪些系统适合 AI 生成、建议、解释或排序，哪些必须由服务器裁决；S44.2 继续补 red-team/eval 测试。
- 依赖与插件：允许引入有价值的依赖和插件，但必须记录用途、许可证、替代方案、测试和回滚策略。
...
- 游戏与考试路由会返回 `examCalendarView`、`examRivalView`、`relationshipView`、`activeNpcRequestView`、`roleWorldCouplingView`、`worldThreadView`、`longTermEventView`、`officialCareerView` 等视图；其中 `officialCareerView` 用于浏览器官场面板，`worldThreadView` 用于浏览器“世界议程”面板和 AI prompt 摘要。
...
- [docs/DEVELOPMENT_STEPS.md](docs/DEVELOPMENT_STEPS.md)：第四阶段路线图与进度台账。
...
- 长期事件、官场结果、科举日历、角色世界联动和 World Threads 仍是确定性规则集；S42.3 已让入仕后可在浏览器中查看官署、差事、考成、关系和风险，S43.2 已把这些信号汇入浏览器世界议程总览。世界议程只提示介入方向，不提供一键结算。
