...
当前活动路线图已切到本地动态世界数据库专项，见 [动态世界本地数据库规划](docs/DYNAMIC_WORLD_DATABASE_PLAN.md)：短期继续保持 JSON 存档可玩，S49.3 已提供可选本地 SQLite session row 原型，一行一 session，保留 JSON `world_state`，并继续让 JSON 作为默认 adapter；S49.4 已加入本地事件日志与 AI proposal 审计；S50.1/S50.2 已把天下地理 seed 实例化进每局 `worldState.worldGeography`，并通过 `worldGeographyView` 与 capped prompt 摘要提供可见地理上下文；S51.1/S51.2 已新增 [人物、家族、资产、田产与关系 Schema 契约](docs/NPC_HOUSEHOLD_ASSET_RELATIONSHIP_CONTRACT.md)、`src/game/worldPeopleSchemas.js` 和 `src/game/worldPeople.js`，先固定未来人物域 schema，再把旧 `characters`、`relationshipLedger` 与可见 active requests 桥接为安全 `worldPeople` projection；S52.1/S52.2 已新增 [官职、官署、任所与迁转数据库契约](docs/OFFICIAL_POSTING_DATABASE_CONTRACT.md)、`src/game/officialPostingSchemas.js` 和 `src/game/officialPostings.js`，先固定未来 `officialPostings` / `office_postings` 数据形状，再把官职目录、当前官场/地方官任所和 S50 城市/辖区数据桥接为安全 `officialPostingsView` / prompt projection；S53.2 已新增 [浏览器信息面板规划](docs/BROWSER_INFORMATION_PANEL_PLAN.md)，固定天下格局、任所地理、人物谱牒、官职簿和事件档案后续只能读取 route player-facing view；S53.3 已在浏览器接入 `#information-panel` tab 壳；S53.4 已把“天下格局”和“任所地理”两类细面板落地为可扫读卡片；S53.5 已把“人物谱牒”和“官职簿”接入同一局势簿，只读 `worldPeopleView`、`officialPostingsView` 与可见辅助 view。事件档案仍必须先有服务器 sanitized projection。当前不规划远程存档、账号体系、多人同步或云端数据库。AI 只能提交结构化建议，不能直接写数据库或执行 SQL。
...
- 本地存档簿、审计、天下地理、人物与官职任所 schema：JSON session envelope、legacy 迁移、原子写入、revision、轻量 lock、存档列表 API 和浏览器存档簿已经落地；可选 SQLite adapter 可用 `STORAGE_ADAPTER=sqlite` 开启，仍返回同一套路由 payload；S49.4 新增本地 `event_log` / `ai_change_proposals` 审计记录，用于复盘服务器接受/拒绝模型建议；S50.1/S50.2 新增 `src/game/worldGeographySeeds.js` 与 `src/game/worldGeography.js`，让每局拥有 server-owned 国家、城市、路线、边境和官署辖区账本，同时只把过滤后的 `worldGeographyView` / prompt summary 给 UI 和模型；S51.1/S51.2 新增 `src/game/worldPeopleSchemas.js` 与 `src/game/worldPeople.js`，把现有可见人物、人脉账本和 active request 近期札记桥接为 `worldPeopleView` / prompt summary；S52.1/S52.2 新增 `src/game/officialPostingSchemas.js` 与 `src/game/officialPostings.js`，定义官署、官职、城市辖区、任命、考成和迁转记录契约，并把当前地方官/入仕官员任所联动到可见城市数据；S53.3-S53.5 新增浏览器“局势簿”tab 壳、天下格局卡、任所地理卡、人物谱牒卡、官职簿卡和 smoke 验收，但事件档案细节仍在后续步骤，也不创建 SQLite 业务表。
...
- 存储一致性：路由通过 `sessionStore` facade 读写；默认 JSON adapter 使用 envelope、atomic temp rename、revision 检查、同 session 队列和本地 `.lock` 文件；可选 SQLite adapter 使用本地 `world_sessions` 表、transaction、revision 检查和同 session 队列保存同一份 JSON `world_state`。S49.4 的审计记录只保存脱敏摘要、接受/拒绝原因和应用事件 id，不进入玩家 API；JSON sidecar 审计是诊断性尽力追加，SQLite 模式提供 session 与审计同事务写入。
...
- 存储：`sessionStore` facade + 默认本地 JSON adapter，文件位于 `data/sessions/`，带 schema envelope、metadata、revision、atomic write、lock 和 save-list API；可选本地 SQLite adapter 通过 Node.js `node:sqlite` 标准库写入 `data/qianqiu.sqlite`；审计日志在 JSON 模式写入 `data/audit/*.jsonl`，SQLite 模式写入 `event_log` 与 `ai_change_proposals` 表。
...
`STORAGE_ADAPTER` 默认是 `json`。设置为 `sqlite` 时，服务器使用本地 SQLite session row adapter；该模式需要当前 Node.js 运行时提供 `node:sqlite`。`SQLITE_DATABASE_PATH` 可覆盖数据库文件位置；`data/*.sqlite` 与派生日志文件会被 Git 忽略。开发迁移可运行 `npm run storage:import:sqlite -- --dry-run` 预览从 `data/sessions/*.json` 导入 SQLite 的结果，再按需去掉 `--dry-run`。
...
- 游戏与考试路由会返回 `examCalendarView`、`examRivalView`、`relationshipView`、`activeNpcRequestView`、`roleWorldCouplingView`、`worldGeographyView`、`worldEntityView`、`worldPeopleView`、`worldThreadView`、`longTermEventView`、`officialCareerView`、`officialPostingsView` 等视图；其中 `officialCareerView` 用于浏览器官场面板，`worldThreadView` 用于浏览器“世界议程”面板和 AI prompt 摘要，`worldEntityView` 用于暴露 S45 多实体世界模型的玩家可见摘要，`worldGeographyView` 是 S50.2 为“天下格局/任所地理”面板准备的可见地理 projection，`worldPeopleView` 是 S51.2 为人物谱牒/家产面板准备的可见人物 projection，`officialPostingsView` 是 S52.2 为官职簿/任所地理面板准备的可见官职任所 projection。S53.4 已把 `worldGeographyView` 与 `officialPostingsView` 渲染进浏览器“局势簿”的天下/任所细卡；S53.5 已把 `worldPeopleView` 和 `officialPostingsView` 渲染为人物/官职簿细卡；后续事件档案必须继续读服务器 view，不能读取 raw ledger、prompt-only `retrievalContext` 或 raw audit。
...
- 当前默认存储仍是本地 JSON。S49.3 已加入可选本地 SQLite session row adapter，S49.4 已加入本地事件日志和 AI proposal 审计；S50.2 已新增每局 `worldState.worldGeography`、路由 `worldGeographyView` 和 prompt projection，但还不把国家/城市拆成 SQLite 业务表；S51.2 已新增每局 `worldState.worldPeople` 的安全可见桥接、路由 `worldPeopleView` 和 prompt projection，但还不把 NPC、家族、资产或关系拆成 SQLite 业务表；S52.2 已新增 `officialPostingsView`，S53.4 已接入浏览器天下格局/任所地理细面板，S53.5 已接入人物谱牒/官职簿细面板；事件档案仍待 S53.6，且必须先补服务器安全 projection。远程存档、账号体系、多人同步和云端数据库不属于当前范围。
