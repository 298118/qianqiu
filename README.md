...
- JSON/SQLite 双模式验收继续由 `npm run smoke:dual-mode` 及 `npm run smoke:dual-mode -- --storage-only` 承担；S74 起 `npm run smoke:browser` 是 React 默认入口 smoke，只接受 `--url`、`--browser`、`--screenshots` 和 `--headed` 等 client smoke 参数。
...
- 前端：默认浏览器入口已由 S74.1 React + TypeScript + Vite 构建产物接管；源码位于 `client/`，生产构建输出为 `dist/client/`，React Router Data Mode 在 `/` 下管理首页、主卷、舆图、人物、史册、科举、皇榜、朝议和印匣等路由。S74.2 起 React client 通过安全 API client 调用 start/saves/player-state/turn/exam/AI settings/connection-test，普通读档不使用 raw state route；S74.3 起 UI 状态层统一管理 route page、`sessionId`、安全玩家摘要、drawer/modal/surface、tab、action draft 和 display preferences；S74.4 起 `AppShell` 与 `SurfaceHost` 统一管理设置/存档/专题 overlay、Esc 关闭、焦点回收和滚动恢复；S74.5 起 `assetRegistry` 通过 `/assets/ui/ink-ui-manifest.json` 按审核状态、安全路径、fallback、缩略图和低清占位提供资产查询，`Portrait` 组件只接收 `portraitRef`；S74.6 起 React 舆图页通过 `InkMapRuntimeBridge` 动态加载本地 Pixi vendor 与 `public/mapRenderer.js`，只读渲染安全 `mapRuntimeView`，地图按钮只写入行动草稿；S74.7 起全局导航在真实案卷中指向当前 `sessionId`，默认入口 smoke 会验证 Mock 表单开局与主要路由刷新恢复。旧 `public/index.html`、`public/app.js` 和 `public/styles.css` 暂留作迁移参考，`public/mapPanel.js` 仅作历史 S72/旧前端参考，不再被 React 新前端调用；产品内不保留 `/legacy.html` 或 `/ink-client/` 双入口。
- 后端：Node.js + Express。
...
- `npm run smoke:browser` 构建 React client 并启动临时 Mock 服务器；无 `--url` 时脚本会显式固定 `AI_PROVIDER=mock`，覆盖默认 `/` React 入口真实表单开局、当前 session 导航、舆图 canvas、人物立绘分页、史册/科举/皇榜/朝议/印匣路由刷新、桌面/移动无横向溢出和 hidden/raw/API 防线。
