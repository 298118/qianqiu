...
- 启动 S73-S77 前端水墨重构规划：新增 [docs/FRONTEND_INK_REDESIGN_ROADMAP.md](docs/FRONTEND_INK_REDESIGN_ROADMAP.md)，把首页“千秋”画卷、水墨云雾、右上角设置/存档/读档/返回首页、底部奏折输入区、身份/场景专属界面、科举考试和放榜全屏界面、NPC/玩家立绘 manifest 与视觉审核流程拆为 S73 素材、S74 信息架构拆分、S75 首页与全局 shell、S76 身份/场景专题界面、S77 验收归档。该专项继续保持纯 HTML/CSS/JS、Mock 默认可玩、S72 地图安全 view、存档/AI 设置安全 API 和完整书生路径；AI 生成素材由 Codex 视觉审核，立绘边界为成年、端庄、身份明确、俊美但不露骨。
...
- [docs/FRONTEND_INK_REDESIGN_ROADMAP.md](docs/FRONTEND_INK_REDESIGN_ROADMAP.md)：S73-S77 前端水墨重构任务书，覆盖素材、信息架构、首页/全局 shell、身份/场景专题界面和验收归档。
