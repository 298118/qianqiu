...
当前活动路线图已切到本地动态世界数据库专项，见 [动态世界本地数据库规划](docs/DYNAMIC_WORLD_DATABASE_PLAN.md)：短期继续保持 JSON 存档可玩，S49.3 已提供可选本地 SQLite session row 原型，一行一 session，保留 JSON `world_state`，并继续让 JSON 作为默认 adapter；S49.4 已加入本地事件日志与 AI proposal 审计；S50.1 已新增 [天下地理静态种子与实例化契约](docs/WORLD_GEOGRAPHY_SEED_CONTRACT.md)，先固定国家、邻国、城市、路线、边境和官署辖区 catalog；S50.2 已把这些种子实例化进每局 `worldState.worldGeography`，并通过 `worldGeographyView` 与 capped prompt 摘要提供可见地理上下文；S51.1 已新增 [人物、家族、资产、田产与关系 Schema 契约](docs/NPC_HOUSEHOLD_ASSET_RELATIONSHIP_CONTRACT.md) 和 `src/game/worldPeopleSchemas.js`，先固定未来 NPC/家族/资产/田产/关系表的归一化、可见性过滤和 prompt cap。之后再逐步桥接旧 `characters`、`relationshipLedger`、active requests、官职和事件等动态数据。当前不规划远程存档、账号体系、多人同步或云端数据库。AI 只能提交结构化建议，不能直接写数据库或执行 SQL。
...
- 本地存档簿、审计、天下地理与人物 schema：JSON session envelope、legacy 迁移、原子写入、revision、轻量 lock、存档列表 API 和浏览器存档簿已经落地；可选 SQLite adapter 可用 `STORAGE_ADAPTER=sqlite` 开启，仍返回同一套路由 payload；S49.4 新增本地 `event_log` / `ai_change_proposals` 审计记录，用于复盘服务器接受/拒绝模型建议；S50.1/S50.2 新增 `src/game/worldGeographySeeds.js` 与 `src/game/worldGeography.js`，让每局拥有 server-owned 国家、城市、路线、边境和官署辖区账本，同时只把过滤后的 `worldGeographyView` / prompt summary 给 UI 和模型；S51.1 新增 `src/game/worldPeopleSchemas.js`，用于未来 NPC、家族、资产、田产与关系表的独立 schema 契约。
...
- 游戏与考试路由会返回 `examCalendarView`、`examRivalView`、`relationshipView`、`activeNpcRequestView`、`roleWorldCouplingView`、`worldGeographyView`、`worldEntityView`、`worldThreadView`、`longTermEventView`、`officialCareerView` 等视图；其中 `officialCareerView` 用于浏览器官场面板，`worldThreadView` 用于浏览器“世界议程”面板和 AI prompt 摘要，`worldEntityView` 用于暴露 S45 多实体世界模型的玩家可见摘要，`worldGeographyView` 是 S50.2 为后续“天下格局/任所地理”面板准备的可见地理 projection。S51.1 的 `worldPeopleSchemas` 尚未接入 route payload；后续人物谱牒/家产面板必须先接服务器 view，不能读取 raw ledger。
...
- [docs/NPC_HOUSEHOLD_ASSET_RELATIONSHIP_CONTRACT.md](docs/NPC_HOUSEHOLD_ASSET_RELATIONSHIP_CONTRACT.md)：S51.1 人物、家族、资产、田产与关系 schema 契约，固定 `worldPeopleSchemas` 的归一化、可见性过滤、引用裁剪、prompt cap 和 AI/server 边界。
...
- 当前默认存储仍是本地 JSON。S49.3 已加入可选本地 SQLite session row adapter，S49.4 已加入本地事件日志和 AI proposal 审计；S50.2 已新增每局 `worldState.worldGeography`、路由 `worldGeographyView` 和 prompt projection，但还不新增浏览器地理面板，也不把国家/城市拆成 SQLite 业务表；S51.1 只新增人物/家族/资产/田产/关系 schema helper，不接入 `worldState` 或 route payload；NPC、官职、关系等业务表拆分仍是后续步骤。远程存档、账号体系、多人同步和云端数据库不属于当前范围。
