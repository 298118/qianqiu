...
- `npm run smoke:browser` 构建 React client 并启动临时 Mock 服务器；无 `--url` 时脚本会显式固定 `AI_PROVIDER=mock`，也可显式传入 `--client react`。当前 smoke 覆盖默认 `/` React 入口真实表单开局、当前 session 导航、右上角印匣 tab 遍历、旧案 `player-state` 读档、印匣返回首页/继续本局、AI 快捷建议草稿、舆图 canvas 与低动效、人物立绘分页、史册/科举/皇榜/朝议/印匣路由刷新、移动端奏折/印匣、桌面/移动无横向溢出和 hidden/raw/API 防线。
