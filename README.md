...
- 完成 S81-S84 NPC、资产与储物系统首轮闭环：新增 [docs/NPC_INVENTORY_SYSTEM_CONTRACT.md](docs/NPC_INVENTORY_SYSTEM_CONTRACT.md)，开局自由背景会先经 `background_claim_parser` 解析，再由服务器裁决宅产、银两、书籍、债务、禁物、军权、官职和虚假身份宣称。新增资产/资源/背包/NPC/交互/交易/委派安全账本、SQLite 安全派生表、`inventory` / `npcs` / `npc` / `npc-interaction` / `trade` / `npc-command` API、React “囊箧” route、人物 NPC 工作台、对话/交易/委派 tabs 和主卷开局裁决摘要。AI 只解析、扮演、议价、计划和回禀，交易 `accepted` 不直接写银钱或物品，NPC/交易/委派 AI 文本命中 hidden/raw/path/key 形状会被服务器拒绝；资源、价格、归属、关系和任务结果仍由服务器裁决。S85 后续施工必须前后端分离，先做后端/API/数据/AI 契约，再集中接入和扩展 React 前端体验。
