...
当前项目已经完成可玩纵切、浏览器验收、时间专项、AI provider 扩展、本地动态数据库基础、S54-S59 SQLite 业务表拆分、S60-S67 超大动态世界数据库内容充实、S68-S69 科举读书深化与归档、S70 AI 编排归档，以及 S71 数据库玩法化、维护、安全检索、redacted player API、财政/刑名/军务外交服务器 resolver、压力驱动事件、多 actor 场景、NPC 记忆账本、AI 调动审计面板和 S71 验收归档；S72 PixiJS 水墨地图专项与 S73-S77 前端水墨重构专项也已完成归档。近期重点更新集中在“本地数据库专项归档”“S68-S69 科举读书深化”“多 provider 能力”“S70 AI 编排”“S71 数据库玩法化归档”“S72 PixiJS 水墨地图归档”“S73-S77 前端水墨重构归档”“S78 官署专题玩法化”“S86 后端 TypeScript 迁移首轮”“S87 route/API 响应类型覆盖”和“AI 编排 v2 baseline”：

- 启动 AI 编排 v2 基线：新增 [docs/AI_ORCHESTRATION_V2_ROADMAP.md](docs/AI_ORCHESTRATION_V2_ROADMAP.md) 与 [docs/AI_V2_BASELINE_REPORT.md](docs/AI_V2_BASELINE_REPORT.md)，把后续 AI Task Runtime、ProviderAdapter、tool loop、trace/eval/replay 和 evidenceRef 改造拆成 Ticket 0-8。新增 `npm run ai:baseline`，会写出 hidden-safe `artifacts/ai-baseline/latest.json`，只摘要 prompt pack、schema、model route、tool definitions 和 provider capabilities；`npm run eval:ai` 继续运行原 eval，并额外写出 `artifacts/ai-eval/latest.json` 安全摘要。两类 artifact 均在 ignored `artifacts/` 下，不含 key、base URL、raw prompt、provider payload、本地路径、raw SQLite row、hidden notes 或内部 `server.*` resolver 名称。本步不改变运行时 AI 行为、provider facade、prompt、API/schema、存档、SQLite、浏览器 UI 或服务器裁决。
...
- 完成 S87 route/API 响应类型覆盖：[docs/TYPESCRIPT_ROUTE_RESPONSE_COVERAGE_ROADMAP.md](docs/TYPESCRIPT_ROUTE_RESPONSE_COVERAGE_ROADMAP.md) 是规划源头，[docs/TYPESCRIPT_ROUTE_RESPONSE_COVERAGE_ARCHIVE.md](docs/TYPESCRIPT_ROUTE_RESPONSE_COVERAGE_ARCHIVE.md) 记录 S87.1-S87.7 完成范围。`src/contracts/serverContracts.ts` 已覆盖 game/exam/AI/inventory/NPC/trade/delegation public response，`src/routes/routeResponses.js` 以局部 `@ts-check` helper 接入大型 route，并在运行时拒绝 public `worldState` raw ledger key；`/api/game/saves` 也纳入 `SavesResponse` helper。后端 route/API response shape 新增或重构时必须对齐 contract 或局部 JSDoc typedef，并运行 `npm run typecheck:server`；不得为了启用类型检查一次性 whole-file `@ts-check` 大型 route 文件，也不得放宽安全 projection、raw ledger 剥离、Ajv/runtime 校验或服务器裁决。
- 启动 S88 全面系统打磨专项：[docs/QIANQIU_POLISHING_ROADMAP.md](docs/QIANQIU_POLISHING_ROADMAP.md) 把完整书生主线、多身份循环、官场世界后果、AI 世界引擎、NPC/关系、资产经济、React 前端、PixiJS 地图、视觉立绘、后端类型安全和 Mock/真实 provider 验收拆为 S88.1-S88.12。S88.1 已先加固 AI remote helper/provider public-safe envelope；S88.2 已固定 SQLite derived row builder 类型边界，`world_sessions.world_state_json` 继续是 SQLite 模式唯一事实源，prompt retrieval、safe search 和 maintenance safe diagnostics 只从服务器安全 view 单向修复；S88.3 已完成书生主线补强一轮，殿试授官后会按服务器授官轨迹生成入仕首月差事并进入 `officialCareerView`，考试取题后 `entryPreparation`、`examProcedureView` 和 `studyProfileView` 会显示服务器按盘费、保结、准考缺口与读书计划派生的备考压力、入场反馈和安全下一步提示，场内推进后 `examProcedureView.phaseFeedback` 会按 `sceneTime`、科场阶段、备考压力和公开行动摘要生成入场后反馈、风险提示和草稿建议，放榜后 `examAftermathView` 会从服务器定榜、科名荣誉、同年座师网络与授官轨迹整理公开过渡摘要和行动草稿建议；`studyProfileView.nextPlan` 现在还包含三旬计划窗口、补弱强度、晨午暮日课、复盘节点、风险提示、首课草稿和服务器权限边界，React 书生面板与科举页只读展示并只写草稿。S88.4 已完成六个官场体验切片：`officialCareerView.firstMonthExperience` 会从首月差事派生官署首月进度、上官同僚反馈、回署回执、考成风险和下一步草稿建议，普通回合推进差事时生成 `[官署回执]` 安全反馈，月末 `playerMonthlyBriefingView` 会摘录该首月差事；`officialCareerView.courtEntry` / `courtEntries` 会把首月回署材料整理为可进入 `memorial-review` / `court-debate` 的安全 evidence 与长期考成追踪，`topic_draft` 可引用这些 evidence 但仍只生成草稿；普通回合提交“入奏折队列 / 付朝议筹议 / 续记考成”后，服务器会写入 `officialCareer.courtEntryResolutions`，并在 `officialCareerView.courtEntry.latestResolution`、`eventArchiveView` 和月报中显示准入复核、转部核议、留中补查、驳回补据或续入考成等公开结果；继续提交“朝议跟进 / 部院覆奏 / 御前摘报 / 考成观察”后，服务器会写入 `officialCareer.courtEntryFollowUps`，并通过 `latestFollowUp`、`followUpHistory`、事件档案 `official_court_follow_up`、`worldThreadView` 和月报公开朝议/部院/御前中间反馈；跨身份 `courtResponseView` / `officialCourtResponses` 让皇帝、大臣、官员围绕公开奏议写入受限 `official_court_response` 中间态，事件档案、世界议程、topic surface 和 React 皇帝/官员面板只读展示；`officialCourtConsequences` / `courtConsequenceView` 继续把公开奏议裁决、跟进和回应转为长期官场后果信号，同步 `official_court_consequence` 事件档案、`worldThreadView`、官职月报和 topic surface。React 官员面板新增“官署首月”“奏折朝议入口”和“考成与弹劾”长期后果区块，皇帝面板新增“奏议回应”并在“赏罚预留”显示长期余波，按钮仍只写行动草稿，近次裁决、跟进批复、跨身份回应与长期后果信号只读展示。兼容响应里的 `worldState` 会递归剥离嵌套 hidden/raw/provider/prompt/statePatch 字段和 raw `officialCourtResponses` / `officialCourtConsequences`，并清洗入场准备、科场局部时间、科场流程、读书计划、放榜过渡和官场首月公开快照。
- S88.5 正在补强六身份循环：`roleCycleView` 统一整理当前身份事务、风险、可查入口和本地草稿建议；S88.5.2 已让地方官读取安全市价/NPC 月账入口、将领读取舆图/战事档案入口；S88.5.3 新增 `roleCycleDomainAdjudication` 后端接缝，把普通回合中的地方官市价/平粜、将领侦察/调粮行动转交既有 `cityPolicyResolver` / `militaryDiplomacyResolver` 裁决，NPC 经济复盘仍只读。公开 turn/SSE 反馈只返回脱敏 outcome、公开事件和 attribute changes，兼容 `worldState` / player-state 不暴露 `cityPolicyLedger`、`militaryDiplomacyLedger` 或 sealed geography。
- S88.12 已启动 Mock/真实 provider 验收：普通 JSON turn 与 SSE turn 在 `AI_PROVIDER=deepseek` 但无 `DEEPSEEK_API_KEY` 时仍通过 Mock 安全回退保持可玩，经济 `draftContext` 仍由服务器从当前 topic surface 重建 allowlist 后再采纳；`/api/ai/connection-test` 缺 key 仍返回 503 作为诊断，不等同普通游玩回退。`npm run smoke:provider:tools` 在无 `MIMO_API_KEY` 时会先 skip，不要求运行时存在 `fetch`；有 key 但无 fetch 仍会失败以暴露真实 smoke 环境问题。`npm run smoke:provider:long` 的内存结算链现在运行 NPC 经济旬/月 tick 并校验 `economyTraceView` 只读账解，不泄露 raw ledger、provider/key/path 或 `evidenceRefs`；`smoke:provider:long` / `smoke:provider:route` 在自动模式无 key 时跳过，但显式点名真实 provider 且缺 key 时必须失败，避免把真实 provider 验收和普通游玩 Mock fallback 混为一谈。
...
- 完成 S68.2 读书账本与学业计划基础：新增 server-owned `studyProfile` 与 route `studyProfileView`，把经义根柢、制艺章法、策论时务、史事典故、律例判断、誊写卷面和科场耐力整理成可见学业画像；普通读书行动会记入日课，交卷后按评分/复核刷新文卷弱点、老师建议、书目和下旬计划，浏览器书生面板与 prompt 只读该 view，provider 不能直接 patch 账本、名位或官职。S88.3 起 `nextPlan` 进一步细化为服务器生成的三旬计划窗口、补弱强度、晨午暮日课、复盘节点、风险提示、下一步短札和权限边界，AI 老师只能补文本建议，不能直接改属性、保结、科名、榜次或官职。
...
- 完成 S68.4 科场制度流程：新增 server-owned `activeExam.procedure` 与 route `examProcedureView`，在不改变外层四级考试 API 的前提下展示童试县试/府试/院试摘要、乡试/会试三场多卷、保结、搜检、号舍、发题、草稿、誊清、交卷、弥封、誊录、对读、磨勘、放榜和归档流程；交卷后把安全流程快照写入 `player.examHistory[].examProcedure`。S88.3 起 `examProcedureView.phaseFeedback` 还会按场内阶段、备考压力和公开行动摘要生成入场后反馈、风险提示、下一步草稿建议和权限边界，React 科举页只读展示，“拟行动”只写本地行动草稿。浏览器和 prompt 只读脱敏 view，不暴露弥封身份映射、保结密注、考官私意、模型原始建议或内部审计。
...
- 加固错误脱敏：AI 连接诊断、provider fallback 日志、provider smoke、事件档案和浏览器 smoke 会避免暴露 key、长 token、raw prompt、provider payload、请求/响应体、base URL、本地路径和 raw provider response。
...
- 后端：Node.js + Express，当前以 CommonJS JavaScript 为主；S86 起已建立 `npm run typecheck:server` 渐进检查地基，优先覆盖契约、API/view 类型、安全 projection、AI schema/provider facade、storage adapter 和核心 resolver，不做全仓重写，TS 类型也不能替代 Ajv 与服务器 runtime 校验；S87 已完成 route/API response shape 首轮覆盖，后续新增或重构 response 必须对齐 `src/contracts/serverContracts.ts` 或局部 JSDoc typedef，禁止为了类型检查一次性 whole-file `@ts-check` 大型 route 文件。
...
- `npm run ai:baseline` 写出 AI v2 改造前的 hidden-safe 结构基线到 `artifacts/ai-baseline/latest.json`，只摘要 prompt/schema/route/tool/provider 能力，不改变运行时行为。
...
- `npm run smoke:provider*` 只在配置真实 provider key 时进行网络调用；自动模式无 key 环境会成功跳过，显式 `AI_PROVIDER=<real provider>` 或 `--provider <real provider>` 缺 key 会失败。普通游玩的 no-key Mock fallback 不适用于这些真实 provider 验收入口。
...
- 游戏与考试路由会返回服务器整理后的可见视图，例如 `examProcedureView`、`examinerPanelView`、`examHonorView`、`appointmentTrackView`、`studyProfileView`、`officialCareerView`、`courtResponseView`、`courtConsequenceView`、`relationshipView`、`worldGeographyView`、`worldPeopleView`、`officialPostingsView`、`localAffairsDocketView`、`militaryDiplomacyView`、`economicFiscalView`、`historicalEventArchiveView`、`eventArchiveView`、`informationPanelPageView`、`aiSettingsView`、`aiInvocationSummaryView`、`playerMonthlyBriefingView`、`actorMemoryView`、`sessionSummaryView`、`mapContextView`、`mapRuntimeView` 和 `aiControlAuditView`。S69.2 的同年/座师/考官事实通过 `relationshipView`、`worldPeopleView`、`eventArchiveView` 和考试历史 `examNetwork` 快照暴露；S69.3 的初授事实通过 `appointmentTrackView`、`officialCareerView`、`eventArchiveView` 和考试历史 `appointmentTrack` 快照暴露；S88.4 的官署首月体验通过 `officialCareerView.firstMonthExperience` 暴露，并由 `playerMonthlyBriefingView` 月末摘录，不读取 raw `officialCareer.hiddenNotes`；首月回署材料还会通过 `officialCareerView.courtEntry` / `courtEntries` 进入 `memorial-review`、`court-debate` 和 `topic_draft` 的安全 evidence，普通回合服务器裁决后的 `courtEntryResolutions` / `latestResolution` 只公开处理结果、微量考成影响和下一步，后续朝议/部院/御前跟进通过 `courtEntryFollowUps`、`latestFollowUp`、`official_court_follow_up` 事件档案、月报和 `worldThreadView` 公开中间反馈；跨身份奏议回应通过 `courtResponseView`、`official_court_response` 事件档案和 `worldThreadView` 公开中间态，兼容 public `worldState` 不暴露 raw `officialCourtResponses`；长期官场后果通过 `courtConsequenceView`、`official_court_consequence` 事件档案、官职月报和 `worldThreadView` 公开证据缺口、考成压力、风宪关注、功绩留痕或朝局余波，兼容 public `worldState` 不暴露 raw `officialCourtConsequences`。这些 view 不让前端或 AI 直接任免、奖惩、处分、财赋动用、奏议终局或风宪定案。S69.4 的浏览器科举档案面板只组合这些 route view 与安全快照，不新增 raw route 入口；S70.9 的 AI 设置面板只读 AI route view，不读取 raw `worldState.aiSettings`；S70.10 的官职月报面板只读 `playerMonthlyBriefingView` 和本回合 `playerMonthlyBriefing` 反馈，不读取 raw `worldState.playerMonthlyBriefing`；S70.11 的跳时叙事只读本回合 `timeSkip` 安全总结，不读取 raw 计划或隐藏状态；S70.12 的记忆与经历摘要只读 `actorMemoryView`、`sessionSummaryView` 和本回合安全反馈，不读取 raw `worldState.actorMemoryLedger` 或 `worldState.sessionSummary`；S70.13 的地图接口只读 `mapContextView` 和稳定 refs，不读取 raw coordinate table 或 hidden enemy truth；S72.2 的 `mapRuntimeView` 只供浏览器地图 runtime 使用，不进入 prompt 或服务器裁决事实；S71.11 的 AI 调动审计只读 `aiControlAuditView`，不读取 raw prompt、provider payload、SQLite row、hidden ledger、本地路径或 key。

- S88.5.3 的 `roleCycleDomainAdjudication` 是普通回合反馈而不是 raw view：它只公开脱敏 outcome、公开事件和 attribute changes，兼容 `worldState` / player-state 继续剥离 raw resolver audit、stateDelta、playerDelta、`cityPolicyLedger`、`militaryDiplomacyLedger` 和 sealed geography。
...
- [docs/TYPESCRIPT_BACKEND_MIGRATION_ARCHIVE.md](docs/TYPESCRIPT_BACKEND_MIGRATION_ARCHIVE.md)：S86 后端 TypeScript 渐进迁移首轮完成范围、验证入口和 Rust 使用边界。
- [docs/TYPESCRIPT_ROUTE_RESPONSE_COVERAGE_ARCHIVE.md](docs/TYPESCRIPT_ROUTE_RESPONSE_COVERAGE_ARCHIVE.md)：S87 route/API 响应类型覆盖完成范围、public response contract、局部 helper、安全边界和验证证据。
...
- [docs/AI_ORCHESTRATION_V2_ROADMAP.md](docs/AI_ORCHESTRATION_V2_ROADMAP.md)：AI 编排 v2 路线图，规划 runtime、provider adapter、tool loop、trace/eval/replay、evidenceRef 和前端 debug 的分阶段边界。
- [docs/AI_V2_BASELINE_REPORT.md](docs/AI_V2_BASELINE_REPORT.md)：AI v2 Ticket 0 基线报告，记录 `npm run ai:baseline` 与 eval artifact 的安全摘要、验证入口和后续建议。
