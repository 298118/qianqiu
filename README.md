...
- 前端：默认浏览器入口已由 S74.1 React + TypeScript + Vite 构建产物接管；源码位于 `client/`，生产构建输出为 `dist/client/`，React Router Data Mode 在 `/` 下管理首页、主卷、舆图、人物、史册、科举、皇榜、朝议和印匣等路由。S74.2 起 React client 通过安全 API client 调用 start/saves/player-state/turn/exam/AI settings/connection-test，普通读档不使用 raw state route；S74.3 起 UI 状态层统一管理 route page、`sessionId`、安全玩家摘要、drawer/modal/surface、tab、action draft 和 display preferences；S74.4 起 `AppShell` 与 `SurfaceHost` 统一管理设置/存档/专题 overlay、Esc 关闭、焦点回收和滚动恢复；S74.5 起 `assetRegistry` 通过 `/assets/ui/ink-ui-manifest.json` 按审核状态、安全路径、fallback、缩略图和低清占位提供资产查询，`Portrait` 组件只接收 `portraitRef`。旧 `public/index.html`、`public/app.js` 和 `public/styles.css` 暂留作迁移参考，`public/assets/`、`public/vendor/`、`public/mapRenderer.js` 和 `public/mapPanel.js` 继续为已审核素材与 S72 地图资源提供静态路径。
- 后端：Node.js + Express。
