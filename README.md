...
- 官场深度：S42.1 已定义深度官场契约，S42.2 已接入静态官职/衙门目录、差事推进、考成卷宗、弹劾流程和官职伪造过滤；S42.3 继续扩展官场 UI、档案和浏览器验收。
- 世界议程：把主动 NPC、长期事件、官场结果、身份联动合并为跨月议题，让 AI 影响体现在人物、衙门、地方、军务、财政和士林网络里。
...
- 游戏与考试路由会返回 `examCalendarView`、`examRivalView`、`relationshipView`、`activeNpcRequestView`、`roleWorldCouplingView`、`longTermEventView`、`officialCareerView` 等视图；其中 `officialCareerView` 已包含 S42.2 官署、差事、考成、关系摘要和弹劾流程摘要。
...
- 长期事件、官场结果、科举日历和角色世界联动仍是确定性规则集；S42.2 已让入仕后有差事/考成/弹劾流程骨架，扩展官场浏览器面板和长期档案仍在后续步骤。
