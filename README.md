...
当前活动路线图已切到本地动态世界数据库专项，见 [动态世界本地数据库规划](docs/DYNAMIC_WORLD_DATABASE_PLAN.md)：短期继续保持 JSON 存档可玩，S49.2 已把 `sessionStore` 路由 facade 与默认 JSON adapter 分开；下一步才是以本地 SQLite 原型承载 session、事件日志、AI proposal 审计，之后逐步拆国家/邻国、城市、NPC、官职、关系和事件等动态数据。当前不规划远程存档、账号体系、多人同步或云端数据库。AI 只能提交结构化建议，不能直接写数据库或执行 SQL。
...
- 存储一致性：路由通过 `sessionStore` facade 读写；默认 JSON adapter 使用 envelope、atomic temp rename、revision 检查、同 session 队列和本地 `.lock` 文件；SQLite/数据库迁移仍是后续方向。
...
- 存储：`sessionStore` facade + 默认本地 JSON adapter，文件位于 `data/sessions/`，带 schema envelope、metadata、revision、atomic write、lock 和 save-list API。
...
- 当前默认存储仍是本地 JSON。S49.2 已把 JSON 实现放入 storage adapter 并用 contract tests 固定 route 兼容行为；本地 SQLite 仍是后续持久化方向，业务表拆分要在 SQLite session 原型和审计日志之后进行。远程存档、账号体系、多人同步和云端数据库不属于当前范围。
