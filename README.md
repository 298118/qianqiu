...
- 完成 S75.1-S75.2 首页画卷与开局表单：默认 `/` 首页已使用 S73.4 审核通过的水墨画卷、透明云雾、题名册表单底、朱印和案卷素材，标题“千秋”作为首屏主视觉；中央表单现已接入朝代、年份、六类身份、姓名、书生家境和自定背景，继续只走安全 `POST /api/game/start`。书生家境由服务器归一化为公开 `familyBackground`，自定背景写入公开 `customSetting`，不作为 hidden 私档；旧案卷只显示脱敏存档 metadata。S75.3 继续处理朱印盖章反馈。
