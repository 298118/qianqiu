...
- [docs/FRONTEND_PORTRAIT_MATRIX.md](docs/FRONTEND_PORTRAIT_MATRIX.md)：S73.10.1 全量立绘矩阵说明，配套 `public/assets/ui/portraits/portrait-pool-matrix-v1.json` 和 `npm run qa:portrait-matrix`；S73.10.2-S73.10.7 的已审核立绘池由各批次 QA 命令校验，S73.10.6 的 `npm run qa:portrait-compression` 统一校验 572 张 active 立绘的缩略图、低清占位、裁切字段、文件预算和懒加载边界。
