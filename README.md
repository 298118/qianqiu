...
- 完成 S73.2 UI manifest schema 与素材台账：新增 `public/assets/ui/ink-ui-manifest.json` 草案、[docs/FRONTEND_ASSET_LEDGER.md](docs/FRONTEND_ASSET_LEDGER.md) 和 `test/frontendInkAssetsManifest.test.js`，固定 `ink-ui-v1` 的审核状态、fallback token、路径策略、usage/role catalog、S73.3-S73.10 planned asset groups，以及 S73.10 全量立绘池的 `portraitRef`、缩略图、低清占位、状态变体、懒加载和审核字段。本步不生成实际图片，`assets` 暂为空。
