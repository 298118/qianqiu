...
- 自动化测试：`npm test` 使用 Node.js 内置测试运行器，目前覆盖 53 项，包括状态边界、JSON session、AI schema、科举门槛、晋级/作弊后果、月度 tick、关系账本、地方官、将领和入仕官员回合。
...
项目已完成第一阶段可玩纵切，并进入第二阶段“可持续模拟”开发。当前已实现世界 tick、关系账本、地方官深度、将领深度和入仕官员深度。下一步计划是继续深化科举竞争、真实 provider 验收和浏览器自动化验收。
