...
- 完成 S73.2-S73.10.6 UI manifest、素材台账、首批材质包、首页资产包、场景插画包、身份背景包、立绘风格基准、动效/fallback 包、素材 QA、全量立绘矩阵、全量立绘池与压缩验收：`public/assets/ui/ink-ui-manifest.json` 已从 schema 草案进入 `assets_active`，登记 594 个 active 素材，其中包括 16 个已审核 UI 材质、6 个首页素材、10 个场景插画、6 个身份背景、8 个动效/fallback 素材和 548 张 active 立绘。S73.10 当前可用立绘为 524 张：192 张玩家池、188 张通用 NPC、72 张重要 NPC 专属池、72 张状态/姿态与场景锚点；另有 S73.7 的 24 张基准立绘。`public/assets/ui/asset-qa-report-v1.json` 校验 594 个 active 素材和 19 个透明素材，`public/assets/ui/portraits/portrait-compression-qa-v1.json` 校验 548 张立绘的主图、缩略图、低清占位、safeArea、focalPoint、移动裁切、文件预算和禁止 eager load。
- S73.7 已完成 24 张玩家/NPC 立绘风格基准并登记 `portrait-baseline-qa-v1.json`；S73.8 已完成 8 个动效/fallback 素材并登记 `effect-motion-qa-v1.json`；S73.9 已新增 `npm run qa:frontend-assets` 与 `npm run qa:frontend-assets:write`；S73.10.1 已新增 [docs/FRONTEND_PORTRAIT_MATRIX.md](docs/FRONTEND_PORTRAIT_MATRIX.md)、`public/assets/ui/portraits/portrait-pool-matrix-v1.json` 和 `npm run qa:portrait-matrix`；S73.10.2-S73.10.5 已完成玩家池、通用 NPC、重要 NPC 专属池、状态/姿态与场景锚点立绘的主图、缩略图、低清占位、视觉/安全审核和 manifest 入库；S73.10.6 新增 `scripts/frontendPortraitCompressionQa.js`、`npm run qa:portrait-compression` / `qa:portrait-compression:write` 和 `public/assets/ui/portraits/portrait-compression-qa-v1.json`，把分散在各批次中的缩略图与压缩规则收束为独立验收点。矩阵只预置 `portraitRef`、路径、prompt 母版和审核字段；实际可用性以 manifest 中已审核资产为准。
...
- [docs/FRONTEND_PORTRAIT_MATRIX.md](docs/FRONTEND_PORTRAIT_MATRIX.md)：S73.10.1 全量立绘矩阵说明，配套 `public/assets/ui/portraits/portrait-pool-matrix-v1.json` 和 `npm run qa:portrait-matrix`；S73.10.2-S73.10.5 的已审核立绘池由各批次 QA 命令校验，S73.10.6 的 `npm run qa:portrait-compression` 统一校验 548 张 active 立绘的缩略图、低清占位、裁切字段、文件预算和懒加载边界。
