...
- 长期模拟骨架：世界月度 tick、关系账本、主动 NPC 请托、长期事件、官场结果、科举日历、角色世界联动、World Entities 多实体模型和 World Threads 世界议程索引已经接入。
...
- 多实体世界：S45.1 已新增 [多实体世界模型契约](docs/WORLD_ENTITIES_CONTRACT.md) 和 `worldEntities` 账本，把朝廷衙门、地方士绅、书院同门、军镇边墙、盐漕税赋和灾荒赈务整理为服务器拥有的实体、玩家可见 `worldEntityView` 与 AI 可读摘要。
...
- 长期事件、官场结果、科举日历、角色世界联动、World Entities 和 World Threads 仍是确定性规则集；S42.3 已让入仕后可在浏览器中查看官署、差事、考成、关系和风险，S43.2 已把这些信号汇入浏览器世界议程总览，S45.1 先提供实体 view/API/prompt 摘要，尚未新增浏览器实体面板。世界议程只提示介入方向，不提供一键结算。
