...
- 完成 S75.1 首页画卷布局：默认 `/` 首页已使用 S73.4 审核通过的水墨画卷、透明云雾、题名册表单底、朱印和案卷素材，标题“千秋”作为首屏主视觉，中央保留现有安全 Mock 开局表单，旧案卷只显示脱敏存档 metadata；移动端无横向溢出，低动效偏好下切换为静态底并关闭云雾漂移。S75.2/S75.3 继续处理完整开局字段和朱印盖章反馈。
