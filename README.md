...
- 世界议程：S43.1 已新增服务器拥有的 `worldThreads` 聚合层，把主动 NPC、长期事件、官场差遣/结果、身份联动、地方压力、边事和派系斗争整理成可追踪的跨月议题；S43.2 将继续做前端检查视图、玩家介入点和后续结算。
- AI 权限审查：逐项判断哪些系统适合 AI 生成、建议、解释或排序，哪些必须由服务器裁决，并补 red-team/eval 测试。
...
- 游戏与考试路由会返回 `examCalendarView`、`examRivalView`、`relationshipView`、`activeNpcRequestView`、`roleWorldCouplingView`、`worldThreadView`、`longTermEventView`、`officialCareerView` 等视图；其中 `officialCareerView` 已用于浏览器官场面板，`worldThreadView` 是 S43.1 新增的世界议程 API/prompt 摘要，前端专门面板留给 S43.2。
...
- 长期事件、官场结果、科举日历、角色世界联动和 World Threads 仍是确定性规则集；S42.3 已让入仕后可在浏览器中查看官署、差事、考成、关系和风险，S43.1 已把这些信号接入世界议程摘要，S43.2 仍需补前端议程面板和介入点。
