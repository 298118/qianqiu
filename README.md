...
- 完成 S73.2-S73.4 UI manifest、素材台账、首批材质包与首页资产包：`public/assets/ui/ink-ui-manifest.json` 已从 schema 草案进入 `assets_active`，登记 16 个已审核 UI 材质和 6 个已审核/限用首页素材；[docs/FRONTEND_ASSET_LEDGER.md](docs/FRONTEND_ASSET_LEDGER.md) 记录来源、prompt 摘要、后处理、许可说明、Codex 视觉审核和安全审核；`public/assets/ui/materials/`、`public/assets/ui/home/` 与 `public/assets/ui/thumbs/` 提供宣纸、旧绢、奏折、竹简、卷轴、残纸、水渍、淡墨线、朱印按钮、印匣纹理、试卷界栏、皇榜纸底、首页水墨画卷、云雾层、题名册表单底、开始朱印、案卷存档素材和低动效静态底。`test/frontendInkAssetsManifest.test.js` 现在校验真实图片存在性、尺寸、alpha、性能预算、审核状态和安全字段。
