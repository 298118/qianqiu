...
- 前端：当前发布入口仍为原生 HTML、CSS、JavaScript；S74.0 已安装 React + TypeScript + Vite + React Router Data Mode 工具链并新增迁移契约，真正的 `client/`、构建配置和默认 `/` 接管从 S74.1 开始。旧 `public/index.html`、`public/app.js` 和 `public/styles.css` 后续可替换或删除，不再要求旧前端双入口。
- 后端：Node.js + Express。
...
需要 Node.js `20.19+`、`22.13+` 或 `24.0+`。S74.0 前端工具链本机验证环境为 Node.js `v24.13.1`、npm `11.8.0`。
...
- [docs/FRONTEND_REACT_MIGRATION_CONTRACT.md](docs/FRONTEND_REACT_MIGRATION_CONTRACT.md)：S74.0 React/Vite 迁移契约，记录已安装依赖、React Router Data Mode、构建输出、脚本草案、安全 API、素材懒加载和回滚边界。
