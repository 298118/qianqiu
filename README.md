...
- 完成 S73.2-S73.10.3 UI manifest、素材台账、首批材质包、首页资产包、场景插画包、身份背景包、立绘风格基准、动效/fallback 包、素材 QA、全量立绘矩阵、玩家立绘池与通用 NPC 立绘池：`public/assets/ui/ink-ui-manifest.json` 已从 schema 草案进入 `assets_active`，登记 16 个已审核 UI 材质、6 个已审核/限用首页素材、10 个已审核场景插画、6 个已审核身份背景、24 张已审核玩家/NPC 立绘基准、8 个已审核动效/fallback 素材、192 张已审核玩家池立绘（72 张身份阶段 + 60 张女性风格补充 + 60 张男性风格补充）和 188 张已审核通用 NPC 立绘；新增 `public/assets/ui/asset-qa-preview.html` 与 `public/assets/ui/asset-qa-report-v1.json`，可预览并校验 450 个 active 素材和 19 个透明素材的路径、尺寸、alpha、缩略图、fallback、审核状态、宣纸底/深色底合成和 chroma-key 色边风险。`test/frontendInkAssetsManifest.test.js` 现在校验真实图片存在性、尺寸、alpha、性能预算、审核状态、场景/身份覆盖、角色色彩权重、`portraitRef`、缩略图、低清占位、懒加载、`motion`/`reducedMotionFallback`、S73.9 QA 报告、S73.10.1 立绘矩阵、S73.10.2 玩家立绘池、S73.10.3 通用 NPC 池和安全字段。
- S73.7 已完成 24 张玩家/NPC 立绘风格基准并登记 `portrait-baseline-qa-v1.json`；S73.8 已完成 8 个动效/fallback 素材并登记 `effect-motion-qa-v1.json`；S73.9 已新增 `npm run qa:frontend-assets` 与 `npm run qa:frontend-assets:write`；S73.10.1 已新增 [docs/FRONTEND_PORTRAIT_MATRIX.md](docs/FRONTEND_PORTRAIT_MATRIX.md)、`public/assets/ui/portraits/portrait-pool-matrix-v1.json` 和 `npm run qa:portrait-matrix`，把后续全量池锁定为 336 张 planned 立绘；S73.10.2 已新增 `scripts/frontendPlayerPortraitAssets.js`、`scripts/frontendPlayerFemalePortraitAssets.js`、`scripts/frontendPlayerMalePortraitAssets.js`、`npm run qa:player-portraits`、`npm run qa:player-female-portraits`、`npm run qa:player-male-portraits`、`public/assets/ui/portraits/portrait-player-pool-qa-v1.json`、`portrait-player-female-reset-qa-v1.json` 和 `portrait-player-male-extra-qa-v1.json`，并完成 72 张玩家身份阶段立绘、60 张女性玩家风格补充和 60 张男性玩家风格补充；S73.10.3 已新增 `scripts/frontendGenericNpcPortraitAssets.js`、`npm run qa:generic-npc-portraits`、`public/assets/ui/portraits/portrait-generic-npc-pool-qa-v1.json`，并完成 120 张矩阵通用 NPC、20 张旧版 bonus 和 48 张宫装/唐装女性风格扩展立绘的主图、缩略图、低清占位、视觉/安全审核和 manifest 入库。矩阵只预置 `portraitRef`、路径、prompt 母版和审核字段；实际可用性以 manifest 中已审核资产为准。
- 细化 S73-S77 前端水墨重构规划：[docs/FRONTEND_INK_REDESIGN_ROADMAP.md](docs/FRONTEND_INK_REDESIGN_ROADMAP.md) 已扩展为可执行开发蓝图，把 S73 拆成视觉资产指南、UI manifest/台账、UI 材质、首页资产、场景插画、身份背景、玩家/NPC 立绘基准、动效/fallback、素材 QA 和 S73.10 全量立绘生产；S74 规划为通过依赖治理后引入 React + TypeScript + Vite + React Router Data Mode 并直接接管默认 `/`，旧原生前端可替换或删除，S75-S76 接管首页/全局 shell、设置/存档/返回首页、底部奏折、身份/场景专题、考试/放榜、独立舆图页和立绘接线，S77 做默认入口确认、安全/性能/可访问性和归档验收。
...
- 前端：当前发布入口仍为原生 HTML、CSS、JavaScript，无构建步骤；S74.0 已规划可在依赖治理通过后引入 React + TypeScript + Vite + React Router Data Mode，并按用户重构口径直接接管默认 `/`，旧 `public/index.html`、`public/app.js` 和 `public/styles.css` 可替换或删除，不再要求旧前端双入口。
- 后端：Node.js + Express。
...
- [docs/FRONTEND_PORTRAIT_MATRIX.md](docs/FRONTEND_PORTRAIT_MATRIX.md)：S73.10.1 全量立绘矩阵说明，配套 `public/assets/ui/portraits/portrait-pool-matrix-v1.json` 和 `npm run qa:portrait-matrix`，用于 336 张 planned 立绘分批生成和审核；S73.10.2 玩家池已由 `npm run qa:player-portraits` 校验，S73.10.3 通用 NPC 池已由 `npm run qa:generic-npc-portraits` 校验。
