...
- 新增官职任所 SQLite 表契约：已固定 `office_bureaus`、`office_catalog`、`office_city_jurisdictions`、`office_postings`、`office_assessments`、`office_transfers` 的字段、引用和 AI 权限；运行时持久化留给 S56.2。
...
- 存储：默认 JSON session files；可选 SQLite session row、审计表、地理 `geo_*` 业务表、可见人物 `people_*` 业务表和人物事件关联；官职任所 `office_*` 表已完成契约，尚未创建运行时表。
...
SQLite 模式需要当前 Node.js 运行时提供 `node:sqlite`。它会保留完整 `world_sessions.world_state_json`，同步地理 `geo_*` 业务表和可见人物 `people_*` bridge rows，并把服务器人物事件关联到本地 `people_*.last_event_id`。官职任所 `office_*` 表目前是文档契约，尚未在运行时创建。raw SQLite table 不进入浏览器、prompt 或 save-list payload；读取修复也只从 `world_state_json` 单向重建，不把 raw row 或事件 id 回填为 route state。
...
- SQLite 目前已经包含 session row、审计表、地理 `geo_*` 业务表、可见人物 `people_*` bridge rows，以及人物事件到 `people_*.last_event_id` 的本地关联；官职任所 `office_*` 表已完成契约但尚未创建运行时表，raw audit 外安全事件索引尚未创建，已排入 S56-S57。
