...
- 完成 S75.1-S75.4 首页画卷、开局表单、朱印开始反馈与右上角印匣：默认 `/` 首页已使用 S73.4 审核通过的水墨画卷、透明云雾、题名册表单底、朱印和案卷素材，标题“千秋”作为首屏主视觉；中央表单现已接入朝代、年份、六类身份、姓名、书生家境和自定背景，继续只走安全 `POST /api/game/start`。书生家境由服务器归一化为公开 `familyBackground`，自定背景写入公开 `customSetting`，不作为 hidden 私档；朱印按钮现有短促落章反馈、loading/disabled/error 视觉、同步提交锁和低动效静态替代，连续 Enter 或双提交不会重复开局。右上角工具已收束为单一“印匣”入口，抽屉内以 AI 设置、旧案、显示、安全 tab 统一当前案卷 AI preset、provider 试连、旧案刷新/读档、返回首页、运行时显示偏好和安全摘要，继续只读安全 API 与 UI store 摘要，不显示密钥、连接地址、完整提示词、本地路径或模型原始返回。
