...
- 当前默认存储仍是本地 JSON。S49.3 已加入可选本地 SQLite session row adapter，S49.4 已加入本地事件日志和 AI proposal 审计；S50.2 已新增每局 `worldState.worldGeography`、路由 `worldGeographyView` 和 prompt projection，但还不新增浏览器地理面板，也不把国家/城市拆成 SQLite 业务表；NPC、官职、关系等业务表拆分仍是后续步骤。远程存档、账号体系、多人同步和云端数据库不属于当前范围。
