...
当前项目已经完成可玩纵切、浏览器验收、时间专项、AI provider 扩展、本地动态数据库基础、S54-S59 SQLite 业务表拆分、S60-S67 超大动态世界数据库内容充实、S68-S69 科举读书深化与归档、S70 AI 编排归档，以及 S71 数据库玩法化、维护、安全检索、redacted player API、财政/刑名/军务外交服务器 resolver、压力驱动事件、多 actor 场景、NPC 记忆账本、AI 调动审计面板和 S71 验收归档；S72 PixiJS 水墨地图专项也已完成归档。当前活动方向已进入 S73-S77 前端水墨重构规划。近期重点更新集中在“本地数据库专项归档”“S68-S69 科举读书深化”“多 provider 能力”“S70 AI 编排”“S71 数据库玩法化归档”“S72 PixiJS 水墨地图归档”和“S73-S77 前端水墨重构规划”：
...
- 完成 S73.2-S73.10.7 UI manifest、素材台账、首批材质包、首页资产包、场景插画包、身份背景包、立绘风格基准、动效/fallback 包、素材 QA、全量立绘矩阵、全量立绘池、年轻女性补丁池、女性单张重制覆盖与压缩验收：`public/assets/ui/ink-ui-manifest.json` 已从 schema 草案进入 `assets_active`，登记 642 个 active 素材，其中包括 16 个已审核 UI 材质、6 个首页素材、10 个场景插画、6 个身份背景、8 个动效/fallback 素材和 596 张 active 立绘。S73.10 当前可用立绘为 572 张：192 张玩家池、188 张通用 NPC、72 张重要 NPC 专属池、72 张状态/姿态与场景锚点、48 张年轻成年女性补充池；另有 S73.7 的 24 张基准立绘。`public/assets/ui/asset-qa-report-v1.json` 校验 642 个 active 素材和 19 个透明素材，`public/assets/ui/portraits/portrait-compression-qa-v1.json` 校验 596 张立绘的主图、缩略图、低清占位、safeArea、focalPoint、移动裁切、文件预算和禁止 eager load。
- S73.7 已完成 24 张玩家/NPC 立绘风格基准并登记 `portrait-baseline-qa-v1.json`；S73.8 已完成 8 个动效/fallback 素材并登记 `effect-motion-qa-v1.json`；S73.9 已新增 `npm run qa:frontend-assets` 与 `npm run qa:frontend-assets:write`；S73.10.1 已新增 [docs/FRONTEND_PORTRAIT_MATRIX.md](docs/FRONTEND_PORTRAIT_MATRIX.md)、`public/assets/ui/portraits/portrait-pool-matrix-v1.json` 和 `npm run qa:portrait-matrix`；S73.10.2-S73.10.5 已完成玩家池、通用 NPC、重要 NPC 专属池、状态/姿态与场景锚点立绘的主图、缩略图、低清占位、视觉/安全审核和 manifest 入库；S73.10.6 新增 `scripts/frontendPortraitCompressionQa.js`、`npm run qa:portrait-compression` / `qa:portrait-compression:write` 和 `public/assets/ui/portraits/portrait-compression-qa-v1.json`，把分散在各批次中的缩略图与压缩规则收束为独立验收点；S73.10.7 新增 `scripts/frontendYoungFemalePortraitAssets.js`、`npm run qa:young-female-portraits` 和 `public/assets/ui/portraits/portrait-young-female-pool-qa-v1.json`，单独入库 48 张年轻成年女性补充立绘；S73.10 单张覆盖新增 `scripts/frontendSinglePortraitOverrides.js`、`npm run qa:single-portrait-overrides` 和 `public/assets/ui/portraits/portrait-single-override-qa-v1.json`，当前把 60 张女性/偏女性画像替换为单张高清重制，减少网格切片带来的脏噪、模糊和比例问题。矩阵只预置 `portraitRef`、路径、prompt 母版和审核字段；实际可用性以 manifest 中已审核资产为准。
- 细化并启动 S74 前端迁移：[docs/FRONTEND_INK_REDESIGN_ROADMAP.md](docs/FRONTEND_INK_REDESIGN_ROADMAP.md) 已扩展为可执行开发蓝图，S73 完成视觉资产、manifest、素材 QA 和全量立绘池；S74.0 已按 [docs/FRONTEND_REACT_MIGRATION_CONTRACT.md](docs/FRONTEND_REACT_MIGRATION_CONTRACT.md) 安装 React + TypeScript + Vite + React Router Data Mode 工具链并记录依赖治理，S74.1 已创建 `client/`、Vite/TS/Vitest 配置、最小 React Router 多页壳和 Express history fallback，构建产物从 `dist/client/` 接管默认 `/`；S74.2 已新增安全 API client、会话 store 和首页/主卷/科举/印匣最小接线，普通读档只走 `player-state`；S74.3 已新增 UI 状态 store、route page bridge、显示偏好抽屉、安全摘要 modal 和全局 action draft；S74.4 已新增 `AppShell`、`SurfaceHost`、设置/存档/专题 surface registry、Esc 关闭、焦点回收和滚动恢复；S74.5 已新增 manifest 驱动 `assetRegistry`、`useAssetRegistry` 和 `Portrait` 组件，人物页按 8 张一组分页接入全部人物页可用立绘，女性高清重制优先、未重制项使用 manifest 原图；S74.6 已新增 React `InkMapRuntimeBridge`，动态加载本地 Pixi vendor 与 `public/mapRenderer.js`，只读包装安全 `mapRuntimeView`；S74.7 已让新默认入口 smoke 从 `/` 表单真实 Mock 开局，校验当前 session 导航、舆图/人物/史册/科举/皇榜/朝议/印匣路由刷新和 unsafe API 防线；S75.8 已将主卷行动入口替换为固定底部 `MemorialComposer`，支持身份 placeholder、Enter 呈上、Shift+Enter 换行、失败保留本地输入、空白禁用和本地规则“可行事”快捷草稿；S75.9 已新增只读 `POST /api/ai/quick-actions/:sessionId`、`quick_action` AI task、Mock/provider 快捷草稿建议、身份/证据/工具意图校验、provider 失败降级和前端 loading/stale/failed/applied 状态；S75.10 已完成 S75 浏览器总验收；S76.1-S76.6 已完成主卷和六类身份面板，书生、地方官、入仕官员/大臣、将领与皇帝面板都只读安全 projection 并把按钮写入底部奏折草稿；S76.7-S76.8 已完成科举考试全屏和放榜全屏，取题、交卷、评分、放榜、晋级和授官仍由服务器裁决；S76.9 已将 S72 地图升级为独立 `/game/:sessionId/map` 舆图页；S76.10 已完成开局案主立绘与人物谱牒接线，只使用已审核 `portraitRef`、缩略图、低清占位和 fallback；S76.11 已完成专题 surface 扩展位，S78 已将 `/game/:sessionId/court` 的奏折队列、拟圣旨、朝议、堂审、军议和人物档案六类专题升级为安全材料、证据勾选、`topic_draft` AI/Mock 草稿、玩家改稿和底部奏折写入闭环。S76.12 已完成 S76 总验收，S77.2 已把 `npm run smoke:browser -- --client react` 扩展到浏览器 history back/forward 和地图运行时资源失败 fallback；S77.3 已补强视觉像素 smoke，加入首页/考试/放榜背景、舆图 canvas、人物缩略图、可见重叠和玩家开发口径文案检查，并通过 `@fontsource/noto-serif-sc` 自托管简体中文字体避免 Linux/CI 方框字；S77.4 已补强安全污染防线，React smoke 统一扫描 DOM、浏览器存储、运行时 manifest 安全字段和截图产物名，科举/皇榜不再把弥封映射、考官隐藏意图或模型原始提案词样作为玩家可见文案。旧原生前端仅保留为迁移参考，不再是交付入口；仓库不提供产品内 `/legacy.html` 或 `/ink-client/` 双入口，失败回退依赖 Git revert 或恢复上一提交。下一步进入 S77.5 性能资源预算，并继续可访问性和归档验收。
- 完成 S75.1-S75.7 首页画卷、开局表单、朱印开始反馈、右上角印匣、案卷式存档/读档、继续本局与显示偏好持久化：默认 `/` 首页已使用 S73.4 审核通过的水墨画卷、透明云雾、题名册表单底、朱印和案卷素材，标题“千秋”作为首屏主视觉；中央表单现已接入朝代、年份、六类身份、姓名、书生家境和自定背景，继续只走安全 `POST /api/game/start`。书生家境由服务器归一化为公开 `familyBackground`，自定背景写入公开 `customSetting`，不作为 hidden 私档；朱印按钮现有短促落章反馈、loading/disabled/error 视觉、同步提交锁和低动效静态替代，连续 Enter 或双提交不会重复开局。右上角工具已收束为单一“印匣”入口，抽屉内以 AI 设置、旧案、显示、安全 tab 统一当前案卷 AI preset、provider 试连、旧案刷新/读档、返回首页、显示偏好和安全摘要；首页与印匣旧案列表现在以案卷卡片显示 sessionId 短码、玩家名、身份/功名/官职、朝代年月旬、回合数、摘要和最近更新时间，缺字段显示中文 fallback，读档继续只走 `GET /api/game/player-state/:sessionId`。游戏中返回首页只清理前端抽屉/弹窗/临时行动草稿并保留当前安全 session 指针，首页显示“继续本局”入口，点击后回到 `/game/:sessionId`；返回首页本身不删除 session、不提交空行动、不改服务器状态。显示偏好只以 `qianqiu.displayPreferences.v1` 白名单保存动效、字号、对比度、自动滚动和舆图动效，不写 raw state、完整提示词、provider payload、本地路径或密钥。
...
- `npm run smoke:browser` 构建 React client 并启动临时 Mock 服务器；无 `--url` 时脚本会显式固定 `AI_PROVIDER=mock`，也可显式传入 `--client react`。当前 smoke 覆盖默认 `/` React 入口真实表单开局、当前 session 导航、右上角印匣 tab 遍历、旧案 `player-state` 读档、印匣返回首页/继续本局、AI 快捷建议草稿、舆图 canvas 与低动效、人物立绘分页、史册/科举/皇榜/朝议/印匣路由刷新、移动端奏折/印匣、首页/考试/放榜背景和地图 canvas 像素非空、人物缩略图资源、可见文本粗重叠、桌面/移动无横向溢出和 hidden/raw/API/玩家开发口径文案防线。
...
- [docs/DEVELOPMENT_STEPS.md](docs/DEVELOPMENT_STEPS.md)：当前活动路线图与进度台账；当前活动方向为 S73-S77 前端水墨重构。
- [docs/FRONTEND_INK_REDESIGN_ROADMAP.md](docs/FRONTEND_INK_REDESIGN_ROADMAP.md)：S73-S77 前端水墨重构任务书，覆盖素材矩阵、React/Vite/React Router 多页迁移、首页/全局 shell、身份/场景专题、考试/放榜、独立舆图页、S73.10 全量立绘生产与后续 `portraitRef` 使用规则，以及验收归档。
