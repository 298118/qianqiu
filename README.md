...
- 启动 S88 全面系统打磨专项：[docs/QIANQIU_POLISHING_ROADMAP.md](docs/QIANQIU_POLISHING_ROADMAP.md) 把完整书生主线、多身份循环、官场世界后果、AI 世界引擎、NPC/关系、资产经济、React 前端、PixiJS 地图、视觉立绘、后端类型安全和 Mock/真实 provider 验收拆为 S88.1-S88.12。S88.1 已先加固 AI remote helper/provider public-safe envelope：`remoteHelpers` 进入 `npm run typecheck:server`，AI connection public response 和 provider fallback 日志不允许回显 raw provider payload、完整 prompt、请求/响应体、base URL、key、本地路径、`statePatch` 或 `worldState`。
