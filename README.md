...
当前活动路线图已切到本地动态世界数据库专项，见 [动态世界本地数据库规划](docs/DYNAMIC_WORLD_DATABASE_PLAN.md)：短期继续保持 JSON 存档可玩，中期先抽存储 adapter，再以本地 SQLite 原型承载 session、事件日志、AI proposal 审计，之后逐步拆国家/邻国、城市、NPC、官职、关系和事件等动态数据。当前不规划远程存档、账号体系、多人同步或云端数据库。AI 只能提交结构化建议，不能直接写数据库或执行 SQL。
...
- 当前存储仍是本地 JSON。虽然已有 envelope、revision、atomic write 和 lock，本地 SQLite 或数据库适配器仍是下一阶段更稳的持久化方向；动态数据库规划已明确先 adapter、后 SQLite、再拆国家/城市/NPC/官职等业务表。远程存档、账号体系、多人同步和云端数据库不属于当前范围。
