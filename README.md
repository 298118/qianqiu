...
- 游戏与考试路由会返回 `examCalendarView`、`examRivalView`、`relationshipView`、`activeNpcRequestView`、`roleWorldCouplingView`、`worldGeographyView`、`worldEntityView`、`worldPeopleView`、`worldThreadView`、`longTermEventView`、`officialCareerView` 等视图；其中 `officialCareerView` 用于浏览器官场面板，`worldThreadView` 用于浏览器“世界议程”面板和 AI prompt 摘要，`worldEntityView` 用于暴露 S45 多实体世界模型的玩家可见摘要，`worldGeographyView` 是 S50.2 为后续“天下格局/任所地理”面板准备的可见地理 projection，`worldPeopleView` 是 S51.2 为后续人物谱牒/家产面板准备的可见人物 projection。后续人物谱牒/家产 UI 必须读服务器 view，不能读取 raw ledger。
...
- [docs/OFFICIAL_POSTING_DATABASE_CONTRACT.md](docs/OFFICIAL_POSTING_DATABASE_CONTRACT.md)：S52.1 官职、官署、任所、城市辖区、考成和迁转记录 schema 契约，固定 `officialPostingSchemas` 的归一化、可见性过滤、引用裁剪、prompt cap 和 future `officialPostings` server-owned 边界。
...
- 当前默认存储仍是本地 JSON。S49.3 已加入可选本地 SQLite session row adapter，S49.4 已加入本地事件日志和 AI proposal 审计；S50.2 已新增每局 `worldState.worldGeography`、路由 `worldGeographyView` 和 prompt projection，但还不新增浏览器地理面板，也不把国家/城市拆成 SQLite 业务表；S51.2 已新增每局 `worldState.worldPeople` 的安全可见桥接、路由 `worldPeopleView` 和 prompt projection，但还不新增浏览器人物/家产面板，也不把 NPC、家族、资产或关系拆成 SQLite 业务表。远程存档、账号体系、多人同步和云端数据库不属于当前范围。
