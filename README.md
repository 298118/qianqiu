...
- 新增 S87 route/API 响应类型覆盖规划：[docs/TYPESCRIPT_ROUTE_RESPONSE_COVERAGE_ROADMAP.md](docs/TYPESCRIPT_ROUTE_RESPONSE_COVERAGE_ROADMAP.md) 把下一轮拆为 route response contract、`buildCommonTurnViews`、game start/state/player-state/turn、inventory/NPC/trade/delegation、exam route、AI route 和验收归档。后端 route/API response shape 新增或重构时必须对齐 `src/contracts/serverContracts.ts` 或局部 JSDoc typedef，并运行 `npm run typecheck:server`；不得为了启用类型检查一次性 whole-file `@ts-check` 大型 route 文件，也不得放宽安全 projection、raw ledger 剥离、Ajv/runtime 校验或服务器裁决。
...
- 后端：Node.js + Express，当前以 CommonJS JavaScript 为主；S86 起已建立 `npm run typecheck:server` 渐进检查地基，优先覆盖契约、API/view 类型、安全 projection、AI schema/provider facade、storage adapter 和核心 resolver，不做全仓重写，TS 类型也不能替代 Ajv 与服务器 runtime 校验；S87 起 route/API response shape 新增或重构时必须对齐 `src/contracts/serverContracts.ts` 或局部 JSDoc typedef，禁止为了类型检查一次性 whole-file `@ts-check` 大型 route 文件。
