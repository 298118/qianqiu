...
- 前端：默认浏览器入口已由 S74.1 React + TypeScript + Vite 构建产物接管；源码位于 `client/`，生产构建输出为 `dist/client/`，React Router Data Mode 在 `/` 下管理首页、主卷、舆图、人物、史册、科举、皇榜、朝议和印匣等路由。旧 `public/index.html`、`public/app.js` 和 `public/styles.css` 暂留作迁移参考，`public/assets/`、`public/vendor/`、`public/mapRenderer.js` 和 `public/mapPanel.js` 继续为已审核素材与 S72 地图资源提供静态路径。
- 后端：Node.js + Express。
