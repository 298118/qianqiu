...
- 前端：当前发布入口仍为原生 HTML、CSS、JavaScript，无构建步骤；S74.0 已规划可在依赖治理通过后引入 React + TypeScript + Vite 并行前端岛，S77 验收后才切默认入口。
- 后端：Node.js + Express。
...
- [docs/FRONTEND_INK_REDESIGN_ROADMAP.md](docs/FRONTEND_INK_REDESIGN_ROADMAP.md)：S73-S77 前端水墨重构任务书，覆盖素材矩阵、React/Vite 渐进迁移、首页/全局 shell、身份/场景专题、考试/放榜、舆图、立绘管线和验收归档。
